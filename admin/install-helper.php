 ...
 * plugin installation. This file is not included by Mtaandao and it is
 ...
 *
 *     check_column( 'mn_links', 'link_description', 'mediumtext' );
 *     if ( check_column( $mndb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     $error_count = 0;
 *     $tablename = $mndb->links;
 *     // Check the column.
 *     if ( ! check_column($mndb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $mndb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $mndb->query( $ddl );
 *     }
 *
 *     if ( check_column( $mndb->links, 'link_description', 'varchar( 255 )' ) ) {
...
function maybe_create_table($table_name, $create_ddl) {
	global $mndb;
	foreach ($mndb->get_col("SHOW TABLES",0) as $table ) {
		if ($table == $table_name) {
			return true;
		}
	}
...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
	global $mndb;
	$diffs = 0;
	$results = $mndb->get_results("DESC $table_name");
