...
if ( isset( $_REQUEST['action'] ) && 'update' === $_REQUEST['action'] ) {
	...
	if ( is_writeable($real_file) ) {
		...
		if ( is_plugin_active( $plugin ) || isset( $_POST['phperror'] ) ) {
			...
			mn_redirect( add_query_arg( '_mnnonce', mn_create_nonce( 'edit-plugin-test_' . $file ), "plugin-editor.php?file=$file&plugin=$plugin&liveupdate=1&scrollto=$scrollto&networkwide=" . $network_wide ) );
			exit;
		}
		mn_redirect( self_admin_url( "plugin-editor.php?file=$file&plugin=$plugin&a=te&scrollto=$scrollto" ) );
	} else {
		mn_redirect( self_admin_url( "plugin-editor.php?file=$file&plugin=$plugin&scrollto=$scrollto" ) );
	}
	exit;
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __('<a href="https://mtaandao.github.io/Plugins_Editor_Screen">Documentation on Editing Plugins</a>') . '</p>' .
		'<p>' . __('<a href="https://mtaandao.github.io/Writing_a_Plugin">Documentation on Writing Plugins</a>') . '</p>' .
		'<p>' . __('<a href="https://mtaandao.co.ke/support/">Support Forums</a>') . '</p>'
	);

	require_once(ABSPATH . 'admin/admin-header.php');

	update_recently_edited(PLUGIN_DIR . '/' . $file);
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php mn_nonce_field('edit-plugin_' . $file) ?>
		...
		<div id="documentation" class="hide-if-no-js"><label for="docs-list"><?php _e('Documentation:') ?></label> <?php echo $docs_select ?> <input type="button" class="button" value="<?php esc_attr_e( 'Look Up' ) ?> " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'https://api.mtaandao.co.ke/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_user_locale() ) ?>&amp;version=<?php echo urlencode( get_bloginfo( 'version' ) ) ?>&amp;redirect=true'); }" /></div>
		<?php endif; ?>
