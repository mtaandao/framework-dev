/* global adminCommentsL10n, thousandsSeparator, list_args, QTags, ajaxurl, mnAjax */
...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			mnListsData = $( settings.target ).attr( 'data-mn-lists' );
		...
		if ( mnListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( mnListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = mnListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_mnnonce=' + settings.data._ajax_nonce);
			a.attr('data-mn-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	delAfter = function( r, settings ) {
		...
		theList.get(0).mnList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
		...
		animatedCallback = function () {
			if ( ! $( '#the-comment-list tr:visible' ).length ) {
				theList.get(0).mnList.add( theExtraList.find( '.no-items' ).clone() );
			}
		};
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see MN_Comments_List_Table::prepare_items() @ class-mn-comments-list-table.php
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).mnList.add( response.rows );
			}
		});
	};

	theExtraList = $('#the-extra-comment-list').mnList( { alt: '', delColor: 'none', addColor: 'none' } );
	theList = $('#the-comment-list').mnList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('mnListDelEnd', function(e, s){
			var mnListsData = $(s.target).attr('data-mn-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( mnListsData.indexOf(':trash=1') != -1 || mnListsData.indexOf(':spam=1') != -1 )
...
commentReply = {
	...
	show : function(xml) {
		...
		r = mnAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': mnAjax.broken});
			return false;
		}
...
$(document).ready(function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		...
		toggle_all = function() {
			$('#cb-select-all-1').data( 'mn-toggle', 1 ).trigger( 'click' ).removeData( 'mn-toggle' );
		};
