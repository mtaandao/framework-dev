...
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = mn.customize;
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes ) {
		...
		request = mn.ajax.post( 'customize_save', data );
	...
	Container = api.Class.extend({
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.mn-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.mn-full-overlay-sidebar-content' ),
	...
	api.Section = Container.extend({
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.mn-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.mn-full-overlay' ),
	...
	api.ColorControl = api.Control.extend({
		ready: function() {
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.mn-picker-container' );
				if ( pickerContainer.hasClass( 'mn-picker-active' ) ) {
					picker.mnColorPicker( 'close' );
					control.container.find( '.mn-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	...
	api.HeaderControl = api.Control.extend({
		ready: function() {
			...
			mn.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme = api.settings.theme.stylesheet;
		},
	...
	api.PreviewFrame = api.Messenger.extend({
		...
		run: function( deferred ) {
			...
			previewFrame.iframe.attr( 'onmousewheel', '' ); // Workaround for Safari bug. See MN Trac #38149.
	...
	api.Previewer = api.Messenger.extend({
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in res or main.
				if ( /\/mn-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
	...
	$( function() {
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.mn-full-overlay-sidebar-content' ),
			 ...
			 * @access private
			 *
			 * @param {mn.customize.Panel|mn.customize.Section} container Construct.
			 * @returns {void}
			 */
		...
		api.control( 'background_preset', function( control ) {
			...
			defaultValues = [
				_mnCustomizeBackground.defaults['default-position-x'],
				_mnCustomizeBackground.defaults['default-position-y'],
				_mnCustomizeBackground.defaults['default-size'],
				_mnCustomizeBackground.defaults['default-repeat'],
				_mnCustomizeBackground.defaults['default-attachment']
			];
		...
		( function() {
			...
			$( window ).on( 'blur.mn-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );

			// Save changeset before unloading window.
			$( window ).on( 'beforeunload.mn-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		} ());
