...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.mn.revisions.
	 */
	revisions = mn.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back end.
	revisions.settings = window._mnRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			mn.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				mn.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = mn.ajax.send( options ),
	...
	/**
	 * mn.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see mn.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = mn.Backbone.View.extend({
		className: 'revisions',
		template: mn.template('revisions-frame'),
		...
		render: function() {
			mn.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#mnbody-content .wrap').append( this.el );
	...
	revisions.view.Controls = mn.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.mn.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.mn.revisions.pinning click.mn.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.mn.revisions.pinning');
	...
	// The tickmarks view
	revisions.view.Tickmarks = mn.Backbone.View.extend({
	...
	// The metabox view
	revisions.view.Metabox = mn.Backbone.View.extend({
	...
	revisions.view.Meta = mn.Backbone.View.extend({
		template: mn.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = mn.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: mn.template('revisions-checkbox'),
	...
	revisions.view.Tooltip = mn.Backbone.View.extend({
	...
	revisions.view.Buttons = mn.Backbone.View.extend({
		className: 'revisions-buttons',
		template: mn.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = mn.Backbone.View.extend({
		className: 'mn-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.mn.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.mn.revisions');
	...
	revisions.view.Diff = mn.Backbone.View.extend({
		className: 'revisions-diff',
		template:  mn.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// mn_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// mn_localize_script does not allow for top-level booleans so do a comparator here.
