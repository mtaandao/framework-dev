...
( function( $ ) {
	 ...
	 *
	 * Example: `$( element ).mnTagsSuggest( options )`.
	 *
	 * The taxonomy can be passed in a `data-mn-taxonomy` attribute on the element or
	 ...
	 */
	$.fn.mnTagsSuggest = function( options ) {
		var cache;
		var last;
		var $element = $( this );
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-mn-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'mn-tags-autocomplete-' + ui.item.id );
			...
			select: function( event, ui ) {
				...
				if ( $.ui.keyCode.TAB === event.keyCode ) {
					...
					window.mn.a11y.speak( window.tagsSuggestL10n.termSelected, 'assertive' );
					event.preventDefault();
		...
		.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="mn-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
