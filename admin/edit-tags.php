...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ) ) ) {
   mn_die( __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ) );
}
...
if ( ! $referer ) { // For POST requests.
	$referer = mn_unslash( $_SERVER['REQUEST_URI'] );
}
$referer = remove_query_arg( array( '_mn_http_referer', '_mnnonce', 'error', 'message', 'paged' ), $referer );
...
case 'editedtag':
	...
	break;
default:
	if ( ! $mn_list_table->current_action() || ! isset( $_REQUEST['delete_tags'] ) ) {
		break;
	}
	check_admin_referer( 'bulk-tags' );
	$tags = (array) $_REQUEST['delete_tags'];
	/** This action is documented in admin/edit-comments.php */
	$location = apply_filters( 'handle_bulk_actions-' . get_current_screen()->id, $location, $mn_list_table->current_action(), $tags );
	break;
}
...
if ( 'category' == $taxonomy || 'link_category' == $taxonomy || 'post_tag' == $taxonomy  ) {
	...
	if ( 'category' == $taxonomy )
		$help .= '<p>' . __( '<a href="https://mtaandao.github.io/Posts_Categories_Screen">Documentation on Categories</a>' ) . '</p>';
	elseif ( 'link_category' == $taxonomy )
		$help .= '<p>' . __( '<a href="https://mtaandao.github.io/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	else
		$help .= '<p>' . __( '<a href="https://mtaandao.github.io/Posts_Tags_Screen">Documentation on Tags</a>' ) . '</p>';

	$help .= '<p>' . __('<a href="https://mtaandao.co.ke/support/">Support Forums</a>') . '</p>';
