...
final class MN_Screen {
	 ...
	 *
	 * @param string|MN_Screen $hook_name Optional. The hook name (also known as the hook suffix) used to determine the screen.
	 * 	                                  Defaults to the current $hook_suffix global.
	 * @return MN_Screen Screen object.
	 */
	public static function get( $hook_name = '' ) {
		if ( $hook_name instanceof MN_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'MN_NETWORK_ADMIN' ) && MN_NETWORK_ADMIN )
				$in_admin = 'network';
			elseif ( defined( 'MN_USER_ADMIN' ) && MN_USER_ADMIN )
				$in_admin = 'user';
			else
				$in_admin = 'site';
		}
	...
	public function set_current_screen() {
		 ...
		 * @since 3.0.0
		 *
		 * @param MN_Screen $current_screen Current MN_Screen object.
		 */
	 ...
	 * @static
	 *
	 * @param MN_Screen $screen A screen object.
	...
	public function set_screen_reader_content( $content = array() ) {
		...
		$content = mn_parse_args( $content, $defaults );
	...
	public function render_screen_meta() {
		 ...
		 * @param MN_Screen $this            Current MN_Screen instance.
		 */
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $mn_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );
		 ...
		 * @param MN_Screen $this            MN_Screen object.
		 */
	...
	public function render_screen_options( $options = array() ) {
		...
		if ( 'widgets' !== $this->base ) {
			...
			$form_end = "\n" . mn_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false, false ) . "\n</form>\n";
		}
	...
	public function render_meta_boxes_preferences() {
		...
		if ( ! isset( $mn_meta_boxes[ $this->id ] ) ) {
			return;
		}
		?>
		...
		<?php
			...
			if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) {
				if ( isset( $_GET['welcome'] ) ) {
					...
					if ( 2 == $welcome_checked && mn_get_current_user()->user_email != get_option( 'admin_email' ) ) {
						$welcome_checked = false;
					}
				}
				echo '<label for="mn_welcome_panel-hide">';
				echo '<input type="checkbox" id="mn_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' == $option ) {
			...
			/** This filter is documented in admin/includes/class-mn-comments-list-table.php */
			$per_page = apply_filters( 'comments_per_page', $per_page, $comment_status );
		} elseif ( 'categories_per_page' == $option ) {
			/** This filter is documented in admin/includes/class-mn-terms-list-table.php */
			$per_page = apply_filters( 'edit_categories_per_page', $per_page );
		} else {
			/** This filter is documented in admin/includes/class-mn-list-table.php */
		...
		<legend><?php _e( 'Pagination' ); ?></legend>
			<?php if ( $per_page_label ) : ?>
				...
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="mn_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>" maxlength="3"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif; ?>
				<input type="hidden" name="mn_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</fieldset>
		<?php
	}
