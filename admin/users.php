...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="https://mtaandao.github.io/Users_Screen">Documentation on Managing Users</a>') . '</p>' .
    '<p>' . __('<a href="https://mtaandao.github.io/Roles_and_Capabilities">Descriptions of Roles and Capabilities</a>') . '</p>' .
...
case 'promote':
	...
	if ( ! $role || empty( $editable_roles[ $role ] ) ) {
		mn_die( __( 'Sorry, you are not allowed to give users that role.' ) );
	}
...
case 'dodelete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		mn_die(__('Sorry, you are not allowed to delete users.'));
	...
	foreach ( $userids as $id ) {
		if ( ! current_user_can( 'delete_user', $id ) )
			mn_die(__( 'Sorry, you are not allowed to delete that user.' ) );
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new MN_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
...
case 'doremove':
	...
	if ( ! current_user_can( 'remove_users' ) )
		mn_die( __( 'Sorry, you are not allowed to remove users.' ) );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new MN_Error('edit_users', __('Sorry, you are not allowed to remove users.'));
...
default:
	...
	if ( $mn_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
		$userids = $_REQUEST['users'];
		$sendback = mn_get_referer();

		/** This action is documented in admin/edit-comments.php */
		$sendback = apply_filters( 'handle_bulk_actions-' . get_current_screen()->id, $sendback, $mn_list_table->current_action(), $userids );
