...
+     */
    ...
    if (_this.browser.ie10 && !params.onlyExternal) {
        _this.wrapper.classList.add('swiper-mn8-' + (isH ? 'horizontal' : 'vertical'));
    }
...
jQuery(document).ready(function($){ 
	...
	function parallaxSliderPos(){
		if(parallaxSlider.parent().attr('class') == 'mnb_wrapper') { 

            //give the slider the custom row ID if assigned
            if(parallaxSlider.parents('.mnb_row').length > 0) {
                if(parallaxSlider.parents('.mnb_row').not('[id^="fws_"]').length > 0) parallaxSlider.attr('id',parallaxSlider.parents('.mnb_row').attr('id'));
            } 
            //remove the row
            parallaxSlider.parents('.mnb_row').remove(); 
        }
        //alter the DOM pos
    ...
    function nectarSliderMainInit() { 
        ...
        $('.nectar-slider-wrap[data-full-width="boxed-full-width"]').each(function(){ if($(this).parents('.mnb_column').length > 0 && $(this).parents('.full-width-content').length > 0) $(this).attr('data-full-width','false') });
	...
	function dynamicHeightSliders(){
		...
		var $adminBarHeight = ($('#mnadminbar').length > 0) ? 28 : 0 ;
			...
			$('.nectar-slider-wrap').each(function(){
				...
				if($(this).attr('data-fullscreen') == 'true' && $(this).attr('data-full-width') == 'true' || $(this).attr('data-fullscreen') == 'true' && $(this).attr('data-full-width') == 'boxed-full-width') {
                    ...
                    if($(this).parents('#nectar_fullscreen_rows').length == 0) {
    					...
    					if($(this).hasClass('first-section') && $(this).index() == 0 || $(this).parents('.mnb_row').length > 0 && $(this).parents('.mnb_row').hasClass('first-section') && $(this).index() == 0 && $(this).parents('.parallax_slider_outer').length == 0){
				...
				if($(this).attr('data-flexible-height') == 'true' && $(this).attr('data-fullscreen') != 'true') {
					...
					if($(this).parents('.mnb_row.full-width-content').length == 0 && $(this).attr('data-full-width') == 'false') { $(this).attr('data-flexible-height','false'); return false; }
