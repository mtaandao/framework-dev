    	...
    	<div class="iosSlider product-slider">
			...
			<div class="slider">
				...
				<?php if (has_post_thumbnail()) { 
					...
					$img_link = mn_get_attachment_url( get_post_thumbnail_id($post->ID) );
                ...
                <div class="slide">
                	<div class="easyzoom">
	                	<a href="<?php echo $img_link; ?>">
	                		<?php echo mn_get_attachment_image( get_post_thumbnail_id($post->ID), 'shop_single'); ?>
	                	</a>
	                </div>
                </div>
				...
				if ( $product_attach_ids ) {
					...
					foreach ($product_attach_ids as $product_attach_id) {
						...
						$img_link = mn_get_attachment_url( $product_attach_id );
			
						if (!$img_link)
							continue;

						printf( '<div class="slide"><div class="easyzoom"><a href="%s" title="%s"> %s </a></div></div>', mn_get_attachment_url($product_attach_id),get_post($product_attach_id)->post_title, mn_get_attachment_image($product_attach_id, 'shop_single'));
        ...
        <div class="iosSlider product-thumbs" style="min-height:<?php echo ($img_size['height']).'px'; ?>!important">
			<div class ="slider">
						...
						foreach ( $product_attach_ids as $product_attach_id) {
							...
							$img_size = mn_get_attachment_image($product_attach_id, apply_filters('single_product_small_thumbnail_size', 'shop_thumbnail'));
							$classes = array();
