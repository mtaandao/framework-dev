	...
	<td>
		<?php if ( ! empty( $available_methods ) ) : ?>
			...
			<?php if ( 1 === count( $available_methods ) ) :
				...
				echo mn_kses_post( wc_cart_totals_shipping_method_label( $method ) ); ?>
			...
			<?php elseif ( get_option( 'woocommerce_shipping_method_format' ) === 'select' ) : ?>
				...
				<select name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>" class="shipping_method">
					<?php foreach ( $available_methods as $method ) : ?>
						<option value="<?php echo esc_attr( $method->id ); ?>" <?php selected( $method->id, $chosen_method ); ?>><?php echo mn_kses_post( wc_cart_totals_shipping_method_label( $method ) ); ?></option>
					<?php endforeach; ?>
				</select>
				...
				<ul id="shipping_method">
					<?php foreach ( $available_methods as $method ) : ?>
						<li>
							...
							<label for="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>"><?php echo mn_kses_post( wc_cart_totals_shipping_method_label( $method ) ); ?></label>
						</li>
					<?php endforeach; ?>
				</ul>
