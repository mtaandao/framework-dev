...
function pn_get_attachment_id_from_url( $attachment_url = '' ) {
	...
	$upload_dir_paths = mn_upload_dir();
	...
	if ( false !== strpos( $attachment_url, $upload_dir_paths['baseurl'] ) ) {
		...
		$attachment_id = $mndb->get_var( $mndb->prepare( "SELECT mnosts.ID FROM $mndb->posts mnosts, $mndb->postmeta mnostmeta WHERE mnosts.ID = mnostmeta.post_id AND mnostmeta.meta_key = '_mn_attached_file' AND mnostmeta.meta_value = '%s' AND mnosts.post_type = 'attachment'", $attachment_url ) );
...
class Recent_Projects_Widget extends MN_Widget {
	...
	function widget($args, $instance) {
		...
		$cache = mn_cache_get('recent_projects_widget', 'widget');
		...
		$r = new MN_Query(array('post_type' => 'portfolio', 'posts_per_page' => $number));
		if ($r->have_posts()) :
?>
		...
		<div>
			...
			<a href="<?php echo $the_project_link; ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>">
				...
				if ( has_post_thumbnail() ) {
					...
					echo mn_get_attachment_image($attachment_id,'portfolio-widget');
				}
					else {
...
<?php
			mn_reset_query();  // Restore global post data stomped by the_post().
		endif;

		$cache[$args['widget_id']] = ob_get_flush();
		mn_cache_add('recent_projects_widget', $cache, 'widget');
	}
	...
	function flush_widget_cache() {
		mn_cache_delete('recent_projects_widget', 'widget');
	}
