...
if(!function_exists('mn_get_attachment')) {
	function mn_get_attachment( $attachment_id ) {
...
$flex_fx = '';
if ( $type == 'flexslider_style' ) {
    $type = ' mnb_flexslider flex-gallery flexslider';
...
foreach ( $images as $attach_id ) {
    ...
    switch ( $source ) {
		case 'media_library':
			 if ($attach_id > 0) {
		        $post_thumbnail = mnb_getImageBySize(array( 'attach_id' => $attach_id, 'thumb_size' => $img_size ));
		        $fullsize_image = mn_get_attachment_image_src($attach_id, 'full');
	...
	if($type == 'nectarslider_style') {
		...
		switch ( $source ) {
			case 'media_library':
				$img = mn_get_attachment_image_src(  $attach_id, $img_size );
				break;
	...
	if($type == 'flickity_style') {
		...
		switch ( $source ) {
			case 'media_library':
				if ($attach_id > 0) {
			        $post_thumbnail = mnb_getImageBySize(array( 'attach_id' => (int) $attach_id, 'thumb_size' => $img_size ));
			    }
			    else {
			        $post_thumbnail = array();
				...
				ob_start(); ?>
						...
						<?php //project style 1
							...
							<div class="work-item">
								...
								<div class="work-info"> 
									...
									<?php 
										if($attach_id > 0) { 
											$attachment_meta = mn_get_attachment($attach_id);
						...
						<?php } //project style 2 
							...
							<div class="work-item style-3">
								...
								if($attach_id > 0) $attachment_meta = mn_get_attachment($attach_id);
...
$css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'mnb_gallery mnb_content_element'.$el_class.' clearfix', $this->settings['base']);
$output .= "\n\t".'<div class="'.$css_class.'">';
$output .= "\n\t\t".'<div class="mnb_wrapper">';
$output .= mnb_widget_title(array('title' => $title, 'extraclass' => 'mnb_gallery_heading'));
$output .= '<div class="mnb_gallery_slides'.$type.'" data-interval="'.$interval.'"'.$flex_fx.'>'.$slides_wrap_start.$gal_images.$slides_wrap_end.'</div>';
$output .= "\n\t\t".'</div> '.$this->endBlockComment('.mnb_wrapper');
$output .= "\n\t".'</div> '.$this->endBlockComment('.mnb_gallery');
