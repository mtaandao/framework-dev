...
if(!empty($image_url_1)) {
	...
	$bg_image_src = mn_get_attachment_image_src($image_url_1, 'full');
	$style .= 'background-image: url(\''.$bg_image_src[0].'\'); ';
}

if(!empty($image_url_2)) {
	
	$bg_image_src_2 = mn_get_attachment_image_src($image_url_2, 'full');
...
if($icon_family == 'linea') mn_enqueue_style('linea'); 
