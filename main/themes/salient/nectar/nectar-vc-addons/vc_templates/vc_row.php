<?php
   ...
   extract(shortcode_atts(array(
	...
	mn_enqueue_script( 'mnb_composer_front_js' );
	mn_enqueue_style('js_composer_custom_css');
	
	if($mouse_based_parallax_bg == 'true'){
		mn_enqueue_script('nectar_parallax');
	}
	...
	if(!empty($bg_image)) {
		...
		if(!preg_match('/^\d+$/',$bg_image)){
			...
			$bg_image_src = mn_get_attachment_image_src($bg_image, 'full');
    ...
    echo'
	<div id="'.$row_id.'" '.$fullscreen_anchor_id.' data-midnight="'.strtolower($midnight_color).'" data-bg-mobile-hidden="'.$background_image_mobile_hidden.'" class="mnb_row vc_row-fluid vc_row '. $main_class . $disable_class . $equal_height_class . $parallax_class . ' ' . $vertically_center_class . ' '. $class . ' " '.$using_custom_text_color.' style="'.$style.'">';
	...
	if($mouse_based_parallax_bg == 'true') {
        ...
        if(!empty($layer_one_image)) {
        	if(!preg_match('/^\d+$/',$layer_one_image)){
        		...
        		$layer_one_image_src = mn_get_attachment_image_src($layer_one_image, 'full');
        ...
        if(!empty($layer_two_image)) {
        	...
        	if(!preg_match('/^\d+$/',$layer_two_image)){
        		...
        		$layer_two_image_src = mn_get_attachment_image_src($layer_two_image, 'full');
        ...
        if(!empty($layer_three_image)) {
        	...
        	if(!preg_match('/^\d+$/',$layer_three_image)){
        		...
        		$layer_three_image_src = mn_get_attachment_image_src($layer_three_image, 'full');
        ...
        if(!empty($layer_four_image)) {
        	...
        	if(!preg_match('/^\d+$/',$layer_four_image)){
        		...
        		$layer_four_image_src = mn_get_attachment_image_src($layer_four_image, 'full');
        ...
        if(!empty($layer_five_image)) {
        	...
        	if(!preg_match('/^\d+$/',$layer_five_image)){
        		...
        		$layer_five_image_src = mn_get_attachment_image_src($layer_five_image, 'full');
