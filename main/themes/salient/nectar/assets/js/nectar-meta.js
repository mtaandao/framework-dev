...
/*----------------------------------------------------------------------------------*/
	...
	$('.mn-post-format-ui .post-format-options > a').click(checkFormat);
	...
	function checkFormat(){
		...
		// >= MN 3.6 
			...
			var format = $(this).attr('data-mn-format');
			
			if( typeof format == 'undefined' && $('a[data-mn-format="gallery"]').hasClass('active')){
				format = $('a[data-mn-format="gallery"]').attr('data-mn-format');
			}
	...
	if($('.mn-post-format-ui .post-format-options').length > 0 ) {
	...
	if( $('#nectar-metabox-home-slider').length > 0 ){
		$('#preview-action, #admin-bar-view').hide();
    ...
    function toggleParallaxOption(){
    	...
    	if($('#_nectar_header_bg_color').length > 0 && $('#_nectar_header_bg').length > 0) {
	    	$('.mn-picker-holder, .mn-picker-clear, .iris-slider, .iris-square, .nectar-metabox-table').click(function(){
    	...
    	if($('input#post_type').length > 0 && $('input#post_type').attr('value') == 'portfolio'){
		    ...
		    if($('#nectar-metabox-project-configuration ._nectar_portfolio_custom_grid_item .ui-state-active').attr('for') == 'nectar_meta_on') {
		    	...
		    	if($('#nectar-metabox-portfolio-extra').css('display') == 'none') {
		    		$('.composer-switch,  #mnb_visual_composer').removeClass('vc-aspect-hidden');
		    		$('#nectar-metabox-portfolio-extra').addClass('vc-aspect-hidden');
		    		$('.composer-switch,  #mnb_visual_composer').hide();
		    	}
		    	else { 
		    		$('#nectar-metabox-portfolio-extra').removeClass('vc-aspect-hidden');
		    		$('.composer-switch,  #mnb_visual_composer').addClass('vc-aspect-hidden');
		    	...
		    	$('#mn-_nectar_portfolio_custom_grid_item_content-wrap').parents('tr').show();
		    	$('#nectar-metabox-portfolio-extra .hndle span').html('Full Width Content');
		    	$('#nectar-metabox-portfolio-extra .inside > p:not(.composer-switch)').html('Please enter your portfolio item content here - all nectar shortcodes are available for use.');
			    ...
			    $('.composer-switch:not(.vc-aspect-hidden),  #mnb_visual_composer:not(.vc-aspect-hidden), #nectar-metabox-portfolio-extra:not(.vc-aspect-hidden)').fadeIn(500);
		    	...
		    	setTimeout(function(){ $(window).trigger('resize'); },700);
		    	$('#mn-_nectar_portfolio_custom_grid_item_content-wrap').parents('tr').fadeOut(500);
		    	$('#nectar-metabox-portfolio-extra .hndle span').html('Extra Content');
		    	$('#nectar-metabox-portfolio-extra .inside > p:not(.composer-switch)').html('Please use this section to place any extra content you would like to appear in the main content area under your portfolio item. (The above default editor is only used to populate your items sidebar content)');
		    }
    ...
    $('#nectar_slider_canvas_shape #edit-gallery').click(function(){ $('body').addClass('particle-edit'); });
     ...
     if($('#adminmenu .mn-has-current-submenu').length > 0 && $('#adminmenu a.mn-has-current-submenu').css('background-color') == 'rgb(0, 116, 162)' ||
     $('#adminmenu > .current').length > 0 && $('#adminmenu > .current a.current').css('background-color') == 'rgb(0, 116, 162)'){
     	...
     	if($('#toplevel_page_redux_options').length > 0) {
	     	var preloadSrc = $('#toplevel_page_redux_options > a .mn-menu-image img').attr('src');
	     	preloadSrc = preloadSrc.replace('.svg','-hover.svg');
	     	var preloadHoverImage = new Image()
			preloadHoverImage.src = preloadSrc;
		}
		
     	$('#toplevel_page_redux_options:not(.mn-has-current-submenu)').hover(function(){
     		$hoverSrc = $(this).find('> a .mn-menu-image img').attr('src');
     		$hoverSrc = $hoverSrc.replace('.svg','-hover.svg');
     		$(this).find('>a .mn-menu-image img').attr('src',$hoverSrc);
     	},function(){
     		$hoverSrc = $(this).find('>a .mn-menu-image img').attr('src');
     ...
     if($('#adminmenu .mn-has-current-submenu').length > 0 && $('#adminmenu a.mn-has-current-submenu').css('background-color') == 'rgb(136, 136, 136)' ||
     $('#adminmenu > .current').length > 0 && $('#adminmenu > .current a.current').css('background-color') == 'rgb(136, 136, 136)'){
     	...
     	$('#toplevel_page_redux_options:not(.mn-has-current-submenu) > a').hover(function(){
     		$hoverSrc = $(this).find('.mn-menu-image img').attr('src');
     		$hoverSrc = $hoverSrc.replace('-grey.svg','.svg');
     		$(this).find('.mn-menu-image img').attr('src',$hoverSrc).css('opacity',0.6);
     	},function(){
     		$hoverSrc = $(this).find('.mn-menu-image img').attr('src');
     		$hoverSrc = $hoverSrc.replace('.svg','-grey.svg');
     		$(this).find('.mn-menu-image img').attr('src',$hoverSrc).css('opacity',0.7);
     	});
     	
     	$('#toplevel_page_redux_options.mn-has-current-submenu > a .mn-menu-image img').attr('src',preloadSrc).css('opacity',0.6);
     	
     }
     
     
     //page builder starting categories
     $('body').on('change','.edit_form_line select.mnb_vc_param_value.dropdown_multi.category[name="category"]', function(){
