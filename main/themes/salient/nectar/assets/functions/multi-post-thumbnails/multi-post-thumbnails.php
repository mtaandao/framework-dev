...
Plugin URI: http://mtaandao.org/extend/plugins/multiple-post-thumbnails/
...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			add_action("mn_ajax_set-{$this->post_type}-{$this->id}-thumbnail", array($this, 'set_thumbnail'));
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$query_vars = mn_parse_args(parse_url($referer, PHP_URL_QUERY));
			
			if( (isset($_REQUEST['context']) && $_REQUEST['context'] != $this->id) || (isset($query_vars['context']) && $query_vars['context'] != $this->id) )
				return $form_fields;

			$ajax_nonce = mn_create_nonce("set_post_thumbnail-{$this->post_type}-{$this->id}-{$calling_post_id}");
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			add_thickbox();
			mn_enqueue_script( "featured-image-custom", $this->plugins_url( 'js/multi-post-thumbnails-admin.js', __FILE__ ), array( 'jquery', 'media-upload' ) );
		}
		...
		public function action_delete_attachment($post_id) {
			...
			$mndb->query( $mndb->prepare( "DELETE FROM $mndb->postmeta WHERE meta_key = '%s' AND meta_value = %d", $this->get_meta_key(), $post_id ));
		}
		...
		public static function get_the_post_thumbnail($post_type, $thumb_id, $post_id = NULL, $size = 'post-thumbnail', $attr = '' , $link_to_original = false) {
			...
			if ($post_thumbnail_id) {
				do_action("begin_fetch_multi_{$post_type}_thumbnail_html", $post_id, $post_thumbnail_id, $size); // for "Just In Time" filtering of all of mn_get_attachment_image()'s filters
			...
			if ($link_to_original && $html) {
				$html = sprintf('<a href="%s">%s</a>', mn_get_attachment_url($post_thumbnail_id), $html);
			}
		 ...
		 * @param string $size Optional. The thumbnail size to use. If set, use mn_get_attachment_image_src() instead of mn_get_attachment_url()
		...
		public static function get_post_thumbnail_url($post_type, $id, $post_id = 0, $size = null) {
			...
			if ($size) {
				if ($url = mn_get_attachment_image_src($post_thumbnail_id, $size)) {
					$url = $url[0];
				} else {
					$url = '';
				}
			} else {
				$url = mn_get_attachment_url($post_thumbnail_id);
			}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			global $content_width, $_mn_additional_image_sizes, $post_ID;
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				$content_width = 266;
				if ( !isset($_mn_additional_image_sizes["{$this->post_type}-{$this->id}-thumbnail"]))
					$thumbnail_html = mn_get_attachment_image($thumbnail_id, array($content_width, $content_width));
				else
					$thumbnail_html = mn_get_attachment_image($thumbnail_id, "{$this->post_type}-{$this->id}-thumbnail");
				if (!empty($thumbnail_html)) {
					$ajax_nonce = mn_create_nonce("set_post_thumbnail-{$this->post_type}-{$this->id}-{$post_ID}");
		...
		public function set_thumbnail() {
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				$thumbnail_html = mn_get_attachment_image($thumbnail_id, 'thumbnail');
				if (!empty($thumbnail_html)) {
