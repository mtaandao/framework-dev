<?php
    ...
    function myprefix_autocomplete_init() {  
        ...
        mn_register_script( 'my_acsearch', get_template_directory_uri() . '/nectar/assets/functions/ajax-search/mnss-search-suggest.js', array('jquery','jquery-ui-autocomplete'),null,true);  
        mn_localize_script( 'my_acsearch', 'MyAcSearch', array('url' => admin_url( 'admin-ajax.php' ))); 
        ...
        add_action( 'mn_ajax_myprefix_autocompletesearch', 'myprefix_autocomplete_suggestions' );  
        add_action( 'mn_ajax_nopriv_myprefix_autocompletesearch', 'myprefix_autocomplete_suggestions' );  
    }  
	
	function myprefix_autocomplete_search_form(){  
        mn_enqueue_script( 'my_acsearch' );  
    }  
        ...
        foreach ($posts as $post): setup_postdata($post);  
			...
			if(get_post_type($post->ID) == 'post'){
				...
				if(!empty($custom_thumbnail) ){
					...
					$suggestion['image'] = mn_get_attachment_image($attachment_id,'portfolio-widget');
				}
