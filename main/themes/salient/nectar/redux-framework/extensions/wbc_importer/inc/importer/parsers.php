...
class WXR_Parser {
	function parse( $file ) {
		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . __( 'There was an error when reading this WXR file', 'mtaandao-importer' ) . '</strong><br />';
			echo __( 'Details are shown above. The importer will now try again with a different parser...', 'mtaandao-importer' ) . '</p>';
		}
...
class WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new MN_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'mtaandao-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version )
			return new MN_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'mtaandao-importer' ) );
		...
		if ( ! isset( $namespaces['mn'] ) )
			$namespaces['mn'] = 'http://mtaandao.org/export/1.1/';
		if ( ! isset( $namespaces['excerpt'] ) )
			$namespaces['excerpt'] = 'http://mtaandao.org/export/1.1/excerpt/';
