<?php
    ...
    if ( $this->parent->dev_mode_forced ) {
        ...
        if ( Redux_Helpers::isWpDebug() ) {
            $is_debug  = true;
            $debug_bit = __( 'MN_DEBUG is enabled', 'redux-framework' );
        }
...
<div id="redux-header">
    <?php if ( ! empty( $this->parent->args['display_name'] ) ) { ?>
        <div class="display_header">
            ...
            <h2><?php echo mn_kses_post( str_replace('Theme', '', $this->parent->args['display_name']) ); ?></h2>

            <?php if ( ! empty( $this->parent->args['display_version'] ) ) { ?>
                <span><?php echo mn_kses_post( $this->parent->args['display_version'] ); ?></span>
            <?php } ?>
