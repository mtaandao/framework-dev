...
<div class="wrap about-wrap redux-status">
    ...
    <table class="redux_status_table widefat" cellspacing="0" id="status">
        <thead>
        <tr>
            <th colspan="3" data-export-label="Mtaandao Environment">
                <?php esc_html_e( 'Mtaandao Environment', 'redux-framework' ); ?>
            </th>
        </tr>
        </thead>
        <tbody>
        ...
        <tr>
            <td data-export-label="MN Content URL">
                <?php esc_html_e( 'MN Content URL', 'redux-framework' ); ?>:
            </td>
            <td class="help">
                <?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'The location of Wordpress\'s content URL.', 'redux-framework' ) . '">[?]</a>'; ?>
            </td>
            <td>
                <?php echo '<code>' . esc_url($sysinfo['mn_content_url']) . '</code> '; ?>
            </td>
        </tr>        
        <tr>
            <td data-export-label="MN Version">
                <?php esc_html_e( 'MN Version', 'redux-framework' ); ?>:
            </td>
            <td class="help">
                <?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'The version of Mtaandao installed on your site.', 'redux-framework' ) . '">[?]</a>'; ?>
            </td>
            <td>
        ...
        <tr>
            <td data-export-label="MN Multisite">
                <?php esc_html_e( 'MN Multisite', 'redux-framework' ); ?>:
            </td>
            <td class="help">
                <?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'Whether or not you have Mtaandao Multisite enabled.', 'redux-framework' ) . '">[?]</a>'; ?>
            </td>
            <td><?php if ( $sysinfo['mn_multisite'] == true ) {
                    echo '&#10004;';
                } else {
                    echo '&ndash;';
                } ?>
...
?>
        <tr>
            <td data-export-label="MN Memory Limit">
                <?php esc_html_e( 'MN Memory Limit', 'redux-framework' ); ?>:
            </td>
            <td class="help">
            ...
            <td>
<?php
                    $memory = $sysinfo['mn_mem_limit']['raw'];

                    if ( $memory < 40000000 ) {
                        echo '<mark class="error">' . sprintf( __( '%s - We recommend setting memory to at least 40MB. See: <a href="%s" target="_blank">Increasing memory allocated to PHP</a>', 'redux-framework' ), esc_html($sysinfo['mn_mem_limit']['size']), 'http://codex.mtaandao.org/Editing_mn-config.php#Increasing_memory_allocated_to_PHP' ) . '</mark>';
                    } else {
                        echo '<mark class="yes">' . esc_html($sysinfo['mn_mem_limit']['size']) . '</mark>';
                    }
?>
            </td>
        </tr>
        ...
        <tr>
            <td data-export-label="MN Debug Mode">
                <?php esc_html_e( 'MN Debug Mode', 'redux-framework' ); ?>:
            </td>
            <td class="help">
                <?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'Displays whether or not Mtaandao is in Debug Mode.', 'redux-framework' ) . '">[?]</a>'; ?>
            </td>
            <td>
                <?php if ( $sysinfo['mn_debug'] === 'true' ) {
        ...
        <tr>
            ...
            <td class="help">
                <?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'The current language used by Mtaandao. Default = English', 'redux-framework' ) . '">[?]</a>'; ?>
            </td>
            <td>
                <?php echo esc_html($sysinfo['mn_lang']); ?>
            </td>
        </tr>
        </tbody>
    </table>
...
?>            
        ...
        <tr>
            ...
            <td class="help"><?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'The largest filesize that can be uploaded to your Mtaandao installation.', 'redux-framework' ) . '">[?]</a>'; ?></td>
...
?>
        ...
        <?php
            ...
            */
            ...
            $posting['mn_remote_post']['name'] = esc_html__( 'Remote Post', 'redux-framework' );
            $posting['mn_remote_post']['help'] = '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'Used to send data to remote servers.', 'redux-framework' ) . '">[?]</a>';

            if ( $sysinfo['mn_remote_post'] === 'true' ) {
                $posting['mn_remote_post']['success'] = true;
            } else {
                $posting['mn_remote_post']['note'] = esc_html__( 'mn_remote_post() failed. Many advanced features may not function. Contact your hosting provider.', 'redux-framework' );

                if ( $sysinfo['mn_remote_post_error'] ) {
                    $posting['mn_remote_post']['note'] .= ' ' . sprintf( __( 'Error: %s', 'redux-framework' ), redux_clean( $sysinfo['mn_remote_post_error'] ) );
                }
            ...
            // MN Remote Get Check
            $posting['mn_remote_get']['name'] = esc_html__( 'Remote Get', 'redux-framework' );
            $posting['mn_remote_get']['help'] = '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'Used to grab information from remote servers for updates updates.', 'redux-framework' ) . '">[?]</a>';

            if ( $sysinfo['mn_remote_get'] === 'true' ) {
                $posting['mn_remote_get']['success'] = true;
            } else {
                $posting['mn_remote_get']['note'] = esc_html__( 'mn_remote_get() failed. This is needed to get information from remote servers. Contact your hosting provider.', 'redux-framework' );
                if ( $sysinfo['mn_remote_get_error'] ) {
                    $posting['mn_remote_get']['note'] .= ' ' . sprintf( __( 'Error: %s', 'redux-framework' ), redux_clean( $sysinfo['mn_remote_get_error'] ) );
                }
                ...
                <tr>
                    ...
                    <td class="help">
                        <mark class="<?php echo esc_attr($mark); ?>">
                            ...
                            <?php echo ! empty( $post['note'] ) ? mn_kses_data( $post['note'] ) : ''; ?>
                        </mark>
                    </td>
                </tr>
            <?php
            ...
            foreach ( $sysinfo['redux_instances'] as $inst => $data ) {
                ...
                <table class="redux_status_table widefat" cellspacing="0" id="status">
                    ...
                    <tr>
                        ...
                        <td class="help"><?php echo '<a href="#" class="redux-hint-qtip" qtip-content="' . esc_attr__( 'The page parent variable sets where the options menu will be placed on the Mtaandao admin sidebar for this instance of Redux.', 'redux-framework' ) . '">[?]</a>'; ?></td>
...
?>
        ...
        <tr>
            ...
            <td>
<?php
                echo is_child_theme() ? '<mark class="yes">' . '&#10004;' . '</mark>' : '&#10005; <br /><em>' . sprintf( __( 'If you\'re modifying Redux Framework or a parent theme you didn\'t build personally, we recommend using a child theme. See: <a href="%s" target="_blank">How to create a child theme</a>', 'redux-framework' ), 'http://codex.mtaandao.org/Child_Themes' ) . '</em>';
?>
            </td>
        </tr>
<?php
