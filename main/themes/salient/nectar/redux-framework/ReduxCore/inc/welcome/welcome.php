<?php
    ...
    class Redux_Welcome {
        ...
        public function __construct() {
            ...
            add_action( 'mn_ajax_redux_support_hash', array( $this, 'support_hash' ) );
        ...
        public function init() {
            ...
            if ( isset( $_GET['page'] ) ) {
                if ( substr( $_GET['page'], 0, 6 ) == "redux-" ) {
                    ...
                    add_filter( 'admin_footer_text', array( $this, 'change_mn_footer' ) );
        ...
        public function check_version() {
            ...
            if ( $compare ) {
                ...
                if ( $redirect && ! defined( 'MN_TESTS_DOMAIN' ) ) {
        ...
        public function do_redirect() {
            if ( !defined('MN_CLI')) {
                //mn_redirect( admin_url( 'tools.php?page=redux-about' ) );
                //exit();
            }
        }

        public function change_mn_footer() {
            echo __('If you like <strong>Redux</strong> please leave us a <a href="' . 'https://' . 'mtaandao.org/support/view/plugin-reviews/redux-framework?filter=5#postform" target="_blank" class="redux-rating-link" data-rated="Thanks :)">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating. A huge thank you from Redux in advance!','redux-framework');
        }

        public function support_hash() {

            if ( ! mn_verify_nonce( $_POST['nonce'], 'redux-support-hash' ) ) {
                die();
            }

            $data          = get_option( 'redux_support_hash' );
            $data          = mn_parse_args( $data, array( 'check' => '', 'identifier' => '' ) );
            $generate_hash = true;
            ...
            if ( isset( $generate_hash ) && $generate_hash ) {
                $data['check']      = $newHash;
                $data['identifier'] = "";
                $response           = mn_remote_post( 'http://' . 'support.redux.io/v1/', array(
                        'method'      => 'POST',
                        'timeout'     => 65,
                        'redirection' => 5,
                        'httpversion' => '1.0',
                ...
                if ( is_mn_error( $response ) ) {
                    ...
                    if ( $response_code == 200 ) {
                        $response = mn_remote_retrieve_body( $response );
        ...
        public function tabs() {
            ...
            $nonce    = mn_create_nonce( 'redux-support-hash' );
            ?>
        ...
        public function parse_readme() {
            if ( file_exists( ReduxFramework::$_dir . 'inc/fields/raw/parsedown.php' ) ) {
                ...
                return $Parsedown->text( trim( str_replace( '# Redux Framework Changelog', '', mn_remote_retrieve_body( mn_remote_get( ReduxFramework::$_url . '../CHANGELOG.md' ) ) ) ) );
            }
        ...
        public function actions() {
            ...
            <p class="redux-actions">
                ...
                <a href="http://mtaandao.org/plugins/redux-framework/" class="review-us button button-primary"
                ...
                <a href="https://twitter.com/share" class="twitter-share-button" data-url="http://reduxframework.com"
                    data-text="Reduce your dev time! Redux is the most powerful option framework for Mtaandao on the web"
