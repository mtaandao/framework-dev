    ...
    if ( ! class_exists( 'Redux_Helpers' ) ) {
        ...
        class Redux_Helpers {
            ...
            public static function isWpDebug() {
                return ( defined( 'MN_DEBUG' ) && MN_DEBUG == true );
            }
            ...
            public static function getTrackingObject() {
                ...
                if ( ! function_exists( 'get_plugin_data' ) ) {
                    if ( file_exists( ABSPATH . 'admin/includes/plugin.php' ) ) {
                        require_once ABSPATH . 'admin/includes/plugin.php';
                    }
                    if ( file_exists( ABSPATH . 'admin/includes/admin.php' ) ) {
                ...
                $data['environment']['mysql'] = $mndb->db_version();
...
//                }
            ...
            public static function localize( $localize ) {
                ...
                $nonce            = mn_create_nonce( 'redux-ads-nonce' );
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
                ...
                $sysinfo['mn_content_url']       = MN_CONTENT_URL;
                $sysinfo['mn_ver']               = get_bloginfo( 'version' );
                $sysinfo['mn_multisite']         = is_multisite();
                ...
                $sysinfo['mn_mem_limit']['raw']  = self::let_to_num( MN_MEMORY_LIMIT );
                $sysinfo['mn_mem_limit']['size'] = size_format( $sysinfo['mn_mem_limit']['raw'] );

                $sysinfo['db_table_prefix'] = 'Length: ' . strlen( $mndb->prefix ) . ' - Status: ' . ( strlen( $mndb->prefix ) > 16 ? 'ERROR: Too long' : 'Acceptable' );

                $sysinfo['mn_debug'] = 'false';
                if ( defined( 'MN_DEBUG' ) && MN_DEBUG ) {
                    $sysinfo['mn_debug'] = 'true';
                }

                $sysinfo['mn_lang'] = get_locale();
                ...
                $sysinfo['mysql_ver']         = $mndb->db_version();
                $sysinfo['max_upload_size']   = size_format( mn_max_upload_size() );
                ...
                if ( $remote_checks == true ) {
                    $response = mn_remote_post( 'https://www.paypal.com/cgi-bin/webscr', array(
                        'sslverify'  => false,
                        'timeout'    => 60,
                    ...
                    if ( ! is_mn_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
                        $sysinfo['mn_remote_post']       = 'true';
                        $sysinfo['mn_remote_post_error'] = '';
                    } else {
                        $sysinfo['mn_remote_post']       = 'false';
                        $sysinfo['mn_remote_post_error'] = $response->get_error_message();
                    }

                    $response = mn_remote_get( 'http://reduxframework.com/admin/admin-ajax.php?action=get_redux_extensions' );

                    if ( ! is_mn_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
                        $sysinfo['mn_remote_get']       = 'true';
                        $sysinfo['mn_remote_get_error'] = '';
                    } else {
                        $sysinfo['mn_remote_get']       = 'false';
                        $sysinfo['mn_remote_get_error'] = $response->get_error_message();
                    }
                }
                    ...
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'redux-framework' ), str_replace( MN_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
                                $found_files[ $plugin_name ][] = sprintf( '<code>%s</code>', str_replace( MN_CONTENT_DIR . '/themes/', '', $theme_file ) );
                            }
                        }
                    }
                }
