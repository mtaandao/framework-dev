    ...
    if ( ! class_exists( 'Redux_Validation_unique_slug' ) ) {
        class Redux_Validation_unique_slug {
            ...
            function validate() {
                ...
                $check_sql       = "SELECT post_name FROM $mndb->posts WHERE post_name = %s LIMIT 1";
                $post_name_check = $mndb->get_var( $mndb->prepare( $check_sql, $slug ) );
                 ...
                 */
                if ( $post_name_check || in_array( $slug, $feeds ) || apply_filters( 'mn_unique_post_slug_is_bad_attachment_slug', false, $slug ) ) {
                    $suffix = 2;
                    do {
                        $alt_post_name   = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
                        $post_name_check = $mndb->get_var( $mndb->prepare( $check_sql, $alt_post_name ) );
                        $suffix ++;
