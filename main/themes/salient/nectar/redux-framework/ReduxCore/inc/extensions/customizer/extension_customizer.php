    ...
    if ( ! class_exists( 'ReduxFramework_extension_customizer' ) ) {
        ...
        class ReduxFramework_extension_customizer {
            ...
            public function __construct( $parent ) {
                ...
                global $pagenow, $mn_customize;
                if ( ! isset( $mn_customize ) && $pagenow !== "customize.php" && $pagenow !== "admin-ajax.php" ) {
                    return;
                }
                ...
                if ( isset( $_POST['mn_customize'] ) && $_POST['mn_customize'] == "on" ) {
                    $this->parent->args['customizer_only'] = true;
                }

                if ( isset( $_POST['mn_customize'] ) && $_POST['mn_customize'] == "on" && isset( $_POST['customized'] ) && ! empty( $_POST['customized'] ) && ! isset( $_POST['action'] ) ) {
            ...
            function enqueue_controls_css() {
                ...
                $enqueue->init();
                mn_enqueue_style( 'redux-extension-advanced-customizer', $this->_extension_url . 'extension_customizer.css', '', time() );
                ...
                mn_localize_script( 'redux-extension-customizer', 'redux_customizer', array( 'body_class' => sanitize_html_class( 'admin-color-' . get_user_option( 'admin_color' ), 'fresh' ) ) );
            }
            ...
            public function _register_customizer_controls( $mn_customize ) {
                ...
                if ( ! class_exists( 'Redux_Customizer_Section' ) ) {
                    ...
                    if ( method_exists( $mn_customize, 'register_section_type' ) ) {
                        $mn_customize->register_section_type( 'Redux_Customizer_Section' );
                    }
                }
                if ( ! class_exists( 'Redux_Customizer_Panel' ) ) {
                    require_once dirname( __FILE__ ) . '/inc/customizer_panel.php';
                    if ( method_exists( $mn_customize, 'register_panel_type' ) ) {
                        $mn_customize->register_panel_type( 'Redux_Customizer_Panel' );
                    }
                }
                ...
                //    $section = new Redux_Customizer_rAds( $mn_customize, 'redux_rAds', array(
                //        'priority'    => 0,
                //    ) );
                //    $mn_customize->add_section( $section, array(
                //        'priority'    => 0,
                //    ) );
                //
                //    //$mn_customize->add_control( new Redux_Customizer_Control_rAds( $mn_customize, 'reduxAdsDisplay', array(
                //    //    'section'        => 'redux_rAds',
                ...
                if ( $this->parent->args['dev_mode'] ) {
                    ...
                    //    'priority'    => 0,
                    //);
                    ////$section = new Redux_Customizer_Section( $mn_customize, 'redux_rAds', $args );
                    ////$mn_customize->add_section( $section, $args );
                    ...
                    //), $mn_customize );
                    //
                    //$mn_customize->add_control( new MN_Customize_Color_Control(
                    //    $mn_customize,
                    //    'redux_rAds_display',
                    //    array(
                    //        'section'    => 'redux_rAds',
                    ...
                    //$section = new Redux_Customizer_section_rAds( $mn_customize, 'redux_rAds', array(
                    //    'priority'    => -999,
                    //) );
                    //$mn_customize->add_section( $section, array(
                    //    'priority'    => -999,
                ...
                foreach ( $this->parent->sections as $key => $section ) {
                    ...
                    if ( method_exists( $mn_customize, 'add_panel' ) && ( ! isset( $section['subsection'] ) || ( isset( $section['subsection'] ) && $section['subsection'] != true ) ) && isset( $this->parent->sections[ ( $key + 1 ) ]['subsection'] ) && $this->parent->sections[ ( $key + 1 ) ]['subsection'] ) {
                    ...
                    foreach ( $section['fields'] as $skey => $option ) {
                        ...
                        if ( $option['type'] != "heading" && $option['type'] != "import_export" && ! empty( $option['type'] ) ) {
                            ...
                            $mn_customize->add_setting( $option['id'],
                                array(
                        ...
                        if ( ! empty( $option['data'] ) && empty( $option['options'] ) ) {
                            ...
                            $option['options'] = $this->parent->get_mtaandao_data( $option['data'], $option['args'] );
                        }
                        ...
                        $mn_customize->add_control( new $class_name( $mn_customize, $option['id'], array(
                            'label'          => $option['title'],
                            'section'        => $section['id'],
                            'settings'       => $option['id'],
            ...
            public function add_section( $id, $args = array(), $mn_customize ) {

                if ( is_a( $id, 'MN_Customize_Section' ) ) {
                    $section = $id;
                } else {

                    $section_class = apply_filters( 'redux/customizer/section/class_name', "Redux_Customizer_Section" );
                    $section       = new $section_class( $mn_customize, $id, $args );
                }
             ...
             */
            public function add_panel( $id, $args = array(), $mn_customize ) {
                if ( is_a( $id, 'MN_Customize_Panel' ) ) {
                    $panel = $id;
                } else {
                    $panel_class = apply_filters( 'redux/customizer/panel/class_name', "Redux_Customizer_Panel" );
                    $panel       = new $panel_class( $mn_customize, $id, $args );
                }

                $mn_customize->add_panel( $panel, $args );
            }
