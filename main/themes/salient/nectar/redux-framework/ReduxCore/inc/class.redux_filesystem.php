        ...
        class Redux_Filesystem {
            ...
            public function __construct() {
                $this->parent->admin_notices[] = array(
                    'type'    => 'error',
                    'msg'     => '<strong>' . __( 'File Permission Issues', 'redux-framework' ) . '</strong><br/>' . sprintf( __( 'We were unable to modify required files. Please check your permissions, or modify your mn-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.', 'redux-framework' ), 'https://codex.mtaandao.org/Editing_mn-config.php#Mtaandao_Upgrade_Constants' ),
                    'id'      => 'redux-mn-login',
                    'dismiss' => false,
                );
            }
            ...
            public function ftp_form() {
                if ( isset( $this->parent->ftp_form ) && ! empty( $this->parent->ftp_form ) ) {
                    ...
                    echo '<strong>' . __( 'File Permission Issues', 'redux-framework' ) . '</strong><br/>' . sprintf( __( 'We were unable to modify required files. Please ensure that <code>%1s</code> has the proper read-write permissions, or modify your mn-config.php file to contain your FTP login credentials as <a href="%2s" target="_blank">outlined here</a>.', 'redux-framework' ), Redux_Helpers::cleanFilePath( trailingslashit( MN_CONTENT_DIR ) ) . '/uploads/', 'https://codex.mtaandao.org/Editing_mn-config.php#Mtaandao_Upgrade_Constants' );
            ...
            public static function load_direct() {
                if ( self::$direct === null ) {
                    ...
                    require_once ABSPATH . '/admin/includes/class-mn-filesystem-direct.php';
                    self::$direct = new MN_Filesystem_Direct( array() );
                }
            }
            ...
            public function execute( $action, $file = '', $params = '' ) {
                ...
                if ( ! is_dir( ReduxFramework::$_upload_dir ) ) {
                    mn_mkdir_p( ReduxFramework::$_upload_dir );
                }
                ...
                $url = mn_nonce_url( $base, 'redux-options' );
            ...
            public function do_action( $action, $file = '', $params = '' ) {
                ...
                //$target_dir = $mn_filesystem->find_folder( dirname( $file ) );
                ...
                if ( $action == 'mkdir' ) {
                    ...
                    $res = $mn_filesystem->rmdir( $file, $recursive );
                    ...
                    $res = $mn_filesystem->copy( $file, $destination, $overwrite );
                } elseif ( $action == 'delete' ) {
                    $res = $mn_filesystem->delete( $file, $recursive );
                } elseif ( $action == 'rmdir' ) {
                    $res = $mn_filesystem->rmdir( $file, $recursive );
                    ...
                    $res = $mn_filesystem->dirlist( $file, $include_hidden, $recursive );
                    ...
                    if ( isset( $owner ) && ! empty( $owner ) ) {
                        $res = $mn_filesystem->chmod( $file, $chmod, $recursive );
                    }
                } elseif ( $action == 'owner' ) {
                    // Gets file owner
                    $res = $mn_filesystem->owner( $file );
                } elseif ( $action == 'chmod' ) {
                    ...
                    $res = $mn_filesystem->get_contents_array( $file );
