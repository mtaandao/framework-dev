    ...
    if ( ! class_exists( 'Redux_CDN' ) ) {
        class Redux_CDN {
            ...
            private static function is_enqueued( $handle, $list = 'enqueued', $is_script ) {
                if ( $is_script ) {
                    mn_script_is( $handle, $list );
                } else {
                    mn_style_is( $handle, $list );
                }
            }

            private static function _register( $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
                if ( $is_script ) {
                    mn_register_script( $handle, $src_cdn, $deps, $ver, $footer_or_media );
                } else {
                    mn_register_style( $handle, $src_cdn, $deps, $ver, $footer_or_media );
                }
            }

            private static function _enqueue( $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
                if ( $is_script ) {
                    mn_enqueue_script( $handle, $src_cdn, $deps, $ver, $footer_or_media );
                } else {
                    mn_enqueue_style( $handle, $src_cdn, $deps, $ver, $footer_or_media );
                }
            }
            ...
            private static function _cdn( $register = true, $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
                ...
                if ( $cdn_is_up ) {
                    ...
                    $cdn_response = @mn_remote_get( $prefix . $src_cdn );

                    if ( is_mn_error( $cdn_response ) || mn_remote_retrieve_response_code( $cdn_response ) != '200' ) {
