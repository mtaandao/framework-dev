    ...
    if ( ! class_exists( 'Redux_Admin_Notices' ) ) {
        ...
        class Redux_Admin_Notices {
            ...
            public static function load() {
                add_action( 'mn_ajax_redux_hide_admin_notice', array(
                    'Redux_Admin_Notices',
                        ...
                        if ( true == $notice['dismiss'] ) {
                            ...
                            if ( ! get_user_meta( $userid, 'ignore_' . $notice['id'] ) ) {
                                ...
                                if ( version_compare( $mn_version, '4.2', '>' ) ) {
                                    ...
                                    $nonce = mn_create_nonce( $notice['id'] . $userid . 'nonce' );
                                    $output .= "<input type='hidden' class='dismiss_data' id='" . esc_attr( $notice['id'] ) . $pageName . $curTab . "' value='{$nonce}'> \n";
                                    $output .= '<p>' . mn_kses_post( $notice['msg'] ) . '</p>';
                                    $output .= "</div> \n";
                                    echo $output;
                                } else {
                                    echo '<div ' . $add_style . ' class="' . esc_attr( $notice['type'] ) . ' notice is-dismissable"><p>' . mn_kses_post( $notice['msg'] ) . '&nbsp;&nbsp;<a href="?dismiss=true&amp;id=' . esc_attr( $notice['id'] ) . $pageName . $curTab . '">' . esc_html__( 'Dismiss', 'redux-framework' ) . '</a>.</p></div>';
                                }
                            }
                        } else {
                            // Standard notice
                            echo '<div ' . $add_style . ' class="' . esc_attr( $notice['type'] ) . ' notice"><p>' . mn_kses_post( $notice['msg'] ) . '</a>.</p></div>';
                        }
                        ?>
                        <script>
            ...
            public static function dismissAdminNoticeAJAX() {
                ...
                if ( ! mn_verify_nonce( $_POST['nonce'], $id . $userid . 'nonce' ) ) {
                    die( 0 );
                } else {
                    // Add the dismiss request to the user meta.
