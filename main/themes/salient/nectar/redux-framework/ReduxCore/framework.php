...
// Fix for the GT3 page builder: http://www.gt3themes.com/mtaandao-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            public static function init() {
                ...
                self::$mn_content_url = trailingslashit( Redux_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', MN_CONTENT_URL ) : MN_CONTENT_URL ) ) );

                // See if Redux is a plugin or not
                if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( MN_CONTENT_DIR . '/themes/' ) ) !== false ) {
                    ...
                    if ( strpos( self::$_dir, ABSPATH ) === false ) {
                        ...
                        foreach ( get_plugins() as $key => $value ) {
                            if ( is_plugin_active( $key ) && strpos( $key, 'redux-framework.php' ) !== false ) {
                                self::$_dir = trailingslashit( Redux_Helpers::cleanFilePath( MN_CONTENT_DIR . '/plugins/' . plugin_dir_path( $key ) . 'ReduxCore/' ) );
                                $is_plugin  = true;
                            }
                        }
                        if ( ! $is_plugin ) {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
                        ...
                        $mn_content_dir = trailingslashit( Redux_Helpers::cleanFilePath( MN_CONTENT_DIR ) );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    // Ajax saving!!!
                    add_action( "mn_ajax_" . $this->args['opt_name'] . '_ajax_save', array( $this, "ajax_save" ) );
                    ...
                    /*if ( $this->args['dev_mode'] == true || Redux_Helpers::isLocalHost() == true ) {
                        ...
                        if ( ! isset ( $GLOBALS['redux_notice_check'] ) ) {
                            ...
                            $params = array(
                                'dir_name'    => 'notice',
                                'server_file' => 'http://reduxframework.com/' . 'main/uploads/redux/redux_notice.json',
                                'interval'    => 3,
...
// __construct()
            ...
            public function network_admin_bar( $admin_bar ) {
...
// get_options()
            ...
            /**
             * ->get_mtaandao_date() - Get Wordpress specific data from the DB and return in a usable array
             *
             * @since ReduxFramework 3.0.0
             */
            public function get_mtaandao_data( $type = false, $args = array() ) {
                $data = "";
//return $data;
                /**
                 * filter 'redux/options/{opt_name}/mtaandao_data/{type}/'
                 *
                 * @deprecated
                 *
                 * @param string $data
                 */
                $data = apply_filters( "redux/options/{$this->args['opt_name']}/mtaandao_data/$type/", $data ); // REMOVE LATER
                ...
                if ( empty ( $data ) && ! empty ( $type ) ) {
                    ...
                    if ( ! empty ( $type ) && empty ( $data ) ) {
                        ...
                        if ( $type == "categories" || $type == "category" ) {
                            ...
                            if ( ! empty ( $terms ) && ! is_a( $terms, 'MN_Error' ) ) {
...
// show()
                        ...
                        if ( is_array( $field['data'] ) && ! empty( $field['data'] ) ) {
                            foreach ( $field['data'] as $key => $data ) {
                                if ( ! empty( $data ) ) {
                                    ...
                                    $field['options'][ $key ] = $this->get_mtaandao_data( $data, $field['args'][ $key ] );
                                }
                            }
                        } else {
                            $field['options'] = $this->get_mtaandao_data( $field['data'], $field['args'] );
                        }
                    }
            ...
            private function _default_cleanup() {
                ...
                // Force dev_mode on MN_DEBUG = true and if it's a local server
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
                        // ONLY for non-mn.org themes OR plugins. Theme-Check alert shown if used and IS theme.
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    // Theme-Check notice is displayed for MN.org theme devs, informing them to NOT use this.
                ...
                if ( $menu ) {
                    ...
                    if ( isset ( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
                        foreach ( $submenu[ $this->args["page_slug"] ] as $index => $redux_options_submenu ) {
                            ...
                            $admin_bar->add_node( $subnodeargs );
                        }
                    }
                    ...
                    if ( isset ( $this->args['admin_bar_links'] ) ) {
                        ...
                        foreach ( $this->args['admin_bar_links'] as $link ) {
                            ...
                            $admin_bar->add_node( $externalnodeargs );
                        }
                    }
                } else {
                    // Fetch the title
            ...
            public function _enqueue_output() {
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        mn_register_style( 'redux-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                        mn_enqueue_style( 'redux-google-fonts-' . $this->args['opt_name'] );
                    }
                }
            }
// _enqueue_output()
                ...
                foreach ( $this->sections as $k => $section ) {
                    ...
                    if ( isset ( $section['fields'] ) ) {
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            ...
                            if ( $field['type'] == "media" ) {
                                if ( isset ( $this->options[ $field['id'] ]['id'] ) && isset ( $this->options[ $field['id'] ]['url'] ) && ! empty ( $this->options[ $field['id'] ]['url'] ) && strpos( $this->options[ $field['id'] ]['url'], str_replace( 'http://', '', MN_CONTENT_URL ) ) === false ) {
                            ...
                            if ( $field['type'] == "background" ) {
                                if ( isset ( $this->options[ $field['id'] ]['media']['id'] ) && isset ( $this->options[ $field['id'] ]['background-image'] ) && ! empty ( $this->options[ $field['id'] ]['background-image'] ) && strpos( $this->options[ $field['id'] ]['background-image'], str_replace( 'http://', '', MN_CONTENT_URL ) ) === false ) {
                            ...
                            if ( $field['type'] == "slides" ) {
                                if ( isset ( $this->options[ $field['id'] ] ) && is_array( $this->options[ $field['id'] ] ) && isset ( $this->options[ $field['id'] ][0]['attachment_id'] ) && isset ( $this->options[ $field['id'] ][0]['image'] ) && ! empty ( $this->options[ $field['id'] ][0]['image'] ) && strpos( $this->options[ $field['id'] ][0]['image'], str_replace( 'http://', '', MN_CONTENT_URL ) ) === false ) {
                                    foreach ( $this->options[ $field['id'] ] as $key => $val ) {
                                        $data = mn_get_attachment_url( $val['attachment_id'] );

                                        if ( isset ( $data ) && ! empty ( $data ) ) {
                                            $this->options[ $field['id'] ][ $key ]['image'] = $data;
                                            $data                                           = mn_get_attachment_image_src( $val['attachment_id'], array(
                                                150,
                                                150
                                            ) );
...
//                }
                ...
                if ( defined( 'MN_CACHE' ) && MN_CACHE && class_exists( 'W3_ObjectCache' ) && function_exists( 'w3_instance' ) ) {
                    //echo "here";
            ...
            public function ajax_save() {
                if ( ! mn_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
            ...
            public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
                ...
                if ( isset ( $section['type'] ) && $section['type'] == "divide" ) {
                    ...
                    $string .= '<a href="javascript:void(0);" id="' . esc_attr( $k . $suffix ) . '_section_group_li_a" class="redux-group-tab-link-a" data-key="' . esc_attr( $k ) . '" data-rel="' . esc_attr( $k . $suffix ) . '">' . $extra_icon . $icon . '<span class="group_title">' . mn_kses_post( $section['title'] ) . '</span></a>';
                    ...
                    if ( $subsections ) {
                        ...
                        while ( $doLoop ) {
                            ...
                            if ( count( $sections ) < $nextK || ! isset ( $sections[ $nextK ] ) || ! isset ( $sections[ $nextK ]['subsection'] ) || $sections[ $nextK ]['subsection'] != true ) {
                                ...
                                $string .= '<a href="javascript:void(0);" id="' . esc_attr( $nextK . $suffix ) . '_section_group_li_a" class="redux-group-tab-link-a" data-key="' . esc_attr( $nextK ) . '" data-rel="' . esc_attr( $nextK . $suffix ) . '">' . $icon . '<span class="group_title">' . mn_kses_post( $sections[ $nextK ]['title'] ) . '</span></a>';
                                $string .= '</li>';
                            }
                        }
