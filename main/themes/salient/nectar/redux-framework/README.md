...
Mtaandao options framework which uses the [Mtaandao Settings API](http://codex.mtaandao.org/Settings_API "Mtaandao Settings API"), Custom Error/Validation Handling, Custom Field/Validation Types, and import/export functionality.
...
ReduxFramework has just released to the Mtaandao Plugins directory. Please spread the word, tweet, and (most importantly) post reviews on http://mtaandao.org/plugins/redux-framework/. 
...
* Uses the [Mtaandao Settings API](http://codex.mtaandao.org/Settings_API "Mtaandao Settings API")
...
The tests are built using [mtaandao's make subversion repository](https://make.mtaandao.org/core/handbook/automated-testing/)

`/var/www/mtaandao-develop` as the destination for the core test files.
First download the wordress core tests repository, for these files.

```bash
cd /var/www
svn co http://develop.svn.mtaandao.org/trunk/ mtaandao-develop
```

In the newly created `/var/www/mtaandao-develop` directory rename
`mn-tests-config-sample.php` to `mn-tests-config.php`. Now add your database
...
Set the `MN_TESTS_DIR` environment variable so that the `redux-framework` test bootstrap file can find the mtaandao core tests:
```bash
export MN_TESTS_DIR='/var/www/mtaandao-develop/tests/phpunit/includes/'
```

You should now be able to run the `redux-framework` unit tests:
```bash
redux-framework$ phpunit
Welcome to the TIVMN Test Suite
Version: 1.0

Tests folder: /var/www/mtaandao-develop/tests/phpunit/includes/
