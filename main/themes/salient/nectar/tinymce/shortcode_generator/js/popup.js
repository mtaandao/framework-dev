jQuery(document).ready(function($){
		...
		function initUpload(clone){
	        ...
	        itemToInit.find('.redux-opts-upload').on('click',function( event ) {
	                ...
	                custom_file_frame = mn.media.frames.customHeader = mn.media({
    ...
    $('input.popup-colorpicker').mnColorPicker({
    	palettes: ['#21b68e', '#78cd6e', '#29c1e7', '#ae81f9', '#f78224', '#FF4629']
    });
  ...
  calcPercent();
    ...
    function directToEditor() {
    	...
    	//insert shortcode
		window.mn.media.editor.insert( $('#shortcode-storage-o').text() + content);
    ...
    $('#add-shortcode').click(function(){
		...
		if( dataType != 'direct_to_editor') {
			...
			window.mn.media.editor.insert( $('#shortcode-storage-o').text() + $('#shortcode-storage-d').text() + $('#shortcode-storage-c').text() );
			$.magnificPopup.close();
	...
	function resetFileds(){
		...
		$('#nectar-sc-generator').find('.mn-color-result').attr('style','');
		$('select[name="icon-set-select"]').trigger('change');
