...
if (!class_exists('MNBakeryVisualComposerAbstract') || class_exists('MNBakeryVisualComposerAbstract') && !defined('SALIENT_VC_ACTIVE')) {
...
function nectar_text_with_icon($atts, $content = null) {  
	...
	if($icon_type == 'font_icon'){
		...
		$icon_markup = mn_get_attachment_image_src($icon_image, 'medium');
		if(!empty($icon_markup)) {
			
			$icon_alt = get_post_meta($icon_image, '_mn_attachment_image_alt', true);
...
function nectar_team_member($atts, $content = null) {
	...
	if($team_memeber_style == 'meta_overlaid' || $team_memeber_style == 'meta_overlaid_alt'){
		...
		if(!empty($image_url)){
				...
				if(preg_match('/^\d+$/',$image_url)){
					$image_src = mn_get_attachment_image_src($image_url, 'portfolio-thumb');
					$image_url = $image_src[0];
				}
...
function nectar_client($atts, $content = null) {
	...
	if(preg_match('/^\d+$/',$image)){
		$image_src = mn_get_attachment_image_src($image, 'full');
		$image = $image_src[0];
								...
								<div class="work-info"> 
									...
									//default
										...
										<div class="vert-center">
											...
											$featured_image = mn_get_attachment_image_src( get_post_thumbnail_id(), 'full' );  							
							...
							<div class="work-item style-5" data-custom-content="<?php echo $using_custom_content; ?>" data-text-align="<?php echo $masonry_item_content_pos; ?>">
								...
								if(!empty($parallax_images)) {
									...
									foreach ( $images as $attach_id ) {
										...
										$img = mn_get_attachment_image_src(  $attach_id, $thumb_size );
										//add one sizer img
								...
								//no parallax images set
									...
									if (!empty($custom_thumbnail)) {
										...
										$thumbnail_url = mn_get_attachment_image_src($thumbnail_id,$thumb_size); 
	...
	if($style != 'slider') {
		...
		($title_labels == 'true') ? $title_label_output = '<h2 class="uppercase recent-posts-title">'.$recent_posts_title_text.'<a href="'. $posts_page_link.'" class="button"> / '. $recent_posts_link_text .'</a></h2>' : $title_label_output = null;
			...
			<div class="row blog-recent columns-<?php echo $columns; ?>" data-style="<?php echo $style; ?>">
				...
				$recent_posts_query = new MN_Query($recentBlogPosts);  
				...
				<div class="col <?php echo $col_num; ?>">
					...
					<?php 
						...
						$mn_version = floatval(get_bloginfo('version'));
							...
							if(get_post_format() == 'video'){
								 ...
								 if ( $mn_version < "3.6" ) {
								    ...
								    if( !empty($video_embed) || !empty($video_m4v) ){
							          ...
							          else if( !empty($video_m4v) && $mn_version < "3.6") {
									...
									<div class="flex-gallery"> 
											 <ul class="slides">
												...
												foreach( $gallery_ids as $image_id ) {
												     echo '<li>' . mn_get_attachment_image($image_id, 'portfolio-thumb', false) . '</li>';
												} ?>
									    	</ul>
								   	 </div><!--/gallery-->
		...
		if( $recent_posts_query->have_posts() ) :  while( $recent_posts_query->have_posts() ) : $recent_posts_query->the_post(); global $post; ?>
				...
				<?php 
					...
					if(!empty($bg)){
						...
						$image_src = mn_get_attachment_image_src($bg_image_id, 'full');
