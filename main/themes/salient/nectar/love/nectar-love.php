...
class NectarLove {
	 ...
	 function __construct()   {	
        ...
        add_action('mn_ajax_nectar-love', array(&$this, 'ajax'));
		add_action('mn_ajax_nopriv_nectar-love', array(&$this, 'ajax'));
	}
	
	function enqueue_scripts() {
		
		mn_enqueue_script( 'jquery' );
		//mn_enqueue_script( 'nectar-love', get_template_directory_uri() . '/nectar/love/js/nectar-love.js', 'jquery', '1.0', TRUE );
		...
		mn_localize_script( 'nectarFrontend', 'nectarLove', array(
			'ajaxurl' => admin_url('admin-ajax.php'),
			'postID' => $post->ID,
			'rooturl' => home_url(),
			'pluginPages' => $plugin_pages,
			'disqusComments' => $disqus_comments,
			'loveNonce' => mn_create_nonce('nectar-love-nonce'),
