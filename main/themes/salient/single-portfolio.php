...
$fmn = get_post_meta($post->ID, '_nectar_portfolio_item_layout', true);
if(empty($fmn)) $fmn = 'false';
...
$featured_src =  (has_post_thumbnail( $post->ID )) ? mn_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full') : array('empty') ;
$full_width_portfolio = (!empty($fmn) && $fmn == 'enabled') ? 'id="full_width_portfolio" data-featured-img="'. $featured_src[0] .'"' : 'data-featured-img="'. $featured_src[0] .'"';
...
?>
<div <?php echo $full_width_portfolio; if(!empty($bg) && $fmn != 'enabled' || !empty($bg_color) && $fmn != 'enabled') echo ' data-project-header-bg="true"'?> >
		...
		<div class="container main-content"> 
			...
			<div class="row <?php if(!empty($enable_gallery_slider) && $enable_gallery_slider == 'on') echo 'gallery-slider'; ?>">
				...
				?>
					...
					<div id="post-area" class="col <?php if($fmn != 'enabled') { echo 'span_9'; } else { echo 'span_12'; } ?>">
										...
										<div class="flex-gallery"> 
											 <ul class="slides">
												...
												foreach( $gallery_ids as $image_id ) {
												     echo '<li>' . mn_get_attachment_image($image_id, '', false) . '</li>';
												} ?>
									    	</ul>
								   	  </div><!--/gallery-->
