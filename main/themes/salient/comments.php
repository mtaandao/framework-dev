...
<?php if ( have_comments() ) : ?>
	...
	<ul class="comment-list <?php echo $fw_class; ?>">
		<?php mn_list_comments(array('avatar_size' => 60)); ?>
	</ul>
...
$args = array(
  ...
  'must_log_in' => '<p class="must-log-in">' .
    sprintf(
      ...
      mn_login_url( apply_filters( 'the_permalink', get_permalink() ) )
    ) . '</p>',
  ...
  'logged_in_as' => '<p class="logged-in-as">' .
    ...
    __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', NECTAR_THEME_NAME ),
      ...
      $user_identity,
      mn_logout_url( apply_filters( 'the_permalink', get_permalink( ) ) )
    ) . '</p>',
