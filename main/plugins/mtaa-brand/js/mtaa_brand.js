jQuery( document ).ready( function( $ ) {
	...
	// Admin Branding
	$( '#toplevel_page_mtaa_brand_admin_logo, #toplevel_page_mtaa_brand_admin_logo_folded' ).on( 'click', 'a', function( e ) {
		e.preventDefault();
	} );
	...
	// Colorpicker
	if ( $( 'body' ).is( '.toplevel_page_mtaa_brand_color_schemes' ) ) {
	...
	if ( $( '.colorCustom input[name="mtaa_brand_settings[colorScheme]"]' ).is( ':checked' ) ) {
	...
	$( 'body.plugins-php #mtaa-branding .deactivate' ).on( 'click', 'a', function( e ) {

		var response = confirm('Are you sure you want to disable Slate Pro? Once disabled, all your settings will be lost. \n\nIf you want to save your settings, make sure to export them on the Mtaa Branding Import/Export page first. \n\nClick "OK" to DISABLE Mtaa Branding and lose your settings.');
		if (response === true) {
			//$(this ).click();
		} else {
			e.preventDefault();
