...
abstract class Types_Field_Group {
	 ...
	 */
	const POSTMETA_FIELD_SLUGS_LIST = '_mn_types_group_fields';
	...
	/**
	 * @var MN_Post Post object that represents the field group.
	 */
	private $post;
	 ...
	 *
	 * @param MN_Post $field_group_post Post object that represents the field group.
	 * @throws InvalidArgumentException
	 */
	protected function __construct( $field_group_post ) {
		if( ! $field_group_post instanceof MN_Post ) {
	...
	/**
	 * @return MN_Post Post object representing the field group.
	 */
	 ...
	 *
	 * @param string $value New value of the post name. Note that it may be further modified by Mtaandao before saving.
	 */
	public function set_name( $value ) {
		$result = $this->update_post( array( 'post_name' => sanitize_title( $value ) ) );
		if( true == $result ) {
			do_action( 'mncf_field_group_renamed', $value, $this );
		}
	}
	 ...
	 *
	 * @param array $args Arguments for mn_update_post(). ID doesn't have to be provided, it will be added automatically.
	...
	private function update_post( $args ) {
		...
		$updated_post_id = mn_update_post( $args );

		if( 0 !== $updated_post_id ) {
			// Refresh the post object
			$this->post = MN_Post::get_instance( $updated_post_id );
	...
	/**
	 * Execute the mncf_group_updated action.
	 *
	...
	public function execute_group_updated_action() {
		 ...
		 * @since 1.9
		 */
		do_action( 'mncf_group_updated', $this->get_id(), $this );
	}
	...
	protected function get_keys_for_export_checksum() {
		return array(
			...
			MNCF_AUTHOR, self::XML_META_SECTION 
		);
	}
