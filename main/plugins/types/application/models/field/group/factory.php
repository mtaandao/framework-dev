 ...
 * an 'mncf_field_group_renamed' action immediately after renaming.
 *
 * @since 1.9
 */
abstract class Types_Field_Group_Factory {
	...
	protected function __construct() {
		add_action( 'mncf_field_group_renamed', array( $this, 'field_group_renamed' ), 10, 2 );
	}
	 ...
	 *
	 * @param int|string|MN_Post $field_group Numeric ID of the post, post slug or a post object.
	 *
	 * @return null|MN_Post Requested post object when the post exists and has correct post type. Null otherwise.
	 */
	final protected function get_post( $field_group ) {
		...
		if ( is_numeric( $field_group ) && ( $field_group == (int) $field_group ) && ( (int) $field_group > 0 ) ) {
			$fg_post = MN_Post::get_instance( $field_group );
		} else if ( is_string( $field_group ) ) {
			$query = new MN_Query( array( 'post_type' => $this->get_post_type(), 'name' => $field_group, 'posts_per_page' => 1 ) );
			if( $query->have_posts() ) {
		...
		if( $fg_post instanceof MN_Post && $this->get_post_type() == $fg_post->post_type ) {
			return $fg_post;
		} else {
			return null;
		}
	...
	private function get_from_cache( $field_group_name ) {
		return mncf_getarr( $this->field_groups, $field_group_name, null );
	}
	 ...
	 *
	 * @param int|string|MN_Post $field_group_source Post ID of the field group, it's name or a MN_Post object.
	 *
	 ...
	 *
	 * @param array $query_args Optional. Arguments for the MN_Query that will be applied on the underlying posts.
	 *     Post type query is added automatically.
	 *     Additional arguments are allowed:
	 *     - 'types_search': String for extended search. See MNCF_Field_Group::is_match() for details.
	...
	public function query_groups( $query_args = array() ) {
		...
		$search_string = mncf_getarr( $query_args, 'types_search' );
		$is_active = mncf_getarr( $query_args, 'is_active', null );
