 ...
 * in MNCF_Fields and field types defined through specially named functions. But that is hidden from anyone who uses
 * this class.
 *
 * @since 1.9
 */
class Types_Field_Type_Definition_Factory {
	 ...
	 * MNCF_Fields::getFieldsTypes() applies a filter each time it is called.
	 */
	...
	private function get_legacy_field_types() {
		if( null == $this->legacy_field_types ) {
			$this->legacy_field_types = MNCF_Fields::getFieldsTypes();
		}
	 ...
	 * starts with a "mncf-" prefix, it attempts to remove it and search again. This way, passing a field type ID,
	...
	public function load_field_type_definition( $field_type_slug ) {
		...
		if( !array_key_exists( $field_type_slug, $this->field_type_definitions ) ) {
			...
			$field_type_configuration = MNCF_Fields::getFieldTypeConfig( $field_type_configuration_path );

			$field_type_id = mncf_getarr( $field_type_configuration, 'id', null );
	...
	public function load_multiple_definitions( $field_type_slugs ) {
		$results = array();
		$field_type_slugs = mncf_ensarr( $field_type_slugs );
