...
class Types_Field_Type_Definition {
	...
	public function __construct( $field_type_slug, $args ) {
		...
		$this->display_name = sanitize_text_field( mncf_getarr( $args, 'display_name', mncf_getarr( $args, 'title', $field_type_slug ) ) );

		$this->description = mncf_getarr( $args, 'description', '' );
		$this->args = $args;
	}
	...
	public function get_args( $argument_name = null, $default = '' ) {
		if( null == $argument_name ) {
			...
			return mncf_getarr( $this->args, $argument_name, $default );
		}
	}
	...
	private function sanitize_field_definition_array_generic( $definition_array ) {
		// slug: sanitize_title
		$definition_array['slug'] = sanitize_title( mncf_getarr( $definition_array, 'slug' ) );

		// type: default to textfield
		$definition_array['type'] = mncf_getarr( $definition_array, 'type', Types_Field_Type_Definition_Factory::TEXTFIELD );

		// name: sanitize_text_field
		$definition_array['name'] = sanitize_text_field( mncf_getarr( $definition_array, 'name' ) );

		// description: ensure it is set
		$definition_array['description'] = mncf_getarr( $definition_array, 'description' );

		// meta_key: default to mncf-{$slug}
		$definition_array['meta_key'] = mncf_getarr( $definition_array, 'meta_key', MNCF_Field_Definition::FIELD_META_KEY_PREFIX . $definition_array['slug'] );

		// data: must be an array
		$definition_array['data'] = mncf_ensarr( mncf_getarr( $definition_array, 'data' ) );

		// data[conditional_display]: must be an array
		$definition_array['data']['conditional_display'] = mncf_ensarr( mncf_getarr( $definition_array['data'], 'conditional_display' ) );

		// data[validate]: must be an array
		$definition_array['data']['validate'] = mncf_ensarr( mncf_getarr( $definition_array['data'], 'validate' ) );
	...
	public final function sanitize_field_definition_array( $definition_array ) {
		 ...
		 * @since 2.1
		 */
		$definition_array = mncf_ensarr( apply_filters( 'types_pre_sanitize_field_definition_array', $definition_array ) );
		 ...
		 * @since 2.1
		 */
		$definition_array = mncf_ensarr( apply_filters( 'types_post_sanitize_field_definition_array', $definition_array ) );
	...
	protected function sanitize_numeric_validation( $definition_array ) {
		...
		// This is what mncf_admin_custom_fields_change_type() was doing.
	...
	protected function sanitize_element_isset( $source, $element_name, $default = '', $allowed = null, $nested_key = null ) {
		...
		$value = mncf_getarr( $src_array, $element_name, $default, $allowed );
