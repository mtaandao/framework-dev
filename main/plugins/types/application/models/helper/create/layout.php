...
class Types_Helper_Create_Layout {
	...
	public function for_post( $type, $name = false ) {
		...
		foreach( $posts as $id ) {
			$layout = get_post_meta( $id, MNDDL_LAYOUTS_META_KEY, true );
			if( !empty( $layout ) ) {
				$post_has_layout[] = array(
					'id' => $id,
					'layout-slug' => $layout
		...
		global $mnddlayout;
		$mnddlayout->post_types_manager->handle_set_option_and_bulk_at_once( $layout_id, array( $type ), array( $type ) );

		// restore previously assigned layouts
		if( !empty( $post_has_layout ) ) {
			foreach( $post_has_layout as $post ) {
				update_post_meta( $post['id'], MNDDL_LAYOUTS_META_KEY, $post['layout-slug'] );
			}
		}
	...
	private function needed_components_loaded( ) {
		global $mnddlayout;
		if(
			! is_object( $mnddlayout )
			|| ! class_exists( 'MNDD_Layouts' )
            || ! class_exists( 'MNDDL_Options' )
			|| ! class_exists( 'MNDD_Layouts_Users_Profiles' )
			|| ! method_exists( 'MNDD_Layouts', 'create_layout' )
			|| ! method_exists( 'MNDD_Layouts', 'save_layout_settings' )
			|| ! method_exists( 'MNDD_Layouts_Users_Profiles', 'user_can_create' )
			|| ! method_exists( 'MNDD_Layouts_Users_Profiles', 'user_can_assign' )
            || ! method_exists( 'MNDD_Layouts_Cache_Singleton', 'get_name_by_id' )
			|| ! method_exists( $mnddlayout, 'get_css_framework' )
		) return false;
	...
	private function create_layout( $name ) {
		...
		// permissions
		if( ! current_user_can( 'manage_options' ) && MNDD_Layouts_Users_Profiles::user_can_create() && MNDD_Layouts_Users_Profiles::user_can_assign() )
			return false;

		$layout = MNDD_Layouts::create_layout( 12, 'fluid' );

        $parent_post_name = '';
        $parent_ID = apply_filters('ddl-get-default-' . MNDDL_Options::PARENTS_OPTIONS, MNDDL_Options::PARENTS_OPTIONS);
        if ($parent_ID) {
            $parent_post_name = MNDD_Layouts_Cache_Singleton::get_name_by_id($parent_ID);
        }
        
		// Define layout parameters
		$layout['type'] = 'fluid'; // layout_type
		$layout['cssframework'] = $mnddlayout->get_css_framework();
		$layout['template'] = '';
		...
		$args = array(
			...
			'post_content'	=> '',
			'post_status'	=> 'publish',
			'post_type'     => MNDDL_LAYOUTS_POST_TYPE
		);
		$layout_id = mn_insert_post( $args );

		// force layout object to take right ID
		// @see MNDD_Layouts::create_layout_callback() @ mnddl.class.php
		$layout_post = get_post( $layout_id );
		$layout['id'] = $layout_id;
		$layout['slug'] = $layout_post->post_name;

		// update changes
		MNDD_Layouts::save_layout_settings( $layout_id, $layout );
	...
	private function validate_name( $name, $id = 1 ) {
		$name_exists = get_page_by_title( html_entity_decode( $name ), OBJECT, MNDDL_LAYOUTS_POST_TYPE );
