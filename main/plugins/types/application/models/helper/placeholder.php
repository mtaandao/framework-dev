...
class Types_Helper_Placeholder {
	...
	public static function replace( &$original ) {
		...
		$placeholders = array(
			...
			'%POST-TYPE-EDIT-HAS-ARCHIVE%'      => self::add_referer( $admin_url . 'admin.php?page=mncf-edit-type&mncf-post-type=' . self::$post_type->name . '#types_options' ),
		);
		...
		if( defined( 'MNV_VERSION' ) ) {
			$placeholders = array_merge( $placeholders, array(
				...
				'%POST-CREATE-VIEWS-ARCHIVE%'      => self::add_referer( $admin_url . 'admin.php?page=types-helper&action=new-mtaandao-archive&type='.self::$post_type->name.'&toolset_help_video=views_archives' ),
	...
	public static function get_permalink( $id = 0 ) {
		...
		// cpt edit page
		if( isset( $_GET['mncf-post-type'] ) && $id == 0 ) {
			$get_permalink_post_type = sanitize_text_field( $_GET['mncf-post-type'] );
			$query = new MN_Query( 'post_type=' . $get_permalink_post_type . '&posts_per_page=1' );
			if( $query->have_posts() )
		...
		if( is_object( self::$post_type ) && $id == 0 ){
			$query = new MN_Query( 'post_type=' . self::$post_type->name . '&posts_per_page=1' );
			if( $query->have_posts() )
				return self::get_permalink( $query->posts[0]->ID );
		}
	...
	public static function get_archive_permalink() {
		if( array_key_exists( 'mncf-post-type', $_GET ) ) {
			$post_type = sanitize_text_field( $_GET['mncf-post-type'] );
		} else {
		...
		// get new
		$query = new MN_Query( 'post_type=' . $post_type . '&post_status=publish&posts_per_page=1' );
