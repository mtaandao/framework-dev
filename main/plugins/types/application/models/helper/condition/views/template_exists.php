...
class Types_Helper_Condition_Views_Template_Exists extends Types_Helper_Condition_Views_Views_Exist {
	...
	public function valid() {
		...
		$mnv_options = get_option( 'mnv_options', array() );

		if( empty( $mnv_options )
		    || ! isset( $mnv_options['views_template_for_'.$type] )
		    || ! get_post_type( $mnv_options['views_template_for_'.$type] )
		) {
			self::$template_id[$type] = false;
		...
		$title = get_the_title( $mnv_options['views_template_for_'.$type] );
		self::$template_id[$type] = $mnv_options['views_template_for_'.$type];
