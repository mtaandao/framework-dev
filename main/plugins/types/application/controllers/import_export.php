...
final class Types_Import_Export {
	...
	private function maybe_delete_fields( $domain, $delete_other_fields, $fields_to_preserve ) {
		...
		if ( $delete_other_fields ) {
			...
			$fields_existing = mncf_admin_fields_get_fields( false, false, false, $option_name );
	...
	private function import_field_definition( $domain, $definition_array_import ) {
		...
		if ( !empty( $iclTranslationManagement ) && isset( $definition['mnml_action'] ) ) {
			$iclTranslationManagement->settings['custom_fields_translation'][ mncf_types_get_meta_prefix( $definition ) . $definition['slug'] ] = $definition['mnml_action'];
		...
		return array(
			'is_success' => true,
			'display_message' => sprintf( __( 'Term field "%s" added/updated', 'mncf' ), $definition['name'] )
		);
	}
	...
	public function process_field_group_import_per_domain( $domain, $data, $groups_key, $bulk_overwrite_groups, $delete_other_groups, $group_settings ) {
		...
		foreach( $groups_import_data as $group ) {
			...
			$import_group_id = mncf_getarr( $group, Types_Field_Group::XML_ID );

			$group_actions = mncf_getarr( $group_settings, $import_group_id, array( 'add' => true ) );
			...
			if( $group_should_be_imported ) {
				if( $bulk_overwrite_groups ) {
					...
					$group_action = mncf_getarr( $group_actions, 'update', 'add', array( 'add', 'update' ) );
				}
			} else {
		...
		if( $delete_other_groups && !empty( $groups_to_preserve ) ) {
			...
			foreach( $all_groups as $group_to_delete ) {
				if( !in_array( $group_to_delete->get_id(), $groups_to_preserve ) ) {
					...
					$deleted = mn_delete_post( $group_to_delete->get_id(), true );
					if ( !$deleted ) {
						$results[] = array(
							'type' => 'error',
							'content' => sprintf( __( 'Term field group "%s" delete failed', 'mncf' ), $deleted_group_name )
						);
					} else {
						$results[] = array(
							'type' => 'success',
							'content' => sprintf( __( 'Term field group "%s" deleted', 'mncf' ), $deleted_group_name )
						);
					}
	...
	public function import_field_group( $domain, $group, $conflict_resolution ) {
		...
		$group_slug = mncf_getarr( $group, Types_Field_Group::XML_SLUG );
		...
		if( $update_existing ) {
			...
			$new_group_id = mn_update_post( $new_post );
		} else {
			$new_group_id = mn_insert_post( $new_post, true );
		}

		$is_success = ( ! is_mn_error( $new_group_id ) && 0 < $new_group_id );
		...
		if( $is_success ) {
			if( $update_existing ) {
				$display_message = sprintf( __( 'Term field group "%s" updated', 'mncf' ), $group['post_title'] );
			} else {
				$display_message = sprintf( __( 'Term field group "%s" added', 'mncf' ), $group['post_title'] );
			}
		} else {
			if( $update_existing ) {
				$display_message = sprintf( __( 'Term field group "%s" update failed', 'mncf' ), $group['post_title'] );
			} else {
				$display_message = sprintf( __( 'Term field group "%s" insert failed', 'mncf' ), $group['post_title'] );
			}
		}
