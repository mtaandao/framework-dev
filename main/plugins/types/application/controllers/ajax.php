 ...
 *
 * 1.  All AJAX action names are automatically prefixed with 'mn_ajax_types_'. Only lowercase characters and underscores
 *     can be used.
 ...
 *     'mn_ajax_types_field_control_action' you need to create a class 'Types_Ajax_Handler_Field_Control_Action'.
...
final class Types_Ajax {
	...
	private function register_callbacks() {
		...
		foreach( self::$callbacks as $callback_name ) {
			add_action( 'mn_ajax_types_' . $callback_name, array( $this, self::CALLBACK_PREFIX . $callback_name ) );
		}
	 ...
	 *
	 * Check user capability and nonce. Dies with an error message (mn_json_error() by default) if the authentization
	 * is not successful.
	 *
	 ...
	 *         - 'die': Call mn_json_error with array( 'type' => 'capability'|'nonce', 'message' => $error_message )
	...
	private function ajax_authenticate( $args = array() ) {
		// Read arguments
		$type_of_death = mncf_getarr( $args, 'type_of_death', 'die', array( 'die', 'return' ) );
		$nonce_name = mncf_getarr( $args, 'nonce' );
		$nonce_parameter = mncf_getarr( $args, 'nonce_parameter', 'mnnonce' );
		$capability_needed = mncf_getarr( $args, 'capability_needed', 'manage_options' );
		$parameter_source_name = mncf_getarr( $args, 'parameter_source', 'post', array( 'get', 'post' ) );
		...
		if ( ! current_user_can( $capability_needed ) ) {
			$error_message = __( 'You do not have permissions for that.', 'mnv-views' );
			$error_type = 'capability';
			$is_error = true;
		}

		// Check nonce
		if ( !$is_error && !mn_verify_nonce( mncf_getarr( $parameter_source, $nonce_parameter, '' ), $nonce_name ) ) {
			$error_message = __( 'Your security credentials have expired. Please reload the page to get new ones.', 'mnv-views' );
			$error_type = 'nonce';
			$is_error = true;
		}
		...
		if( $is_error ) {
			...
			switch( $type_of_death ) {
				...
				case 'die':
					mn_send_json_error( $error_description );
					break;
	...
	private function additional_ajax_init() {
		...
		// On the Add Term page, we need to initialize the page controller MNCF_GUI_Term_Field_Editing
	...
	public function prepare_for_term_creation() {
		...
		$screen = mncf_getpost( 'screen', null );
	 ...
	 *
	 * See MNCF_GUI_Term_Field_Editing::maybe_disable_column_autohiding() for details.
	 *
		...
		/** @noinspection PhpUnusedParameterInspection */ $meta_id, $object_id, $meta_key, $_meta_value )
	{
		...
		if( $is_columnshidden_option ) {
			...
			if( $is_tax_edit_page ) {
				...
				$edit_term_page_extension = MNCF_GUI_Term_Field_Editing::get_instance();
