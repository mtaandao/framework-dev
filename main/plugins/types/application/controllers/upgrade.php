...
class Types_Upgrade {
	...
	const TYPES_DATABASE_VERSION_OPTION_LEGACY1 = 'MNCF_VERSION';
	const TYPES_DATABASE_VERSION_OPTION_LEGACY2 = 'mncf-version';
	...
	private function do_upgrade() {
		...
		foreach( $upgrade_routines as $routine ) {
			$upgrade_version = (int) mncf_getarr( $routine, 'version' );
			
			if( $from_version < $upgrade_version && $upgrade_version <= $target_version ) {
				$callback = mncf_getarr( $routine, 'callback' );
	...
	function upgrade_db_to_2010000() {
		...
		$roles_manager = MNCF_Roles::getInstance();

		global $mndb;

		// Will find users without the administrator roles but with one of the Types management roles.
		// A sign of the types-768 bug.
		$user_query = new MN_User_Query(
			array(
				'meta_query' => array(
					'relation' => 'AND',
					array(
						'key' => $mndb->prefix . 'capabilities',
						'value' => '"administrator"',
						'compare' => 'NOT LIKE',
					),
					array(
						'key' => $mndb->prefix . 'capabilities',
						'value' => '"mncf_custom_post_type_view"',
						'compare' => 'LIKE',
					),
				)
			)
