...
final class Types_Field_Type_Converter {
	...
	private function __construct() {
		// Custom Content Maps uses priority 10 for this filter, we need to run later and override.
		add_filter( 'mncf_filter_field_control_change_type_allowed_types_from', array( $this, 'google_address_field_control_change_type_allowed_from' ), 15, 2 );
	}
	...
	public function get_conversion_matrix() {
		...
		if( null == $this->conversion_matrix ) {
			...
			foreach( $allowed_conversion_source as $field_type_slug => $allowed_conversions ) {
				...
				/**
				 * mncf_filter_field_control_change_type_allowed_types_from
				 *
				 ...
				 * @since 1.8.9
				 */
				$filtered_conversion_matrix[ $field_type_slug ] = apply_filters( 'mncf_filter_field_control_change_type_allowed_types_from', $allowed_conversions, $field_type_slug );
			}


			/**
			 * mncf_filter_field_control_change_type_allowed_types
			 *
			 ...
			 * @since 1.8.9
			 */
			$result = apply_filters( 'mncf_filter_field_control_change_type_allowed_types', $filtered_conversion_matrix );
	...
	public function get_possible_conversions( $type ) {
		...
		$allowed_slugs = mncf_ensarr( mncf_getarr( $matrix, $type->get_slug() ) );
	 ...
	 *
	 * This is hooked into the mncf_filter_field_control_change_type_allowed_types_from filter with higher priority
	 * than the Custom Content Maps hook that adds the google_address field type to the conversion matrix.
	 *
	 * This way we can properly handle the allowed conversions not depending on the particular Custom Content Maps version
	 * that is active.
	 *
