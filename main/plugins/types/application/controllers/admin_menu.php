...
final class Types_Admin_Menu {
	 ...
	 *
	 * Custom Content shared menu usage is described here:
	...
	public function on_admin_menu( $pages ) {
		// Add legacy pages
		$pages = mncf_admin_toolset_register_menu_pages( $pages );

		$page_name = sanitize_text_field( mncf_getget( 'page' ) );
		if( !empty( $page_name ) ) {
