...
final class Types_Page_Dashboard extends Types_Page_Abstract {
	...
	protected $table_mtaandao;

	protected $types_by_toolset;
	protected $types_by_3rd;
	protected $types_by_mtaandao;
	...
	public function get_title() {
		return __( 'Custom Content Dashboard', 'types' );
	}
	...
	public function on_admin_enqueue_scripts() {
		...
		mn_enqueue_script(
			...
			array( 'jquery-ui-dialog', 'mn-pointer' ),
			TYPES_VERSION,
			true
		);
		...
		mn_enqueue_style(
			...
			array( 'mn-jquery-ui-dialog', 'mn-pointer' ),
			TYPES_VERSION
		);
		...
		mn_enqueue_style(
			...
			MNCF_EMBEDDED_TOOLSET_RELPATH . '/onthego-resources/onthegosystems-icons/css/onthegosystems-icons.css',
			array(),
			TYPES_VERSION
		);
	}
	...
	private function build_page_context() {
		...
		$context = array(
			...
			'table_mtaandao' => $this->table_mtaandao,
			'labels' => array(
	...
	private function get_types_by_toolset() {
		...
		$cpts_raw = ! isset( $_GET['toolset-dashboard-simulate-no-custom-post-types'] )
			? get_option( MNCF_OPTION_NAME_CUSTOM_TYPES, array() )
			: array();

		// remove buildin types
		$cpts_raw = array_diff_key( $cpts_raw, $this->get_types_by_mtaandao() );
	 ...
	 * @return array
	 */
	private function get_types_by_mtaandao() {
		if( $this->types_by_mtaandao !== null )
			return $this->types_by_mtaandao;
	...
	private function get_types_by_3rd() {
		...
		$cpts = array_diff_key( $cpts, $this->get_types_by_mtaandao(), $this->get_types_by_toolset() );
	...
	private function get_post_types_filtered_by_screen_options( $cpts = false ) {
		if( $cpts === false )
			$cpts = array_merge( $this->get_types_by_toolset(), $this->get_types_by_mtaandao(), $this->get_types_by_3rd() );
	...
	private function get_dashboard() {
		...
		// Types by Wordpress
		$post_types = $this->get_post_types_filtered_by_screen_options( $this->get_types_by_mtaandao() );

		if( ! empty( $post_types ) )
			$this->table_mtaandao = $this->get_dashboard_types_table(
				$post_types,
				__( 'Built-in post types created by Mtaandao', 'types' ),
				false
			);
	...
	public function screen_settings( $status, $args ) {
		...
		// Types by Mtaandao
		$cpts = $this->get_types_by_mtaandao();
		$string_legend = __( 'Built-in post types created by Mtaandao', 'types' );
		$return .= $this->screen_settings_fieldset( $cpts, $cpts_filtered, $string_legend );
	...
	private function screen_settings_fieldset( $cpts, $cpts_filtered, $legend ) {
        ...
        <div><input type="hidden" name="mn_screen_options[option]" value="toolset_dashboard_screen_post_types" /></div>
        <div><input type="hidden" name="mn_screen_options[value]" value="yes" /></div>
	...
	private function help_information() {
		$title = __('Custom Content Dashboard', 'types');
