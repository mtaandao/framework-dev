...
class Types_Page_Hidden_Helper extends Types_Page_Abstract {
		...
		if( isset( $_GET['action'] ) && isset( $_GET['type'] ) ) {
			...
			switch( $action ) {
				...
				case 'new-content-template':
					...
					break;
				case 'new-mtaandao-archive':
					$this->redirect_url = $this->new_mtaandao_archive_action( $type );
					break;
	...
	private function new_form_action( $type ) {
		...
		if( $id = $new_form->for_post( $type ) ) {
			return get_edit_post_link( $id, 'Please Mtaandao, be so nice and do not encode &.' );
		}
	...
	private function new_mtaandao_archive_action( $type ) {
		$new_mtaandao_archive = new Types_Helper_Create_Wordpress_Archive();

		if( $id = $new_mtaandao_archive->for_post( $type ) ) {
	...
	private function new_post_field_group_action( $type ) {
		...
		$url = isset( $_GET['ref'] )
			? 'admin.php?page=mncf-edit&group_id='.$new_post_field_group->get_id().'&ref='.sanitize_text_field( $_GET['ref'] )
			: 'admin.php?page=mncf-edit&group_id='.$new_post_field_group->get_id();
