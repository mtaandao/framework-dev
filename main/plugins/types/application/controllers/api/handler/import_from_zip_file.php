...
class Types_Api_Handler_Import_From_Zip_File implements Types_Api_Handler_Interface {
	...
	function process_call( $arguments ) {
		...
		$path = mncf_getarr( $arguments, 1, null );
		$args = mncf_getarr( $arguments, 2, null );

		if( ! is_string( $path ) || ! file_exists( $path ) ) {
			return new MN_Error( 42, __( 'Invalid path to the import file.', 'mncf' ) );
		}

		ob_start();

		$import_data = $this->get_data_from_file( $path );
		if( $import_data instanceof MN_Error ) {
			return $import_data;
		} elseif( null == $import_data ) {
			return new MN_Error( 42, __( 'The import file could not be processed, it seems to be corrupted.', 'mncf' ) );
		}
	...
	private function hack_around_legacy_import_routine( $import_data, $import_args = null ) {
		...
		add_filter('mncf_admin_message_store', '__return_false');
		...
		/**
         * This can be emtpy string '' or 'mnvdemo', but this second option has a serious bug with xml parsing/looping
         */
		...
		if ( isset($import_args['overwrite']) ){
			$legacy_args['force_import_post_name'] = mncf_getarr( $import_args, 'overwrite', array() );
		}
		if ( isset($import_args['skip']) ){
			$legacy_args['force_skip_post_name'] = mncf_getarr( $import_args, 'skip', array() );
		}
		if ( isset($import_args['duplicate']) ){
			$legacy_args['force_duplicate_post_name'] = mncf_getarr( $import_args, 'duplicate', array() );
		}

		$result = mncf_admin_import_data( $import_data, false, $context, $legacy_args );
	...
	private function get_data_from_file( $path ) {
		...
		if ( $is_zip ) {
			...
			if ( is_resource( $zip ) ) {
				...
				return new MN_Error( 42, __( 'Unable to open zip file', 'mncf' ) );
			}
		} else {
