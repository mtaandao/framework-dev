...
final class Types_Utils {
	...
	public static function get_all_taxonomies() {
		...
		$types_taxonomies = get_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array() );
	 ...
	 *
	 * @link https://codex.mtaandao.org/Function_Reference/get_taxonomies Taxonomy object description.
	 *
	...
	public static function taxonomy_slug_to_label( $slug, $label_name = 'name' ) {
		...
		$taxonomy_display_name = mncf_getnest( $all_taxonomies, array( $slug, 'labels', $label_name ), $slug );
	...
	public static function insert_at_position( $source, $to_insert, $position) {
		...
		if( is_array( $position ) ) {
			$pivot_key = mncf_getarr( $position, 'key', null );
			$direction = mncf_getarr( $position, 'where', 'after', array( 'after', 'before' ) );
	 ...
	 * the mn-content directory. If so, then we search the database for a
	 * partial match consisting of the remaining path AFTER the mn-content
	 * directory. Finally, if a match is found the attachment ID will be
	 * returned.
	 *
	 * Taken from:
	 * @link http://frankiejarrett.com/get-an-attachment-id-by-url-in-mtaandao/
	 *
	...
	public static function get_attachment_id_by_url( $url ) {
		...
		// Split the $url into two parts with the mn-content directory as the separator.
		$parsed_url = explode( parse_url( MN_CONTENT_URL, PHP_URL_PATH ), $url );
		...
		$query = $mndb->prepare(
			"SELECT ID FROM $mndb->posts WHERE post_type = 'attachment' AND guid LIKE %s",
			'%' . $attachment_path
		);

		$attachment = $mndb->get_col( $query );
