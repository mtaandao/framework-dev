 ...
 * the shared Custom Content menu item configuration and then extend the get_help_config() method to return a valid
...
final class Types_Asset_Help_Tab_Loader {
	 ...
	 *
	 * Used as a hook for 'contextual_help_hook' in the shared Custom Content menu.
	 *
	 * @since 2.0
	 */
	public function add_help_tab() {
		...
		$current_page = sanitize_text_field( mncf_getget( 'page', null ) );
		...
		$args = array(
			'title' => mncf_getarr( $help_content, 'title' ),
			'id' => 'mncf',
			'content' => mncf_getarr( $help_content, 'content' ),
			'callback' => false,
		);
	...
	private function get_help_content( $page_name ) {
		...
		return array(
			'title' => mncf_getarr( $config, 'title' ),
			'content' => $twig->render( mncf_getarr( $config, 'template' ), mncf_getarr( $config, 'context' ) )
		);
	}
