...
final class Types_Ajax_Handler_Field_Control_Action extends Types_Ajax_Handler_Abstract {
	...
	public function process_call( $arguments ) {
		...
		$field_action = sanitize_text_field( mncf_getpost( 'field_action' ) );
		$fields = mncf_getpost( 'fields' ); // array of values, will be sanitized when processed

		$current_domain = mncf_getpost( 'domain', null, Types_Field_Utils::get_domains() );
		if( null == $current_domain ) {
			$am->ajax_finish( array( 'message' => __( 'Wrong field domain.', 'mncf' ) ), false );
		}

		if( !is_array( $fields ) || empty( $fields ) ) {
			$am->ajax_finish( array( 'message' => __( 'No fields have been selected.', 'mncf' ) ), false );
		}

		// will be sanitized when/if used by the action-specific method
		$action_specific_data = mncf_getpost( 'action_specific', array() );
		...
		foreach( $fields as $field ) {
			...
			} else if( $result instanceof MN_Error ) {
				// Single error
				$errors[] = $result;
			} else if( false == $result ) {
				// This should not happen...!
				$errors[] = new MN_Error( 0, __( 'An unexpected error happened while processing the request.', 'mncf' ) );
			} else  {
				// Success

				// Save the field definition model as a result if we got a whole definition
				if( $result instanceof MNCF_Field_Definition ) {
					$result = $result->to_json();
				}

				$results[ mncf_getarr( $field, 'slug' ) ] = $result;
			}
		}
	 ...
	 * @return bool|mixed|null|MN_Error|MN_Error[]|MNCF_Field_Definition An error, array of errors, boolean indicating
	...
	private function single_field_control_action( $action_name, $field, $domain, $action_specific_data ) {
		...
		$field_slug = sanitize_text_field( mncf_getarr( $field, 'slug' ) );

		switch ( $action_name ) {

			case 'manage_with_types':
				return $this->start_managing_field( sanitize_text_field( mncf_getarr( $field, 'metaKey' ) ), $domain );
			...
			default:
				return new MN_Error( 42, __( 'Invalid action name.', 'mncf' ) );
		}
	}
	 ...
	 * Check MNCF_Field_Definition_Factory::create_field_definition_for_existing_fields() for details.
	 *
	 * AJAX callback helper only, do not use elsewhere.
	 *
	 * @param string $meta_key
	 * @param string $domain Field domain
	 * @return false|null|MNCF_Field_Definition The updated/newly created field definition or falsy value on failure.
	 * @since 2.0
	 */
	public function start_managing_field( $meta_key, $domain ) {
		$factory = MNCF_Field_Definition_Factory::get_factory_by_domain( $domain );
	 ...
	 * @param string $domain Field domain.
	 * @return MN_Error|MNCF_Field_Definition Error with a user-friendly message on failure
	...
	public static function stop_managing_field( $field_slug, $domain ) {
		...
		if( null == $definition ) {
			...
			return new MN_Error( 42, sprintf( __( 'Field definition for field "%s" not found in options.', 'mncf' ), sanitize_text_field( $field_slug ) ) );
			...
			if( $is_success ) {
				...
				return new MN_Error(
					42,
					sprintf(
						__( 'Unable to set types management status for field definition "%s".', 'mncf' ),
	 ...
	 *
	 * @return MN_Error|MNCF_Field_Definition The updated field definition on success or an error object.
	 * @since 2.0
	 */
	public function change_assignment_to_groups( $field_slug, $domain, $groups ) {
		...
		$new_groups = mncf_ensarr( mncf_getarr( $groups, 'group_slugs' ) );
	 ...
	 * @param string $field_slug
	 * @param string $domain
	 * @return bool|MN_Error True for success, false or MN_Error on error.
	 * @since 2.0
	 */
	public function delete_field( $field_slug, $domain ) {
		...
		if( null == $definition ) {
			...
			return new MN_Error( 42, sprintf( __( 'Field "%s" will not be deleted because it is not managed by Types.', 'mncf' ), sanitize_text_field( $field_slug ) ) );
		}
	 ...
	 * @return false|MN_Error|MNCF_Field_Definition The updated definition on succes, error/false otherwise.
	 * @since 2.0
	 */
	public function change_field_type( $field_slug, $domain, $arguments ) {
		...
		if( null == $definition ) {
			...
			return new MN_Error( 42, sprintf( __( 'Field "%s" will not be converted because it is not managed by Types.', 'mncf' ), sanitize_text_field( $field_slug ) ) );
		}

		$type_slug = sanitize_text_field( mncf_getarr( $arguments, 'field_type' ) );
		$target_type = Types_Field_Type_Definition_Factory::get_instance()->load_field_type_definition( $type_slug );
		if( null == $target_type ) {
			return new MN_Error( 42, sprintf( __( 'Unknown field type "%s".', 'mncf' ), $type_slug ) );
		}
		...
		if( !$is_conversion_possible ) {
			return new MN_Error(
				42,
				sprintf(
					__( 'Conversion from type "%s" to "%s" is currently not allowed.', 'mncf' ),
		...
		if( !$is_cardinality_sustainable ) {
			return new MN_Error(
				42,
				sprintf(
					__( 'Field "%s" cannot be converted from "%s" to "%s" because it is repetitive and the target type doesn\'t support that.', 'mncf' ),
	 ...
	 * @return bool|MN_Error|MNCF_Field_Definition The updated definition on succes, error/false otherwise.
	 * @since 2.0
	 */
	public function change_field_cardinality( $field_slug, $domain, $arguments ) {
		...
		$target_cardinality = mncf_getarr( $arguments, 'target_cardinality', null, array( 'single', 'repetitive' ) );
