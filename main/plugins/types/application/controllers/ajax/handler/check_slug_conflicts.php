...
class Types_Ajax_Handler_Check_Slug_Conflicts extends Types_Ajax_Handler_Abstract {
	...
	function process_call( $arguments ) {
		...
		$domains = mncf_getpost( 'domains' );
		$value = mncf_getpost( 'value' );
		$exclude = mncf_getpost( 'exclude' );
		$exclude_id = mncf_getarr( $exclude, 'id', 0 );
		$exclude_domain = mncf_getarr( $exclude, 'domain' );
	...
	private function check_slug_conflicts_in_taxonomy_rewrite_rules( $value, $exclude_id ) {
		...
		$taxonomy_settings = get_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array() );
		...
		foreach( $taxonomy_settings as $taxonomy ) {
			...
			if( is_object( $taxonomy ) ) {
				$slug = $taxonomy->name;
				$rewrite_slug = mncf_getarr( $taxonomy->rewrite, 'slug' );
				$is_permalink_rewriting_enabled = !empty( $rewrite_slug );
			} else {
				$slug = mncf_getarr( $taxonomy, 'slug' );
				$is_permalink_rewriting_enabled = (bool) mncf_getnest( $taxonomy, array( 'rewrite', 'enabled' ) );
				$rewrite_slug = mncf_getnest( $taxonomy, array( 'rewrite', 'slug' ) );
			}
			...
			if( $is_permalink_rewriting_enabled ) {
				...
				if( $conflict_candidate == $value ) {
					...
					$conflict = array(
						'conflicting_id' => $slug,
						'message' => sprintf(
							__( 'The same value is already used in permalink rewrite rules for the taxonomy "%s". Using it again can cause issues with permalinks.', 'mncf' ),
							esc_html( $slug )
						)
					);
	...
	private function check_slug_conflicts_in_post_type_rewrite_rules( $value, $exclude_id ) {
		...
		$post_type_settings = get_option( MNCF_OPTION_NAME_CUSTOM_TYPES, array() );
		...
		foreach( $post_type_settings as $post_type ) {
			...
			if( is_object( $post_type ) ) {
				$slug = $post_type->name;
				$is_permalink_rewriting_enabled = (bool) mncf_getarr( $post_type->rewrite, 'enabled' );
				$rewrite_slug = mncf_getarr( $post_type->rewrite, 'slug' );
				$is_custom_slug_used = !empty( $rewrite_slug );
			} else {
				$slug = mncf_getarr( $post_type, 'slug' );
				$is_permalink_rewriting_enabled = (bool) mncf_getnest( $post_type, array( 'rewrite', 'enabled' ) );
				$is_custom_slug_used = ( mncf_getnest( $post_type, array( 'rewrite', 'custom' ) ) == 'custom' );
				$rewrite_slug = mncf_getnest( $post_type, array( 'rewrite', 'slug' ) );
			}
			...
			if( $is_permalink_rewriting_enabled ) {
				...
				if( $conflict_candidate == $value ) {
					...
					$conflict = array(
						'conflicting_id' => $slug,
						'message' => sprintf(
							__( 'The same value is already used in permalink rewrite rules for the custom post type "%s". Using it again can cause issues with permalinks.', 'mncf' ),
							esc_html( $slug )
						)
					);
