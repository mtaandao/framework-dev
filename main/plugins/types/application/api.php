...
final class Types_Api {
	...
	private function register_callbacks() {
		...
		foreach( self::$callbacks as $callback_name => $args ) {
			...
			$argument_count = mncf_getarr( $args, 'args', 1 );
	...
	public function __call( $name, $parameters ) {
		...
		$default_return_value = mncf_getarr( $parameters, 0, null );
	...
	{
		$domain = mncf_getarr( $query, 'domain', 'all' );
		...
		if( 'all' == $domain ) {
			...
			/** @var MNCF_Field_Definition[] $definitions */
