...
if( !function_exists( 'mncf_getpost' ) ) {

	/**
	 * Safely retrieve a key from $_POST variable.
	 *
	 * This is a wrapper for mncf_getarr(). See that for more information.
	 *
	 * @param string $key
	 * @param mixed $default
	 ...
	 * @since 1.9
	 */
	function mncf_getpost( $key, $default = '', $valid = null ) {
		return mncf_getarr( $_POST, $key, $default, $valid );
	}

}


if( !function_exists( 'mncf_getget' ) ) {
	 ...
	 * @since 1.9
	 */
	function mncf_getget( $key, $default = '', $valid = null ) {
		return mncf_getarr( $_GET, $key, $default, $valid );
	}

}


if( !function_exists( 'mncf_getarr' ) ) {
	 ...
	 * @since 1.9
	 */
	function mncf_getarr( &$source, $key, $default = '', $valid = null ) {
...
if( !function_exists( 'mncf_ensarr' ) ) {
	 ...
	 * @since 1.9
	 */
	function mncf_ensarr( $array, $default = array() ) {
...
if( !function_exists( 'mncf_wraparr' ) ) {
...
if( !function_exists( 'mncf_getnest' ) ) {
	 ...
	 * you need to write: $my_value = mncf_getnest( $source, array( 'a', 'b', 'c' ) );
	 *
	 ...
	 * @since 1.9
	 */
	function mncf_getnest( &$source, $keys = array(), $default = null ) {
		...
		while( ! empty( $keys ) ) {
			...
			$current_value = mncf_getarr( $current_value, $current_key, $missing_value );
