...
**Types let's you customize the Mtaandao admin by adding content types, custom fields and taxonomies. You will be able to craft the Mtaandao admin and turn it into your very own content management system.**

[vimeo https://vimeo.com/176428571]

= COMPLETE DOCUMENTATION, POWERFUL API, SIMPLE GUI FOR NON-CODERS =
If you're an experienced PHP developer, you'll appreciate Types comprehensive [fields API](https://mn-types.com/documentation/customizing-sites-using-php/functions/).

You will find detailed guides on [adding custom post types, fields and taxonomy to the front-end](https://mn-types.com/documentation/customizing-sites-using-php/), including:

* [Creating templates for single custom posts](https://mn-types.com/documentation/customizing-sites-using-php/creating-templates-single-custom-posts)
* [Creating templates for custom post type archives](https://mn-types.com/documentation/customizing-sites-using-php/creating-templates-custom-post-type-archives)
* [Creating custom user profiles](https://mn-types.com/documentation/customizing-sites-using-php/creating-custom-user-profiles)
* [Create taxonomy term archives](https://mn-types.com/documentation/customizing-sites-using-php/creating-taxonomy-term-archives)

and [more](https://mn-types.com/documentation/customizing-sites-using-php/).

**Too much technical stuff to learn?** The full [Toolset](http://mn-types.com) package lets you build complete Mtaandao sites from within the admin dashboard.

= CUSTOM FIELDS FOR CONTENT AND USERS =
Types lets you add custom fields for both posts (meaning, Mtaandao posts, pages and custom content types), as well as users. You can add any field types to different user profiles.

= ACCESS CONTROL FOR FIELDS =
Using [Access](https://mn-types.com/home/toolset-components/#access), you will be able to control what fields different users can edit and view. This way, you can make some field groups read-only for certain users, and fully-editable for other users.
...
Types fields come with a built-in CSS editor, letting you design how fields appear in the Mtaandao admin. You can design both full-edit and read-only field display modes.

= RELIABLE SUPPORT =
To get support for Types, please join our [technical support forum](http://mn-types.com/forums/). You will receive support directly from our developers, helping you deliver great sites on time and correctly.
...
* **Single-line text**
* **Multi-line text**
* **WYSIWYG** (Mtaandao Visual Editor)
* **Checkbox**
...
Types custom fields use the standard Mtaandao post-meta table, making it cross-compatible with any theme or plugin. Additionally, all fields can be **repeating fields**.
...
Types is the only custom fields and post types plugin that's built multilingual-ready. It plays perfectly with [MNML](http://mnml.org). You'll be able to translate everything, including texts and labels in the Mtaandao admin and user-content for front-page.

= BUILT FOR STABILITY =

Types is part of a family of plugins, including MNML and Toolset, developed and maintained by [OnTheGoSystems](http://www.onthegosystems.com). Our plugins power over 500,000 commercial sites, using Mtaandao as a complete CMS. While we love features, we know that stability, performance, usability and security are critical. All our plugins go through comprehensive testing, QA and security analysis before every release.

== Installation ==

1. Upload 'types' to the '/mn-content/plugins/' directory
2. Activate the plugin through the 'Plugins' menu in Mtaandao
...
By default, Mtaandao will either display your blog posts or a specific page on the home-page.

To display custom post types on the home-page, you have two options:

1. If you're comfortable with PHP and Mtaandao API, edit the site's template files (probably index.php) and load the custom post types there. Different themes do this differently, so we can't really say what single approach works best. You should look at [get_posts](http://codex.mtaandao.org/Template_Tags/get_posts), which is part of the Mtaandao Template Tags system.
2. If you want to build sites right away, without becoming an expert in Mtaandao API and try our [Custom Content Views](http://mn-types.com/). You'll be able to load whatever content you need from the database and display it anywhere and in whatever way you choose.
...
If you also buy Views, with Toolset, you'll have a complete solution for both **defining** and **displaying** custom data. You can achieve everything that Views does if you're fluent in PHP and know Mtaandao API. When you buy Views, you're also supporting Types development, but we're not looking for donations. You should consider Views for its value and nothing else.
...
* Fix an issue with registering custom taxonomies in Mtaandao 4.7.
...
* Support all CPTs in Custom Content Dashboard.
* Change the way we store the context of a Types field for string translation (use field group name instead of ID)
* Fix a Mtaandao 4.7 compatibility issue with direct access to $mn_filter.
...
* Custom Content Dashboard now supports custom post types created by theme or other plugins
* Updated select2 to version 4
* Fixed issue using [types] shortcode in CRED notification
* Fixed minor incompatibility with the latest version of Custom Content Access.
* Fixed issue with custom values for checkboxes fields not being respected.

= 2.2.1 =

* Fixed path for Custom Content Installer
...
* Added types_finished_saving_child_posts API action as a workaround for https://core.trac.mtaandao.org/ticket/17817.
...
* Fixed issue with post relationship query in MNML default language.
...
* Added feature on post type edit screen to also display complex fields in “Post Fields to be shown as columns in Post Type listing in Mtaandao Admin” section, even if they are not available in the listing.
...
* Updated “Cancel” links in dialogs, which are now buttons to match Mtaandao style.
...
* Fixed conflict between post relationship table and Mtaandao heartbeat ajax call.
...
* Added new filter "mncf_exclude_meta_boxes_on_post_type" that allows to exclude own Post Types from mncf_add_meta_boxes() function in order to avoid adding Types meta boxes to certain custom posts.
...
* Fixed compatibility with MNML related to custom field translations.
* Fixed compatibility with Mtaandao 4.4 related to menu management.
* Added 'action' to the list of reserved words that can not be used to name post types or taxonomies.

= 1.8.10 =

* Release date: 2015-11-18
* Added filter "mncf_init_custom_types_taxonomies".
...
* Fixed the problem with select post parent if MNML is active.
...
* Improved MNML integration, replace `icl_t()` by filter `mnml_translate_single_string`.
* Changed Installer version to 1.6.7 - to reduce requests to Custom Content API.
...
* Added the ability to turn off the standard Mtaandao Custom Fields metabox.
...
* Added a new filter “mncf_config_options_(type)”.
...
* Fixed a problem about lost translation when a parent Post was saved (https://mnml.org/forums/topic/custom-post-type-relationships/).
* Fixed a conflict of validation field with CRED plugin.
* Removed "Styling Editor" section in Custom Fields groups and User Fields groups edit screen. In order to enable it again, you can define “TYPES_USE_STYLING_EDITOR” constant in mn-config.php file.
...
* Fixed WYSIWYG field for MN 4.2.3 security release.
...
* Fixed a problem with Custom Fields Group edit screen to allow (again) underscore in Custom Fields names. https://mn-types.com/forums/topic/underscores-in-custom-field-names-possible-bug/
...
* Fixed problem with "View All" in menu builder for Custom Post Types. https://mn-types.com/forums/topic/appearance-menu-php-errornotice/

= 1.7.2 =

* Release date: 2015-06-23
* Field a problem with "file type" field on post edit screen when is no WYSIWYG editor. https://mtaandao.org/support/topic/image-field-not-working-1

= 1.7.1 =

* Release date: 2015-06-22
* Fixed a problem with constant ICL_SITEPRESS_VERSION https://mtaandao.org/support/topic/types-17-notice-undefined-constant-icl_sitepress_version
* Field a problem with "file type" field on user profile screen. https://mtaandao.org/support/topic/image-field-not-working-1
...
* Added the word "mode" to the list of words reserved by Mtaandao. https://mn-types.com/forums/topic/when-types-is-activated-i-cant-filter-articles-by-category-in-the-mn-backend/
...
* Added the "mncf_field_image_max_width" filter which allows user to change image width on admin listing pages.
* Added the option to specify the custom archive slug for the Custom Post Type http://mn-types.com/forums/topic/specify-cpt-archive-slug-as-string/
...
* Fixed a problem with selecting an image for the Custom Image Field in the Child Posts table, after using the "Add New", "Save All" and "Save" buttons.  https://mn-types.com/forums/topic/featured-image-cannot-be-changed-after-first-save/
* Fixed a problem with multi-line field not being wrapped with P (paragraph) HTML tag https://mn-types.com/forums/topic/multi-line-text-fields-are-missing-paragraph-tags/
...
* Fixed problem with "playlist" word.  https://mn-types.com/forums/topic/front-end-warning-from-wysiwyg-php/

= 1.6.6.5 =

* Release date: 2015-05-20
* Fixed problem with Uncaught ReferenceError: pagenow is not defined. http://mn-types.com/forums/topic/nextgen-gallery-broken-urgent/
...
* Fixed problem with File Field which do not work when edited from the Parent Post Type. https://mn-types.com/forums/topic/1-6-6-seems-to-break-child-fields-when-parent-has-an-image-field/
...
* Fixed problem with shortcode "playlist" used in WYSIWYG field. http://mn-types.com/forums/topic/media-play-list-not-outputting-from-custom-wysiwyg-field-js-error/
* Fixed empty title problem for filter "mnt_field_options" on user edit/add screen https://mn-types.com/forums/topic/populate-select-field-in-mncf-um-group/
* Added ability to create CPT without title and editor. https://mn-types.com/forums/topic/inaccurate-warning-message-when-creatingediting-a-cpt/
...
* Added ability to setup meta box callback function. https://mn-types.com/forums/topic/add-support-for-meta_box_cb-in-custom-taxonomy/
* Added ability to add HTML5 placeholder attribute for custom post fields.
* Fixed problem with CPT labels. https://mn-types.com/forums/topic/after-save-cpts-cutom-labels-always-revert-to-default-label/
* Added a filters to the post title as option text in the select dropdown for post parents. mncf_pr_belongs_items for array of options and mncf_pr_belongs_item for one option. https://mn-types.com/forums/topic/help-to-distinguish-duplicate-titles-in-post-relationship/
...
* Fixed problem with "Access Control and User Roles" menu in Types, when Access is active http://mn-types.com/forums/topic/update-issues-fatal-error-require_once-failed-opening-required-mncf_access_/
...
* Added ability to hide custom post types on post relationships list. https://mn-types.com/forums/topic/post-relationship-doesnt-show-post-type-events-created-by-events-espresso/ using filter add_filter('mncf_show_ui_hide_in_relationships', '__return_false');
* Fixed a problem with deleting last children on post relationships table.
* Added filter to allow use "?" in image url. https://mn-types.com/forums/topic/image-custom-field-is-not-storing-image-path-with-parameters/
* Added option for child table, when editing parent to allow show only list of children instead edit form. http://mn-types.com/forums/topic/miss-settings-for-post-relationship-child-options/
...
* Fixed a conflict with Formidable-Pro plugin https://mn-types.com/forums/topic/plugin-conflict/
* Fixed creating new post in relationships. MN 4.1 need real title not faked by one space.
* Fixed problem with validate fields on user create page. http://mn-types.com/forums/topic/custom-usermeta-bypassed-even-required-is-set/
* Improved Edit CPT and Edit CT screens to be more compatible with MN Admin UI
* Fixed problem with default label which contains single quote character (eg. French) https://mn-types.com/forums/topic/default-label-always-shown/
...
* Fixed an issue with editing checkboxes with the option "save 0 to the database" selected, created for Custom Posts. http://mn-types.com/forums/topic/checkbox-custom-field-doesnt-save-value-since-upgrade-to-version-1-6-2/ http://mn-types.com/forums/topic/types-checkbox-field-not-saving-after-save-0-to-the-database/
* Fixed an issue with PHP notices being thrown when relative URLs to images were used. http://mn-types.com/forums/topic/php-notice-undefined-index-host-in-image-php/
...
* Added the "mncf_delete_relation_meta" filter which allows deletion of all post relationships when deleting a custom post type.
* Fixed an issue with the file name being changed when the file was uploaded. https://icanlocalize.basecamphq.com/projects/7393061-toolset/todo_items/189560556/comments http://mn-types.com/forums/topic/types-1-6-update-breaks-layout-that-worked-in-types-1-5-7/
* Fixed a problem with duplicate slugs on "Edit Group" screen. https://icanlocalize.basecamphq.com/projects/7393061-toolset/todo_items/187118123/comments http://mn-types.com/forums/topic/cant-add-more-custom-fields/
...
* Fixed embedding OTGS CSS for the admin area. https://mtaandao.org/support/topic/four-stylesheets-being-loaded-at-frontend
* Fixed a problem with Checkbox field value not being saved. https://mn-types.com/forums/topic/checkbox-value-not-saved/
* Added the option to select posts with the "Private" post status as parents in a parent-child Custom Post Types relationships. http://mn-types.com/forums/topic/cred-child-form-not-working-with-private-ctp/
...
* Fixed a problem with label menu in admin for child posts.
* Fixed a problem with child table when edit parent post and children do not have title.
* Prevent to chose repetitive field in child table on edit parent screen. http://mn-types.com/forums/topic/mn-types-select-child-fields-to-be-displayed-specific-fields-not-working/
* Added a dynamic "posted x minutes/hours ago" for Types fields. https://mn-types.com/forums/topic/adding-a-dynamic-posted-x-minuteshours-ago-in-a-view/
* Improved post relation table by using more precise labels. http://mn-types.com/forums/topic/displaying-the-best-names-of-cpts-in-applicable-contexts/
...
* Fixed a problem where the custom field group’s description was missing from the post/page editing page. http://mn-types.com/forums/topic/custom-field-group-descriptions-no-longer-visible-in-cpt-add-newedit-screen/
* Fixed a problem where the field descriptions weren’t displayed on the user profile editing page. http://mn-types.com/forums/topic/checkbox-description-fields-no-longer-display-in-types-1-6-2/
...
* Fixed a problem where the value of date field couldn’t be cleared and added new button which clears the date field value. http://mn-types.com/forums/topic/problem-2-after-update/
* Replaced the deprecated like_escape function with the mndb::esc_like function.
...
* Fixed a problem where date picker scripts were being enqueued in the front end. https://mtaandao.org/support/topic/datepicker-css-enqueued-on-public
...
* Release date: 2014-08-22
* Added ability to add extra options by filter *mnt_field_options* to fields with "options".
* Added ability to create custom post type with the same singular and plural name.
* Added ability to select menu icon (Mtaandao dashicon) for custom post type.
* Added delete attached repetitive files.
* Added filter "mncf_pr_belongs_post_status" that allows to change post_status for parent/child posts.
...
* Fixed problems with embedding scripts when Mtaandao works on non-standard port.
...
* Removed Zebra library and created new Custom Content fields library.
* Tweak import/export screen when is something wrong with imported file
* Fixed several security issues
* Unify code with other Custom Content plugins
* Added debug information output for improved customer support
* Added "required" validation audio, file and video fields

= 1.5.7 =

* Fixed action "admin_header" to "admin_head" thx for (anarchocoder)[https://mtaandao.org/support/profile/anarchocoder]
* Fixed english suggests in non-english sites.
* Fixed missing icons in Mtaandao menu on custom fields edit screen.
...
* Fixed usage of custom fields with prefix "mncf-" but added by default interface.
...
* Check compatibility with Mtaandao 3.9.1.

= 1.5.6 =

* Avoid to get MNML configuration if MNML is not installed.
* Check compatibility with Mtaandao 3.9.
...
* Fixed bug with child posts and MNML
...
* Fixed missing MNML translations, after changing parent post type slug.
...
= 1.5.3 =
* Fixed caching field data before applying MNML filters
...
* Fixed MNML updating fields from original translation
* Fixed MNML edit post screen forms for copied fields
* Fixed MNML creating and updating child posts
* Fixed MNML copied fields appear locked if post do not have original post
* Fixed MNML deleting translated post fields marked as 'translatable' when original is updated
* Fixed MNML changed all fields to be disabled when copied
* Fixed MNML unlocked copied fields when Translation Management is not active
* Fixed MNML removed translation preferences form when Translation Management is not active
...
* Fixed custom fields named with prefix 'mncf-' put under Types control
...
* MNML synchronization when custom post type or taxonomy is changed (translation preferences, translation connections, belonging terms connections)
* Fixed various issues with MNML-copied fields
...
* MNML and Group terms filter compatibility added
...
* Fixed a problem with conditional fields and mnv_condition
...
= 1.1.3.4 =
* Fixed adding child posts for Mtaandao 3.5

= 1.1.3.2 =
* Fixed 'em' tags in radio.php and select.php
* Added support for localized custom post slugs via MNML

= 1.1.3.1 =
* Fixed saving fields in MN 3.5
...
* Some fixes for MNML compatibility
...
= 1.0.2 =
* Improved MNML support with repeating fields
* Fixed problems with decimal repeating fields
* Post relationship meta box goes through standard Mtaandao filters
...
= 0.9.1 =
* The new Embedded mode allows integrating Types functionality in Mtaandao plugins and themes.
...
= 1.1.3.4 =
Fix adding child posts for Mtaandao 3.5
