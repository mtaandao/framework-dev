...
*
* Generic class to manage the Custom Content shortcodes admin bar entry.
*
...
if ( ! class_exists( 'Toolset_Shortcode_Generator' ) ) {
	 ...
	 */
	public function display_shortcodes_admin_bar_items( $admin_bar ) {
		if ( ! is_admin() ) {
			return;
		}
		$registered_items = self::$registered_admin_bar_items;
	    ...
	    $this->create_admin_bar_item( $admin_bar, 'toolset-shortcodes', __( 'Custom Content shortcodes', 'mnv-views' ), '#', false );
		foreach ( $registered_items as $item_key => $item_args ) {
			$this->create_admin_bar_item( $admin_bar, $item_args['id'], $item_args['title'], $item_args['href'], $item_args['parent'], $item_args['meta'] );
		}
	}
	 ...
	 * 
	 */
	public static function create_admin_bar_item( $admin_bar, $id, $name, $href, $parent, $classes = null ) {
	    $args = array(
			'id'		=> $id,
			'title'		=> $name,
			'href'		=> $href,
