...
MNV_Toolset.activeUrlEditor = null;

if ( typeof MNV_Toolset.CodeMirror_instance === "undefined" ) {
	MNV_Toolset.CodeMirror_instance = [];
}

if ( typeof MNV_Toolset.add_qt_editor_buttons !== 'function' ) {
    MNV_Toolset.add_qt_editor_buttons = function( qt_instance, editor_instance ) {
        QTags._buttonsInit();
		MNV_Toolset.CodeMirror_instance[qt_instance.id] = editor_instance;
			...
			if ( qt_instance.theButtons.hasOwnProperty( button_name ) ) {
                ...
                if ( qt_instance.theButtons[button_name].id == 'img' ){
                    qt_instance.theButtons[button_name].callback = function( element, canvas, ed ) {
						...
						selection = MNV_Toolset.CodeMirror_instance[id].getSelection(),
						e = "http://",
						...
						MNV_Toolset.CodeMirror_instance[id].replaceSelection( selection, 'end' );
						MNV_Toolset.CodeMirror_instance[id].focus();
                    }
                }
                else if ( qt_instance.theButtons[button_name].id == 'mnv_conditional' ) {
                    qt_instance.theButtons[button_name].callback = function ( e, c, ed ) {
						if ( typeof MNViews === "undefined" ) {
							return;
						}
						if ( typeof MNViews.shortcodes_gui === "undefined" ) {
							return;
						}
                        MNV_Toolset.activeUrlEditor = ed;                        
						var id = jQuery( c ).attr( 'id' ),
                        t = this;
                        window.mncfActiveEditor = id;
                        MNV_Toolset.CodeMirror_instance[id].focus();
                        selection = MNV_Toolset.CodeMirror_instance[id].getSelection();
						...
						if ( selection ) {
						   ...
						   MNViews.shortcodes_gui.mnv_insert_popup_conditional('mnv-conditional', icl_editor_localization_texts.mnv_insert_conditional_shortcode, {}, icl_editor_localization_texts.mnv_editor_callback_nonce, current_editor_object );
						} else if ( ed.openTags ) {
					...
					qt_instance.theButtons[button_name].callback = function ( b, c, d, e ) {
						MNV_Toolset.activeUrlEditor = c;var f,g=this;return"undefined"!=typeof mnLink?void mnLink.open(d.id):(e||(e="http://"),void(g.isOpen(d)===!1?(f=prompt(quicktagsL10n.enterURL,e),f&&(g.tagStart='<a href="'+f+'">',a.TagButton.prototype.callback.call(g,b,c,d))):a.TagButton.prototype.callback.call(g,b,c,d)))
					};
					jQuery( '#mn-link-submit' ).off();
					jQuery( '#mn-link-submit' ).on( 'click', function( event ) {
						event.preventDefault();
						if ( mnLink.isMCE() ) {
							mnLink.mceUpdate();
						} else {
							var id = jQuery( MNV_Toolset.activeUrlEditor ).attr('id'),
							selection = MNV_Toolset.CodeMirror_instance[id].getSelection(),
							inputs = {},
							attrs, text, title, html;
							inputs.wrap = jQuery('#mn-link-wrap');
							inputs.backdrop = jQuery( '#mn-link-backdrop' );
							if ( jQuery( '#link-target-checkbox' ).length > 0 ) {
								// Backwards compatibility - before Mtaandao 4.2
								...
								// Mtaandao 4.2+
								inputs.text = jQuery( '#mn-link-text' );
								attrs = mnLink.getAttrs();
								text = inputs.text.val();
								if ( ! attrs.href ) {
									return;
							...
							inputs.wrap.hide();
							jQuery( document ).trigger( 'mnlink-close', inputs.wrap );
							MNV_Toolset.CodeMirror_instance[id].replaceSelection( selection, 'end' );
							MNV_Toolset.CodeMirror_instance[id].focus();
							return false;
						}
...
jQuery(document).ready(function(){
     ...
     *
     * Used on MN editor, Types WYSIWYG, Views Filter Meta HTML,
     * Views Layout Meta HTML, CRED form.
     */
    window.mncfActiveEditor = 'content';
    jQuery('.mn-media-buttons a, .mncf-wysiwyg .editor_addon_dropdown .item, .mnt-wysiwyg .editor_addon_dropdown .item, #postdivrich .editor_addon_dropdown .item, #js-visual-editor-codemirror .editor_addon_dropdown .item,#mnv_filter_meta_html_admin_edit .item, #mnv_layout_meta_html_admin_edit .item').on('click', function(){
        window.mncfActiveEditor = jQuery(this).parents('.mnt-wysiwyg, .mncf-wysiwyg, #postdivrich, #mnv_layout_meta_html_admin, #mnv_filter_meta_html_admin, #js-visual-editor-codemirror')
        .find('textarea#content, textarea.mncf-wysiwyg, textarea.mnt-wysiwyg, textarea#mnv_layout_meta_html_content, textarea#mnv_filter_meta_html_content, textarea#visual-editor-html-editor').attr('id');

    /*
         *
         * TODO 1.3 Why we do not have saving cookie in common?
         */
    //        document.cookie = "mncfActiveEditor="+window.mncfActiveEditor+"; expires=Monday, 31-Dec-2020 23:59:59 GMT; path="+mncf_cookiepath+"; domain="+mncf_cookiedomain+";";
    });
    ...
    jQuery(document).on('click','input[id^="credmailsubject"]', function(){
	    window.mncfActiveEditor = jQuery(this).attr('id');
    });
    jQuery(document).on('click', 'div[id^="mn-credmailbody"] .editor_addon_dropdown img', function(){
	    window.mncfActiveEditor = jQuery(this).parents('div[id^="mn-credmailbody"]').find('textarea[id^="credmailbody"]').attr('id');
    });
    /*
     * Handle the "Add Field" boxes - some layout changes.
     */
    jQuery('.mnv_add_fields_button').on('click', function(e) {
        ...
        if (dropdown_list.css('visibility') == 'hidden') {
            ...
            var pos = jQuery('.mnv_add_fields_button').position();
            dropdown_list.css('top', pos.top + jQuery('.mnv_add_fields_button').height() - iclEditorHeight + 'px');
            dropdown_list.css('left', pos.left + jQuery('.mnv_add_fields_button').width() + 'px');
    ...
    jQuery('#post').on('click', '.editor_addon_dropdown img', function(e){
        ...
        if (drop_down.css('visibility') == 'hidden') {
            ...
            mnv_hide_top_groups(jQuery(this).parent());
     ...
     */
    jQuery(document).on('click', '.js-mnv-shortcode-post-icon-mnv-views', function(e){

        // Set dropdown
        var drop_down = jQuery(this).parent().find('.js-editor_addon_dropdown-mnv-views');
        if ( drop_down.css('visibility') === 'hidden' ) {
            // Hide top links if div too small
            mnv_hide_top_groups(jQuery(this).parent());
            // Popup
     ...
     */
    jQuery(document).on('click', '.js-mnv-shortcode-post-icon-types', function(e){
        // Set dropdown
       // console.log('js-mnv-shortcode-post-icon-types');
...
/**
 * Resizing Custom Content editor dropdowns.
 *
...
function insert_b64_shortcode_to_editor(b64_shortcode, text_area) {
    ...
    window.mncfActiveEditor = text_area;
 ...
 */
function mnv_on_search_filter(el) {
    // get search text
    ...
    mnv_hide_top_groups(parent);
}

/**
 * @TODO Document this
 */
function mnv_hide_top_groups(parent) {
...
// clear search input
function mnv_search_clear(el) {
    var parent = el.parentNode.parentNode;
    var searchbox = jQuery(parent).find('.search_field');
    searchbox.val('');
    mnv_on_search_filter(searchbox[0]);
}
...
function icl_editor_bind_auto_close() {
    ...
    jQuery(document).on('click.icl_editor',function(e){
        ...
        // if we click anything but Custom Content buttons
        if ( ( $target.closest('.mnv_add_fields_button,' +
								'.js-code-editor-toolbar-button-v-icon,' +
								'.js-code-editor-toolbar-button-v-icon,' +
								'.js-mnv-shortcode-post-icon-mnv-views,' +
								'.js-mnv-shortcode-post-icon-types,' +
								'.js-mncf-access-editor-button').length === 0) ) {
...
var icl_editor = (function(window, $){
    ...
    function isCodeMirror($textarea)
    {
		...
		if ( textareaNext ) {
			...
			// Emerson: Mtaandao 4.0+ introduces 'content-textarea-clone' div which in some instances is loaded after our textarea and before the CodeMirror div.
			// This core feature in MN is used in their auto-resize editor and distraction free writing.
    ...
    {
        //        alert(window.mncfActiveEditor);
        InsertAtCursor($('#'+window.mncfActiveEditor), content);
    }
