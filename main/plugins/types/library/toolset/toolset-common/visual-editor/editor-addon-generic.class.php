		...
		function icl_editor_admin_enqueue_styles() {
			...
			*/
	        ...
	        if (
	            ...
	            mn_enqueue_style( 'editor_addon_menu' );
	            mn_enqueue_style( 'editor_addon_menu_scroll' );
                    mn_enqueue_style( 'toolset-dialogs-overrides-css' );
	        }
	    }
	}
        ...
        function icl_editor_admin_enqueue_scripts() {
			...
			*/
			...
			if (
                ...
                mn_enqueue_script( 'icl_editor-script' );
            }
			...
			if (
				...
				&& ! mn_script_is( 'views-redesign-media-manager-js', 'enqueued' )
			) {
				mn_enqueue_media();
				mn_enqueue_script( 'icl_media-manager-js' );
			}
        }
    }
    ...
    class Editor_addon_generic
    {
        ...
        public static function getWpForbiddenNames()
        {
	        ...
	        $reserved_list = array(
				...
				'second', 'sentence', 'shomnosts', 'static', 'subpost', 'subpost_id', 'tag', 'tag__and', 'tag__in', 'tag__not_in',
				'tag_id', 'tag_slug__and', 'tag_slug__in', 'taxonomy', 'tb', 'term', 'theme', 'type', 'w', 'withcomments', 'withoutcomments',
				'year '
			);
	
	
	    	$reserved_post_types = array_keys( $mn_post_types );
	    	
	    	$mnv_taxes = get_taxonomies();
	    	$reserved_taxonomies = array_keys( $mnv_taxes );
	    	
	    	$mnv_forbidden_parameters = array(
				'mtaandao' => $reserved_list,
				'post_types' => $reserved_post_types,
				'taxonomies' => $reserved_taxonomies,
	    	);
	    	
	    	return $mnv_forbidden_parameters;
        }
