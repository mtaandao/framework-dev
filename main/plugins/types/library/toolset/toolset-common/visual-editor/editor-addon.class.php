...
if ( file_exists( dirname(__FILE__) . '/editor-addon-generic.class.php') && !class_exists( 'Editor_addon' )  ) {
    ...
    class Editor_addon extends Editor_addon_generic
    {
        ...
        function add_form_button( $context, $text_area = '', $standard_v = true, $add_views = false, $codemirror_button = false )
        {
            ...
            if ( version_compare( $mn_version, '3.1.4', '>' ) && !empty( $context ) ) {
                $text_area = $context;
            }
            ...
            // add_filter('editor_addon_parent_items', array($this, 'mnv_add_parent_items'), 10, $this->items);
            ...
            $icon_class = 'js-mnv-shortcode-post-icon-'.$this->name;
            if ( $this->name == 'mnv-views' ) {
                $button_label = __( 'Fields and Views', 'mnv-views' );
            } else if ( $this->name == 'types' ) {
                $button_label = __( 'Types', 'mnv-views' );
            } else {
                $button_label = '';
            }
            ...
            {
                $addon_button = '<span class="button mnv-shortcode-post-icon '. $icon_class .'"><img src="' . $this->media_button_image . '" />' . $button_label . '</span>';
            }
            else if( '' !== $this->icon_class ){

                $addon_button = '<span class="button mnv-shortcode-post-icon '. $icon_class .'"><i class="'.$this->icon_class.'"></i><span class="button-label">' . $button_label . '</span></span>';
            }
            ...
            if ( !$standard_v ) {
                ...
                if( '' !== $this->media_button_image )
                {
                    $addon_button = '<span class="button vicon mnv-shortcode-post-icon '. $icon_class .'"><img src="' . $this->media_button_image . '" />' . $button_label . '</span>';
                }
                else if( '' !== $this->icon_class )
                {
                    $addon_button = '<span class="button vicon mnv-shortcode-post-icon '. $icon_class .'"><i class="'.$this->icon_class.'"></i><span class="button-label">' . $button_label . '</span></span>';
                }
            }
            ...
            if ( $codemirror_button ){
                 $addon_button = '<button class="js-code-editor-toolbar-button js-code-editor-toolbar-button-v-icon button-secondary">'.
                        '<i class="icon-views-logo ont-icon-18"></i><span class="button-label">'. __('Fields and Views', 'mnv-views') .'</span></button>';
            }
            // add search box
            ...
            $out .= '<div class="close">&nbsp;</div>
                <div class="editor_addon_dropdown_content">
                        ' . apply_filters( 'editor_addon_dropdown_top_message_' . $this->name,
                                        '' ) . '
                        <p class="direct-links-desc">'. __('Go to','mnv-views') .': </p>
            ...
            if ( version_compare( $mn_version, '3.1.4', '>' ) ) {
                echo apply_filters( 'mnv_add_media_buttons', $out );
            } else {
                return apply_filters( 'mnv_add_media_buttons', $context . $out );
            }
        }
        ...
        function add_users_form_button( $context, $text_area = 'textarea#content', $codemirror_button = false ) {
            global $mn_version, $sitepress, $mndb, $MN_Views;
            $standard_v = true;
            // MN 3.3 changes ($context arg is actually a editor ID now)
            if ( version_compare( $mn_version, '3.1.4', '>' ) && !empty( $context ) ) {
                $text_area = $context;
            ...
            $unused_field = array('comment_shortcuts','managenav-menuscolumnshidden','dismissed_mn_pointers','meta-box-order_dashboard','nav_menu_recently_edited',
            ...
            $this->items = array(
				array(__('User ID', 'mnv-views'), 'mnv-user field="ID"',__('Basic', 'mnv-views'),''),
                array(__('User Email', 'mnv-views'), 'mnv-user field="user_email"',__('Basic', 'mnv-views'),''),
                array(__('User Login', 'mnv-views'), 'mnv-user field="user_login"',__('Basic', 'mnv-views'),''),
                array(__('First Name', 'mnv-views'), 'mnv-user field="user_firstname"',__('Basic', 'mnv-views'),''),
                array(__('Last Name', 'mnv-views'), 'mnv-user field="user_lastname"',__('Basic', 'mnv-views'),''),
                array(__('Nickname', 'mnv-views'), 'mnv-user field="nickname"',__('Basic', 'mnv-views'),''),
                array(__('Display Name', 'mnv-views'), 'mnv-user field="display_name"',__('Basic', 'mnv-views'),''),
                array(__('Description', 'mnv-views'), 'mnv-user field="description"',__('Basic', 'mnv-views'),''),
                array(__('Yahoo IM', 'mnv-views'), 'mnv-user field="yim"',__('Basic', 'mnv-views'),''),
                array(__('Jabber', 'mnv-views'), 'mnv-user field="jabber"',__('Basic', 'mnv-views'),''),
                array(__('AIM', 'mnv-views'), 'mnv-user field="aim"',__('Basic', 'mnv-views'),''),
                array(__('User Url', 'mnv-views'), 'mnv-user field="user_url"',__('Basic', 'mnv-views'),''),
                array(__('Registration Date', 'mnv-views'), 'mnv-user field="user_registered"',__('Basic', 'mnv-views'),''),
                array(__('User Status', 'mnv-views'), 'mnv-user field="user_status"',__('Basic', 'mnv-views'),''),
                array(__('User Spam Status', 'mnv-views'), 'mnv-user field="spam"',__('Basic', 'mnv-views'),'')
                );

            if ( isset( $sitepress ) && function_exists( 'mnml_string_shortcode' ) ) {
				$nonce = mn_create_nonce('mnv_editor_callback');
				$this->items[] = array(
					__('Translatable string', 'mnv-views'), 
					'mnml-string',
					__('Basic', 'mnv-views'),
					"MNViews.shortcodes_gui.mnv_insert_popup('mnml-string', '" . __( 'Translatable string', 'mnv-views' ) . "', {}, '" . $nonce . "', this )"
				);
			}
            ...
            foreach ($meta_keys as $key) {
                $key_nicename = '';
                if ( function_exists('mncf_init') ){
                    if (stripos($key, 'mncf-') === 0) {
                        //
                    }
                    else {
            ...
            if ( function_exists('mncf_init') ){
                ...
                $groups = mncf_admin_fields_get_groups( 'mn-types-user-group' );
                ...
                if ( !empty( $groups ) ) {
                    foreach ( $groups as $group_id => $group ) {
                        ...
                        $fields = mncf_admin_fields_get_fields_by_group( $group['id'],
                                'slug', true, false, true, 'mn-types-user-group',
                                'mncf-usermeta' );

                        if ( !empty( $fields ) ) {
                            foreach ( $fields as $field_id => $field ) {
                                $add[] = $field['meta_key'];
                                $callback = 'mncfFieldsEditorCallback(\'' . $field['id'] . '\', \'views-usermeta\', -1)';
                ...
                foreach ( $cf_types as $cf_id => $cf ) {
                     if ( !in_array( $cf['meta_key'], $add) ){
                         $callback = 'mncfFieldsEditorCallback(\'' . $cf['id'] . '\', \'views-usermeta\', -1)';
                                $this->items[] = array($cf['name'],
                                  'types usermeta="'.$cf['meta_key'].'"][/types',
                                  __('Types fields', 'mnv-views'),$callback);

                     }
                }
             }

		$view_available = $mndb->get_results("SELECT ID, post_title FROM {$mndb->posts} WHERE post_type='view' AND post_status in ('publish')");
		foreach($view_available as $view) {

			$view_settings = get_post_meta($view->ID, '_mnv_settings', true);
			if (isset($view_settings['query_type'][0]) && $view_settings['query_type'][0] == 'posts' && !$MN_Views->is_archive_view($view->ID)) {
            ...
            if ( !$standard_v ) {
                $addon_button = '<span class="button vicon mnv-shortcode-post-icon '. $icon_class .'"><img src="' . $this->media_button_image . '" />' . $button_label . '</span>';
                // $addon_button = '<input id="addingbutton" alt="#TB_inline?inlineId=add_field_popup" class="thickbox mnv_add_fields_button button-primary field_adder" type="button" value="'. __('Add field', 'mnv-views') .'" name="" />';
                //$addon_button = '<span class="mnv_add_fields_button button-primary field_adder">'. __('Add field', 'mnv-views') .'</span>';
            }
        ...
        function _output_media_menu( $menu, $text_area, $standard_v ) {
                        ...
                        if ( $menu_item[3] != '' ) {
                            if ( !($key == 'css') ) { // hide unnecessary elements from the V popup
                                if ( !$standard_v && (strpos( $menu_item[3],
                                                'mncfFieldsEditorCallback' ) !== false ||
                                        strpos( $menu_item[3],
                                                'mncfFieldsEmailEditorCallback' ) !== false ||
                                        strpos( $menu_item[3],
                                                'mnv_insert_view_form_popup' ) !== false) ) {
                                    $out .= $this->mnv_parse_menu_item_from_addfield( $menu_item );
                                } else {
                                    $out .= sprintf(
                            ...
                            if ( $standard_v ) {
                                ...
                                if ( $menu_item[2] == __( 'Post View',
                                                'mnv-views' ) || $menu_item[2] == __( 'Taxonomy View',
                                                'mnv-views' ) || $menu_item[2] == __( 'User View',
                                                'mnv-views' ) ) {
                                    $short_code = 'mnv-view name="' . $short_code . '"';
            ...
            if ( 'mncf' != $key && 'views' != $key ) {  // for some reason it displays a group mncf on sites with MNLANG different from ''
                        // a sum menu.
                        /*
                         * SRDJAN
...
//                        if($key == __('Taxonomy', 'mnv-views') || $key == __('Basic', 'mnv-views')) {
...
//                        $out .= '<div class="group '. $css_classes .'"><div class="group-title" id="editor-addon-link-' . md5($key) . '-target">' . $key . "&nbsp;&nbsp;\n</div>\n";
         ...
         * @return string
         */
        function mnv_parse_menu_item_from_addfield( $menu_item ) {
            $param1 = '';
            $slug = $menu_item[1];

            // search for mnv- starting fields first
            if ( strpos( $slug, 'mnv-' ) !== false ) {
                $menuitem_parts = explode( ' ', $slug );
                $slug = $menuitem_parts[0];
            }
            // find types fields
            else if ( (strpos( $menu_item[3], 'mncfFieldsEditorCallback' ) !== false)
                    || (strpos( $menu_item[3], 'mncfFieldsEmailEditorCallback' ) !== false)
                    || (strpos( $menu_item[3], 'mnv_insert_view_form_popup' ) !== false) ) {
                return '<li class="item" onclick="on_add_field_mnv_types_callback(\'' . esc_js( $menu_item[3] ) . '\', \'' . esc_js( $menu_item[0] ) . '\'); return false;">' . $menu_item[0] . "</li>\n";
            ...
            if ( $menu_item[2] == __( 'Basic', 'mnv-views' ) ) {
                // don't use slug here, just field name.
                $slug = $menu_item[0];
            }
            // View Templates here
            if ( $menu_item[2] == __( 'Content Template', 'mnv-views' ) ) {
                $param1 = 'Content Template';
            }
            if ( $menu_item[2] == __( 'Post View', 'mnv-views' ) || $menu_item[2] == __( 'Taxonomy View',
                            'mnv-views' ) || $menu_item[2] == __( 'User View', 'mnv-views' ) ) {
                $param1 = 'Child View';
            }
            if ( strpos( $slug, 'mnv-post-field' ) !== false ) {
                $param1 = __('Field', 'mnv-views');
                $slug = $menu_item[0];
            }
            // Taxonomies
            if ( strpos( $menu_item[1], 'mnv-post-taxonomy' ) !== false ) {
                $slug = $menu_item[1];
                $param1 = 'Taxonomy';
                if ( preg_match( '/mnv-post-taxonomy type="([^"]*)"/', $slug,
                                $matches ) > 0 ) {
                    $slug = 'mnvtax-' . $matches[1]; // split up and pass text only
                } else {
                    $slug = esc_html( $menu_item[1] );
                    $slug = str_replace( 'mnv-post-taxonomy', 'mnv-taxonomy',
                            $slug );
                }
                /* $slug = esc_html($menu_item[1]);
                  $slug = str_replace('mnv-post-taxonomy', 'mnv-taxonomy', $slug); */
            }
            ...
            return '<li class="item" onclick="on_add_field_mnv(\'' . $param1 . '\', \'' . esc_js( $slug ) . '\', \'' . base64_encode( $menu_item[0] ) . '\')">' . $link_text . "</li>\n";
        }

        // add parent items for Views and Content Templates
        function mnv_add_parent_items( $items ) {
            global $post, $pagenow;
        ...
        function add_view_template_parent_groups( $items ) {
            ...
            foreach ( $view_template_relations as $relation => $value ) {
                if ( $value == $view_template_id ) {
                    ...
                    if ( function_exists( 'mncf_pr_get_belongs' ) ) {
                        $parent_types[] = mncf_pr_get_belongs( $relation );
                    }
                }
            }
            ...
            foreach ( $parent_types as $type ) {
                foreach ( $type as $typename => $typeval ) {
                    $parent_groups = mncf_admin_get_groups_by_post_type( $typename );
                }
            }
        ...
        function get_search_bar() {
            ...
            $searchbar .=   '<label for="searchbar-input">' . __( 'Search', 'mnv-views' ) . ': </label>';
            $searchbar .=   '<input id="searchbar-input" type="text" class="search_field" onkeyup="mnv_on_search_filter(this)" />';
            $searchbar .=   '<input type="button" class="button-secondary search_clear" value="' . __( 'Clear','mnv-views' ) . '" onclick="mnv_search_clear(this)" style="display: none;" />';
            $searchbar .= '</p>';
            return $searchbar;
        }
        ...
        function add_view_type( &$menus, $post_type, $post_name ) {
            ...
            $view_templates_available = $mndb->get_results(
                $mndb->prepare(
                    "SELECT ID, post_title, post_name FROM {$mndb->posts} 
					WHERE post_type = %s 
                ...
                if ( $post_type == 'view' ) {
                    ...
                    if ( isset( $view_settings['query_type'] ) && isset( $view_settings['query_type'][0] ) && $view_settings['query_type'][0] == 'taxonomy' ) {
                        $title = $vtemplate->post_title . ' - ' . __( 'Taxonomy View',
                                        'mnv-views' );
                        if ( $post_name == __( 'Post View', 'mnv-views' ) || $post_name == __( 'User View', 'mnv-views' ) ) {
                            continue;
                        }
                        ...
                        $title = $vtemplate->post_title . ' - ' . __( 'User View',
                                        'mnv-views' );
                        if ( $post_name == __( 'Post View', 'mnv-views' ) || $post_name == __( 'Taxonomy View', 'mnv-views' ) ) {
                            continue;
                        }
                    } else {
                        if ( $post_name == __( 'Taxonomy View', 'mnv-views' ) || $post_name == __( 'User View', 'mnv-views' ) ) {
                            continue;
                        }
                    }
...
/*
      Add the mnv_views button to the toolbar.
     */
    function mnv_mce_add_button( $buttons )
    {
     ...
     */
    if( !function_exists('mnv_mce_register') )
    {
        function mnv_mce_register( $plugin_array )
        {
