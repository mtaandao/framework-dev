...
window.Custom Content = window.Custom Content || {};
...
if (typeof MNV_Toolset == 'undefined') {
    var MNV_Toolset = {};
    MNV_Toolset.message = {};
    MNV_Toolset.message.container = null;
}

if (typeof MNV_Toolset.Utils == 'undefined') MNV_Toolset.Utils = {};

MNV_Toolset.Utils.eventDispatcher = _.extend({}, Backbone.Events);

MNV_Toolset.Utils.restoreEventPropagation = function (event) {
...
MNV_Toolset.Utils.do_ajax_post = function (params, callback_object) {
    jQuery.post(ajaxurl, params, function (response) {
            ...
            if ((typeof(response) !== 'undefined') && response !== null && ( response.message || response.Data )) {
                ...
                MNV_Toolset.Utils.eventDispatcher.trigger('on_ajax_success_' + params.action, response, params);
            }
                ...
                MNV_Toolset.Utils.eventDispatcher.trigger('on_ajax_error_' + params.action, response, params);
            }
        }, 'json')
        .fail(function (jqXHR, textStatus, errorThrown) {
            ...
            MNV_Toolset.Utils.eventDispatcher.trigger('on_ajax_fail_' + params.action, textStatus, errorThrown, params);
        })
        .always(function () {
            ...
            MNV_Toolset.Utils.eventDispatcher.trigger('on_ajax_complete_' + params.action, arguments, params);
        });
};
...
;(function ($, window, document, undefined) {
    ...
    var pluginName = "mnvToolsetMessage",
        ...
        init: function () {
            ...
            if (self.prms.close) {
                ...
                self.remove.on('click', function (event) {
                    self.mnvMessageRemove();
                });
            }


            //if( is_open ) self.mnvMessageRemove();
            ...
            self.box.fadeTo(null != prev ? 0 : self.prms.fadeIn, 1, function () {
                ...
                if (self.prms.stay) {
                    ...
                    var remove_message = _.bind(self.mnvMessageRemove, self);
        ...
        mnvMessageRemove: function () {
            ...
            if (self.box || self.container.data('message-box')) {
                ...
                box.fadeTo(self.prms.fadeOut, 0, function () {
                    $(this).trigger('mnv-message-remove');
                    is_open = false;
                    prev = null;
                    prev_text = '';
                    has_stay = false;
...
if (typeof jQuery.fn.mnvToolsetHelp === 'undefined') {

    /* Help messages */
    (function ($) {

        $.fn.mnvToolsetHelp = function (options) {
            ...
            var $footer = $('<div class="toolset-help-footer"><button class="js-toolset-help-close js-toolset-help-close-forever button-secondary">' + mnv_help_box_texts.mnv_dont_show_it_again + '</button><button class="js-toolset-help-close js-toolset-help-close-once button-primary">' + mnv_help_box_texts.mnv_close + '</button></div>');
...
(function ($) {
    ...
    $.fn.ddlWpPointer = function (action, options) {
                ...
                $el
                    .pointer({
                        ...
                        close: function () {
                            ...
                            $el.data('has-mnpointer', false);
                            $el.trigger('help_tooltip_closes', options);
                        }
                    })
                    .pointer('open');

                $el.data('has-mnpointer', true);
            }
        };
        ...
        if (typeof(action) !== 'undefined') { // When 'action' parameter is given
            ...
            if ($el.data('has-mnpointer')) { // hide pointer if it's displayed
                hidePointer();
            }
...
MNV_Toolset.Utils.Loader = function () {
    //fake comment
    var self = this;
...
if (typeof _ != 'undefined') {
    MNV_Toolset.Utils.flatten = function (x, result, prefix) {
        if (_.isObject(x)) {
            _.each(x, function (v, k) {
                MNV_Toolset.Utils.flatten(v, result, prefix ? prefix + '_' + k : k)
            })
        } else {
            result[prefix] = x
        }
    ...
    MNV_Toolset.Utils.flatten_filter_by_key = function (x, result, prefix, filter) {
        var res = [],
            find = MNV_Toolset.Utils.flatten(x, result, prefix);
    ...
    MNV_Toolset.Utils.containsObject = function (obj, list) {
...
MNV_Toolset.Utils.escapeHtml = function (str) {
...
MNV_Toolset.Utils.editor_decode64 = function (input) {
    var output = "",
        chr1, chr2, chr3 = "",
        enc1, enc2, enc3, enc4 = "",
        i = 0,
    ...
    return MNV_Toolset.Utils.editor_utf8_decode(output);
};

MNV_Toolset.Utils.editor_utf8_decode = function (utftext) {
    var string = "";
    var i = 0;
    var c = c1 = c2 = 0;
...
MNV_Toolset.Utils.fixedCharCodeAt = function (str, idx) {
...
MNV_Toolset.replace_unicode_characters = function (string) {
    ...
    for (var i = 0; i < unicode.length; i++) {
        string = string.replace(new RegExp(unicode.charAt(i).regexEscape(), 'g'), MNV_Toolset.Utils.fixedCharCodeAt(unicode.charAt(i)));
    }
...
MNV_Toolset.Utils._strip_scripts = function (data) {
 ...
 *
 *     MNV_Toolset.Utils._strip_tags_and_preserve_text(_.unescape(text))
 *
 ...
 * @since unknown
 */
MNV_Toolset.Utils._strip_tags_and_preserve_text = function (text) {
 ...
 */
MNV_Toolset.Utils.setConfirmUnload = function (checkIfConfirmationNeededCallback, onBeforeUnloadCallback, confirmationMessage) {
...
MNV_Toolset.Utils.has_shortcode = function (string) {
 ...
 * @returns int
 */
MNV_Toolset.Utils.versionCompare = function(left, right) {
 ...
 */
MNV_Toolset.Utils.getParameterByName = function(name, url) {
 ...
 */
MNV_Toolset.Utils.updateUrlQuery = function(key, value, url) {
    if(!url) {
        url = window.location.href;
    }
...
/**
 * Tools for manipulating spinners in a native MN way.
 *
 * If MN changes the behaviour, we'll need only to update this object.
 *
 * @type {{create: MNV_Toolset.Utils.Spinner.create, show: MNV_Toolset.Utils.Spinner.show, hide: MNV_Toolset.Utils.Spinner.hide, find: MNV_Toolset.Utils.Spinner.find}}
 */
MNV_Toolset.Utils.Spinner = {
    create: function() {
    ...
    show: function(spinner, show) {
        ...
        if(!show) {
            MNV_Toolset.Utils.Spinner.hide(spinner);
            return;
        }
 ...
 * AJAX call helper.
 *
 * WIP, to be merged with MNV_Toolset.Utils.do_ajax_post().
 *
 * @type {{parseResponse: MNV_Toolset.Utils.Ajax.parseResponse, call: MNV_Toolset.Utils.Ajax.call}}
 * @since 2.1
 */
MNV_Toolset.Utils.Ajax = {
     ...
     *
     * @param {*} data AJAX call parameters. Needs to include 'action' and 'mnnonce'.
    ...
    call: function(data, successCallback, failCallback) {
        ...
        jQuery.post({
            ...
            success: function(originalResponse) {
                var response = MNV_Toolset.Utils.Ajax.parseResponse(originalResponse);
...
MNV_Toolset.Utils._template = function( template, data, settings ){
    var options = _.defaults({}, settings, _.templateSettings),
        _template = null;

    if( MNV_Toolset.Utils.versionCompare(_.VERSION, '1.7') >= 0 ){
                ...
                if ( qt_instance.theButtons[button_name].id == 'img' ){
                    ...
                    qt_instance.theButtons[button_name].callback =
                        function ( b, c, d, e ) {
                            activeUrlEditor = c;
                            var f,g=this;
                            return "undefined" != typeof mnLink ?void mnLink.open(d.id) : (e||(e="http://"), void(g.isOpen(d)===!1 ? (f=prompt(quicktagsL10n.enterURL,e), f && (g.tagStart='<a href="'+f+'">', a.TagButton.prototype.callback.call(g,b,c,d))) : a.TagButton.prototype.callback.call(g,b,c,d)))
                        };
