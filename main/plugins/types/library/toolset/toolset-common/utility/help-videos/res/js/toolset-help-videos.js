var MN_Custom Content = MNV_Toolset || {};

MN_Toolset.HelpVideos = {};
...
MN_Toolset.HelpVideosFactory = function ($) {
    var self = this,
        videos = MN_ToolsetVideoSettings.video_instances,
        current = MN_ToolsetVideoSettings.current,
        seen = MN_ToolsetVideoSettings.seen,
        collection,
        append_done = false,
    ...
    self.populate_collection = function(){
        collection = new MN_Toolset.HelpVideosCollection();
        _.each(videos, function(v){
            collection.add( new MN_Toolset.HelpVideo( v ) );
        });
        return collection;
    };
    ...
    self.handle_list = function(){
        ...
        collection_view = new MN_Toolset.HelpVideosListView({model:models});
        jQuery( '.js-toolset-videos-wrapper' ).append( jQuery(MN_ToolsetVideoSettings.VIDEOS_LIST_TITLE), collection_view.$el );
    };

    self.show_video = function( video ){
        if ( videos.hasOwnProperty(video) ) {

            if( adminpage !== MN_ToolsetVideoSettings.detached_page && videos[video].hasOwnProperty('append_to') && videos[video].append_to !== '' ){
                self.create_on_the_fly( videos[video].element ? videos[video].element : MN_ToolsetVideoSettings.GENERIC_ELEMENT, videos[video].append_to );
            }

            var model = MN_Toolset.HelpVideos.hasOwnProperty(video) ? MN_Toolset.HelpVideos[video] : new MN_Toolset.HelpVideo(videos[video]),
                view  = new MN_Toolset.HelpVideoView({
                    el: videos[video].element ? videos[video].element : MN_ToolsetVideoSettings.GENERIC_ELEMENT,
                    model: model
                });

            if( adminpage !== MN_ToolsetVideoSettings.detached_page && self.get_seen() === 'seen' && triggered_manually === false ){
                view.$el.hide();
                view.manual_trigger();
            ...
            MN_Toolset.HelpVideos[video] = model;

            return MN_Toolset.HelpVideos[video];
        }
...
MN_Toolset.HelpVideo = Backbone.Model.extend({
    defaults: {
        title:'',
        name: '',
        url: '',
...
MN_Toolset.HelpVideosCollection = Backbone.Collection.extend({
    model:MN_Toolset.HelpVideo,
    current:null
});


MN_Toolset.HelpVideoView = Backbone.View.extend({
    DELAY:200,
    initialize: function (options) {
        var self = this;
        self.template_selector = '#toolset-video-template';
        self.template = _.template(jQuery(self.template_selector).html());
        self.deatch_url = MN_ToolsetVideoSettings.detach_url;
    ...
    handle_detach:function(){
        ...
        if( adminpage === MN_ToolsetVideoSettings.detached_page ){
            $button.hide();
        } else {
    ...
    remove_video:function(){
        ...
        self.remove_button.on('click', function(event){
                ...
                self.$el.hide(400, function(){
                    if( adminpage === MN_ToolsetVideoSettings.detached_page ){
                        MN_Toolset.HelpVideos.main.handle_list();
                    } else {
    ...
    insert_title_view:function(){
        var self = this,
            ...
            element = self.model.get('element') ? self.model.get('element').substring(1) : MN_ToolsetVideoSettings.GENERIC_ELEMENT,
            new_me = new MN_Toolset.HelpVideoListView({
                model:self.model,
...
MN_Toolset.HelpVideoListView = Backbone.View.extend({
    initialize:function(options){
        var self = this;
    ...
    show:function(){
        ...
        self.$el.on('click', function(event){
            ...
            event.preventDefault();
            if( adminpage === MN_ToolsetVideoSettings.detached_page ) {
                MN_Toolset.HelpVideos.main.show_new_video(self.model);
            } else {
    ...
    insert_video_view:function(){
        ...
        self.remove();
        MN_Toolset.HelpVideos.main.show_video( self.model.get('name') );
    }
});

MN_Toolset.HelpVideosListView = Backbone.View.extend({
    el:'.js-videos-list',
    tagName: 'ul',
    initialize:function(options){
        var self = this;
    ...
    appendModelElement:function( option ){
        ...
        self.model.each(function(model){
            ...
            try{
                ...
                view = new MN_Toolset.HelpVideoListView({
                        model:model,
...
(function ($) {
    $(function () {
        MN_Toolset.HelpVideos.main = {};
        MN_Toolset.HelpVideosFactory.call(MN_Toolset.HelpVideos.main, $);
    });
}(jQuery));