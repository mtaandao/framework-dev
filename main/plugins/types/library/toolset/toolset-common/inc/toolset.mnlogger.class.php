...
if ( ! defined( 'MNT_MNLOGGER' ) ) {
    define( 'MNT_MNLOGGER', true );
}
...
* 
* Usage:
* 	toolset_mnlog( mixed php_expression [, const message_type] )
...
if ( ! function_exists( 'toolset_mnlog' ) ) {

	function toolset_mnlog( $message = '', $msgType = null, $file = '', $function = '', $line = '' ) {
		$toolset_mnlog = Toolset_MNLogger::get_instance();
		$toolset_mnlog->log( $message, $msgType, $file, $function, $line );
	}
...
if ( ! class_exists( 'Toolset_MNLogger' ) ) {
	...
	class Toolset_MNLogger {
		...
		public function __construct() {
			add_action( 'mn_footer',	array( $this, 'flushLogMessages' ) );
			add_action('admin_footer',	array( $this, 'flushLogMessages' ) );
		}
		
		public static function get_instance() {
			if ( ! self::$instance ) {
				self::$instance = new Toolset_MNLogger();
			}
