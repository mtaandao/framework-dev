...
if ( ! defined( 'MNT_SETTINGS_SCREEN' ) ) {
    define( 'MNT_SETTINGS_SCREEN', true );
}

/**
* Toolset_Settings_Screen
*
* Generic class for the shared settings entry for the Custom Content family.
*
* @since 1.9
*/
    ...
    class Toolset_Settings_Screen {
		...
		public function register_settings_page_in_menu( $pages ) {
			$pages[] = array(
				...
				'menu_title'	=> __( 'Settings', 'mnv-views' ),
				'page_title'	=> __( 'Settings', 'mnv-views' ),
		...
		function init() {
			...
			add_action( 'mn_ajax_toolset_update_toolset_admin_bar_options',			array( $this, 'toolset_update_toolset_admin_bar_options' ) );
		...
		function settings_page() {
			...
			<div class="wrap">
				<h1><?php _e( 'Custom Content Settings', 'mnv-views' ) ?></h1>
		...
		function toolset_admin_bar_settings( $sections, $toolset_options ) {
			...
			ob_start();
			?>
			<h3><a name="shortcodes-settings" href="#"></a><?php echo __( 'Custom Content shortcodes menu in the admin bar', 'mnv-views' ); ?></h3>
			<div class="toolset-advanced-setting">
				<p>
					<?php _e( "Custom Content can display an admin bar menu in the backend to let you generate Custom Content shortcodes in any page that you need them.", 'mnv-views' ); ?>
				</p>
				<ul class="js-shortcode-generator-form">
					<?php
						...
						$shortcodes_generator_options = array(
							array(
								'label' =>  __( 'Disable the Custom Content shortcodes menu in the admin bar', 'mnv-views' ),
								'value' => 'disable'
							),
							array(
								'label' => __( 'Show the Custom Content shortcodes menu in the admin bar only when editing content', 'mnv-views' ),
								'value' => 'editor'
							),
							array(
								'label' => __( 'Show the Custom Content shortcodes menu in the admin bar in all the admin pages', 'mnv-views' ),
								'value' => 'always'
							)
						);
						foreach( $shortcodes_generator_options as $option ) {
							printf(
								'<li><label><input type="radio" name="mnv-shortcodes-generator" class="js-toolset-shortcodes-generator js-toolset-admin-bar-options" value="%s" %s autocomplete="off" />%s</label></li>',
								$option['value'],
			...
			<h3><a name="design-with-toolset-settings" href="#"></a><?php echo __( 'Design with Toolset', 'mnv-views' ); ?></h3>
			<div class="toolset-advanced-setting">
				<p>
					<?php _e( "Custom Content can display an admin bar menu in the frontend to let you create or edit Views and Content Templates related to the current page.", 'mnv-views' ); ?>
				</p>
				<p>
					<label>
						<input type="checkbox" name="mnv-toolset-admin-bar-menu" id="js-toolset-admin-bar-menu" class="js-toolset-admin-bar-menu js-toolset-admin-bar-options" value="1" <?php checked( $toolset_admin_bar_menu_show ); ?> autocomplete="off" />
						<?php _e( "Enable the Design with Custom Content menu in the frontend", 'mnv-views' ); ?>
					</label>
				</p>
			</div>
			<?php
			mn_nonce_field( 'toolset_admin_bar_settings_nonce', 'toolset_admin_bar_settings_nonce' );
			?>
			<?php
			...
			$sections['admin-bar-settings'] = array(
				...
				'title'		=> __( 'Admin bar options', 'mnv-views' ),
		...
		function toolset_update_toolset_admin_bar_options() {
			...
			if ( 
				! isset( $_POST["mnnonce"] )
				|| ! mn_verify_nonce( $_POST["mnnonce"], 'toolset_admin_bar_settings_nonce' ) 
			) {
				$data = array(
					'type' => 'nonce',
