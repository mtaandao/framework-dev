...
/**
* Helper functions for the Custom Content family
*
* @since unknown
*/
...
if ( ! function_exists( 'mnv_form_control' ) ) {
	function mnv_form_control( $elements ) {
...
if ( ! function_exists( 'icl_get_file_relpath' ) ) {
	function icl_get_file_relpath( $file ) {
		...
		$path_to_file = mn_normalize_path($file);
		// get file directory
		$file_dir = mn_normalize_path( dirname( $path_to_file ) );
		// get the path to 'mn-content'
		$from_content_dir = mn_normalize_path( realpath( MN_CONTENT_DIR ) );
		// get mn-content dirname
		$content_dir = mn_normalize_path( basename(MN_CONTENT_DIR) );

		// remove absolute path part until 'mn-content' folder
		$path = str_replace( $from_content_dir, '', $file_dir);
		// add mn-content dir to path
		$path = mn_normalize_path( $content_dir.$path );
...
/**
 * Fix MN's multiarray parsing.
 * 
 * @param type $arg
 * @param type $defaults
 * @return type 
 */
if ( ! function_exists( 'mnv_parse_args_recursive' ) ) {
	function mnv_parse_args_recursive( $arg, $defaults ) {
		$temp = false;
		if ( isset( $arg[0] ) ) {
			$temp = $arg[0];
		...
		$arg = mn_parse_args( $arg, $defaults );
		if ( $temp ) {
			$arg[0] = $temp;
		}
		foreach ( $defaults as $default_setting_parent => $default_setting ) {
			...
			$arg[$default_setting_parent] = mnv_parse_args_recursive( $arg[$default_setting_parent],
...
if ( ! function_exists( 'mnv_filter_parse_date' ) ) {
     ...
     */
    function mnv_filter_parse_date($date_format)
    {
            ...
            for ($i = 0; $i < $occurences; $i++) {
                ...
                switch (strtoupper($date_func)) {
                    ...
                    case "YEARS_FROM_NOW":
                        ...
                        break;
                    case "DATE":
                        $date_object = mnv_filter_get_date_and_format($date_value);
                        $date_value = $date_object->date;
                        $format = $date_object->format;
                        $resulting_date = mnv_filter_parse_date_get_resulting_date( $date_value, $format );
                        break;
...
if ( ! function_exists( 'mnv_filter_parse_date_get_resulting_date' ) )
{
	function mnv_filter_parse_date_get_resulting_date( $date_value, $format )
	{
...
if ( ! function_exists( 'mnv_filter_get_date_and_format' ) )
{
	function mnv_filter_get_date_and_format($date_value)
	{
...
/*
 * Extra check for date for shortcode in shortcode. Called as filter in mnv_condition bellow.
 *
 * @note As of 1.9 this is not used in Views anymore
 */
if ( ! function_exists( 'mnv_add_time_functions' ) ) {
	function mnv_add_time_functions( $value ) {
		return mnv_filter_parse_date( $value );
	}
}
...
if ( ! function_exists( 'mnv_condition' ) ) {
	function mnv_condition( $atts, $post_to_check = null ) {
		extract(
	...
	//    global $post;
		...
		if ( $has_post ) {
			do_action( 'mnv_condition', $post );
		}

		$logging_string = "Original expression: " . $evaluate;

		add_filter( 'mnv-extra-condition-filters', 'mnv_add_time_functions' );
		$evaluate = apply_filters( 'mnv-extra-condition-filters', $evaluate );
		...
		if ( $strings_count && $strings_count > 0 ) {
			for ( $i = 0; $i < $strings_count; $i++ ) {
				...
				if ( !(is_numeric( $first_string ) && is_numeric( $second_string )) ) {
					...
					$compared_str_result = mnv_compare_strings( $first_string,
		...
		if ( $has_post ) {
			...
			if ( $count && $count > 0 ) {
				...
				$matches[1] = mnv_sort_matches_by_length( $matches[1] );
		...
		toolset_mnlog( $logging_string, 'debug', __FILE__, 'mnv_condition', 307 );
		// evaluate the prepared expression using the custom eval script
		$result = mnv_evaluate_expression( $evaluate );
		
		if ( $has_post ) {
			do_action( 'mnv_condition_end', $post );
		}
...
if ( ! function_exists( 'mnv_eval_check_syntax' ) ) {
	function mnv_eval_check_syntax( $code ) {
		try {
 ...
 */
if ( ! function_exists( 'mnv_sort_matches_by_length' ) ) {
	function mnv_sort_matches_by_length( $matches ) {
 ...
 * 
 */
if ( ! function_exists( 'mnv_compare_strings' ) ) {
	function mnv_compare_strings( $first, $second, $sign ) {
 ...
 */
if ( ! function_exists( 'mnv_evaluate_expression' ) ) {
	function mnv_evaluate_expression( $expression ){
		//Replace AND, OR, ==
		...
		if ( $count > 0 ) {
			if ( strlen( $matches[0] ) == strlen( $expression ) ) {
				$valid_eval = mnv_eval_check_syntax( "return $expression;" );
				if ( $valid_eval ) {
