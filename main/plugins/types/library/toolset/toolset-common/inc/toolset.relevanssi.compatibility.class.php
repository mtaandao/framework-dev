	...
	class Toolset_Relevanssi_Compatibility {
		...
		function init() {
			...
			if ( $this->toolset_types_installed ) {
				...
				add_action( 'mn_ajax_toolset_update_toolset_relevanssi_settings',			array( $this, 'toolset_update_toolset_relevanssi_settings' ) );
				
				$this->toolset_settings_url = admin_url( 'admin.php?page=toolset-settings&tab=relevanssi' );
				
				// Custom Content Types
				
				add_filter( 'mncf_form_field',												array( $this, 'types_extend_field_settings' ), 5, 3 );
				add_filter( 'mncf_field_pre_save',											array( $this, 'types_flag_field_on_save' ) );
				add_action( 'mncf_postmeta_fields_group_saved',								array( $this, 'types_store_fields_on_group_save' ) );
				...
				if ( 
					...
					add_filter( 'mnv_filter_query',											array( $this, 'mnv_filter_query_compatibility' ), 99, 3 );
					add_filter( 'mnv_filter_query_post_process',							array( $this, 'mnv_filter_query_post_proccess_compatibility' ), 99, 3 );
					// Register search content options
					add_filter( 'mnv_filter_mnv_extend_post_search_content_options',		array( $this, 'mnv_extend_post_search_content_options' ) );
					// Fix Relevanssi on archive pages by forcing the Relevanssi query when needed
					add_action( 'mnv_action_mnv_before_clone_archive_loop',					array( $this, 'mnv_fix_relevanssi_on_archive_loops' ), 10, 2 );
					...
					add_action( 'toolset_action_toolset_relevanssi_do_query_before',		array( $this, 'mnv_fix_relevanssi_orderby' ) );
					// Fix Relevanssi max_num_pages
					add_action( 'toolset_action_toolset_relevanssi_do_query_processed',		array( $this, 'mnv_fix_relevanssi_max_num_pages' ) );
					// Fix Relevanssi returning posts as objects but not as MN_Post objects
					add_action( 'toolset_action_toolset_relevanssi_do_query_processed',		array( $this, 'mnv_fix_relevanssi_return_as_post_objects' ) );
				}
			}
		...
		/*
		* ---------------------
		* Custom Content Common integration
		* ---------------------
		*/
		...
		/**
		* Text search tab inside the Custom Content Settings page.
		*
		* @since 2.2
		*/
		...
		function register_relevanssi_settings_section( $registered_sections ) {
			$registered_sections['relevanssi'] = array(
				'slug'	=> 'relevanssi',
				'title'	=> __( 'Text Search', 'mnv-views' )
			);
			return $registered_sections;
		}
		
		/**
		* Text search tab content inside the Custom Content Settings page.
		*
		* @since 2.2
		*/
		...
		function toolset_relevanssi_settings( $sections, $toolset_options ) {
			...
			$section_content = '';
			$text_search_documentation_link = 'https://mn-types.com/documentation/user-guides/views-text-search/?utm_source=viewsplugin&utm_campaign=views&utm_medium=views-text-search-settings&utm_term=Text Search documentation';
			...
			if ( ! $this->relevanssi_installed ) {
				...
				<div class="notice inline notice-warning notice-alt">
					<p><?php _e( 'You need to install <strong>Relevanssi</strong>', 'mnv-views' ); ?></p>
				</div>
				<p>
				<?php
				echo sprintf( 
					__( '%1$sRelevanssi%2$s plugin extends and improves the Mtaandao text search. Relevanssi allows to search in custom fields and returns the most relevant results first.', 'mnv-views' ),
				...
				if ( current_user_can( 'install_plugins' ) ) {
					echo sprintf(
						__( 'Please %1$sinstall Relevanssi%2$s to allow Custom Content to search texts in custom fields.', 'mnv-views' ),
						'<a href="' . admin_url( 'plugin-install.php?s=relevanssi&tab=search&type=term' ) . '">',
						'</a>'
					);
				} else {
					echo sprintf(
						__( 'You can download Relevanssi it from %3$smtaandao.org%4$s and ask your site administrator to install it.', 'mnv-views' ),
						'<a href="https://mtaandao.org/plugins/relevanssi/" target="_blank">',
						'</a>'
					);
				}
				?>
				...
				if ( empty( $indexable_fields ) ) {
					...
					<div class="notice inline notice-warning notice-alt">
						...
						<?php
						echo sprintf(
							__( '%1$sOnce you %2$ssetup textual custom fields in Types%3$s (single line, multiple lines, WYSIWYG), you will be able to include them in text searches here.', 'mnv-views' ),
							'<i class="icon-types-logo ont-color-orange ont-icon-24" style="margin-right:5px;vertical-align:-2px;"></i>',
							'<a href="' . admin_url( 'admin.php?page=mncf-cf' ) . '">',
							'</a>'
						);
						?>
						</p>
					...
					?>
					<h3>
						<?php echo esc_html( __( 'Select which custom fields to include in text searches', 'mnv-views' ) ); ?>
					</h3>
					...
					<div class="js-toolset-relevanssi-list-summary"<?php echo ( count( $actual_fields_to_index ) > 0 ) ? '' : ' style="display:none"'; ?>>
						<h3>
							<?php echo esc_html( __( 'Update the Relevanssi settings and rebuild the index', 'mnv-views' ) ); ?>
						</h3>
						<ol style="list-style-type:upper-roman">
						<li>
						<?php
						_e( 'Copy this list of field names:', 'mnv-views' );
						?>
						<input type="text" 
							readonly="readonly" 
						...
						<?php
						echo sprintf(
							__( 'Paste into "Custom fields to index" in the %1$sRelevanssi settings%2$s. Remember to double check the post types included in the index, and notice that you can also add taxonomies to it.', 'mnv-views' ),
						...
						<?php
						echo sprintf(
							__( 'Click on "Save indexing options, erase index and rebuild the index" in the %1$sRelevanssi settings%2$s.', 'mnv-views' ),
							'<a href="' . admin_url( 'options-general.php?page=relevanssi/relevanssi.php#indexing' ) . '">',
							'</a>'
						);
						?>
						...
						<?php
						_e( 'Your site has one list of custom fields to index. This means that all the text searches will return results from these custom fields.', 'mnv-views' );
						?>
						</p>
						<p>
						<?php
						echo sprintf(
							__( 'See how to do this in the %1$sText Search documentation%2$s.', 'mnv-views' ),
					...
					<div class="js-toolset-relevanssi-list-summary-empty"<?php echo ( count( $actual_fields_to_index ) == 0 ) ? '' : ' style="display:none"'; ?>>
						...
						<?php
						_e( 'You have not selected any field to be indexed in Relevanssi searches. Please select some below.', 'mnv-views' );
						?>
						</p>
					</div>					
					<?php
					mn_nonce_field( 'toolset_relevanssi_settings_nonce', 'toolset_relevanssi_settings_nonce' );
				}
			...
			$sections['relevanssi-settings'] = array(
				...
				'title'		=> __( 'Text search in custom fields', 'mnv-views' ),
		...
		function toolset_update_toolset_relevanssi_settings() {
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				$data = array(
					'type' => 'capability',
					'message' => __( 'You do not have permissions for that.', 'mnv-views' )
				);
				mn_send_json_error( $data );
			}
			if ( 
				! isset( $_POST["mnnonce"] )
				|| ! mn_verify_nonce( $_POST["mnnonce"], 'toolset_relevanssi_settings_nonce' ) 
			) {
				$data = array(
					'type' => 'nonce',
					'message' => __( 'Your security credentials have expired. Please reload the page to get new ones.', 'mnv-views' )
				);
				mn_send_json_error( $data );
			}
		...
		function types_extend_field_settings( $form, $data, $field_type = '' ) {
			...
			if (
				...
				$this_field_description		= '<span class="js-toolset-toggle-relevanssi-index-description"' . $this_field_description_style . '>' . sprintf(
							__( 'Go to %1$sText Search settings%2$s to build the search index.', 'mncf' ),
				...
				$form['relevanssi_index'] = array(
						...
						'#inline' => true,
						'#title' => __( 'Include in search', 'mncf' ),
						'#label' => __( 'Include this field in text searches throughout the site', 'mncf' ),
						'#description' => $this_field_description,
						'#attributes' => array(
							'autocomplete'	=> 'off',
							'class'			=> 'js-toolset-toggle-relevanssi-index'
						),
						'#pattern' => '<tr class="mncf-border-top"><td><TITLE></td><td><ERROR><BEFORE><ELEMENT><LABEL><AFTER><DESCRIPTION></td></tr>'
					);
		...
		function types_store_fields_on_group_save( $group_id ) {
			...
			foreach ( $group_fields as $field ) {
				if ( in_array( $field['slug'], $this->pending_to_add ) ) {
					$real_custom_field_name = mncf_types_get_meta_prefix( $field ) . $field['slug'];
		...
		*/
		...
		/**
		* mnv_filter_query_compatibility
		*
		...
		*/
		...
		function mnv_filter_query_compatibility( $query, $view_settings, $view_id ) {
		...
		/**
		* mnv_filter_query_post_proccess_compatibility
		*
		...
		*/
		...
		function mnv_filter_query_post_proccess_compatibility( $post_query, $view_settings, $view_id ) {
			if ( 
				...
				if ( $search_in == 'content_extended' ) {
					...
					if (
						isset( $_GET['mnv_post_search'] ) 
						&& ! empty( $_GET['mnv_post_search'] )
					) {
		...
		/**
		* mnv_extend_post_search_content_options
		*
		...
		*/
		...
		function mnv_extend_post_search_content_options( $options ) {
			$options['content_extended'] = array(
				'label'			=> __( 'Title, body and custom fields', 'mnv-views' ),
				'description'	=> sprintf(
										__( 'Search in titles, content and custom fields. %1$sText Search settings%2$s.', 'mnv-views' ),
										'<a href="' . admin_url( 'admin.php?page=toolset-settings&tab=relevanssi' ) . '" target="_blank">',
										'</a>'
									),
				'summary'		=> __( 'post title, content and fields', 'mnv-views' )
			);
			return $options;
		}
		
		/**
		* mnv_fix_relevanssi_on_archive_loops
		*
		...
		*/
		...
		function mnv_fix_relevanssi_on_archive_loops( $query, $args ) {
			...
			if (
				isset( $args['mna_slug'] ) 
				&& $args['mna_slug'] != 'view_search-page'
			) {
				$mna_settings = isset( $args['mna_settings'] ) ? $args['mna_settings'] : array();
				if ( 
					isset( $mna_settings['search_mode'] ) 
					&& isset( $_GET['mnv_post_search'] )
					&& ! empty( $_GET['mnv_post_search'] )
				) {
					$search_in = isset( $mna_settings['post_search_content'] ) ? $mna_settings['post_search_content'] : 'full_content';
					if ( $search_in == 'content_extended' ) {
			...
			if ( $do_relevanssi_query ) {
				...
				$mna_object = isset( $args['mna_object'] ) ? $args['mna_object'] : null;
				if ( $mna_object ) {
					if ( empty( $relevanssi_posts ) ) {
						$mna_object->loop_has_no_posts = true;
						$query->post_count = 1;
						...
						$dummy_post = new MN_Post( $dummy_post_obj );
						$query->posts = array( $dummy_post );
					} else {
						$mna_object->loop_has_no_posts = false;
					}
				}
			}
		}
		...
		function mnv_fix_relevanssi_orderby( $query ) {
			$query->set( 'orderby', null );
		}
		
		/**
		* mnv_fix_relevanssi_max_num_pages
		*
		...
		*/
		...
		function mnv_fix_relevanssi_max_num_pages( $query ) {
			if ( 
		...
		/**
		* mnv_fix_relevanssi_return_as_post_objects
		*
		* Relevanssi return posts as dummy objects instead of MN_Post instances, and we need them.
		*
		* @since 2.3
		*/
		
		function mnv_fix_relevanssi_return_as_post_objects( $query ) {
			if ( $query->posts ) {
