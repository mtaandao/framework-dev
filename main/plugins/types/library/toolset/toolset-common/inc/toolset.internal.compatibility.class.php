...
if ( ! class_exists( 'Toolset_Internal_Compatibility' ) ) {
	...
	class Toolset_Internal_Compatibility {
		...
		function __construct() {
			 ...
			 */
			add_filter( 'mnv_filter_mnv_before_set_meta_query', array( $this, 'resolve_types_checkboxes_views_meta_query' ), 10, 2 );
		}
		
		/**
		 * Resolve Types checkboxes query filters in Views and Mtaandao Archives
		 *
		...
		function resolve_types_checkboxes_views_meta_query( $meta_query, $data = array() ) {
			...
			$fields_to_check_slugs	= mn_list_pluck( $fields_to_check, 'meta_key' );
			...
			foreach ( $meta_query as $meta_query_index => $meta_query_entry ) {
				if ( 
					...
					$field_definition_candidates	= mn_list_filter( $fields_to_check, array( 'meta_key' => $meta_query_entry['key'] ) );
