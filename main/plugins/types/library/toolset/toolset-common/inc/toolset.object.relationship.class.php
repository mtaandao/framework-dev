...
if ( ! class_exists( 'Toolset_Object_Relationship' ) ) {
	class Toolset_Object_Relationship
	{
		...
		public function __construct()
        {
			...
			add_filter( 'mnv_filter_mnv_the_content_suppressed', 	array( $this, 'record_post_relationship_belongs' ), 0, 1 );
			
			add_filter( 'the_content', 								array( $this, 'restore_post_relationship_belongs' ), PHP_INT_MAX, 1 );
			add_filter( 'mnv_filter_mnv_the_content_suppressed', 	array( $this, 'restore_post_relationship_belongs' ), PHP_INT_MAX, 1 );
        }
		...
		function record_post_relationship_belongs( $content ) {
			...
			if ( 
				! empty( $post->ID ) 
				&& function_exists( 'mncf_pr_get_belongs' ) 
			) {

				if ( ! isset( $this->relations[ $post->post_type ] ) ) {
					$this->relations[ $post->post_type ] = mncf_pr_get_belongs( $post->post_type );
				}
				if ( is_array( $this->relations[ $post->post_type ] ) ) {
					foreach ( $this->relations[ $post->post_type ] as $post_type => $data ) {
						$related_id = mncf_pr_post_get_belongs( $post->ID, $post_type );
						if ( $related_id ) {
...
/**
 * class MNV_mncf_switch_post_from_attr_id
 *
 * This class handles the "id" attribute in a mnv-post-xxxxx shortcode
 ...
 */
if ( ! class_exists( 'MNV_mncf_switch_post_from_attr_id' ) ) {
	class MNV_mncf_switch_post_from_attr_id
	{
			...
			if ( isset( $atts['id'] ) ) {
				...
				if ( strpos( $atts['id'], '$' ) === 0 ) {
					...
					if ( 
						...
						if ( is_single() || is_page() ) {
							global $mn_query;
							if ( isset( $mn_query->posts[0] ) ) {
								$current_post = $mn_query->posts[0];
						...
						global $MN_Views;
						if ( isset( $MN_Views ) ) {
							$post_id = $MN_Views->get_variable( $atts['id'] . '_id' );
						}
						if ( $post_id == 0 ) {
				...
				if ( $post_id > 0 ) {
					...
					if( get_post_status( $post_id ) === false ) {
						...
						// save original post
						$this->post = ( isset( $post ) && ( $post instanceof MN_Post ) ) ? clone $post : null;
					...
					$authordata = new MN_User( $post->post_author );
		...
		function __destruct() {
			if ( $this->found ) {
				...
				$post = ( isset( $this->post ) && ( $this->post instanceof MN_Post ) ) ? clone $this->post : null;
				if ( $this->authordata ) {
