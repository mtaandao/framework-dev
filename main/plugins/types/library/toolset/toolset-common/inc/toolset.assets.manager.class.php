...
if ( defined( 'MNT_ASSETS_MANAGER' ) ) {
    return; 
}

define( 'MNT_ASSETS_MANAGER', true );

class Toolset_Style
{

	public function __construct( $handle, $path = 'mtaandao_default', $deps = array(), $ver = false, $media = 'screen' ) {
		$this->handle	= $handle;
		$this->path		= $path;
		$this->deps		= $deps;
		$this->ver		= $ver;
		...
		if ( 
			...
			&& $this->path != 'mtaandao_default'
		) {
			mn_register_style( $this->handle, $this->path, $this->deps, $this->ver, $this->media );
		}
	}

	public function enqueue() {
		if ( $this->is_enqueued() === false ) {
			mn_enqueue_style( $this->handle );
		}
	}
	...
	protected function compare_versions() {
			...
			if ( isset( $mn_styles->registered ) ) {
				$mnt_registered_styles = $mn_styles->registered;
				if ( isset( $mnt_registered_styles[ $this->handle ] ) ) {
					$registered = $mnt_registered_styles[ $this->handle ];
					if ( (float) $registered->ver < (float) $this->ver ) {
						$mn_styles->remove( $this->handle );
						return true;
					} else {
						return false;
					}
	...
	public function deregister() {
		if ( $this->is_registered() !== false ) {
			mn_deregister_style( $this->handle );
		}
	}


	protected function is_registered() {
		return mn_style_is( $this->handle, 'registered' );
	}

	protected function is_enqueued() {
		return mn_style_is( $this->handle, 'enqueued' );
	}
}

class Toolset_Script
{
	public function __construct( $handle, $path = 'mtaandao_default', $deps = array(), $ver = false, $in_footer = false ) {
		$this->handle		= $handle;
		$this->path			= $path;
		$this->deps			= $deps;
		$this->ver			= $ver;
		...
		if ( 
			...
			mn_register_script( $this->handle, $this->path, $this->deps, $this->ver, $this->in_footer );
		}
	}

	public function enqueue() {
		if ( $this->is_enqueued() === false ) {
			mn_enqueue_script( $this->handle );
		}
	}
    ...
    protected function compare_versions() {
        ...
        if ( isset( $mn_scripts->registered ) ) {
			$mnt_registered_scripts = $mn_scripts->registered;
			if ( isset( $mnt_registered_scripts[ $this->handle ] ) ) {
				$registered = $mnt_registered_scripts[$this->handle];
				if ( (float) $registered->ver < (float) $this->ver ) {
					$mn_scripts->remove( $this->handle );
					return true;
				} else {
					return false;
				}
	...
	public function localize( $object, $args ) {
		if ( $this->is_registered() ) {
			mn_localize_script( $this->handle, $object, $args );
		}
	}

	public function deregister() {
		if ( $this->is_registered() !== false ) {
			mn_deregister_script( $this->handle );
		}
	}

	protected function is_registered() {
		return mn_script_is( $this->handle, 'registered' );
	}

	protected function is_enqueued() {
		return mn_script_is( $this->handle, 'enqueued' );
	}
}
...
class Toolset_Assets_Manager
{
	...
	* assets_url
	*
	* Base URL for the Custom Content Common instance.
	*
	...
	*/
	...
	protected function __construct() {
		...
		//fe
		add_action( 'mn_enqueue_scripts',		array( $this, 'get_rid_of_default_scripts' ) );
		add_action( 'mn_enqueue_scripts',		array( $this, 'get_rid_of_default_styles' ) );
	...
	public function get_rid_of_default_scripts() {
		global $mn_scripts;
		if ( is_array( $mn_scripts->registered ) ) {
			foreach ( $mn_scripts->registered as $registered ) {
	...
	public function get_rid_of_default_styles() {
		...
		if ( is_array( $mn_styles->registered ) ) {
			foreach ( $mn_styles->registered as $registered ) {
	...
	protected function __initialize_styles() {
        ...
        $this->styles['mnt-toolset-backend']						= new Toolset_Style(
																		'mnt-toolset-backend', 
																		$this->assets_url . '/toolset-forms/css/mnt-toolset-backend.css',
																		array(),
        ...
        $this->styles['ddl-dialogs-general-css'] = new Toolset_Style('ddl-dialogs-general-css', $this->assets_url . '/utility/dialogs/css/dd-dialogs-general.css', array( 'mn-jquery-ui-dialog' ), TOOLSET_VERSION);
	...
	protected function __initialize_scripts() {
		...
		/**
		 * For compatibility with ACF Plugin that's not using the right handle for this module (mn-event-manager)
		 * we are using ACF handle to prevent unwanted overrides of window.mn.hooks namespace (******!)
		 */
        ...
        $this->scripts['icl_editor-script']									= new Toolset_Script(
																				...
																				array(  'jquery', 'quicktags', 'mnlink', 'toolset-codemirror-script' ), 
																				TOOLSET_COMMON_VERSION
																			);
		...
		$this->scripts['mntoolset-parser']									= new Toolset_Script(
																				'mntoolset-parser', 
		...
		$this->localize_script(
            ...
            'toolset_utils_texts',
            array(
                'mnv_dont_show_it_again'			=> __( "Got it! Don't show this message again", 'mnv-views'),
				'mnv_close'							=> __( 'Close', 'mnv-views')
            )
        );
		...
		$this->localize_script(
            ...
            array(
                'mnv_insert_conditional_shortcode'	=> __( 'Insert conditional shortcode', 'mnv-views' ),
                'mnv_conditional_button'			=> __( 'conditional output', 'mnv-views' ),
                'mnv_editor_callback_nonce'			=> mn_create_nonce( 'mnv_editor_callback' )
            )
        );
		...
		$this->localize_script(
            ...
            'icl_media_manager',
            array(
                'only_img_allowed_here'				=> __( "You can only use an image file here", 'mnv-views' )
            )
        );
		...
		$this->localize_script(
            ...
            array(
                'autosave_saving'					=> '<i class="fa fa-refresh fa-spin"></i>' . __( 'Saving...', 'mnv-views' ),
				'autosave_saved'					=> '<i class="fa fa-check"></i>' . __( 'All changes saved', 'mnv-views' ),
				'autosave_failed'					=> '<i class="fa fa-exclamation-triangle"></i>' . __( 'Saving failed. Please reload the page and try again.', 'mnv-views' )
            )
        );
