...
class Toolset_User_Editors_Editor_Screen_Beaver_Frontend
	...
	public function __construct() {
		...
		add_action( 'mnv_before_shortcode_post_body', array( $this, '_actionSetPostIdForViewsBodyShortcode' ) );
		add_action( 'mnv_after_shortcode_post_body',  array( $this, '_actionSetMediumIdAfterViewsBodyShortcode' ) );
		add_action( 'mn', array( $this, '_actionGlobalizeMediumId' ) );

		add_filter( 'mnv_filter_content_template_output', array( $this, '_filterArchiveContent' ), 10, 4 );
		*/
		add_filter( 'fl_builder_post_types',					array( $this, '_filterSupportMedium' ) );
		
		add_filter( 'body_class',								array( $this, 'bodyClass' ) );
		
		add_filter( 'mnv_filter_content_template_output',		array( $this, 'filterContentTemplateOutput' ), 10, 4 );
	...
	public function beforeRenderShortcodes( $content ) {
		...
		if ( $current_post_id ) {
			...
			$authordata = new MN_User( $current_post_author );
		}
		*/
		$content = MNV_Frontend_Render_Filters::pre_process_shortcodes( $content );
		/*
	...
	public function filterContentTemplateOutput( $content, $template_selected, $id, $kind ) {
		...
		if (
			...
			// There is a CT applied, either on single/archive pages or on a mnv-post-body shortcode
			...
			if ( isset( $post->view_template_override ) ) {
				$this_id = get_the_ID();
				// This is coming from a mnv-post-body shortcode with view_template="None" so we do need to apply BB here 
	...
	public function _actionSetPostIdForViewsBodyShortcode() {
		...
		add_filter( 'mnv_filter_content_template_output', 'FLBuilder::render_content' );
	}
