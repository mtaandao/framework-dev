...
class Toolset_User_Editors_Editor_Screen_Beaver_Backend
	...
	public function __construct() {
		add_action( 'mn_ajax_toolset_user_editors_beaver', array( $this, 'ajax' ) );
		
		// Views and MNAs editor integration
		...
		add_filter( 'mnv_filter_mnv_layout_template_extra_attributes',	array( $this, 'layoutTemplateAttribute' ), 10, 3 );
	...
	public function ajax() {
		if ( ! mn_verify_nonce( $_REQUEST['nonce'], 'toolset_user_editors_beaver' ) ) {
			die();
		}
	...
	public function _actionPrintScripts() {
		echo '<script>';
		echo 'var toolsetUserEditorsBeaverNonce = "' . mn_create_nonce( 'toolset_user_editors_beaver' ) . '";';
	...
	public function _actionStyleAndScripts() {
		...
		mn_localize_script( 'toolset-user-editors-beaver-script', 'toolset_user_editors_beaver', array(
			'nonce' => mn_create_nonce( 'toolset_user_editors_beaver' ),
	...
	public function layoutTemplateRegisterAssets() {
		...
		$beaver_layout_template_i18n = array(
			...
			'template_overlay'		=> array(
										'title'		=> sprintf( __( 'This Content Template uses %1$s', 'mnv-views' ), $this->editor->getName() ),
										'text'		=> sprintf( __( 'To modify this Content Template, go to edit it and launch the %1$s.', 'mnv-views' ), $this->editor->getName() ),
										'button'	=> __( 'Edit this Content Template', 'mnv-views' )
									),
		);
		mn_localize_script( 'toolset-user-editors-beaver-layout-template-script', 'toolset_user_editors_beaver_layout_template_i18n', $beaver_layout_template_i18n );
	}
	
	public function layoutTemplateEnqueueAssets() {
		$page = mnv_getget( 'page' );
		if ( 
			'views-editor' == $page 
			|| 'view-archives-editor' == $page 
		) {
			mn_enqueue_script( 'toolset-user-editors-beaver-layout-template-script' );
		}
	}
	/**
	* Set the builder used by a Content Template, if any.
	*
	* On a Content Template used inside a View or MNA loop output, we set which builder it is using
			...
			if (
				...
				<div class="toolset-alert toolset-alert-error">
					<p><strong><?php echo sprintf( 
						__( '%1$s %2$s design inside %2$s templates may cause problems', 'mnv-views' ), 
					...
					<p>
						<?php echo sprintf(
							__( 'You are using %1$s to design this page, but there is already a template <em>%2$s</em> created by %3$s for %4$s. This may work, but could produce visual glitches. Please consider using %5$s only for the template OR for this page.', 'mnv-views' ),
							$this->editor->getName(),
							$ct_title,
							$this->editor->getName(),
					...
					<p>
						<?php echo sprintf(
							__( '%1$sDesigning templates with %2$s%3$s.', 'mnv-views' ),
