...
class Toolset_User_Editors_Editor_Visual_Composer
	...
	public function requiredPluginActive() {
		...
		if( version_compare( MNB_VC_VERSION, $this->minimum_version ) < 0 ) {
			add_filter( 'mnv_ct_control_switch_editor_buttons', array( $this, '_filterAddDisabledButton' ) );
			return false;
		}
	...
	public function _filterAddDisabledButton( $buttons ) {
		$buttons[] = '<button class="button-secondary" onClick="javascript:alert( jQuery( this ).attr( \'title\' ) );" title="' . sprintf( __( 'Version %s or higher required', 'mnv-views' ), $this->minimum_version ) . '">' . $this->name . '</button>';
