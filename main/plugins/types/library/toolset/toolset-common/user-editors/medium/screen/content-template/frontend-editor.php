...
class Toolset_User_Editors_Medium_Screen_Content_Template_Frontend_Editor
	...
	public function __construct() {
		add_action( 'mn_ajax_set_preview_post', array( $this, 'ajax_set_preview_post' ) );
	}
	...
	public function equivalentEditorScreenIsActive() {
		...
		add_action( 'mn', array( $this, '_actionLoadMediumIdByPost' ), -1 );
	}
	...
	public function _actionLoadMediumIdByPost() {
		...
		add_action( 'mn_footer', array( $this, 'render_preview_post_selector') );
		...
		add_action( 'mn_enqueue_scripts', array( $this, '_actionStyleAndScripts' ) );
	}
	...
	public function _actionStyleAndScripts() {
		...
		mn_localize_script( 'toolset-user-editors-ct-frontend-editor-script', 'toolset_user_editors', array(
			'nonce' => mn_create_nonce( 'toolset_user_editors' ),
	...
	public function render_preview_post_selector() {
		if( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
			...
			if( $toolset_frontend_editor['preview_domain'] == 'post' ) {
				$preview_posts =  mn_get_recent_posts( array( 'post_type' => $preview_slug ), ARRAY_A );
			} else {
				...
				$preview_posts =  mn_get_recent_posts( array( 'post_type' => 'any', 'tax_query' => array(
					array(
			...
			echo __( 'Preview with:', 'mnv-views' );

			echo ' <select id="mnv-ct-preview-post">';
			echo '<option value="0"' . $selected . '>'.__( 'No Shortcode Rendering', 'mn-views' ).'</option>';
			...
			echo '</select>';
			echo '<i class="fa fa-question-circle fa-lg mnv-ct-preview-post-help js-mnv-ct-preview-post-help" aria-hidden="true" onclick="alert(\'' . esc_js( 'You are using Beaver Builder to design a template. This selector lets you choose with what content to preview the template.', 'mnv-views' ) . '\');"></i>';
			echo '</div>';
		}
	}

	public function ajax_set_preview_post() {
		if ( ! mn_verify_nonce( $_REQUEST['nonce'], 'toolset_user_editors' ) ) {
			die( -1 );
		}
