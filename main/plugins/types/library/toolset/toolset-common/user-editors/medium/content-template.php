...
class Toolset_User_Editors_Medium_Content_Template
	...
	public function userEditorChoice() {
		...
		$content_template_id  = mnv_getget( 'ct_id' );
		...
		} // backward compatibility (since Views Visual Comopser Beta we used 'mnv_ct_editor_choice')
		elseif ( $editor_choice = get_post_meta( $content_template_id, 'mnv_ct_editor_choice', true ) ) {
			$this->user_editor_choice = $editor_choice;
			update_post_meta( $content_template_id, $this->option_name_editor_choice, $editor_choice );
			delete_post_meta( $content_template_id, 'mnv_ct_editor_choice' );
			return $editor_choice;
	...
	public function getFrontendTemplates() {
		...
		$theme_template_files    = (array) mn_get_theme()->get_files( 'php', 1, true );

		$mnv_options_patterns = array(
			'views_template_for_'         => array(
				'label'              => __( 'Single page', 'mnv-views' ),
				'domain'             => 'post',
			...
			'views_template_archive_for_' => array(
				'label'              => __( 'Post archive', 'mnv-views' ),
				'domain'             => 'post',
				'template_hierarchy' => array(
					'archive-%NAME%.php',
					'archive.php',
			...
			'views_template_loop_'        => array(
				'label'              => __( 'Taxonomy archive', 'mnv-views' ),
				'domain'             => 'taxonomy',
				'template_hierarchy' => array(
					'taxonomy-%NAME%.php',
					'taxonomy.php',
			...
			'view_mna_loop_preview_post_type_'	=> array(
				'label'              => __( 'Mtaandao Archive loop', 'mnv-views' ),
				'domain'             => 'post',
				'template_hierarchy' => array(
					'archive-%NAME%.php',
					'archive.php',
			...
			'view_mna_loop_preview_taxonomy_'	=> array(
				'label'              => __( 'Mtaandao Archive loop', 'mnv-views' ),
				'domain'             => 'taxonomy',
				'template_hierarchy' => array(
					'taxonomy-%NAME%.php',
		...
		foreach( $content_template_usages as $usage => $ct_id ) {
			foreach( $mnv_options_patterns as $pattern => $settings ) {
		...
		$allowed_paths = mn_list_pluck( $this->allowed_templates, 'path' );
		...
		if ( 
			...
			$settings_to_store = array(
				'template_path' => mn_slash( $first_allowed_template['path'] ),
		...
		if ( isset( $_GET['ct_id'] ) ) {
			...
			if ( 
				...
				foreach ( $preview_post_type_archive as $prev_cpt ) {
					$views_options[ 'view_mna_loop_preview_post_type_' . $prev_cpt ] = (int) $_GET['ct_id'];
				}
			}
			...
			if ( 
				...
				foreach ( $preview_taxonomy_archive as $prev_cpt ) {
					$views_options[ 'view_mna_loop_preview_taxonomy_' . $prev_cpt ] = (int) $_GET['ct_id'];
				}
			}
			...
			foreach( $editors as $editor ) {
				if ( $editor->getId() == $this->manager->getActiveEditor()->getId() ) {
					if ( 'basic' != $editor->getId() ) {
						$editor_current = sprintf( __( 'Using %1$s ', 'mnv-views' ), '<strong>' . $editor->getName() . '</strong>' );
					}
				} else {
					$editor_switch_buttons[] = '<a class="button" href="'.$admin_url.'&ct_editor_choice='.$editor->getId().'">'.sprintf( __( 'Design with %1$s', 'mnv-views' ), $editor->getName() ).'</a>';
				}
			}

			$control_editor_select .= '<div class="mnv-ct-control-switch-editor">';
			$control_editor_select .= $editor_current;
			//$control_editor_select .= __( 'Select Editor: ', 'mnv-views' );
