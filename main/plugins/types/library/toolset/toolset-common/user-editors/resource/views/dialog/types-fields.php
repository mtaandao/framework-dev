...
class Toolset_User_Editors_Resource_Views_Dialog_Types_Fields
	...
	public function load() {
		...
		if( defined( 'TYPES_RELPATH' ) )
			add_filter( 'editor_addon_menus_mnv-views', array( $this, '_filterTypesFieldsToViewsDialog' ) );
	...
	public function _filterTypesFieldsToViewsDialog( $menu ) {
		...
		if ( !empty( $groups ) ) {
			...
			foreach ( $groups as $group_id => $group ) {
				...
				$fields = $this->mncf_admin_fields_get_fields_by_group( $group['id'],
	...
	public function types_js_settings() {
		...
		echo '
		<style>
		#colorbox.js-mncf-colorbox-with-iframe { z-index: 110000 !important; }
</style>
	...
	protected function mncf_admin_fields_get_fields_by_group( $group_id, $key = 'slug',
	                                                          $only_active = false, $disabled_by_type = false,
	                                                          $strictly_active = false, $post_type = 'mn-types-group',
