...
*
* General class to manage common code loading for all Custom Content plugins
*
* This class is used to load Custom Content Common into all Custom Content plugins that have it as a dependency.
* Note that Assets, Menu, Utils, Settings, Localization, Promotion, Debug, Admin Bar and MNML compatibility are always loaded when first instantiating this class.
...
* 	toolset_forms						Custom Content Forms, the shared component for Types and CRED
* 	toolset_visual_editor				Visual Editor Addon, to display buttons and dialogs over editors
* 	toolset_parser						Custom Content Parser, to parse conditionals
*
* New sections can be added here, following the same structure.
*
* Note that you have available the following constants:
* 	TOOLSET_COMMON_VERSION				The Custom Content Common version
* 	TOOLSET_COMMON_PATH					The path to the active Custom Content Common directory
* 	TOOLSET_COMMON_DIR					The name of the directory of the active Custom Content Common
* 	TOOLSET_COMMON_URL					The URL to the root of Custom Content Common, to be used in backend - adjusted as per SSL settings
* 	TOOLSET_COMMON_FRONTEND_URL			The URL to the root of Custom Content Common, to be used in frontend - adjusted as per SSL settings
*
...
class Toolset_Common_Bootstrap {
    ...
    private function __construct() {
	    ...
	    // Register assets, utils, settings, localization, promotion, debug, admin bar and MNML compatibility
		$this->register_utils();
		$this->register_res();
		$this->register_libs();
		$this->register_inc();
        ...
        /**
         * Action when the Custom Content Common Library is completely loaded.
         *
         * @since m2m
         */
	...
	public function register_inc() {
		...
		if ( ! $this->is_section_loaded( self::TOOLSET_INCLUDES ) ) {
			...
			if ( ! class_exists( 'Toolset_MNLogger' ) ) {
				require_once( TOOLSET_COMMON_PATH . '/inc/toolset.mnlogger.class.php' );
			}
			...
			if ( ! class_exists( 'Toolset_MNML_Compatibility' ) ) {
				require_once( TOOLSET_COMMON_PATH . '/inc/toolset.mnml.compatibility.class.php' );
				$this->mnml_compatibility = new Toolset_MNML_Compatibility();
			}
	...
	public function register_toolset_forms() {
		...
		if ( ! $this->is_section_loaded( self::TOOLSET_FORMS ) ) {
			...
			if ( ! class_exists( 'MNToolset_Forms_Bootstrap' ) ) {
     ...
     * - MODE_ADMIN when showing a MN admin page
