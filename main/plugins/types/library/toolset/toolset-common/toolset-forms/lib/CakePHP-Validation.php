...
class MNToolset_Cake_Validation {
    ...
    function &getInstance() {
        ...
        if (!$instance) {
            $instance[0] = new MNToolset_Cake_Validation();
        }
        return $instance[0];
    }
    ...
    function notEmpty($check) {
        $_this = &MNToolset_Cake_Validation::getInstance();
        $_this->__reset();
        $_this->check = $check;
     ...
     * @return boolean Success
     * @access public
     * @see MNToolset_Cake_Validation::_luhn()
     */
    ...
    function comparison($check1, $operator = null, $check2 = null) {
        ...
        switch ($operator) {
            ...
            default:
                $_this = &MNToolset_Cake_Validation::getInstance();
                $_this->errors[] = __('You must define the $operator parameter for MNToolset_Cake_Validation::comparison()', 'mncf');
                break;
        }
        return false;
    }
    ...
    function custom($check, $regex = null) {
        ...
        if ($_this->regex === null) {
            $_this->errors[] = __('You must define a regular expression for MNToolset_Cake_Validation::custom()', 'mncf');
            return false;
        }
        return $_this->_check();
    }
    ...
    function date($check, $format = 'ymd', $regex = null) {
        ...
        if (is_numeric($check)) {
            return MNToolset_Field_Date_Scripts::_isTimestampInRange($check);
        }
        // TODO Change to use date strtotime
        $valid = mntoolset_strtotime($check);
        return $valid !== false;
    ...
    function extension($check, $extensions = array('gif', 'jpeg', 'png', 'jpg')) {
        ...
        //##########################################################################################
        if (is_array($check)) {
            return MNToolset_Cake_Validation::extension(array_shift($check), $extensions);
        }
