 ...
 * @author Srdjan
 */
if ( !class_exists('MNToolset_Types') ){   
class MNToolset_Types
{
     ...
     * @return array
     */
    static function filterField($field, $post_id = null, $_post_mncf = array())
    {
        // Get field settings (as when using get_option('mncf-fields') from DB)
         ...
         * Example main field:
         * ID: mncf-text
         * conditional: '$mncf-date > DATE(01,02,2014)'
         *
         * Example child field:
         * ID: mncf-text-123
         * conditional: '$mncf-date-123 > DATE(01,02,2014)'
         * Suffix is child post ID (mncf-text-$child_post_id).
         *
        ...
        $_field = array(
            'id' => $prefix . $field['id'] . $suffix, // Used as main ID (raw date mnt-id), used to connect conditional relations
            ...
            'name' => isset( $field['form_name'] ) ? $field['form_name'] : "mncf[{$field['id']}]", // Form element name, default mncf[$no-prefix-slug]
            'repetitive' => self::isRepetitive( $field ), // Is repetitive?
            'validation' => self::filterValidation( $field ), // Validation settings
            'conditional' => self::filterConditional( $field, $post_id, $_post_mncf ), // Conditional settings
     ...
     *
     * [conditional] => Array(
            [custom] => ($mncf-my-date = DATE(01,02,2014)) OR ($mncf-my-date > DATE(07,02,2014))
            [values] => Array(
                [mncf-my-date] => 32508691200
            )
      )
     ...
     * @return array
     */
    public static function filterConditional($field, $post_id, $_post_mncf = array())
    {
        $field = self::getConfig( $field );
        if ( is_null( $field ) ) return array();
		...
		if ( function_exists('mncf_fields_get_field_by_slug') ){
            ...
            foreach ( $_post_mncf as $field_slug => $field_value ) {
                ...
                $field_value = apply_filters( 'mncf_fields_type_' . $field['type'] . '_value_save', $field_value, $field, null );
        ...
        if ( !empty( $field['data']['conditional_display']['custom_use'] ) ) {
            ...
            foreach ( $field['data']['conditional_display']['conditions'] as $d ) {
                ...
                if ( !empty( $c_field ) ) {
                    ...
                    mntoolset_form_field_add_filters( $field['type'] );
                    $key = $c_field['meta_key'];
                    if ( isset( $cond_values[$key] ) ) {
                        $cond['values'][$key . $suffix] = apply_filters( 'mntoolset_conditional_value_php',
    ...
    public static function getCustomConditional($custom, $suffix = '', $cond_values = array()) {
        $c_fields = MNToolset_Forms_Conditional::extractFields( $custom );
        $c_values = array();
        ...
        foreach ( $c_fields as $c_field_id ) {
            ...
            if ( !empty( $c_field ) ) {
                ...
                // Apply filters from field (that is why we set 'type' property)
                mntoolset_form_field_add_filters( $c_field['type'] );
                $c_key = $c_field['meta_key'];
                if ( isset( $cond_values[$c_key] ) ) {
                    $c_values[$c_key . $suffix] = apply_filters( 'mntoolset_conditional_value_php',
    ...
    {
        return self::$is_user_meta ? get_option( 'mncf-usermeta', array() ) : get_option( 'mncf-fields', array() );
    }
    ...
    {
        return !empty( $config['data']['controlled'] ) ? '' : 'mncf-';
    }

    /**
     * Translates various strings connected to Types using MNML icl_t().
     *
     * @param string $name
     * @param string $string
