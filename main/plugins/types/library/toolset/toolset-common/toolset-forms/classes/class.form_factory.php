...
define( "CLASS_NAME_PREFIX", "MNToolset_Field_" );
...
class FormFactory extends FormAbstract
{
    ...
    public function __construct( $nameForm = 'default' )
    {
        ...
        mn_register_script( 'mntoolset-forms',
            MNTOOLSET_FORMS_RELPATH . '/js/main.js',
            array('jquery', 'underscore', 'suggest'), MNTOOLSET_FORMS_VERSION, true );
        mn_enqueue_script( 'mntoolset-forms' );
		$mntoolset_forms_localization = array(
			'ajaxurl' => admin_url( 'admin-ajax.php', null )
		);
		mn_localize_script( 'mntoolset-forms', 'mntoolset_forms_local', $mntoolset_forms_localization );

        if ( is_admin() ) {
            mn_register_style( 'mntoolset-forms-admin',
                MNTOOLSET_FORMS_RELPATH . '/css/mnt-toolset-backend.css', array(),
                MNTOOLSET_FORMS_VERSION );
            mn_enqueue_style( 'mntoolset-forms-admin' );
        } else {
            /**
            ...
            if ( $load_cred_css ) {
                mn_register_style(
                    'mntoolset-forms-cred',
                    MNTOOLSET_FORMS_RELPATH . '/css/mnt-toolset-frontend.css',
                    array(),
                    MNTOOLSET_FORMS_VERSION
                );
                mn_enqueue_style( 'mntoolset-forms-cred' );
            }
        }
    }
    ...
    public function metaform( $config, $global_name_field, $value )
    {
         ...
         * WMPL configuration
         */
        $config['mnml_action'] = $this->get_mnml_action($config['id']);
        ...
        if (
            ...
            $config['options'] = apply_filters(
                'mncf_config_options_'.$config['type'],
                $config['options'],
                $config['slug'],
                $config['type']
            );
        ...
        foreach ( $value as $val ) {            
            ...
            if ( !is_mn_error( $field = $this->loadField( $_cfg, $_gnf, $val ) ) ) {
                $form = $field->metaform();
                // Set $config['validate'] to trigger PHP validation
                // when rendering metaform
                if ( !empty( $_cfg['validate'] ) && 
                     is_mn_error( $valid = $this->validateField( $field, $val ) ) ) {
                    $key = key( $form );
                ...
                if ( current_user_can('manage_options') ) {
                    $htmlArray[] = sprintf(
                        ...
                        sprintf(
                            __('There is a problem rendering field <strong>%s (%s)</strong>.', 'mnv-views'),
                            $_cfg['title'],
                            $_cfg['type']
                        ),
        ...
        if ( !empty( $htmlArray ) && isset($config['repetitive']) && $config['repetitive'] ) {
            $_gnf = $_cfg['name'] = "{$global_name_field}[%%{$count}%%]";
            if ( !is_mn_error( $field = $this->loadField( $_cfg, $_gnf, null ) ) ) {
    ...
    public function loadField( $config, $global_name_field, $value ){
        ...
        if ( is_null( $field ) ) {
            return new MN_Error( 'mntoolset_forms', 'wrong field type' );
        }

        $settings = $field->getSettings();
        if ( isset( $settings['min_mn_version'] ) && version_compare( $mn_version,
                        $settings['min_mn_version'], '<' ) ) {
            return new MN_Error( 'mntoolset_forms', 'Higher MN version required' );
        }
    ...
    protected function _checkValidation( $config ) {        
        if ( isset( $config['validation'] ) && is_null( $this->_validation ) ) {
            ...
            $this->_validation = new MNToolset_Forms_Validation( $this->nameForm , isset($config['form_settings']) ? $config['form_settings'] : "" );
        }
    }
    ...
    public function getConditionalClass(){
        if ( is_null( $this->_conditional ) ) {
            ...
            $this->_conditional = new MNToolset_Forms_Conditional( $this->nameForm );
        }
    ...
    protected function _repetitive() {
        if ( is_null( $this->_repetitive ) ) {
            ...
            $this->_repetitive = new MNToolset_Forms_Repetitive();
        }
    ...
    protected function _tpl( $cfg, $html ) {
        ob_start();
        include MNTOOLSET_FORMS_ABSPATH . '/templates/metaform.php';
        $o = ob_get_contents();
        ob_get_clean();
        return $o;
    }
    ...
    protected function _tplItem( $cfg, $out ) {
        ob_start();
        include MNTOOLSET_FORMS_ABSPATH . '/templates/metaform-item.php';
        $o = ob_get_contents();
        ob_get_clean();
        return $o;
    }
    ...
    public function validateField( $field, $value ) {
         ...
         */
        if (!is_admin()) {            
            //I receive mncf-id and mncf[id] for the same type
            ...
            {   
                    ...
                    !isset($field_value)) 
                {
                    ...
                    return new MN_Error( 'mntoolset_forms', $mess,
    ...
    public function loadFieldClass( $type ) {
        ...
        if ( !class_exists( $class ) ) {
            $file = MNTOOLSET_FORMS_ABSPATH . "/classes/class.{$type}.php";
            if ( file_exists( $file ) ) {
                require_once $file;
                return $class;
            } else {
                // third party fields array $type => __FILE__
                $third_party_fields = apply_filters( 'mntoolset_registered_fields',
                        array() );
    ...
    private function get_mnml_action($id)
    {
