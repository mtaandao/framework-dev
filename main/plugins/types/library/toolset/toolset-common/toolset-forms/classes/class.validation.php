 ...
 * - Filters data-mnt-validation (adds array of rules) to form element
 ...
 * @author Srdjan
 */
class MNToolset_Forms_Validation {
    ...
    function __construct($formID, $formSET) {
        ...
        // Register
        mn_register_script('mntoolset-form-jquery-validation', MNTOOLSET_FORMS_RELPATH . '/lib/js/jquery-form-validation/jquery.validate.js', array('jquery'), MNTOOLSET_FORMS_VERSION, true);
        mn_register_script('mntoolset-form-jquery-validation-additional', MNTOOLSET_FORMS_RELPATH . '/lib/js/jquery-form-validation/additional-methods.min.js', array('mntoolset-form-jquery-validation'), MNTOOLSET_FORMS_VERSION, true);
        mn_register_script('mntoolset-form-validation', MNTOOLSET_FORMS_RELPATH . '/js/validation.js', array('mntoolset-form-jquery-validation-additional', 'underscore', 'toolset-utils'), MNTOOLSET_FORMS_VERSION, true);

        $my_formID = str_replace("-", "_", $formID);
        mn_localize_script('mntoolset-form-validation', 'cred_settings_' . $my_formID, array(
            'form_id' => $formID,
            'use_ajax' => (!is_admin() && isset($formSET->form['use_ajax']) && $formSET->form['use_ajax'] == 1) ? true : false,
            'operation_ok' => __('Operation completed successfully', 'mnv-views'),
            'operation_ko' => __('Operation not completed successfully', 'mnv-views'),
            'delay_message' => __('You are being redirectd. Please Wait.', 'mnv-views')
                )
        );

        // Filter JS validation data
        add_action('mntoolset_forms_field_js_validation_data_' . $this->__formID, array($this, 'filterJsValidation'));
        // Filter form field PHP validation
        add_filter('mntoolset_form_' . $this->__formID . '_validate_field', array($this, 'filterFormField'), 10, 2);
        // Render classes
        add_action('mntoolset_field_class', array($this, 'actionFieldClass'));

        // Render settings
        add_action('admin_print_footer_scripts', array($this, 'renderJsonData'), 30);
        add_action('mn_footer', array($this, 'renderJsonData'), 30);
        add_action('mn_footer', array($this, 'loadCustomAssets'), 30);

        mn_enqueue_script('mntoolset-form-validation');
    }
    ...
    public function loadCustomAssets() {
        echo '<div class="mnt-modal"></div>';
    }

    /**
     * Adjusts validation data for JS processing (data-mnt-validate HTML attribute)
     *
     * @param type $rules
     * @return type
     */
    public function filterJsValidation($rules) {
        foreach ($rules as $r => $rule) {
            ...
            $_r = apply_filters('mntoolset_validation_rule_js', $r);
            if ($_r != $r) {
                $rules[$_r] = $rule;
                unset($rules[$r]);
                continue;
        ...
        foreach ($rules as $r => &$rule) {
            $rule['args'] = apply_filters('mntoolset_validation_args_js', $rule['args'], $r);
...
//            unset( $rule['message'] );
     ...
     * Form Factory should check if element has 'error' property (MN_Error)
     * and use MN_Error::get_error_message() to display error message
     *
     * @param type $element
     * @param type $value
     * @return type
    ...
    public function filterFormField($element, $value) {
        ...
        try {
            ...
            $element['error'] = new MN_Error(__CLASS__ . '::' . __METHOD__, 'Field not validated', $errors);
        }
        return $element;
    }
    ...
    public function validateField($field) {
        $value = apply_filters('mntoolset_validation_value_' . $field->getType(), $field->getValue());
        ...
        try {
            ...
            return new MN_Error(__CLASS__ . '::' . __METHOD__, 'Field not validated', $errors);
        }
        return true;
    }
    ...
    protected function _parseRules($rules, $value) {
        $_rules = array();
        foreach ($rules as $rule => $args) {
            $rule = apply_filters('mntoolset_validation_rule_php', $rule);
            $args['args'] = apply_filters('mntoolset_validation_args_php', $args['args'], $rule);
    ...
    protected function _cake() {
        if (is_null($this->_cake)) {
            require_once MNTOOLSET_FORMS_ABSPATH . '/lib/CakePHP-Validation.php';
            $this->_cake = new MNToolset_Cake_Validation;
        }
        return $this->_cake;
    }
    ...
    public function renderJsonData() {
        printf('<script type="text/javascript">mntValidationForms.push("#%s");</script>', $this->__formID);
    }

    public function actionFieldClass($config) {
        if (!empty($config['validation'])) {
            foreach ($config['validation'] as $rule => $data) {
                echo " mnt-validation-{$rule}";
            }
        }
    }
