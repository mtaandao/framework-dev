...
//    }
    ...
    public function validateElement(&$element) {
        ...
        $element = apply_filters('mntoolset_form_' . $this->_id . '_validate_field', $element, $value);
    ...
    public function renderElement($element) {
        ...
        if (is_callable(array($this, $method))) {
            ...
            if (!empty($element['#validate'])) {
                ...
                $element['#attributes']['data-mnt-validate'] = esc_html(self::json_encode(apply_filters('mntoolset_forms_field_js_validation_data_' . $this->_id, $element['#validate'])));
                $element['#attributes']['data-mnt-field-title'] = esc_js($custom_field_title);
            }
            if ($element['#type'] == 'radios' && !empty($element['#options'])) {
                foreach ($element['#options'] as &$option) {
                    if (!empty($option['#validate'])) {
                        $option['#attributes']['data-mnt-validate'] = esc_html(self::json_encode(apply_filters('mntoolset_forms_field_js_validation_data_' . $this->_id, $option['#validate'])));
                        $option['#attributes']['data-mnt-field-title'] = esc_js($custom_field_title);
                    }
                }
            }
            /**
             * MNML - lock CF is has option "copy from original".
             */
            if (is_admin() && function_exists('mncf_mnml_field_is_copied') && mncf_mnml_field_is_copied($element)) {
    ...
    private function _wrapElement($element, $output) {
        ...
        if (preg_match('/_hidden$/', $element['#id']) && !is_admin()) {
            $classes[] = 'mnt-form-hide-container';
        }
        if (is_admin()) {
            return sprintf(
...
//                . "\r\n";
    ...
    public function checkbox($element) {
         ...
         * type and data_id
         */
        $element['_render']['element'] .= sprintf(' data-mnt-type="%s"', __FUNCTION__);
         ...
         */
        $is_types_field = (
	        ( strpos( $element['#name'], 'mncf[' ) === 0 )
	        || ( strpos( $element['#name'], 'mncf_post_relationship[' ) === 0 )
        );
    ...
    public function select($element) {
        ...
        foreach ($element['#options'] as $id => $value) {
             ...
             * type and data_id
             */
            $element['_render']['element'] .= ' data-mnt-type="option"';
            $element['_render']['element'] .= $this->_getDataWptId($element);
            /**
             * selected
             */
    ...
    private function _getDataWptId($element) {
        ...
        if (array_key_exists('#id', $element)) {
            if (is_admin()) {
                $html .= sprintf(' data-mnt-id="%s"', preg_replace('/\[/', '-', preg_replace('/\]/', '', $element['#name'])));
            } else {
                $html .= sprintf(' data-mnt-id="%s_%s"', $this->_id, $element['#id']);
            }
            if (array_key_exists('#name', $element) && $element['#name']) {
                if (!is_admin() && $this->_isRepetitive($element)) {
                    $html .= sprintf(' data-mnt-name="%s"', preg_replace('/\[.+$/', '', $element['#name']));
                } else {
                    if (preg_match('/^mncf_post_relationship\[\d+\]\[\d+\]\[[^\]]+\]/', $element['#name'])) {
                        $html .= sprintf(
                                ' data-mnt-name="%s"', preg_replace('/^mncf_post_relationship\[\d+\]\[(\d+)\]\[mncf-([^\]]+)\]/', "mncf[$2-$1]", $element['#name']));
                    } else {
                        $html .= sprintf(' data-mnt-name="%s"', $element['#name']);
                    }
                }
            }
        }
