...
abstract class FieldFactory extends FieldAbstract
{
    ...
    public function getValue()
    {
        ...
        $value = apply_filters( 'mncf_fields_value_get', $value, $post );
        if ( array_key_exists('slug', $this->_data ) ) {
            $value = apply_filters( 'mncf_fields_slug_' . $this->_data['slug'] . '_value_get', $value, $post );
        }
        $value = apply_filters( 'mncf_fields_type_' . $this->_data['type'] . '_value_get', $value, $post );
        return $value;
    }
    ...
    {
        return mnautop( mn_filter_post_kses( $this->_data['description'] ) );
    }
    ...
    public function getMNMLAction()
    {
        if ( array_key_exists( 'mnml_action', $this->_data ) ) {
            return $this->_data['mnml_action'];
        }
        return 0;
    }
