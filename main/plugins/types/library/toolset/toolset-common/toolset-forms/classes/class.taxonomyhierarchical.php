...
class MNToolset_Field_Taxonomyhierarchical extends MNToolset_Field_Textfield {
    ...
    public function init() {
        ...
        if (isset($post)) {
            $terms = apply_filters('toolset_filter_taxonomyhierarchical_terms', mn_get_post_terms($post->ID, $this->getName(), array("fields" => "all")));
    ...
    public function metaform() {
        ...
        $metaform[] = array(
            ...
            '#attributes' => array(
                ...
                'data-after-selector' => 'js-mnt-hierarchical-taxonomy-add-new-' . $taxname,
                'data-open' => apply_filters('toolset_button_add_new_text', esc_attr($attributes['add_new_text'])),
                'data-close' => apply_filters('toolset_button_cancel_text', esc_attr(__('Cancel', 'mnv-views'))), // TODO adjust the button value depending on open/close action
                'class' => $use_bootstrap ? 'btn btn-default mnt-hierarchical-taxonomy-add-new-show-hide js-mnt-hierarchical-taxonomy-add-new-show-hide' : 'mnt-hierarchical-taxonomy-add-new-show-hide js-mnt-hierarchical-taxonomy-add-new-show-hide',
            ),
        ...
        if ($use_bootstrap) {
            $container = '<div style="display:none" class="form-group mnt-hierarchical-taxonomy-add-new js-mnt-hierarchical-taxonomy-add-new-' . $taxname . '" data-taxonomy="' . $taxname . '">';
        } else {
            $container = '<div style="display:none" class="mnt-hierarchical-taxonomy-add-new js-mnt-hierarchical-taxonomy-add-new-' . $taxname . '" data-taxonomy="' . $taxname . '">';
        }
        ...
        $metaform[] = array(
            ...
            '#attributes' => array(
                ...
                'data-taxonomy' => $taxname,
                'class' => 'js-taxonomy-parent mnt-taxonomy-parent'
            ),
            '#validate' => $this->getValidationData(),
        );
        ...
        $metaform[] = array(
            ...
            '#attributes' => array(
                'data-taxonomy' => $taxname,
                'data-build_what' => $build_what,
                'class' => $use_bootstrap ? 'btn btn-default mnt-hierarchical-taxonomy-add-new js-mnt-hierarchical-taxonomy-add-new' : 'mnt-hierarchical-taxonomy-add-new js-mnt-hierarchical-taxonomy-add-new',
            ),
            '#validate' => $this->getValidationData(),
            '#after' => '</div>',
        );
    ...
    private function buildCheckboxes($index, &$childs, &$names, &$metaform, $level = 0, $parent = -1) {
        if (isset($childs[$index])) {
            ...
            foreach ($childs[$index] as $tkey => $tid) {
                ...
                if ($tkey == 0) {
                    if ($level > 0) {
                        $item['#before'] = '<li class="tax-children-of-' . $parent . '"><ul class="mnt-form-set-children mnt-form-set-children-level-' . $level . '" data-level="' . $level . '">' . $item['#before'];
                    } else {
                        $item['#before'] = '<ul class="mnt-form-set mnt-form-set-checkboxes mnt-form-set-checkboxes-' . $this->getName() . '" data-level="0">' . $item['#before'];
                    }
                }
