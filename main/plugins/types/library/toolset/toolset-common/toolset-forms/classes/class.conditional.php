 ...
 * 
 * Data
 * [id] - Trigger ID to match data-mnt-id
 ...
 * Custom conditionals
 * 
 * Variable name should match trigger ID - data-mnt-id
 * Example
 * $config['conditional'] = array(
 *  'custom' => '($mncf-text = show) OR ($mncf-date > '21-01-2014')'
 * );
 */
 ...
 * Class description
 *
 * @todo BUG common function mnv_condition has some flaws
 *      (dashed names, mixed checks for string and numeric values causes failure)
 *
 * @author Srdjan
 */
if ( !class_exists('MNToolset_Forms_Conditional') ){  
class MNToolset_Forms_Conditional {
    ...
    public function __construct($formID) {
        ...
        mn_register_script('mntoolset-form-conditional', MNTOOLSET_FORMS_RELPATH . '/js/conditional.js', array('jquery', 'jquery-effects-scale'), MNTOOLSET_FORMS_VERSION, true);
        mn_enqueue_script('mntoolset-form-conditional');
        $js_data = array(
            'ajaxurl' => admin_url('admin-ajax.php', null),
        );
        mn_localize_script('mntoolset-form-conditional', 'mntConditional', $js_data);

        mn_enqueue_script('mntoolset-parser');
        $js_data = array(
            'ajaxurl' => admin_url('admin-ajax.php', null),
        );
        // Render settings
        ...
        foreach ($this->_collected as $id => $config) {
            if (!empty($config['custom'])) {
                ...
                if (strpos($evaluate, "REGEX") === false) {
                    $evaluate = mnv_filter_parse_date($evaluate);
                ...
                if (isset($config) && isset($config['conditions'])) {
                    ...
                    foreach ($config['conditions'] as &$c) {
                        ...
                        $c['args'] = apply_filters('mntoolset_conditional_args_js', $c['args'], $c['type']);
    ...
    public static function evaluate($config) {
        ...
        foreach ($config['conditions'] as $c) {
            // Add filters
            mntoolset_form_field_add_filters($c['type']);
            $c['args'] = apply_filters('mntoolset_conditional_args_php', $c['args'], $c['type']);
            $value = isset($config['values'][$c['id']]) ? $config['values'][$c['id']] : null;
            $value = apply_filters('mntoolset_conditional_value_php', $value, $c['type']);
            $compare = $c['args'][0];
            switch ($c['operator']) {
                case '=':
                case '==':
    ...
    private static function _update_values_in_expression($evaluate, $fields, $values) {
        ...
        usort($keys, 'MNToolset_Forms_Conditional::sortByLength');
    ...
    public static function handle_user_function($evaluate) {
        ...
        if ($occurrences > 0) {
            for ($i = 0; $i < $occurrences; $i++) {
                ...
                if ($function === 'USER') {
                    $result = MNV_Handle_Users_Functions::get_user_field($field);
                }
        ...
        foreach ($conditional as $k => $c) {
            ...
            foreach ($post_values as $fid => $value) {
                if (isset($_POST['field_types'][$fid])) {
                    ...
                    mntoolset_form_field_add_filters($field_type);
                    $value = apply_filters('mntoolset_conditional_value_php', $value, $field_type);
                }
                $values[$fid] = $value;
            }
    ...
    public function actionFieldClass($config) {
        if (
            ...
            echo ' mnt-hidden js-mnt-remove-on-submit js-mnt-validation-ignore';
        }
    }
...
if (!class_exists('MNV_Handle_Users_Functions')) {

    class MNV_Handle_Users_Functions {
