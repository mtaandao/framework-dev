 ...
 * @author Srdjan
 */
class MNToolset_Field_Radios extends FieldFactory {
...
//            }
        ...
        $options = apply_filters('mnt_field_options', $options, $title, 'select');
        /**
         * default_value
         */
        ...
        $form_attr = array(
            ...
            '#after' => '<input type="hidden" name="_mntoolset_radios[' . $this->getId() . ']" value="1" />',
        );
        
        if (!is_admin()) {// TODO maybe add a doing_ajax() check too, what if we want to load a form using AJAX?
            $form_attr['#before'] = '<ul class="mnt-form-set mnt-form-set-radios mnt-form-set-radios-' . $name . '">';
            $form_attr['#after'] = '</ul>';
        }
