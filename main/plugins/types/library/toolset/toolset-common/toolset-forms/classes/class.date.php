 ...
 * @author Srdjan
 */
class MNToolset_Field_Date extends FieldFactory {
    ...
    public static function addFilters() {
        if (has_filter('mntoolset_validation_value_date', array('MNToolset_Field_Date', 'filterValidationValue'))) {
            return;
        }
        // Filter validation
        add_filter('mntoolset_validation_value_date', array('MNToolset_Field_Date', 'filterValidationValue'));
        add_filter('mntoolset_validation_rule_js', array('MNToolset_Field_Date', 'filterValidationRuleJs'));
        add_filter('mntoolset_validation_args_php', array('MNToolset_Field_Date', 'filterValidationArgsPhp'), 10, 2);
        // Filter conditional
        add_filter('mntoolset_conditional_args_php', array('MNToolset_Field_Date', 'filterConditionalArgsPhp'), 10, 2);
        add_filter('mntoolset_conditional_value_php', array('MNToolset_Field_Date', 'filterConditionalValuePhp'), 10, 2);
        add_filter('mntoolset_conditional_args_js', array('MNToolset_Field_Date', 'filterConditionalArgsJs'), 10, 2);
    }
    ...
    public function metaform() {
        ...
        $field_disable = false;
        $def_class_aux = 'js-mnt-date-auxiliar';

        if ( // if field should get duplicated
                is_admin() && defined('MNML_TM_VERSION') && intval($mnml_action) === 1 && function_exists('mncf_mnml_post_is_original') && !mncf_mnml_post_is_original() && function_exists('mncf_mnml_have_original') && mncf_mnml_have_original()
        ) {
            $field_disable = true;
        ...
        if (
            ...
            $def_class .= ' js-mnv-date-readonly';
            $def_class_aux .= ' js-mnt-date-readonly';
            $readonly = true;
        }
        ...
        $attr_visible = array(
            ...
            'readonly' => 'readonly',
            'title' => esc_attr(__('Select', 'mnv-views')) . " Date"
        );
        $attr_hidden = array('class' => $def_class_aux, 'data-ts' => $timestamp, 'data-mnt-type' => 'date');
        ...
        if (!empty($data['add_time'])) {
            ...
            if ($readonly) {
                ...
                $hour_element = array(
                    '#type' => 'select',
                    '#before' => '<span class="mnt-form-label">' . __('Hour', 'mnv-views') . '</span>',
                    '#options' => $options,
                    '#default_value' => $hour,
                    '#name' => '_' . $this->getName() . '[hour]',
                    '#inline' => true,
                    '#attributes' => array('title' => esc_attr(__('Select', 'mnv-views')) . " Date"),
                );
            } else {
                $hour_element = array(
                    '#type' => 'select',
            ...
            if ($readonly) {
                ...
                $minute_element = array(
                    '#type' => 'select',
                    '#before' => '<span class="mnt-form-label">' . __('Minute', 'mnv-views') . '</span>',
                    '#options' => $options,
                    '#default_value' => $minute,
                    '#name' => '_' . $this->getName() . '[minute]',
                    '#inline' => true,
                    '#attributes' => array('title' => esc_attr(__('Select minute', 'mnv-views'))),
                );
            } else {
                $minute_element = array(
                    '#type' => 'select',
        ...
        $form[] = array(
            ...
            '#inline' => true,
            '#markup' => sprintf(
                    '<input type="button" class="button button-secondary js-mnt-date-clear mnt-date-clear" value="%s" %s/>', esc_attr(__('Clear', 'mnv-views')) . " Date",
                    /**
    ...
    public static function getDateFormat() {
        return MNToolset_Field_Date_Scripts::getDateFormat();
    }
    ...
    public static function timetodate($timestamp, $format = null) {
        return MNToolset_Field_Date_Scripts::timetodate($timestamp, $format);
    }
