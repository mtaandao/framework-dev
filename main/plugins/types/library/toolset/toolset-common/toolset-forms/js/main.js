...
var mntCallbacks = {};
mntCallbacks.validationInit = jQuery.Callbacks('unique');
mntCallbacks.addRepetitive = jQuery.Callbacks('unique');
mntCallbacks.removeRepetitive = jQuery.Callbacks('unique');
mntCallbacks.conditionalCheck = jQuery.Callbacks('unique');
mntCallbacks.reset = jQuery.Callbacks('unique');

jQuery(document).ready(function () {
    if (typeof mntValidation !== 'undefined') {
        mntCallbacks.validationInit.add(function () {
            mntValidation.init();
        });
    }
    if (typeof mntCond !== 'undefined') {
        mntCond.init();
    } else {
        mntCallbacks.validationInit.fire();
    }
    /**
    ...
    jQuery('.cred-taxonomy', jQuery('form.is_submitted')).each(function () {
        parent = jQuery(this);
        setTimeout(function () {
            jQuery('input.mnt-taxonomy-add-new', parent).click();
        }, 50);
    });
    
    setTimeout(function(){
        
        if(jQuery(".mnt-hierarchical-taxonomy-add-new-show-hide").is(":hidden")){
            jQuery(".mnt-hierarchical-taxonomy-add-new:hidden").remove();
            jQuery(".mnt-hierarchical-taxonomy-add-new-show-hide:hidden").remove();
        }
...
function add_filter(name, callback, priority, args_num) {
    ...
    if (typeof mntFilters[name] === 'undefined')
        mntFilters[name] = {};
    if (typeof mntFilters[name][args[2]] === 'undefined')
        mntFilters[name][args[2]] = [];
    mntFilters[name][args[2]].push([callback, args[3]]);
}
function apply_filters(name, val) {
    if (typeof mntFilters[name] === 'undefined')
        return val;
    var args = _.rest(_.toArray(arguments));
    _.each(mntFilters[name], function (funcs, priority) {
...
function showHideMostPopularButton(taxonomy, form)
{
    ...
    var $tag_list = $taxonomy_box.find('.js-mnt-taxonomy-popular-add');

    if (!$button.hasClass('js-mnt-taxonomy-popular-show-hide'))
        return true;
...
jQuery(document).off('click', '.js-mnt-taxonomy-popular-show-hide', null);
jQuery(document).off('click', '.js-mnt-taxonomy-popular-add', null);
jQuery(document).off('click', '.js-mnt-taxonomy-add-new', null);
jQuery(document).off('keypress', '.js-mnt-new-taxonomy-title', null);

jQuery(document).on('click', '.js-mnt-taxonomy-popular-show-hide', function () {
...
jQuery(document).on('click', '.js-mnt-taxonomy-popular-add', function () {
...
jQuery(document).on('click', '.js-mnt-taxonomy-add-new', function () {
    var thiz = jQuery(this),
            taxonomy = thiz.data('taxonomy');
    setTaxonomy(taxonomy, this);
});

jQuery(document).on('keypress', '.js-mnt-new-taxonomy-title', function (e) {
    if (13 === e.keyCode) {
        e.preventDefault();
        var thiz = jQuery(this),
                taxonomy = thiz.data('taxonomy'),
...
function updateTaxonomies(taxonomy, form)
{
    ...
    for (var i = 0; i < toshow.length; i++) {
        ...
        str += '<span><a href="#" class=\'ntdelbutton\' data-mncf-i=\'' + i + '\' id=\'post_tag-check-num-' + i + '\'>X</a>&nbsp;' + sh + '</span>';
    }
...
function initTaxonomies(values, taxonomy, url, fieldId)
{
    ...
    jQuery('input[name=tmp_' + taxonomy + ']').suggest(
        mntoolset_taxonomy_settings.ajaxurl + '?action=mnt_suggest_taxonomy_term&taxonomy=' + taxonomy,
        {
            resultsClass: 'mnt-suggest-taxonomy-term',
            selectClass: 'mnt-suggest-taxonomy-term-select'
        }        
    );
...
//        jQuery('.js-mnt-hierarchical-taxonomy-add-new-' + taxonomy, form).toggle();
//        self.hide_parent_button_if_no_terms(taxonomy, button);
//    }
    self.add_new_show_hide = function (taxonomy, button) {
        var form = jQuery(button).closest('form');
        var add_wrap = jQuery(".js-mnt-hierarchical-taxonomy-add-new-" + taxonomy);
    ...
    jQuery('.js-mnt-hierarchical-taxonomy-add-new').on('click', function () {
        var thiz = jQuery(this),
                taxonomy = thiz.data('taxonomy');
        self.add_taxonomy(taxonomy, this);
    });
    
    jQuery('.js-mnt-hierarchical-taxonomy-add-new-show-hide').on('click', function () {
    ...
    self.add_taxonomy = function (taxonomy, button)
    {
        ...
        if (build_what === 'checkboxes') {
            ...
            var new_checkbox = '<li><input data-parent="' + parent + '" class="mnt-form-checkbox form-checkbox checkbox" type="checkbox" name="' + taxonomy + '[]" checked="checked" value="' + new_taxonomy + '"></input><label>' + new_taxonomy + '</label></li>';
            ...
            if (first_checkbox.length == 0) {
                ...
                level++;
                new_checkbox = '<ul class="mnt-form-set-children" data-level="' + level + '">' + new_checkbox + '</ul>';
                //first_checkbox = ;
                //add_before = false;
                //add_position = jQuery('input[name="' + taxonomy + '\[\]"][value="' + parent + '"]').closest('li');
