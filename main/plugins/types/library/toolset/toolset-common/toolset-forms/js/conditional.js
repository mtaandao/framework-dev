/**
 * @see MNToolset_Forms_Conditional (classes/conditional.php)
 *
 *
 */
var mntCondTriggers = {}
, mntCondFields = {}
, mntCondCustomTriggers = {}
, mntCondCustomFields = {}
, mntCondDebug = false;

var mntCond = (function ($) {

    function init()
    {
        _.each(mntCondTriggers, function (triggers, formID) {
        ...
        _.each(mntCondCustomTriggers, function (triggers, formID) {
            _.each(triggers, function (fields, trigger) {
                var $trigger = _getTrigger(trigger, formID);
                _bindChange(formID, $trigger, function (e) {
...
//            });
    ...
    {
        var $trigger = $('[data-mnt-name="' + trigger + '"]', formID);
        /**
         * admin
         */
        if ($('body').hasClass('admin')) {
            trigger = trigger.replace(/mncf\-/, 'mncf[') + ']';
            $trigger = $('[data-mnt-name="' + trigger + '"]', formID);
        }
        /**
         * handle skype field
         */
        if ($trigger.length < 1) {
            $trigger = $('[data-mnt-name="' + trigger + '[skypename]"]', formID);
        }
        /**
         * handle date field
         */
        if ($trigger.length < 1) {
            $trigger = $('[data-mnt-name="' + trigger + '[datepicker]"]', formID);
        }
        /**
         ...
         * handle select
         */
        if ($trigger.length > 0 && 'option' == $trigger.data('mnt-type')) {
    ...
    function _getTriggerValue($trigger, formID)
    {
        if (mntCondDebug) {
            ...
            console.log('$trigger', $trigger);
            console.log('$trigger.type', $trigger.data('mnt-type'));
        }
    ...
    function _getAffected(affected, formID)
    {
        ...
        var $el = $('[data-mnt-id="' + affected + '"]', formID);
        if ($('body').hasClass('admin')) {
            $el = $el.closest('.mnt-field');
            if ($el.length < 1) {
            ...
            if ($el.length < 1) {
                 ...
                 * try get element
                 */
                $obj = $('[data-mnt-id="' + affected + '_file"]', formID);
                /**
                 * handle by mnt field name
                 */
                if ($obj.length < 1) {
                    $obj = $('[data-mnt-name="' + name + '"]', formID);
                }
                /**
                 * handle date field
                 */
                if ($obj.length < 1) {
                    $obj = $('[data-mnt-name="' + name + '[datepicker]"]', formID);
                    if ($obj.length < 1) {
                 ...
                 */
                if ($obj.length < 1) {
                    $obj = $('[data-mnt-name="' + name + '[skypename]"]', formID);
                    if ($obj.length < 1) {
             ...
             */
            if ($obj.length > 0) {
                $el = $obj.closest('.js-mnt-conditional-field');
                if ($el.length < 1) {
                    $el = $obj.closest('.cred-field');// This for backwards compatibility
                    if ($el.length < 1) {
                        $el = $obj.closest('.js-mnt-field-items');
                    }
                }
            }
            /**
    ...
    {
        var __ignore = false;
        var c = mntCondFields[formID][field];
        ...
        _.each(c.conditions, function (data) {
            ...
            var field_type = $trigger.data('mnt-type');
            if (data.type == 'date') {
                field_type = 'date';
            }
        ...
        //if ( $trigger.length && next && $trigger.hasClass('js-mnt-date' ) ) {
        //    setTimeout(function() {
    ...
    function _check(formID, fields)
    {
        ...
        mntCallbacks.conditionalCheck.fire(formID);
    }
    ...
    function _bindChange(formID, $trigger, func)
    {
        ...
        var binded = do_action('conditional_trigger_bind_' + $trigger.data('mnt-type'), $trigger, func, formID);
        if (binded) {
            return;
        }
        /**
        ...
        if (mntCondDebug) {
            ...
            console.log('$trigger', $trigger);
            console.log('mnt-type', $trigger.data('mnt-type'));
        }
        switch ($trigger.data('mnt-type')) {
            case 'checkbox':
            ...
            case 'file':
                ...
                break;
            default:
                if ($trigger.hasClass('js-mnt-colorpicker')) {
    ...
    function _custom(formID, fields)
    {
        ...
        _.each(fields, function (field) {
            var c = mntCondCustomFields[formID][field];
            ...
            _.each(c.triggers, function (t) {
                ...
                if ($trigger.data('mnt-type') == 'radio')
                    is_array = false;
    ...
    function _showHide(show, $el)
    {
        ...
        /*if (jQuery('.mnt-form-error').length) {            
         jQuery('.mnt-form-error').hide();
         }*/
        ...
        if (show) {
            if ($el.hasClass('mnt-date') && 'object' == typeof mntDate) {
                $('.js-mnt-date', $el).removeAttr('disabled');
                mntDate.init('body');
            }
            $el.addClass('mnt-conditional-visible').removeClass('mnt-conditional-hidden js-mnt-remove-on-submit js-mnt-validation-ignore');
            switch (effectmode) {
                case 'fade-slide':
                    setTimeout(function () {
            ...
            $el.addClass('mnt-conditional-hidden js-mnt-remove-on-submit js-mnt-validation-ignore').removeClass('mnt-conditional-visible');
            switch (effectmode) {
                case 'fade-slide':
                    setTimeout(function () {
    ...
    function ajaxCheck(formID, field, conditions)
    {
        ...
        var data = {
            'action': 'mntoolset_conditional',
            'conditions': conditions,
            'values': values
        };
        $.post(mntConditional.ajaxurl, data, function (passed) {
            _showHide(passed, _getAffected(field, formID));
            mntCallbacks.conditionalCheck.fire(formID);
        }).fail(function (data) {
            //alert(data);
    ...
    function addConditionals(data)
    {
        _.each(data, function (c, formID) {
            if (typeof c.triggers != 'undefined'
                    && typeof mntCondTriggers[formID] != 'undefined') {
                _.each(c.triggers, function (fields, trigger) {
                    mntCondTriggers[formID][trigger] = fields;
                    var $trigger = _getTrigger(trigger, formID);
            ...
            if (typeof c.fields != 'undefined'
                    && typeof mntCondFields[formID] != 'undefined') {
                _.each(c.fields, function (conditionals, field) {
                    mntCondFields[formID][field] = conditionals;
                });
            }
            if (typeof c.custom_triggers != 'undefined'
                    && typeof mntCondCustomTriggers[formID] != 'undefined') {
                _.each(c.custom_triggers, function (fields, trigger) {
                    mntCondCustomTriggers[formID][trigger] = fields;
                    var $trigger = _getTrigger(trigger, formID);
                    _bindChange(formID, $trigger, function () {
                        _custom(formID, fields);
                    });
            ...
            if (typeof c.custom_fields != 'undefined'
                    && typeof mntCondCustomFields[formID] != 'undefined') {
                _.each(c.custom_fields, function (conditionals, field) {
                    mntCondCustomFields[formID][field] = conditionals;
                });
            }
        });
    }
