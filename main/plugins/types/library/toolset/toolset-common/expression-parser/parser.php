...
if ( defined( 'MNT_PARSER' ) ) {
    return; 
}

define( 'MNT_PARSER', true );

/**
*
*   Custom Content Parser, advanced parser for arithmetic,logical and comparision expressions
...
class Toolset_Parser
{
    ...
    private function _clonePostFix($pf)
    {
        $nemnf=array();
        for ($i=0; $i<count($pf); $i++)
            $nemnf[$i]=Toolset_Tokenizer::cloneToken($pf[$i]);
        return $nemnf;
    }
