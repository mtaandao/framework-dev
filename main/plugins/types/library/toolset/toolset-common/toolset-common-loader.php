...
if ( ! function_exists( 'toolset_common_boostrap' ) ) {
	...
	/**
	 * Set Custom Content Common constants.
	 *
	 * TOOLSET_COMMON_URL				Base URL for the Custom Content Common instance. Note that is does not have a trailing slash.
	 * TOOLSET_COMMON_FRONTEND_URL		Base frontend URL for the Custom Content Common instance. Note that is does not have a trailing slash.
	 *
	 ...
	 * https://developer.mtaandao.org/reference/functions/set_url_scheme/
	 *
	 ...
	 * and we ned to build a frontend URL based on the Custom Content Common URL, while they have different SSL schemas,
    ...
    // Load early
	// We register scripts and styles that are dependences for Custom Content assets
    add_action( 'after_setup_theme', 'toolset_common_boostrap' );
}


if( !function_exists('toolset_disable_mnml_admin_lang_switcher') ){
	add_filter( 'mnml_show_admin_language_switcher', 'toolset_disable_mnml_admin_lang_switcher' );
	function toolset_disable_mnml_admin_lang_switcher( $state ) {
		global $pagenow;
		...
		$toolset_pages = apply_filters( 'toolset_filter_disable_mnml_lang_switcher_in_admin', $toolset_pages );
