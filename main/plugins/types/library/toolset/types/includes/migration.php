 ...
 * @return array
 */
function mncf_admin_migration_form()
{
    global $mndb;
    $mncf_types = get_option(MNCF_OPTION_NAME_CUSTOM_TYPES, array());
    $mncf_taxonomies = get_option(MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array());
    $mncf_types_defaults = mncf_custom_types_default();
    $mncf_taxonomies_defaults = mncf_custom_taxonomies_default();

    $form = array();
    $form['#form']['callback'] = 'mncf_admin_migration_form_submit';
    ...
    if (!empty($cfui_types)) {
        $form['types_title'] = array(
            '#type' => 'markup',
            '#markup' => '<h3>' . __('Custom Types UI Post Types', 'mncf') . '</h3>',
        );
    ...
    if (!empty($cfui_taxonomies)) {
        $form['tax_titles'] = array(
            '#type' => 'markup',
            '#markup' => '<h3>' . __('Custom Types UI Taxonomies', 'mncf') . '</h3>',
        );
    ...
    if (!empty($cfui_types) || !empty($cfui_taxonomies)) {
        $form['deactivate-cfui'] = array(
            ...
            '#before' => '<br /><br />',
            '#default_value' => 1,
            '#title' => __('Disable Custom Types UI after importing the configuration (leave this checked to avoid defining custom types twice)', 'mncf'),
        );
    };
    ...
    if (class_exists('acf') && !class_exists('acf_pro')) {
        ...
        if (!empty($acf_groups)) {
            $mncf_types = mncf_admin_fields_get_available_types();
            $mncf_types_options = array();
            foreach ($mncf_types as $type => $data) {
                $mncf_types_options[$type] = array(
                    '#title' => $data['title'],
                    '#value' => $type,
                );
            }
            ...
            if (!empty($acf_groups)) {
                $form['acf_title'] = array(
                    '#type' => 'markup',
                    '#markup' => '<h3>' . __('Advanced Custom Fields', 'mncf') . '</h3>',
                );
            }

            foreach ($acf_groups as $acf_key => $acf_post) {
                $group_id = $mndb->get_var(
                    $mndb->prepare(
                ...
                if (empty($group_id)) {
                    $add = __('Group will be created', 'mncf');
                } else {
                    $add = __('Group will be updated', 'mncf');
                }
                ...
                foreach ($metas as $meta_name => $meta) {
                    if (strpos($meta_name, 'field_') === 0) {
                        ...
                        $exists = mncf_types_cf_under_control('check_exists',
                            $data['name']);
                        $outsider = mncf_types_cf_under_control('check_outsider',
                            $data['name']);
                        $supported = !empty($acf_types[$data['type']]);
                        if (!$supported) {
                            $attributes = array('style' => 'margin-left: 20px;', 'readonly' => 'readonly', 'disabled' => 'disabled');
                            $add = __('Field conversion not supported by Types', 'mncf');
                        } else if ($exists && !$outsider) {
                            $attributes = array('style' => 'margin-left: 20px;', 'readonly' => 'readonly', 'disabled' => 'disabled');
                            $add = __('Field with same name is already controlled by Types', 'mncf');
                        } else if ($exists && $outsider) {
                            $attributes = array('style' => 'margin-left: 20px;');
                            $add = __('Field will be updated', 'mncf');
                        } else {
                            $attributes = array('style' => 'margin-left: 20px;');
                            $add = __('Field will be created', 'mncf');
                        }
    ...
    $form['submit'] = array(
        '#type' => 'submit',
        '#name' => 'submit',
        '#value' => __('Import custom field settings', 'mncf'),
 ...
 */
function mncf_admin_migration_form_submit() {
    $cfui_types = get_option('cpt_custom_post_types', array());
    $cfui_taxonomies = get_option('cpt_custom_tax_types', array());
    $mncf_types = get_option(MNCF_OPTION_NAME_CUSTOM_TYPES, array());
    $mncf_taxonomies = get_option(MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array());
    $redirect_page = 'mncf-ctt';

    if (!empty($_POST['cfui']['types'])) {
        $data = array();
        foreach ($_POST['cfui']['types'] as $key => $types_slug) {
            if (array_key_exists(sanitize_title($types_slug), $mncf_types)) {
                continue;
            }
            foreach ($cfui_types as $cfui_type) {
                if (sanitize_title($cfui_type['name']) == $types_slug) {
                    $data[$types_slug] = mncf_admin_migrate_get_cfui_type_data($cfui_type);
                    mncf_admin_message_store(
                        sprintf(__("Post Type %s added", 'mncf'),
                        '<em>' . $cfui_type['name'] . '</em>')
                    );
                }
            }
        }
        $mncf_types = array_merge($mncf_types, $data);
    }
    if (!empty($_POST['cfui']['tax'])) {
        $data = array();
        foreach ($_POST['cfui']['tax'] as $key => $tax_slug) {
            if (array_key_exists(sanitize_title($tax_slug), $mncf_taxonomies)) {
                continue;
            }
            foreach ($cfui_taxonomies as $cfui_tax) {
                if (sanitize_title($cfui_tax['name']) == $tax_slug) {
                    $data[$tax_slug] = mncf_admin_migrate_get_cfui_tax_data($cfui_tax);
                    mncf_admin_message_store(
                        sprintf(__("Taxonomy %s added", 'mncf'),
                        '<em>' . $cfui_tax['name'] . '</em>')
                    );
                    if (
                        ...
                        foreach( $cfui_tax[1] as $key) {
                            ...
                            if ( array_key_exists($types_slug, $mncf_types) ) {
                                if ( !array_key_exists('taxonomies', $mncf_types[$types_slug] )) {
                                    $mncf_types[$types_slug]['taxonomies'] = array();
                                }
                                $mncf_types[$types_slug]['taxonomies'][$tax_slug] = 1;
                                $mncf_types[$types_slug][TOOLSET_EDIT_LAST] = time();
                            }
                        }
        ...
        $mncf_taxonomies = array_merge($mncf_taxonomies, $data);
        update_option(MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, $mncf_taxonomies);
    }
    update_option(MNCF_OPTION_NAME_CUSTOM_TYPES, $mncf_types);
    ...
    if (!empty($_POST['acf_posts']['migrate_groups'])) {
        foreach ($_POST['acf_posts']['migrate_groups'] as $acf_group_id) {
            ...
            $group = $mndb->get_row(
                $mndb->prepare(
                    "SELECT ID, post_title FROM $mndb->posts WHERE post_title = %s AND post_type = %s",
            ...
            if (empty($group)) {
                ...
                $group_id = mncf_admin_fields_save_group($group);
                mncf_admin_message_store(sprintf(__("Group %s added", 'mncf'),
                                '<em>' . $group['name'] . '</em>'));
            } else {
                $group_id = $group->ID;
                mncf_admin_message_store(sprintf(__("Group %s updated", 'mncf'),
            ...
            if ($group_id && !empty($_POST['acf_posts'][$acf_group_id]['fields'])) {
                foreach ($_POST['acf_posts'][$acf_group_id]['fields'] as $field_id => $field) {
                    ...
                    if (!mncf_types_cf_under_control('check_exists',
                                    $field['slug'])
                            || mncf_types_cf_under_control('check_outsider',
                                    $field['slug'])) {
                        // save field
                        $field['controlled'] = 1;
                        $temp = mncf_admin_fields_save_field($field);
                        $fields_to_add[] = $temp;
                        mncf_admin_message_store(sprintf(__("Field %s added", 'mncf'),
                                        '<em>' . $temp . '</em>'));
                    }
                }
                mncf_admin_fields_save_group_fields($group_id, $fields_to_add,
                        false);
            }
            mncf_admin_fields_save_group_post_types($group_id, array());
            mncf_admin_fields_save_group_terms($group_id, array());
        }
        $redirect_page = 'mncf';
    }
    flush_rewrite_rules();
    ...
    mn_safe_redirect(esc_url_raw(admin_url('admin.php?page=' . $redirect_page)));
    die();
}
 ...
 * @return type
 */
function mncf_admin_migrate_get_cfui_type_data($cfui_type) {
    $cfui_types_migrated = array();
    $supports = array();
    ...
    foreach ($mncf_types_defaults['labels'] as $label_id => $label_text) {
        if (empty($labels[$label_id])) {
            $labels[$label_id] = $label_text;
        }
    }
    foreach ($labels as $label_id => $label_text) {
        if (!isset($mncf_types_defaults['labels'][$label_id])) {
 ...
 * @return type
 */
function mncf_admin_migrate_get_cfui_tax_data($cfui_tax) {
    $cfui_tax_migrated = array();
    $supports = array();
    if (!empty($cfui_tax[1])) {
    ...
    foreach ($mncf_taxonomies_defaults['labels'] as $label_id => $label_text) {
        if (empty($labels[$label_id])) {
            $labels[$label_id] = $label_text;
        }
    }
    foreach ($labels as $label_id => $label_text) {
        if (!isset($mncf_taxonomies_defaults['labels'][$label_id])) {
            unset($labels[$label_id]);
        }
    }
    // Force menu_name label
    ...
    $cfui_tax_migrated[$slug] = array_merge($mncf_taxonomies_defaults,
