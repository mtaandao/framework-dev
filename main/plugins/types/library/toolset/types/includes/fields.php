 ...
 * @return type
 */
function mncf_admin_get_taxonomies_by_group( $group_id )
{
    /**
     * sanitized $group_id
     */
    ...
    global $mndb;
    $terms = get_post_meta( $group_id, '_mn_types_group_terms', true );
    if ( $terms == 'all' ) {
        return array();
    }
    ...
    if ( !empty( $terms ) ) {
        foreach ( $terms as $term ) {
            $term = $mndb->get_row(
                $mndb->prepare(
                    "SELECT tt.term_taxonomy_id, tt.taxonomy, t.term_id, t.slug, t.name FROM {$mndb->term_taxonomy} tt JOIN {$mndb->terms} t WHERE t.term_id = tt.term_id AND tt.term_taxonomy_id = %d",
                    $term
                ),
                ARRAY_A
            );
 ...
 * @return type
 */
function mncf_admin_get_templates_by_group( $group_id )
{
    /**
     * sanitized $group_id
     */
    ...
    $data = get_post_meta( $group_id, '_mn_types_group_templates', true );
    if ( $data == 'all' ) {
        return array();
    }
 ...
 *
 */
function mncf_admin_fields_activate_group( $group_id, $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME )
{
    global $mndb;
    return $mndb->update( $mndb->posts, array('post_status' => 'publish'),
 ...
 *
 */
function mncf_admin_fields_deactivate_group( $group_id,
        $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME ) {
    global $mndb;
    return $mndb->update( $mndb->posts, array('post_status' => 'draft'),
                    array('ID' => intval( $group_id ), 'post_type' => $post_type),
                    array('%s'), array('%d', '%s')
    );
}
 ...
 * @return bool
 */
function mncf_admin_fields_remove_field_from_group( $group_id, $field_id ) {
    $group_fields = get_post_meta( $group_id, '_mn_types_group_fields', true );
    if ( empty( $group_fields ) ) {
        return false;
    }
    $group_fields = str_replace( ',' . $field_id . ',', ',', $group_fields );
    return update_post_meta( $group_id, '_mn_types_group_fields', $group_fields );
}
 ...
 * @deprecated
 */
function mncf_admin_fields_remove_field_from_group_bulk( $group_id, $fields ) {
    foreach ( $fields as $field_id ) {
		$field_id = sanitize_text_field( $field_id );
        mncf_admin_fields_remove_field_from_group( $group_id, $field_id );
    }
}
 ...
 *
 */
function mncf_admin_fields_delete_field( $field_id,
        $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, $meta_name = 'mncf-fields' ) {
    global $mndb;
    $fields = get_option( $meta_name, array() );
    if ( isset( $fields[$field_id] ) ) {
        // Remove from groups
        $groups = mncf_admin_fields_get_groups( $post_type );
        foreach ( $groups as $key => $group ) {
            mncf_admin_fields_remove_field_from_group( $group['id'], $field_id );
        }
        // Remove from posts
        if ( !mncf_types_cf_under_control( 'check_outsider', $field_id, $post_type, $meta_name ) ) {
            $results = $mndb->get_results(
                $mndb->prepare(
                    "SELECT post_id, meta_key FROM $mndb->postmeta WHERE meta_key = %s",
                    mncf_types_get_meta_prefix( $fields[$field_id] ) . strval( $field_id )
                )
            );
        ...
        mncf_admin_fields_save_fields( $fields, true, $meta_name );
        return true;
    } else {
        return false;
    }
 ...
 * @return int
 */
function mncf_admin_fields_delete_group( $group_id,
        $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME ) {
    $group = get_post( $group_id );
    if ( empty( $group ) || $group->post_type != $post_type ) {
        return false;
    }
    mn_delete_post( $group_id, true );
}
 ...
 * @return int Group ID.
 */
function mncf_admin_fields_save_group( $group, $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, $which_fields = 'none' )
{
    ...
    if( empty( $post['post_title'] ) ) {
        mncf_admin_message( __( 'Please set name', 'mncf' ), 'error' );
        return false;
    }
    ...
    if ( $update ) {
        $group_id = mn_update_post( $post );
        if ( !$group_id ) {
            return false;
        }
        update_post_meta( $group_id, TOOLSET_EDIT_LAST, time());
    } else {
        $group_id = mn_insert_post( $post, true );
        if ( is_mn_error( $group_id ) ) {
            return false;
        }
    }

    if( isset( $group['admin_styles'] ) ) {
        mncf_admin_fields_save_group_admin_styles( $group_id, $group['admin_styles'] );
    }

    if ( !empty( $group['filters_association'] ) ) {
        update_post_meta( $group_id, '_mn_types_group_filters_association', $group['filters_association'] );
    } else {
        delete_post_meta( $group_id, '_mn_types_group_filters_association' );
    }
    ...
    if ( function_exists( 'icl_register_string' ) ) {
	    ...
	    try {
		    ...
		    if( null != $field_group ) {
			    ...
			    $group_mnml->register( $slug_pre_save );
		    }
    ...
    // admin message
    mncf_admin_fields_save_message( $update, $which_fields );
 ...
 */
function mncf_admin_fields_save_fields( $fields, $forced = false, $option_name = 'mncf-fields' ) {
 ...
 * @param string $option_name
 *
 * @return string|MN_Error Field slug or an error object.
 */
function mncf_admin_fields_save_field( $field, $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, $option_name = 'mncf-fields' )
{

    if ( !isset( $field['name'] ) || !isset( $field['type'] ) ) {
        return new MN_Error( 'mncf_save_field_no_name_or_type', __( "Error saving field", 'mncf' ) );
    }

    $field = mncf_sanitize_field( $field );

    if ( empty( $field['name'] ) || empty( $field['slug'] ) ) {
        return new MN_Error( 'mncf_save_field_no_name', __( "Please set name for field", 'mncf' ) );
    }
    ...
    // @TODO Remember why
    if ( mncf_types_cf_under_control( 'check_outsider', $field['id'],
                    $post_type, $option_name ) ) {
        $field_previous_data = mncf_admin_fields_get_field( $field['id'], false,
    ...
    $field['data'] = apply_filters( 'mncf_fields_' . $field['type'] . '_meta_data', $field['data'], $field );
    ...
    if( isset($field['slug-pre-save'] ) && $field['slug'] != $field['slug-pre-save'] ) {
        ...
        $belongs_to_groups = mncf_admin_fields_get_groups_by_field( $field['slug-pre-save'], $post_type );
        if( isset( $_GET['group_id'] )
            ...
            $mndb->update(
                $mndb->postmeta,
                array( 'meta_key' => 'mncf-'.$field['slug'] ),
                array( 'meta_key' => 'mncf-'.$field['slug-pre-save'] ),
                array( '%s'),
                array( '%s')
            );
            ...
            $groups_conditions = $mndb->get_results( "
                SELECT post_id, meta_value
                FROM   $mndb->postmeta
                WHERE  meta_key = '_mncf_conditional_display'
                    ...
                    if( isset( $meta_value['conditions'] ) ) {
                        foreach( $meta_value['conditions'] as &$single_condition ) {
                            if( $single_condition['field'] == $field['slug-pre-save'] ) {
                                ...
                                $single_condition['field'] = $field['slug'];
                                update_post_meta( $group_condition->post_id, '_mncf_conditional_display', $meta_value );
                                break;
                            }
                        }
                    }
            ...
            // delete field
            mncf_admin_fields_delete_field( $field['slug-pre-save'], $post_type, $option_name );
			        ...
			        if ( is_array( $items ) && ! empty( $items ) ) {
				        ...
				        foreach ( $items as $item ) {
					        $mndb->update(
						        $mndb->postmeta,
						        array( 'meta_key' => 'mncf-' . $field['slug'] ),
						        array( 'meta_key' => 'mncf-' . $field['slug-pre-save'], 'post_id' => $item->ID ),
						        array( '%s', '%d' ),
						        array( '%s' )
					        );
				        }
    ...
    // MNML register strings
    if ( function_exists( 'icl_register_string' ) ) {
        if ( isset($_POST['mnml_cf_translation_preferences'][$field_id] ) ) {
            $__mnml_action = intval( $_POST['mnml_cf_translation_preferences'][$field_id] );
        } else {
            $__mnml_action = mncf_mnml_get_action_by_type( $field['type'] );
        }
    ...
    /**
     * MNML update translation status
     */
    if (
        isset($save_data['data'])
        && isset($save_data['data']['submit-key'])
    ) {
        if ( isset( $_POST[ 'mnml_cf_translation_preferences' ][ $save_data['data']['submit-key'] ] ) ) {
            do_action(
                'mnml_config',
                array(
                    'section'   => 'custom-fields',
                    'key'       => mncf_types_get_meta_prefix( mncf_admin_fields_get_field( $save_data['slug']) ) . $save_data['slug'],
                    'value'     => intval( $_POST[ 'mnml_cf_translation_preferences' ][ $save_data['data']['submit-key'] ] ),
                    'read_only' => true
                )
            );
        }
 ...
 */
function mncf_admin_fields_save_message( $updated = false, $which_fields = 'none' ) {

    switch( $which_fields ) {
        case 'custom':
            $msg = $updated
                ? __( 'Post Field Group saved.', 'mncf' )
                : __( 'New Post Field Group created.', 'mncf' );
            break;

        case 'user':
            $msg = $updated
                ? __( 'User Field Group saved.', 'mncf' )
                : __( 'New User Field Group created.', 'mncf' );
            break;

        case 'term':
            $msg = $updated
                ? __( 'Term Field Group saved.', 'mncf' )
                : __( 'New Term Field Group created.', 'mncf' );
            break;
    }
 ...
 */
function mncf_admin_custom_fields_change_type_allowed_matrix() {
 ...
 * @return bool
 */
function mncf_admin_fields_save_group_fields( $group_id, $fields, $add = false, $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, $option_name = null )
{
	if( null == $option_name ) {
		$option_name = ( $post_type == TYPES_CUSTOM_FIELD_GROUP_CPT_NAME ? 'mncf-fields' : 'mncf-usermeta' );
	}
    $fields = mncf_types_cf_under_control( 'add', array('fields' => $fields), $post_type, $option_name );
    if ( $add ) {
        $existing_fields = mncf_admin_fields_get_fields_by_group( $group_id, 'slug', false, true, false, $post_type, $option_name );
        $order = array();
    ...
    update_post_meta( $group_id, '_mn_types_group_fields', $fields );
}
 ...
 */
function mncf_admin_fields_save_group_post_types( $group_id, $post_types )
{
    if ( !empty($post_types) ) {
    ...
    if ( empty( $post_types ) ) {
        update_post_meta( $group_id, '_mn_types_group_post_types', 'all' );
        return true;
    }
    $post_types = ',' . implode( ',', (array) $post_types ) . ',';
    update_post_meta( $group_id, '_mn_types_group_post_types', $post_types );
}
 ...
 * @param type $terms
 */
function mncf_admin_fields_save_group_terms( $group_id, $terms ) {
    if ( empty( $terms ) ) {
        update_post_meta( $group_id, '_mn_types_group_terms', 'all' );
        return true;
    }
    $terms = ',' . implode( ',', (array) $terms ) . ',';
    update_post_meta( $group_id, '_mn_types_group_terms', $terms );
}
 ...
 * @param type $terms
 */
function mncf_admin_fields_save_group_templates( $group_id, $templates ) {
    if ( empty( $templates ) ) {
        update_post_meta( $group_id, '_mn_types_group_templates', 'all' );
        return true;
    }
    $templates = ',' . implode( ',', (array) $templates ) . ',';
    update_post_meta( $group_id, '_mn_types_group_templates', strip_tags($templates) );
}
 ...
 *
 */
function mncf_admin_fields_get_ajax_activation_link( $group_id, $action = 'activate_group' ) {
    return '<a href="' . admin_url( 'admin-ajax.php?action=mncf_ajax&amp;'
                    . 'mncf_action=' . $action . '&amp;group_id='
                    . $group_id . '&amp;mncf_ajax_update=mncf_list_ajax_response_'
                    . $group_id ) . '&amp;_mnnonce=' . mn_create_nonce( $action )
 ...
 * @return string
 */
function mncf_admin_fields_get_ajax_deactivation_link( $group_id, $action = 'deactivate_group' ) {
    return '<a href="' . admin_url( 'admin-ajax.php?action=mncf_ajax&amp;'
                    . 'mncf_action=' . $action . '&amp;group_id='
                    . $group_id . '&amp;mncf_ajax_update=mncf_list_ajax_response_'
                    . $group_id ) . '&amp;_mnnonce=' . mn_create_nonce( $action )
 ...
 * @return boolean|int
 */
function mncf_admin_fields_checkbox_migrate_empty_check( $field, $action ) {
    if ( $field['type'] != 'checkbox' ) {
        return false;
    }
    if ($field['meta_type'] == 'postmeta') {
        $filter = mncf_admin_fields_get_filter_by_field( $field['id'] );
        if ( !empty( $filter ) ) {
            $posts = array();
            $meta_key = esc_sql( mncf_types_get_meta_prefix( $field ) . $field['id'] );
            $meta_query = '';
            if ( $action == 'do_not_save_check' ) {
                $meta_query = "(m.meta_key = '$meta_key' AND m.meta_value = '0')";
                $posts = mncf_admin_fields_get_posts_by_filter( $filter, $meta_query );
            } else if ( $action == 'save_check' ) {
                $posts = mncf_admin_fields_get_posts_by_filter_missing_meta( $filter,
                        $meta_key );
            }
            $option = get_option( 'mncf_checkbox_migration', array() );
            $cache_key = $action == 'do_not_save_check' ? 'do_not_save' : 'save';
            $option[$cache_key] = $posts;
            update_option( 'mncf_checkbox_migration', $option );
            return $posts;
        }
    } else if ($field['meta_type'] == 'usermeta') {
        $option = get_option( 'mncf_checkbox_migration_usermeta', array() );
        $cache_key = $action == 'do_not_save_check' ? 'do_not_save' : 'save';
        if ( $action == 'do_not_save_check' ) {
            $user_query = new MN_User_Query( array('meta_key' => $field['meta_key'],
                'meta_value' => '0', 'meta_compare' => '=', 'fields' => 'ID') );
            $r =  $user_query->results;
        } else if ( $action == 'save_check' ) {
            global $mndb;
            $_query = "SELECT u.ID FROM {$mndb->users} u WHERE NOT EXISTS (SELECT um.user_id FROM {$mndb->usermeta} um WHERE u.ID = um.user_id AND um.meta_key = '%s')";
            $r = $mndb->get_col($mndb->prepare( $_query, $field['meta_key']) );
        }
        $option[$field['meta_key']][$cache_key] = $r;
        update_option( 'mncf_checkbox_migration_usermeta', $option );
        return $r;
    }
    return false;
}
 ...
 * @return boolean|int
 */
function mncf_admin_fields_checkbox_migrate_empty( $field, $action ) {
    if ( $field['type'] != 'checkbox' ) {
        return false;
    }
    if ( $field['meta_type'] == 'usermeta' ) {
        ...
        if ( empty( $option[$field['meta_key']][$action] ) ) {
            $users = mncf_admin_fields_checkbox_migrate_empty_check( $field,
                    $action . '_check' );
        } else {
    ...
    $option = get_option( 'mncf_checkbox_migration', array() );
    $meta_key = mncf_types_get_meta_prefix( $field ) . $field['id'];
 ...
 */
function mncf_admin_fields_get_filter_by_field( $field ) {
    $field = mncf_admin_fields_get_field( $field );
    if ( empty( $field ) ) {
        return false;
    }
    $filter = array();
    ...
    $groups = mncf_admin_fields_get_groups_by_field( $field['id'] );
    foreach ( $groups as $group_id => $group_data ) {
        ...
        $filter['terms'] = array_merge( $filter['terms'],
                explode( ',',
                        trim( get_post_meta( $group_id, '_mn_types_group_terms',
                                        true ), ',' ) ) );
...
/**
 * Gets posts by filter fetched with mncf_admin_fields_get_filter_by_field().
 *
 * @param array $filter
 * @param string $meta_query This argument needs to be allways sanitized!
 * @return array
 */
function mncf_admin_fields_get_posts_by_filter( $filter, $meta_query = '' ) {
    global $mndb, $mncf;
    $query = array();
    $join = array();
    ...
    if ( $filter['terms'] != 'all' && !empty( $filter['terms'] ) ) {
        ...
        foreach ( $filter['terms'] as $term_id ) {
            $term_taxonomy_id = $mndb->get_var(
                $mndb->prepare(
                    "SELECT term_taxonomy_id FROM $mndb->term_taxonomy WHERE term_id=%d",
                    $term_id
                )
            );
        ...
        $join[] = "LEFT JOIN $mndb->term_relationships t ON p.ID = t.object_id ";
    }
    if ( $filter['templates'] != 'all' && !empty( $filter['templates'] ) ) {
        ...
        $query[] = '(m.meta_key = \'_mn_page_template\' AND m.meta_value IN (\'' . implode( '\',\'',
                        $templates ) . '\'))';
    }
    if ( !empty( $meta_query )
            || ($filter['templates'] != 'all' && !empty( $filter['templates'] )) ) {
        $join[] = "LEFT JOIN $mndb->postmeta m ON p.ID = m.post_id ";
    }

    $_query = "SELECT p.ID FROM $mndb->posts p " . implode( '', $join );
    if ( !empty( $query ) ) {
    ...
    $posts = $mndb->get_col( $_query );
    return $posts;
}

function mncf_admin_fields_get_posts_by_filter_missing_meta_helper( $e )
{
    return '%s';
}
 ...
 * with mncf_admin_fields_get_filter_by_field().
 *
 * @global object $mndb
 * @param type $filter
 * @return type
 */
function mncf_admin_fields_get_posts_by_filter_missing_meta( $filter, $meta_key = '' )
{
    global $mndb, $mncf;
    $query = $join = array();
    $args = array(
    ...
    if ( $filter['types'] != 'all' && !empty( $filter['types'] ) ) {
        ...
        foreach ( $post_types_all as $post_type ) {
            if ( in_array( $post_type, $mncf->excluded_post_types ) ) {
                continue;
            }
    ...
    $query[] = sprintf(
        ' p.post_type IN ( %s ) ',
        implode( ', ', array_map('mncf_admin_fields_get_posts_by_filter_missing_meta_helper', $args['post_types'] ))
    );
    ...
    if ( $filter['templates'] != 'all' && !empty( $filter['templates'] ) ) {
        ...
        $query[] = sprintf(
            ' (m.meta_key = \'_mn_page_template\' AND m.meta_value IN (%s))',
            implode( ', ', array_map('mncf_admin_fields_get_posts_by_filter_missing_meta_helper', $args['templates'] ))
        );
        $join[] = "LEFT JOIN $mndb->postmeta m ON p.ID = m.post_id ";
    }

    $_query = "SELECT DISTINCT p.ID FROM $mndb->posts p " . implode( '', $join );
    $_query .= "WHERE NOT EXISTS ( SELECT * FROM $mndb->postmeta mm WHERE p.ID = mm.post_id AND mm.meta_key = %s )";
    ...
    return $mndb->get_col( $mndb->prepare($_query, $args));
}
 ...
 * @return boolean|int
 */
function mncf_admin_fields_checkboxes_migrate_empty_check( $field, $action ) {
    ...
    if ( $field['meta_type'] == 'usermeta' ) {
        ...
        if ( $action == 'do_not_save_check' ) {
            ...
            $meta_query = "SELECT u.ID FROM {$mndb->users} u
                LEFT JOIN {$mndb->usermeta} um ON u.ID = um.user_id
        ...
        $users = $mndb->get_col( $mndb->prepare( $meta_query, $field['meta_key'] ) );
        $option = get_option( 'mncf_checkboxes_migration_usermeta', array() );
        $cache_key = $action == 'do_not_save_check' ? 'do_not_save' : 'save';
        $option[$field['meta_key']][$cache_key] = $users;
        update_option( 'mncf_checkboxes_migration_usermeta', $option );
        return $users;
    }
    $filter = mncf_admin_fields_get_filter_by_field( $field['id'] );
    if ( !empty( $filter ) ) {
        ...
        $meta_key = esc_sql( mncf_types_get_meta_prefix( $field ) . $field['id'] );
        $meta_query = '';
        // "mncf-fields-checkboxes-option-1873650245";s:1:"1";
        if ( $action == 'do_not_save_check' ) {
            $query = array();
            foreach ( $field['data']['options'] as $option_id => $option_data ) {
                $query[] = '\"' . esc_sql( $option_id ) . '\";i:0;';
        ...
        $option = get_option( 'mncf_checkboxes_migration', array() );
        $cache_key = $action == 'do_not_save_check' ? 'do_not_save' : 'save';
        $option[$cache_key] = $posts;
        update_option( 'mncf_checkboxes_migration', $option );
        return $posts;
    }
    return false;
}
 ...
 * @return boolean|int
 */
function mncf_admin_fields_checkboxes_migrate_empty( $field, $action ) {
    if ( $field['type'] != 'checkboxes' || empty( $field['data']['options'] ) ) {
        return false;
    }
    if ( $field['meta_type'] == 'usermeta' ) {
        $option = get_option( 'mncf_checkboxes_migration_usermeta', array() );
        if ( empty( $option[$field['meta_key']][$action] ) ) {
            $users = mncf_admin_fields_checkboxes_migrate_empty_check( $field,
                    $action . '_check' );
        } else {
            $users = $option[$field['meta_key']][$action];
        }
...
function mncf_admin_fields_form_fix_styles()
{
    $suffix = SCRIPT_DEBUG ? '' : '.min';
    mn_enqueue_style(
        'mncf-dashicons',
        site_url( "/mn-includes/css/dashicons$suffix.css" )
    );
}
