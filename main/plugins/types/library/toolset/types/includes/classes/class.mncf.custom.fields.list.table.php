 ...
 */
class MNCF_Custom_Fields_List_Table extends MN_List_Table
{
    var $bulk_action_field_name = 'mncf_cf_ids';
    /** ************************************************************************
    ...
    function column_default($item, $column_name)
    {
        switch($column_name){
            ...
            case 'post_types':
                ...
                $post_types = mncf_admin_get_post_types_by_group($item['id']);
                $supports = array();
                ...
                if( ! empty( $post_types ) ) {
                    ...
                    foreach( $post_types as $key => $post_type_slug ) {
                        ...
                        if( isset( $mn_post_types[$post_type_slug]->labels->singular_name ) ) {
                            $supports[] = $mn_post_types[$post_type_slug]->labels->singular_name;
                        } else {
                ...
                return empty($post_types) ? __('All post types', 'mncf') : implode(', ', $supports);
                break;
            case 'taxonomies':
                global $mn_taxonomies;

                $taxonomies = mncf_admin_get_taxonomies_by_group($item['id']);
                $output = '';
                if (empty($taxonomies)) {
                    $output = __('None', 'mncf');
                } else {
                    foreach ($taxonomies as $taxonomy => $terms) {
                        $output .= isset( $mn_taxonomies[$taxonomy]->labels->singular_name )
                            ? '<em>' . $mn_taxonomies[$taxonomy]->labels->singular_name . '</em>: '
                            : '<em>' . $taxonomy . '</em>: ';
                        $terms_output = array();
    ...
    function column_title($item)
    {
        ...
        if ( MNCF_Roles::user_can_edit('custom-field', $item ) ) {
            //Build row actions
            $actions = array(
                'edit'      => sprintf('<a href="%s">%s</a>', $edit_link, __('Edit', 'mncf')),
                'status' => 'active' == $item['status']? mncf_admin_fields_get_ajax_deactivation_link($item['id']):mncf_admin_fields_get_ajax_activation_link($item['id']),
                'delete'     => sprintf(
                    ...
                    esc_url(
                        add_query_arg(
                            array(
                                'action' => 'mncf_ajax',
                                'mncf_action' => 'delete_group',
                                'group_id' => $item['id'],
                                'mncf_ajax_update' => 'mncf_list_ajax_response_'.$item['id'],
                                '_mnnonce' => mn_create_nonce('delete_group'),
    ...
    function process_bulk_action()
    {
        ...
        if (!empty($action)) {
            ...
            if ( !mn_verify_nonce($nonce, 'bulk-customfieldgroups')) {
                die( 'Security check' );
            }
        }
        ...
        if (
            ...
            foreach( $_POST[$this->bulk_action_field_name] as $key ) {
                 ...
                 * check capability
                 */
                if ( !MNCF_Roles::user_can_edit('custom-field', $items[$key]) ){
                    continue;
                }
                /**
                 * do it!
            ...
            mn_cache_delete(md5('group::_get_group'.TYPES_CUSTOM_FIELD_GROUP_CPT_NAME),'types_cache_groups');
        }
    }
    ...
    function prepare_items()
    {
         ...
         */
        $per_page = $this->get_items_per_page('mncf_cf_per_page', 10);;
        ...
        if ( !empty($groups) ){
            foreach( array_values($groups) as $group ) {
                $one = array(
                    ...
                    'title' => mn_kses_post($group['name']),
                    MNCF_AUTHOR => isset($group[MNCF_AUTHOR])? $group[MNCF_AUTHOR]:0,
                );
                $add_one = true;
                if ( $s ) {
                    $add_one = false;
    ...
    {
        if ( isset($_POST['s']) ) {
            _e('No custom field groups found.','mncf');
            return;
        }
        echo mnautop(__('To use post fields, please create a group to hold them.', 'mncf'));
        printf(
