 ...
 */
class Types_Admin_Usermeta_Groups_List_Table extends MN_List_Table
{
    var $bulk_action_field_name = 'mncf_ids';
    /** ************************************************************************
    ...
    function column_default($item, $column_name)
    {
        switch($column_name){
            ...
            case 'show_for':
                $show_for = mncf_admin_get_groups_showfor_by_group($item['id']);
                if (function_exists('mncf_access_register_caps')){
                    $show_for = __('This groups visibility is also controlled by the Access plugin.',
                        'mncf');
                }
                else{
                    $show_for = (count($show_for) == 0) ? __('Displayed for all users roles', 'mncf') : ucwords(implode($show_for, ', '));
                }
                return $show_for;
            case 'status':
    ...
    function column_title($item)
    {
        ...
        if ( MNCF_Roles::user_can_edit('user-meta-field', $item ) ) {
            ...
            $actions = array(
                ...
                'status' => 'active' == $item['status']? mncf_admin_usermeta_get_ajax_deactivation_link($item['id']):mncf_admin_usermeta_get_ajax_activation_link($item['id']),
                'delete'     => sprintf(
                    ...
                    esc_url(
                        add_query_arg(
                            array(
                                'action' => 'mncf_ajax',
                                'mncf_action' => 'delete_usermeta_group',
                                'group_id' => $item['id'],
                                'mncf_ajax_update' => 'mncf_list_ajax_response_'.$item['id'],
                                '_mnnonce' => mn_create_nonce('delete_usermeta_group'),
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'show_for'    => __('Available for', 'mncf'),
        );
        if ( !MNCF_Roles::user_can_create('user-meta-field') ) {
            unset($columns['cb']);
        }
        return $columns;
    }
...
//                'delete'     => __('Delete', 'mncf'),
    ...
    function process_bulk_action()
    {
        ...
        if (!empty($action)) {
            ...
            if ( !mn_verify_nonce($nonce, 'bulk-userfieldgroups')) {
                die( 'Security check' );
            }
        }
        ...
        if (
            ...
            foreach( $_POST[$this->bulk_action_field_name] as $key ) {
                 ...
                 * check capability
                 */
                if ( !MNCF_Roles::user_can_edit('user-meta-field', $items[$key]) ){
                    continue;
                }
                /**
                 * do it!
            ...
            mn_cache_delete(md5('group::_get_group'.TYPES_USER_META_FIELD_GROUP_CPT_NAME),'types_cache_groups');
        }
    }
    ...
    function prepare_items()
    {
         ...
         */
        $per_page = $this->get_items_per_page('mncf_uf_per_page', 10);;
    ...
    {
        if ( isset($_POST['s']) ) {
            _e('No user field groups found.','mncf');
            return;
        }
        printf(
            '<p>%s</p>',
            __('To use user fields, please create a group to hold them.','mncf')
        );
        printf(
