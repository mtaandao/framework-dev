...
class Types_Admin_Edit_Meta_Fields_Group extends Types_Admin_Edit_Fields
{
    ...
    public function init_admin()
    {
        ...
        $this->boxes = array(
            ...
            'types_where' => array(
                ...
                'title' => __('Where to display this group', 'mncf'),
                'default' => 'side',
            ),
        );
        ...
        $this->boxes = apply_filters('mncf_meta_box_user_meta', $this->boxes, $this->post_type);

        mn_enqueue_script(
            __CLASS__,
            MNCF_RES_RELPATH . '/js/' . 'taxonomy-form.js',
    ...
    public function form()
    {
        ...
        $this->current_user_can_edit = MNCF_Roles::user_can_create('user-meta-field');

        // If it's update, get data
        $this->update = false;
        if (isset($_REQUEST[$this->get_id])) {
            $this->update = mncf_admin_fields_get_group(intval($_REQUEST[$this->get_id]), TYPES_USER_META_FIELD_GROUP_CPT_NAME);
            $this->current_user_can_edit = MNCF_Roles::user_can_edit('user-meta-field', $this->update);
            if (empty($this->update)) {
                $this->update = false;
                mncf_admin_message(sprintf(__("Group with ID %d do not exist", 'mncf'), intval($_REQUEST[$this->get_id])));
            } else {
                $this->update['fields'] = mncf_admin_fields_get_fields_by_group( sanitize_text_field( $_REQUEST[$this->get_id] ), 'slug', false, true, false, TYPES_USER_META_FIELD_GROUP_CPT_NAME, 'mncf-usermeta');
                $this->update['show_for'] = mncf_admin_get_groups_showfor_by_group( sanitize_text_field( $_REQUEST[$this->get_id] ) );
...
var mncfDefaultCss = ' .  json_encode( base64_encode($admin_styles_value) ) . ';
        ...
        $admin_styles = _mncf_filter_wrap( 'admin_styles',
            __( 'Admin styles for fields:', 'mncf' ), '', '', $temp,
            __( 'Open style editor', 'mncf' ) );
    ...
    public function save()
    {
        ...
        if (isset($_REQUEST[$this->get_id])) {
            ...
            if (empty($post) || $post->post_type != TYPES_USER_META_FIELD_GROUP_CPT_NAME) {
                $form->triggerError();
                mncf_admin_message(sprintf(__("Wrong group ID %d", 'mncf'), intval($_REQUEST[$this->get_id])), 'error');
                return $form;
            }
            $group_id = $post->ID;
        ...
        // Rename if needed
        if (isset($_REQUEST[$this->get_id])) {
            $_POST['mncf']['group']['id'] = intval($_REQUEST[$this->get_id]);
        }

        $group_id = mncf_admin_fields_save_group($_POST['mncf']['group'], TYPES_USER_META_FIELD_GROUP_CPT_NAME, 'user');

        // Set open fieldsets
        if ($new_group && !empty($group_id)) {
            $open_fieldsets = get_user_meta(get_current_user_id(), 'mncf-group-form-toggle', true);
            if (isset($open_fieldsets[-1])) {
                $open_fieldsets[$group_id] = $open_fieldsets[-1];
                unset($open_fieldsets[-1]);
                update_user_meta(get_current_user_id(), 'mncf-group-form-toggle', $open_fieldsets);
            }
        }
    ...
    {
        if (empty($post_types)) {
            update_post_meta($group_id, '_mn_types_group_showfor', 'all');
            return true;
        }
        $post_types = ',' . implode(',', (array) $post_types) . ',';
        update_post_meta($group_id, '_mn_types_group_showfor', $post_types);
    }
}
