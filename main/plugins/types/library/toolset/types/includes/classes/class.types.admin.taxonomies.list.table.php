 ...
 */
class Types_Admin_Taxonomies_List_Table extends MN_List_Table
{
    var $custom_taxonomies;
    var $builtin_taxonomies;
    var $bulk_action_field_name = 'mncf_ct_ids';
    ...
    function __construct()
    {
        ...
        $this->custom_taxonomies = get_option(MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array());
        $this->builtin_taxonomies = mncf_get_builtin_in_taxonomies();
    }
            ...
            case 'supports':
                ...
                if (!empty($item[$column_name])) {
                    foreach ($item[$column_name] as $temp_post_type => $true) {
                        $rows[] = stripslashes(mncf_translate($temp_post_type . ' name', $temp_post_type, 'Types-CPT'));
                    }
                }
    ...
    function column_title($item)
    {
        ...
        if ( MNCF_Roles::user_can_edit('custom-taxonomy', $item ) ) {
            $edit_link = esc_url(
                add_query_arg(
                    array(
                        'page' => 'mncf-edit-tax',
        ...
        $actions['status'] = 'active' == $item['status']? mncf_admin_custom_taxonomies_get_ajax_deactivation_link($item['slug']):mncf_admin_custom_taxonomies_get_ajax_activation_link($item['slug']);

        if ( 'custom' == $item['type'] ) {
            //Build row actions
            $actions += array(
                'edit'      => sprintf('<a href="%s">%s</a>', $edit_link, __('Edit', 'mncf')),
                'duplicate'     => sprintf(
                    '<a href="%s" class="mncf-ajax-link">%s</a>',
                    esc_url(
                        add_query_arg(
                            array(
                                'action' => 'mncf_ajax',
                                'mncf_action' => 'taxonomy_duplicate',
                                'mncf-tax' => $item['slug'],
                                'mncf_ajax_update' => 'mncf_list_ajax_response_'.$item['slug'],
                                '_mnnonce' => mn_create_nonce('taxonomy_duplicate'),
                            ),
                ...
                'delete'     => sprintf(
                    ...
                    esc_url(
                        add_query_arg(
                            array(
                                'action' => 'mncf_ajax',
                                'mncf_action' => 'delete_taxonomy',
                                'mncf-tax' => $item['slug'],
                                'mncf_ajax_update' => 'mncf_list_ajax_response_'.$item['slug'],
                                '_mnnonce' => mn_create_nonce('delete_taxonomy'),
    ...
    function process_bulk_action()
    {
        ...
        if (
            ...
            foreach( $_POST[$this->bulk_action_field_name] as $key ) {
                 ...
                 * check capability
                 */
                if ( !MNCF_Roles::user_can_edit('custom-taxonomy', $this->custom_taxonomies[$key]) ){
                    continue;
                }
                /**
                 * do it!
                ...
                case 'delete':
                    ...
                    break;
                case 'deactivate':
                    if ( mncf_is_builtin_taxonomy($key) ) {
                        get_taxonomy($key);
                    }
             ...
             */
            update_option(MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, $this->custom_taxonomies);
            /**
             * update post types
             */
    ...
    function prepare_items()
    {
         ...
         */
        $per_page = $this->get_items_per_page('mncf_ctt_per_page', 10);;
            ...
            foreach( $this->custom_taxonomies as $slug => $taxonomy ) {
                ...
                if( isset( $taxonomy['supports'] ) ) {
                    foreach( $taxonomy['supports'] as $post_type_slug => $value_one  ) {
                        if( isset( $mn_post_types[$post_type_slug]->labels->name ) ) {
                            $supports[$mn_post_types[$post_type_slug]->labels->name] = 1;
                        } else {
                ...
                $one = array(
                    'description' => isset($taxonomy['description'])? mn_kses_post($taxonomy['description']):'',
                    'supports' => $supports,
                    'slug' => $taxonomy['slug'],
                    'status' => (isset($taxonomy['disabled']) && $taxonomy['disabled'])? 'inactive':'active',
                    'title' => isset($taxonomy['labels']['name']) ? mn_kses_post(stripslashes($taxonomy['labels']['name'])) : '',
                    MNCF_AUTHOR => isset($taxonomy[MNCF_AUTHOR])? $taxonomy[MNCF_AUTHOR]:0,
                    'type' => 'custom',
                    '_builtin' => false,
                );
                $add_one = true;
        ...
        foreach( $this->builtin_taxonomies as $taxonomy ) {
            ...
            $one = array(
                'description' => __('This is built-in Mtaandao Taxonomy.', 'mncf'),
        ...
        foreach(array('category', 'post_tag') as $slug ) {
            if ( isset($data[$slug]) ) {
                ...
                $data[$slug]['description'] = __('This is built-in Mtaandao Taxonomy.', 'mncf');
            }
        }
    ...
    {
        if ( isset($_POST['s']) ) {
            _e('No custom taxonomies found.','mncf');
            return;
        }
