...
include_once MNCF_INC_ABSPATH.'/classes/class.mncf.marketing.php';
 ...
 */
class MNCF_Types_Marketing_Messages extends MNCF_Types_Marketing
{
    private $state;
    private $taxonomies;
    ...
    {
        if(!function_exists('MN_Installer')){
            return false;
        }
        $repos = array(
            'toolset'
        ...
        foreach( $repos as $repository_id ) {
            $key = MN_Installer()->repository_has_subscription($repository_id);
            if ( empty($key) ) {
                continue;
            }
            return true;
    ...
    private function replace_placeholders($text)
    {
        ...
        case 'cpt':
            if (
                is_array($_GET)
                && array_key_exists('mncf-post-type', $_GET)
            ) {
                $types = get_option(MNCF_OPTION_NAME_CUSTOM_TYPES, array());
                $candidate_key = sanitize_text_field( $_GET['mncf-post-type'] );
        ...
        case 'taxonomy':
            if (
                is_array($_GET)
                && array_key_exists('mncf-tax', $_GET)
            ) {
                $taxonomies = get_option(MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array());
                $taxonomies = $this->taxonomies->get();

                $candidate_key = sanitize_text_field( $_GET['mncf-tax'] );
    ...
    public function register_scripts()
    {
         ...
         * common question
         */
        $data['message'] = __('Saving your changes', 'mncf');
        $data['spinner'] = apply_filters('mncf_marketing_message', admin_url('/images/spinner.gif'), $data, 'spinner');
        $data['question'] = apply_filters('mncf_marketing_message', __('Did you know?', 'mncf'), $data, 'question');
        /**
         * random image & class
         */
        ...
        $data['image'] = apply_filters('mncf_marketing_message', $src, $data, 'image');
        $data['class'] = apply_filters('mncf_marketing_message', $image, $data, 'class');
        /**
        ...
        foreach ( array('header', 'description') as $key ) {
            ...
            $data[$key] = apply_filters('mncf_marketing_message', $value, $data, $key );
            $data['state'] = $this->state;
        }
        mn_register_script( 'types-modal', MNCF_EMBEDDED_RES_RELPATH.'/js/modal.js', array('toolset-colorbox'), MNCF_VERSION, true);
        mn_localize_script( 'types-modal', 'types_modal', $data);
        mn_enqueue_script('types-modal');
    }
    ...
    public function update_options()
    {
        ...
        if ( !mn_verify_nonce($_POST['marketing'], 'update')) {
            return;
        }
        $this->set_state();
    }
    ...
    public function show_top($update = true)
    {
        ...
        $content .= sprintf('<p class="mncf-notif-header">%s</p>', $update? __('Updated!', 'mncf'):__('Created!', 'mncf') );
        if ( 'endabled' == $this->state) {
            $content .= '<p class="mncf-notif-description">';
            if ( isset($data['link']) ) {
                $content .= sprintf(
                    '<a href="%s">%s</a>',
					esc_url(
