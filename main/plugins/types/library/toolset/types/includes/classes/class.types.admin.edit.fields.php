...
if ( defined( 'DOING_AJAX' ) ) {
    require_once MNCF_ABSPATH.'/embedded/classes/validate.php';
}
...
abstract class Types_Admin_Edit_Fields extends Types_Admin_Page
{
    ...
    public function __construct()
    {
         ...
         * actions
         */
        add_action('mn_ajax_mncf_edit_field_choose', array($this, 'field_choose'));
        add_action('mn_ajax_mncf_edit_field_insert', array($this, 'field_insert'));
        add_action('mn_ajax_mncf_edit_field_select', array($this, 'field_select'));
        add_action('mn_ajax_mncf_edit_field_add_existed', array($this, 'field_add_existed'));
    }
    ...
    {
        $button_text = __( 'Save Field Group', 'mncf' );
    ...
    {
        /**
         * this function replace: mncf_fields_get_field_form_data()
         */
         ...
         * row fir field data
         */
        $table_row_typeproof = '<tr class="js-mncf-fields-typeproof"><td><LABEL></td><td><ERROR><BEFORE><ELEMENT><AFTER></td></tr>';
        ...
        $required = (isset($form_data['data']['validate']['required']['active']) && $form_data['data']['validate']['required']['active'] === "1") ? __('- required','mncf') : '';
        $form_data['id'] = $id;

        // Set title
        $title = !empty( $form_data['name'] ) ? $form_data['name'] : __( 'Untitled', 'mncf' );
        $title = sprintf(
            '<span class="mncf-legend-update">%s</span> <span class="description">(%s)</span> <span class="mncf_required_data">%s</span>',
            $title,
            $field_data['title'],
            $required
        );
	    ...
	    */
        ...
        $form_field['box-open'] = array(
            ...
            '#markup' => sprintf(
                ...
                esc_attr__('Click to toggle', 'mncf'),
                $icon,
                $title
            )
        );
        ...
        $form_field['table-open'] = array(
            '#type' => 'markup',
            '#markup' => '<table class="widefat inside js-mncf-slugize-container">',
        );
        ...
        $form_field['name'] = array(
            ...
            '#name' => 'name',
            '#attributes' => array(
                'class' => 'widefat mncf-forms-set-legend mncf-forms-field-name js-mncf-slugize-source',
                'placeholder' => __( 'Enter field name', 'mncf' ),
                'tooltip' => __('This will be the label for the field in the post editor.', 'mncf'),
            ),
            '#validate' => array('required' => array('value' => true)),
            '#inline' => true,
            '#pattern' => $table_row_typeproof,
            '#title' => __('Field name', 'mncf'),
        );
        ...
        $form_field['slug'] = array(
            ...
            '#name' => 'slug',
            '#attributes' => array(
                'class' => 'widefat mncf-forms-field-slug js-mncf-slugize',
                'maxlength' => 255,
                'placeholder' => __( 'Enter field slug', 'mncf' ),
                'tooltip' => __('This is the machine name of the field.', 'mncf'),
            ),
            '#validate' => array('nospecialchars' => array('value' => true)),
            '#inline' => true,
            '#pattern' => $table_row_typeproof,
            '#title' => __('Field slug', 'mncf'),
        );
        ...
        if ( empty( $options ) ) {
            ...
            $form_field['type'] = array(
                ...
                '#options' => $options,
                '#default_value' => $type,
                '#description' => __('Options for this filed will be available after group save.', 'mncf'),
                '#attributes' => array(
                    'class' => 'js-mncf-fields-type',
                    'data-message-after-change' => esc_attr__('Field options will be available after you save this group.', 'mncf'),
                ),
            );
        }
        $form_field['type']['#title'] = __('Field type', 'mncf');
        ...
        if ( function_exists( 'mncf_fields_' . $type . '_insert_form' ) ) {
            $form_field_temp = call_user_func( 'mncf_fields_' . $type . '_insert_form', $form_data, 'mncf[fields][' . $id . ']' );
        ...
        $form_field['description'] = array(
            ...
            '#attributes' => array(
                'rows' => 5,
                'cols' => 1,
                'placeholder' => __( 'Enter field description', 'mncf' ),
                'class' => 'widefat',
                'tooltip' => __( 'This optional text appears next to the field and helps users understand what this field is for.', 'mncf'),
            ),
            '#inline' => true,
            '#pattern' => $table_row,
        ...
        case 'video':
            $form_field['placeholder'] = array(
                ...
                '#attributes' => array(
                    ...
                    'class' => 'widefat',
                    'tooltip' => __( 'This value is being displayed when the field is empty in the post editor.', 'mncf' ),
                ),
                '#pattern' => preg_replace('/<tr>/', '<tr class="mncf-border-top">', $table_row),
            );
            break;
        }
        ...
        case 'video':
            $form_field['user_default_value'] = array(
                ...
                '#attributes' => array(
                    ...
                    'class' => 'widefat',
                    'tooltip' => __('This is the initial value of the field.', 'mncf'),
            ),
                '#pattern' => $table_row,
            );
            break;
        ...
        switch( $type ) {
            ...
            case 'video':
                $form_field['user_default_value']['#validate'] = array(
                    'url2' => array(
                        'active' => 1,
                        'message' => __( 'Please enter a valid URL address.', 'mncf' )
                    )
                );
                break;
        ...
        /* Add validation box
         */
        $validate_function = sprintf('mncf_fields_%s', $type);
        if ( is_callable($validate_function) ) {
            $form_validate = $this->form_validation(
                'mncf[fields][' . $id . '][validate]',
         ...
         *
         */
        $form_field += $this->mnml($form_data);
	    ...
	    $form_field = apply_filters( 'mncf_form_field', $form_field, $form_data, $type );
        ...
        $form_field['remove-field'] = array(
            '#type' => 'markup',
            '#markup' => sprintf(
                '<a href="#" class="js-mncf-field-remove mncf-field-remove" data-message-confirm="%s"><i class="fa fa-trash"></i> %s</a>',
                esc_attr__( 'Are you sure?', 'mncf' ),
                __('Remove field', 'mncf')
            ),
        ...
        foreach ( $form_field as $k => $field ) {
            ...
            $name = sprintf('mncf-%s[%s]', $id, $k);

            $form[$name] = $field;

            // Check if nested
            if ( isset( $field['#name'] ) && strpos( $field['#name'], '[' ) === false ) {
                $form[$name]['#name'] = 'mncf[fields][' . $id . '][' . $field['#name'] . ']';
            } else if ( isset( $field['#name'] ) && strpos( $field['#name'], 'mncf[fields]' ) === false ) {
                $form[$name]['#name'] = 'mncf[fields][' . $id . ']' . $field['#name'];
    ...
    {
        $clasess[] = 'mncf-fields-add-new';
        $clasess[] = 'js-mncf-fields-add-new';
        return array(
            'fields-button-add-'.rand() => array(
                '#type' => 'button',
                '#value' => '<span class="dashicons dashicons-plus"></span> ' . __('Add New Field', 'mncf'),
                '#attributes' => array(
                    'class' => esc_attr(implode(' ', $clasess)),
                    'data-mncf-dialog-title' => esc_attr__('Add New Field', 'mncf'),
                    'data-mncf-id' => esc_attr($this->ct['id']),
                    'data-mncf-message-loading' => esc_attr__('Please Wait, Loading…', 'mncf'),
                    'data-mncf-nonce' => mn_create_nonce('mncf-edit-'.$this->ct['id']),
	                // This can be mncf-postmeta, mncf-usermeta or mncf-termmeta.
                    'data-mncf-type' => $this->type,
	                'data-mncf-page' => esc_attr( mncf_getget( 'page' ) )
                ),
                '_builtin' => true,
    ...
    private function get_available_types($type = false)
    {
        ...
        if ( $type ) {
            ...
            $allowed = mncf_admin_custom_fields_change_type_allowed_matrix();
            ...
            if ( Wpcf_Validate::canValidate( $method ) && Wpcf_Validate::hasForm( $method ) ) {
                ...
                foreach ( $form_validate as $key => $element ) {
                    if ( isset( $element['#type'] ) ) {
                        $form_validate[$key]['#id'] = $element['#type'] . '-' . mncf_unique_id( serialize( $element ) );
                    }
                    if ( $is_first && isset($element['#pattern'] ) ) {
                        $is_first = false;
                        $form_validate[$key]['#pattern'] = preg_replace(
                            '/<tr>/',
                            '<tr class="mncf-border-top">',
                            $element['#pattern']
                        );
                    }
                }
     ...
     * @return type Description.
     */
    public function mnml($form_data)
    {
        $form = array();
        if (
            true
            ...
            $mnml_prefs = mnml_cf_translation_preferences(
                $form_data['id'],
                $custom_field,
                'mncf',
                false,
            ...
            $form['mncf-' . $form_data['id'].'-mnml-preferences'] = array(
    ...
    protected function add_description($description)
    {
        return array(
            ...
            'description' => array(
                '#type' => 'markup',
                '#markup' => sprintf(
                    '<p class="description js-mncf-description">%s</p>',
                    $description
                ),
            ),
        );
    ...
    public function fields()
    {
        ...
        $form['fields-open'] = array(
            '#type' => 'markup',
            '#markup' => '<div class="js-mncf-fields mncf-fields meta-box-sortables ui-sortable">',
            '_builtin' => true,
        );
        ...
        $form += $this->button_add_new( array( 'js-mncf-fields-add-new-last' . $class_bottom_elements ));
        ...
        $form['submit-bottom-container'] = array(
            '#type' => 'markup',
            '#markup' => '<div class="js-mncf-second-submit-container mncf-second-submit-container' . $class_bottom_elements.'">',
            '_builtin' => true,
        );
        ...
        $form['submit-bottom'] = array(
            '#type' => 'submit',
            '#value' => __('Save Field Group', 'mncf'),
            '#attributes' => array(
                'class' => 'js-mncf-second-submit button-primary mncf-disabled-on-submit',
            ),
            '_builtin' => true,
    ...
    public function field_choose()
    {
        ...
        if (
            ...
            || !isset($_REQUEST['id'])
            || !mn_verify_nonce($_REQUEST['_mnnonce'], 'mncf-edit-'.$_REQUEST['id'])
        ) {
            $this->verification_failed_and_die();
        }
        $form = array();
        $form += $this->add_description(
            __('You can choose from the available fields:', 'mncf')
        );
        // Get field types
        $form['nonce'] = array(
            '#type' => 'hidden',
            '#value' => mn_create_nonce('fields_insert'),
            '#name' => 'mncf-fields-add-nonce',
            '#id' => 'mncf-fields-add-nonce',
        );
        ...
        if( $this->previously_added_fields_available() ) {
            $form['switch-to-exists'] = array(
                ...
                '#type' => 'button',
                '#value' => __('Choose from previously created fields', 'mncf'),
                '#before' => sprintf('<p>%s ', __('or you can', 'mncf')),
                '#after' => '</p>',
                '#attributes' => array(
                    'class' => 'mncf-switch-to-exists js-mncf-switch-to-exists',
                    'data-mncf-id' => esc_attr($_REQUEST['id']),
                    'data-mncf-type' => $this->type,
                ),
            );
        }
    ...
    private function previously_added_fields_available() {
        $fields = mncf_admin_fields_get_fields( true, true, false, $this->type );
    ...
    public function field_select()
    {
        ...
        if (
            ...
            || !isset($_REQUEST['_mnnonce'])
            || !isset($_REQUEST['id'])
            || !mn_verify_nonce($_REQUEST['_mnnonce'], 'fields_insert')
        ) {
            $this->verification_failed_and_die();
        }
        $fields = mncf_admin_fields_get_fields( true, true, false, $this->type );
        ...
        if ( !empty( $fields ) ) {
            $form = array();
            $form += $this->add_description(
                __('You can choose from the previously created fields:', 'mncf')
            );
            ...
            if ( count($fields) > 8 ) {
                $form['Search'] = array(
                    '#type' => 'textfield',
                    '#name' => 'mncf-fields-search',
                    '#attributes' => array(
                        'class' => 'regular-text js-mncf-fields-search',
            ...
            foreach ( $fields as $key => $field ) {
                ...
                $form['fields-existing'.$key] = array(
                    ...
                    '#value' => $value,
                    '#attributes' => array(
                        'class' => 'js-mncf-field-button-use-existed',
                        'data-mncf-field-id' => $key,
                        'data-mncf-field-type' => $field['type'],
                        'data-mncf-type' => $this->type,
                    ),
                );
            }
            $form['switch-to-exists'] = array(
                '#name' => 'switch-to-exists',
                '#type' => 'button',
                '#value' => __('Choose from the default fields', 'mncf'),
                '#before' => sprintf('<p>%s ', __('or you can', 'mncf')),
                '#after' => '</p>',
                '#attributes' => array(
                    'class' => 'mncf-switch-to-new js-mncf-switch-to-new',
                    'data-mncf-id' => esc_attr($_REQUEST['id']),
                    'data-mncf-type' => $this->type,
                ),
            );
    ...
    public function field_insert()
    {
	    ...
	    // passed through the mncf_form_field filter in get_field_from_data().
	    //
	    // This information is required by the Relevanssi integration.
        $field_kind = mncf_getpost( 'field_kind', 'mncf-postmeta', array( 'mncf-postmeta', 'mncf-usermeta', 'mncf-termmeta' ) );
	    $faux_form_data = array(
	    	'meta_type' => substr( $field_kind, 5 ) // get rid of the mncf- prefix.
	    );

	    $enlimbo_form = $this->get_field_form_data( $_REQUEST['type'], $faux_form_data );

        echo mncf_form_simple( $enlimbo_form );
        die;
    }
    ...
    public function field_add_existed()
    {
        ...
        $field = mncf_admin_fields_get_field( sanitize_text_field( $_REQUEST['id'] ), false, true, false, $this->type );
        if ( !empty( $field ) ) {
            echo mncf_form_simple($this->get_field_form_data($field['type'], $field));
        }
        die;
    }
    ...
    protected function get_fields_list($form = array())
    {
        ...
        foreach ( $fields_registered as $filename => $data ) {
            ...
            $form[$filename] = array(
                ...
                '#value' => $value,
                '#attributes' => array(
                    'data-mncf-field-type' => basename( $filename, '.php' ),
                    'class' => 'js-mncf-field-button-insert',
                    'data-mncf-type' => $this->type,
                ),
            );
        }
     ...
     * @return type Description.
     */
    public function get_group_list($form = array(), $type = 'mncf-fields')
    {
        $message = '';
        $groups = array();
        switch( $type ) {
	        case 'mncf-fields':
		        ...
		        $message = __( 'There is no Post Field Group. Please define one first.', 'mncf' );
		        break;
	        case 'mncf-usermeta':
		        $groups = mncf_admin_fields_get_groups( TYPES_USER_META_FIELD_GROUP_CPT_NAME );
		        $message = __( 'There is no User Field Group. Please define one first.', 'mncf' );
		        break;
	        case MNCF_Field_Definition_Factory_Term::FIELD_DEFINITIONS_OPTION:
		        $groups = mncf_admin_fields_get_groups( Types_Field_Group_Term::POST_TYPE );
		        $message = __( 'There is no Term Field Group. Please define one first.', 'mncf' );
		        break;
        }
        ...
        $form['groups-ul-open'] = array(
            '#type' => 'markup',
            '#markup' => '<ul class="mncf-list-of-items js-mncf-list-of-items">',
        );
	...
	public function ajax_filter_dialog() {
		 ...
		 * check nonce
		 */
		if( ! isset( $_REQUEST['id'] ) || ! isset( $_REQUEST['_mnnonce'] ) || ! isset( $_REQUEST['type'] ) || ! mn_verify_nonce( $_REQUEST['_mnnonce'], $_REQUEST['type'] ) ) {
			_e( 'Verification failed.', 'mncf' );
			die;
		}
		...
		// html: open tabs menu
		$html_tabs_menu = '<ul class="mncf-tabs-menu">';

		// css: class for first menu
		$first_menu_active = ' class="mncf-tabs-menu-current"';
