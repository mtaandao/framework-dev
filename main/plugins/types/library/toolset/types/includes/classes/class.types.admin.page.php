...
abstract class Types_Admin_Page
{
    ...
    public function add_meta_boxes()
    {
         ...
         * @param type  $var Description.
         */
        do_action('mncf_closedpostboxes', $screen->base);

        foreach ($this->boxes as $id => $box) {
            $defaults[$box['default']][] = $id;
        }

        $order = mn_parse_args( $user_settings, $defaults );
    ...
    protected function prepare_screen()
    {
        ...
        /**
         * MN control for meta boxes
         */
        include_once ABSPATH.'/admin/includes/meta-boxes.php';
        mn_enqueue_script( 'post' );

        $markup = mn_nonce_field('closedpostboxes', 'closedpostboxesnonce', false, false );
        $markup .= mn_nonce_field('meta-box-order', 'meta-box-order-nonce', false, false );
        return array(
            '#form' => array(),
    ...
    {
        if ( MNCF_Roles::user_can_edit($type, $this->ct) ) {
            ...
            if(
                ( isset( $_GET['group_id'] ) || isset( $_GET['mncf-tax'] ) )
                && isset( $_GET['page'] )
                && ! $built_in
            ) {
                switch( $_GET['page'] ) {
                    case 'mncf-edit':           // post fields
                        $action = 'delete_group';
                        break;
                    case 'mncf-edit-usermeta':  // user fields
                        $action = 'delete_usermeta_group';
                        break;
                    case 'mncf-termmeta-edit':  // term fields
                        $action = 'delete_term_group';
                        break;
                    case 'mncf-edit-tax':       // taxonomy
                ...
                if( isset( $action ) ) {
                    $args = array(
                        'action' => 'mncf_ajax',
                        'mncf_action' => $action,
                        '_mnnonce' => mn_create_nonce( $action ),
                        'mncf_warning' => urlencode(__('Are you sure?', 'mncf'))
                    );

                    if( isset( $_GET['group_id'] ) ) {
                        $args['group_id'] = sanitize_text_field( $_GET['group_id'] );
                        $args['mncf_ajax_update'] = 'mncf_list_ajax_response_'.sanitize_text_field( $_GET['group_id'] );
                        $delete_id_addition = sanitize_text_field( $_GET['group_id'] );
                    } else if( isset( $_GET['mncf-tax'] ) ) {
                        $args['mncf-tax'] = sanitize_text_field( $_GET['mncf-tax'] );
                        $args['mncf_ajax_update'] = 'mncf_list_ajax_response_'.sanitize_text_field( $_GET['mncf-tax'] );
                        $delete_id_addition = sanitize_text_field( $_GET['mncf-tax'] );
                    }

                    $args['mncf_ref'] = isset( $_GET['ref'] )
                        ? $_GET['ref']
                        : 'list';
                    ...
                    $form['delete'] = array(
                        '#type' => 'markup',
                        '#markup' => sprintf(
                            '<div id="delete-action"><a href="%s" class="submitdelete mncf-ajax-link mncf-group-delete-link" id="mncf-list-delete-%d"">%s</a></div>',
                            esc_url(
                                add_query_arg(
                                    $args,
            ...
            $form['submit'] = array(
                '#type' => 'submit',
                '#name' => 'mncf-submit', // do not use only 'submit', because there is JQuery bug which prevents using form.submit() if input submit has name='submit'
                '#value' => $button_text,
                '#attributes' => array(
                    'class' => 'button-primary mncf-disabled-on-submit',
                ),
                '_builtin' => true,
            );
            /**
    ...
    {
        if( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'mncf-edit-tax' ) {
            $this->print_notice( __('These options are not available for built-in taxonomies.', 'mncf'), 'add-mncf-inside');
        } else {
            $this->print_notice( __('These options are not available for built-in post types.', 'mncf'), 'add-mncf-inside');
        }
    }
    ...
    protected function filter_wrap( $type, $data = array(), $button_only = false, $render_closing_hr = true )
    {
        ...
        $form[$unique_id.'-open'] = array(
            '#type' => 'markup',
            '#markup' => sprintf(
                '<div id="%s" class="mncf-filter-container js-mncf-filter-container">',
                esc_attr($unique_id)
            ),
        );
        ...
        if( ! $button_only && isset( $data['description'] ) ) {
            $form[$unique_id.'-description'] = array(
                '#type' => 'markup',
                '#markup' => mnautop($data['description']),
            );
        }
        ...
        if( ! $button_only )
            $form[$unique_id.'-content'] = array(
                '#type' => 'markup',
                '#markup' => sprintf(
                    '<span class="js-mncf-filter-ajax-response">%s</span>',
        ...
        if ( $this->current_user_can_edit ) {
            $form[$unique_id.'-button'] = array(
                ...
                '#value' => __('Edit', 'mncf'),
                '#attributes' => array(
                    'class' => 'js-mncf-filter-button-edit mncf-filter-button-edit',
                    'data-mncf-type' => esc_attr($type),
	                'data-mncf-page' => esc_attr( mncf_getget( 'page' ) ),
                    'data-mncf-nonce' => mn_create_nonce($type),
                ),
                '#inline' => true,
                '#before' => '<div class="mncf-filter-button-edit-container">',
                '#after' => '</div>',
            );
            foreach( $data as $key => $value ) {
                if ( preg_match('/^data\-mncf\-/', $key) ) {
    ...
    {
        $form = array();
        if ( 'add-mncf-inside' == $add_wrap ) {
            $form['wrap-begin'] = array(
                '#type' => 'markup',
                '#markup' => '<div class="mncf-inside">',
            );
        }
        $form['alert'] = array(
            '#type' => 'notice',
