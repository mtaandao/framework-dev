...
function mncf_admin_metabox_end($table_show = true, $_builtin = false)
{
    $markup = '';
    if ( $table_show ) {
...
function mncf_admin_metabox_begin($title, $id = false, $table_id = false, $table_show = true, $_builtin = false )
{
    ...
    $markup = sprintf(
        ...
        $id? sprintf('id="%s"', $id):'',
        __('Click to toggle', 'mncf'),
        $title
    );
    if ( $table_show ) {
        $markup .= sprintf(
            '<table %s class="mncf-types-form-table widefat"><tbody><tr><td>',
...
function mncf_admin_common_metabox_save($ct, $button_text, $type = 'custom-post-type' )
{
    $form = array();
    if ( MNCF_Roles::user_can_edit($type, $ct) ) {
        $form['submit-open'] = mncf_admin_metabox_begin(__( 'Save', 'mncf' ), 'submitdiv', false, false, '_builtin');
        ...
        */
        ...
        $form['submit-close'] = mncf_admin_metabox_end(true, '_builtin');
    }
    return $form;
}

function mncf_admin_common_only_show($form)
{
    foreach( $form as $key => $data ) {
        ...
        case 'checkbox':
        case 'radio':
            $form[$key]['#markup'] = mncf_admin_common_only_show_checkbox_helper($data);
            break;
        ...
        case 'checkboxes':
            ...
            if ( isset($data['#options']) && is_array($data['#options']) ) {
                foreach( $data['#options'] as $option_key => $option_value ) {
                    $markup .= mncf_admin_common_only_show_checkbox_helper($option_value);
                }
            }
        ...
        case 'textarea':
        case 'textfield':
            $form[$key]['#markup'] = mnautop(empty($form[$key]['#value'])? __('[empty]', 'mncf'):stripcslashes($form[$key]['#value']));
            break;
        ...
        case 'fieldset':
            $fieldset_form = array(
                ...
                'description' => array(
                    'value' => $data['description']['#value']? $data['description']['#value']:__('[empty]', 'mncf'),
                    'label' => __('Description', 'mncf'),
                ),
                'repetitive' => array(
                    'value' => isset($data['repetitive']) && $data['repetitive']['#default_value']? __('Allow multiple-instances of this field', 'mncf'):__('This field can have only one value', 'mncf'),
                    'label' => __('Repetitive', 'mncf'),
                ),
            );
...
function mncf_admin_common_only_show_checkbox_helper($data)
{
    return sprintf(
