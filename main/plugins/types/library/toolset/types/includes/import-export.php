...
function mncf_admin_export_form() {
	$form = array();
    $form['types_export_mnnonce'] = array(
        '#type'		=> 'hidden',
        '#name'		=> 'types_export_mnnonce',
        '#value'	=> mn_create_nonce( 'mncf_export' ),
    );
	$form['types_export'] = array(
		'#type'			=> 'submit',
		'#name'			=> 'types_export',
		'#value'		=> __( 'Export', 'mncf' ),
		'#attributes'	=> array('class' => 'button-primary'),
		'#before' 		=> '<p>' 
						. __( 'Download all custom fields, post types and taxonomies created by Types plugin.', 'mncf' ) 
						. '</p>'
						. '<p class="toolset-update-button-wrap">',
		'#after'		=> '</p>'
	);
	return mncf_form_simple( $form );
}

function mncf_admin_import_form() {
	$form = array();
	$form['types_import_mnnonce'] = array(
        '#type'		=> 'hidden',
        '#name'		=> 'types_import_mnnonce',
        '#value'	=> mn_create_nonce( 'mncf_import' ),
    );
    if ( extension_loaded( 'simplexml' ) ) {
		$attributes = !mncf_admin_import_dir() ? array('disabled' => 'disabled') : array();
		$form['intro'] = array(
			'#type'		=> 'markup',
			'#markup'	=> '<p>'
							. __( 'You can import Types data with the following methods:', 'mncf' )
							. '</p>'
		);
		$form['types-import-method'] = array(
			...
			'#options'			=> array(
				'types-import-method-file'	=> array(
												'#title'		=> __( 'XML or ZIP file', 'mncf' ),
												'#value'		=> 'file',
											),
				'types-import-method-text'	=> array(
												'#title'		=> __( 'XML text input', 'mncf' ),
												'#value'		=> 'text',
											)
			),
			'#default_value'	=> 'file',
		...
		$form['types-import-file'] = array(
			...
			'#before'		=> '<div class="toolset-advanced-setting js-types-import-method-extra js-types-import-method-extra-file">'
								. '<p>' . __( 'Upload a .zip or .xml file from your computer:', 'mncf' ) . '</p>',
			'#after'		=> '</div>',
			'#inline'		=> true,
		...
		$form['type-import-before-textarea'] = array(
			...
			'#markup'	=> '<div class="toolset-advanced-setting js-types-import-method-extra js-types-import-method-extra-text hidden">'
							. '<p>' . __( 'Paste your XML here:', 'mncf' ) . '</p>',
		);
		...
		$form['types-import-text-encoding'] = array(
			...
			'#description'	=> __( 'If encoding is set in text input, it will override this setting.', 'mncf' ),
			'#label'		=> __( 'Encoding', 'mncf' ),
		);
		...
		$form['types-import-submit'] = array(
			...
			'#value'		=> __( 'Review and import', 'mncf' ),
		...
		. __( 'PHP SimpleXML extension not loaded: Importing not available', 'mncf' )
		. '</p></div>';
	}
	return mncf_form_simple( $form );
}

function mncf_admin_import_confirmation_form() {
	$form = array();
	if ( isset( $_POST['types-import-step'] ) ) {
		...
		if ( 
			...
			*/
			if ( ! empty( $_FILES['types-import-file']['error'] ) ) {
				echo '<div class="message error"><p>'
					. __( 'Error uploading file', 'mncf' )
					. '</p></div>';
				echo mncf_admin_import_form();
				return;
			}
			$mn_upload_dir = mn_upload_dir();
			$new_file = $mn_upload_dir['basedir'] . '/' . sanitize_file_name($_FILES['types-import-file']['name']);
			$move = move_uploaded_file( $_FILES['types-import-file']['tmp_name'], $new_file );
			if ( ! $move ) {
				echo '<div class="message error"><p>'
					. __( 'Error moving uploaded file', 'mncf' )
					. '</p></div>';
				echo mncf_admin_import_form();
				return;
			}
			...
			if ( $is_zip ) {
				...
				if ( is_resource( $zip ) ) {
					...
					echo '<div class="message error"><p>'
						. __( 'Unable to open zip file', 'mncf' )
						. '</p></div>';
					echo mncf_admin_import_form();
					return;
				}
		...
		if ( empty( $data ) ) {
			echo '<div class="message error"><p>'
				. __( 'Data to import not set or not valid', 'mncf' )
				. '</p></div>';
			echo mncf_admin_import_form();
			return;
		} else {
			$data = mncf_admin_import_export_settings( $data );
			if ( empty( $data ) ) {
				echo '<div class="message error"><p>'
					. __( 'Data not valid', 'mncf' )
					. '</p></div>';
				echo mncf_admin_import_form();
				return;
			} else {
...
function mncf_admin_import_final_data() {
	$return = array();
	if ( 
			...
			if ( file_exists($file) ) {
				...
				if ( $is_zip ) {
					...
					if ( is_resource( $zip ) ) {
						...
						$return[] = array(
							'type'		=> 'error',
							'content'	=> __( 'Unable to open zip file', 'mncf' )
						);
						return $return;
					}
				} else {
				...
				if ( $data ) {
					$return = mncf_admin_import_data( $data, false );
				} else {
					$return[] = array(
						'type'		=> 'error',
						'content'	=> __( 'Unable to process file', 'mncf' )
					);
					return $return;
				}
			} else {
		...
		if ( $_POST['types-import-mode'] == 'text' && !empty( $_POST['types-import-text'] ) ) {
			$charset = !empty( $_POST['types-import-text-encoding'] ) ? sanitize_text_field( $_POST['types-import-text-encoding'] ) : get_option( 'blog_charset' );
			$return = mncf_admin_import_data( stripslashes( html_entity_decode( $_POST['types-import-text'], ENT_QUOTES, $charset ) ), false );
		}
	}
	return $return;
}
 ...
 */
function mncf_admin_import_export_form()
{
    $form = array();
    $form['mnnonce'] = array(
        '#type' => 'hidden',
    ...
    if ( isset( $_POST['_mnnonce'] ) && mn_verify_nonce( $_POST['_mnnonce'], 'mncf_import' ) ) {
        ...
        if ( isset( $_POST['import-final'] ) ) {
            if ( $_POST['mode'] == 'file' && !empty( $_POST['file'] ) ) {
                ...
                if ( file_exists($file) ) {
                    ...
                    if ( $data ) {
                        mncf_admin_import_data( $data );
                    } else {
                        echo '<div class="message error"><p>'
                            . __( 'Unable to process file', 'mncf' )
                            . '</p></div>';
                        return array();
                    }
                } else {
            ...
            if ( $_POST['mode'] == 'text' && !empty( $_POST['text'] ) ) {
                ...
                mncf_admin_import_data( stripslashes( html_entity_decode( $_POST['text'],
                                        ENT_QUOTES, $charset ) ) );
            }
            ...
            if ( !empty( $_POST['import-file'] ) && !empty( $_FILES['file']['tmp_name'] ) ) {
                ...
                $mn_upload_dir = mn_upload_dir();
                $new_file = $mn_upload_dir['basedir'] . '/' . sanitize_file_name($_FILES['file']['name']);
    ...
    if ( $show_first_screen ) {
        ...
        $form['submit'] = array(
            ...
            '#attributes' => array('class' => 'button-primary'),
            '#after' => '<br /><br />',
            '#before' => '<h3>' . __( 'Export Types data', 'mncf' ) . '</h3>'
            . __( 'Download all custom fields, post types and taxonomies created by Types plugin.', 'mncf' ) . '<br /><br />',
        );
        /**
         * check is temp folder available?
         */
        $temp = mncf_get_temporary_directory();
        if ( empty($temp) ) {
            unset($form['submit']);
        }
        if ( extension_loaded( 'simplexml' ) ) {
            ...
            $form['file'] = array(
                '#type' => 'file',
                '#name' => 'file',
                '#prefix' => __( 'Upload XML file', 'mncf' ) . '<br />',
                '#before' => '<h3>' . __( 'Import Types data file', 'mncf' ) . '</h3>',
                '#inline' => true,
                '#attributes' => $attributes,
            );
            ...
            $form['text'] = array(
                '#type' => 'textarea',
                '#title' => __( 'Paste code here', 'mncf' ),
                '#name' => 'text',
                '#attributes' => array('rows' => 20),
                '#before' => '<h3>' . __( 'Import Types data text input', 'mncf' ) . '</h3>',
            );
            $form['text-encoding'] = array(
                '#type' => 'textfield',
                '#title' => __( 'Encoding', 'mncf' ),
                '#name' => 'text-encoding',
                '#value' => get_option( 'blog_charset' ),
                '#description' => __( 'If encoding is set in text input, it will override this setting.', 'mncf' ),
            );
 ...
 */
function mncf_admin_import_export_file_upload_error($file, $error_msg)
{
 ...
 * @return string
 */
function mncf_admin_import_export_settings($data)
{
    global $mndb;
    $form = array();
    $form['title'] = array(
        '#type' => 'markup',
        '#markup' => '<h2>' . __( 'General Settings', 'mncf' ) . '</h2>',
    );
    $form['overwrite-settings'] = array(
        '#type' => 'checkbox',
        '#title' => __( 'Overwrite settings', 'mncf' ),
    ...
    $form['overwrite-or-add-groups'] = array(
        '#type' => 'checkbox',
        '#title' => __( 'Bulk overwrite groups if exist', 'mncf' ),
    ...
    $form['delete-groups'] = array(
        '#type' => 'checkbox',
        '#title' => __( "Delete group if don't exist", 'mncf' ),
        '#name' => 'delete-groups',
        '#inline' => true,
        '#after' => '<br />',
    );
    $form['delete-fields'] = array(
        '#type' => 'checkbox',
        '#title' => __( "Delete field if don't exist", 'mncf' ),
        '#name' => 'delete-fields',
        '#inline' => true,
        '#after' => '<br />',
    );
    $form['delete-types'] = array(
        '#type' => 'checkbox',
        '#title' => __( "Delete Post Type if don't exist", 'mncf' ),
        '#name' => 'delete-types',
        '#inline' => true,
        '#after' => '<br />',
    );
    $form['delete-tax'] = array(
        '#type' => 'checkbox',
        '#title' => __( "Delete Taxonomy if don't exist", 'mncf' ),
        '#name' => 'delete-tax',
        '#inline' => true,
        '#after' => '<br />',
    );
...
//    $data = new SimpleXMLElement($data);
    ...
    if ( !empty( $data->groups ) ) {
        $form['title-1'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'Groups to be added/updated', 'mncf' ) . '</h2>',
        );
        $groups_check = array();
        foreach ( $data->groups->group as $group ) {
            ...
            if ( !empty( $post ) ) {
                $form['group-add-' . $group['ID']]['#after'] = mncf_form_simple(
                        array('group-add-update-' . $group['ID'] => array(
                                '#type' => 'radios',
                                '#name' => 'groups[' . $group['ID'] . '][update]',
                                '#inline' => true,
                                '#options' => array(
                                    __( 'Update', 'mncf' ) => 'update',
                                    __( 'Create new', 'mncf' ) => 'add'
                                ),
        ...
        $groups_existing = get_posts( 'post_type=mn-types-group&post_status=null' );
        if ( !empty( $groups_existing ) ) {
            ...
            if ( !empty( $groups_to_be_deleted ) ) {
                $form['title-groups-deleted'] = array(
                    '#type' => 'markup',
                    '#markup' => '<h2>' . __( 'Groups to be deleted', 'mncf' ) . '</h2>',
                );
    ...
    if ( !empty( $data->fields ) ) {
        $form['title-fields'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'Fields to be added/updated', 'mncf' ) . '</h2>',
        );
        ...
        if ( !empty( $fields_to_be_deleted ) ) {
            $form['title-fields-deleted'] = array(
                '#type' => 'markup',
                '#markup' => '<h2>' . __( 'Fields to be deleted', 'mncf' ) . '</h2>',
            );
    ...
    if ( !empty( $data->user_groups ) ) {
        $form['title-users'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'User Groups to be added/updated', 'mncf' ) . '</h2>',
        );
        $groups_check = array();
        foreach ( $data->user_groups->group as $group ) {
            ...
            if ( !empty( $post ) ) {
                $form['user-group-add-' . $group['ID']]['#after'] = mncf_form_simple(
        ...
        $groups_existing = get_posts( 'post_type=mn-types-user-group&post_status=null' );
        if ( !empty( $groups_existing ) ) {
            $groups_to_be_deleted = array();
            foreach ( $groups_existing as $post ) {
                if ( !in_array( $post->post_title, $groups_check ) ) {
    ...
    if ( !empty( $data->user_fields ) ) {
        $form['user-title-fields'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'User fields to be added/updated', 'mncf' ) . '</h2>',
        );
        $fields_existing = mncf_admin_fields_get_fields( false, false, false, 'mncf-usermeta' );
        $fields_check = array();
        $fields_to_be_deleted = array();
	...
	$term_form_additions = mncf_admin_import_export_settings_for_terms( $data );
    ...
    if ( !empty( $data->types ) ) {
        $form['title-types'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'Post Types to be added/updated', 'mncf' ) . '</h2>',
        );
        ...
        if ( !empty( $types_to_be_deleted ) ) {
            $form['title-types-deleted'] = array(
                '#type' => 'markup',
                '#markup' => '<h2>' . __( 'Post Types to be deleted', 'mncf' ) . '</h2>',
            );
    ...
    if ( !empty( $data->taxonomies ) ) {
        $form['title-tax'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'Custom taxonomies to be added/updated', 'mncf' ) . '</h2>',
        );
        ...
        if ( !empty( $taxonomies_to_be_deleted ) ) {
            $form['title-taxonomies-deleted'] = array(
                '#type' => 'markup',
                '#markup' => '<h2>' . __( 'Custom taxonomies to be deleted', 'mncf' ) . '</h2>',
            );
    ...
    if ( !empty( $data->post_relationships ) ) {
        $form['title-post-relationships'] = array(
            '#type' => 'markup',
            '#markup' => '<h2>' . __( 'Post relationship', 'mncf' ) . '</h2>',
        );
        $form['pr-add'] = array(
            '#type' => 'checkbox',
            '#name' => 'post_relationship',
            '#default_value' => true,
            '#title' => '<strong>' . __( 'Create relationships', 'mncf' ) . '</strong>',
            '#inline' => true,
            '#after' => '<br />',
        );
    }
 ...
 * @since 2.1
 */
function mncf_admin_import_export_settings_for_terms( $data ) {
	...
	if ( !empty( $data->term_groups ) ) {
		...
		$form['title-terms'] = array(
			'#type' => 'markup',
			'#markup' => '<h2>' . __( 'Term field groups to be added or updated', 'mncf' ) . '</h2>',
		);
		...
		foreach ( $data->term_groups->group as $group ) {
			...
			if ( $group_already_exists ) {
				$form[ 'term-group-add-' . $group_id ]['#after'] = mncf_form_simple(
					array(
		...
		if ( !empty( $groups_existing ) ) {
			...
			if ( !empty( $groups_to_be_deleted ) ) {
				$form['title-term-groups-deleted'] = array(
					'#type' => 'markup',
					'#markup' => '<h2>' . __( 'Term groups to be deleted', 'mncf' ) . '</h2>',
				);
	...
	if ( !empty( $data->term_fields ) ) {
		$form['term-title-fields'] = array(
			'#type' => 'markup',
			'#markup' => '<h2>' . __( 'Term fields to be added/updated', 'mncf' ) . '</h2>',
		);
		$fields_existing = mncf_admin_fields_get_fields( false, false, false, MNCF_Field_Definition_Factory_Term::FIELD_DEFINITIONS_OPTION );
		$fields_check = array();
		$fields_to_be_deleted = array();
		...
		if ( !empty( $fields_to_be_deleted ) ) {
			$form['term-title-fields-deleted'] = array(
				'#type' => 'markup',
				'#markup' => '<h2>' . __( 'Term ields to be deleted', 'mncf' ) . '</h2>',
			);
 ...
 * Exports data to XML.
 */
function mncf_admin_export_data($download = true)
{
    /**
     *
     * Since Types 1.2
     ...
     * mncf_admin_export_selected_data( array $items, $_type = 'all', $return = 'download' )
     *
     */
    $return = $download ? 'download' : 'xml';
    return mncf_admin_export_selected_data( array(), 'all', $return );
}
 ...
 * @return type
 */
function mncf_admin_import_dir()
{
    $dir = get_temp_dir();
