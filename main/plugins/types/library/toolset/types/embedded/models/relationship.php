    ...
    public static function getChildrenByPostType( $post, $post_type, $data, $params = array(), $user_query = array() )
    {
        ...
        $query = mn_parse_args(
            ...
            array(
                ...
                'meta_key' => '_mncf_belongs_' . $post->post_type . '_id',
                'meta_value' => $post->ID,
                'suppress_filters' => 0,
            )
        );
        ...
        if ( isset( $params['sort'] ) && isset( $params['field'] ) ) {
             ...
             * Order by title
             */
            if ( $params['field'] == '_mn_title' ) {
                $query['orderby'] = 'title';
                $query = apply_filters( 'mncf_relationship_get_children_query',
                 ...
                 * Order by parents
                 */
            } else if ( $params['field'] == '_mncf_pr_parent' ) {
                $query = apply_filters( 'mncf_relationship_get_children_query',
                        $query, $post, $post_type, $data,
                        esc_attr( $params['field'] ) );
                $items = get_posts( $query );
                if ( !empty( $items ) ) {
                    ...
                    foreach ( $items as $item ) {
                        $meta = mncf_get_post_meta( $item->ID,
                                '_mncf_belongs_'
                ...
                $sorted = $mndb->get_results( $mndb->prepare( $_query, $post_type ) );
                if ( !empty( $sorted ) ) {
                ...
                $field = mncf_admin_fields_get_field( str_replace( 'mncf-', '',
                                $params['field'] ) );
                if ( !empty( $field ) ) {
                ...
                if ( !empty( $items ) ) {
                    ...
                    foreach ( $items as $item ) {
                        $meta = mncf_get_post_meta( $item->ID,
                                'mncf-' . $field['slug'], true );
                        if ( empty( $meta ) ) {
                            $additional[] = $item;
                            continue;
                        }
                        $check = mncf_cd_post_edit_field_filter( array(),
                                $field, $item, 'post-relationship-sort' );
                        if ( isset( $check['__mncf_cd_status'] )
                                && $check['__mncf_cd_status'] == 'failed' ) {
                            $additional[] = $item;
                            continue;
                        }
            ...
            $query = apply_filters( 'mncf_relationship_get_children_query', $query, $post, $post_type, $data );
            $items = get_posts( $query );
        }
        return $items;
    }
