 ...
 * 
 * Use MNCF_RUNNING_EMBEDDED or mncf_is_embedded() to check if
...
define('MNCF_OPTION_NAME_CUSTOM_TYPES', 'mncf-custom-types');
define('MNCF_OPTION_NAME_CUSTOM_TAXONOMIES', 'mncf-custom-taxonomies');
 ...
 */
if ( defined( 'MNCF_ABSPATH' ) ) {
    if ( dirname( dirname( __FILE__ ) ) == MNCF_ABSPATH ) {
        // Mark plugin file
...
if ( defined( 'MNCF_RUNNING_EMBEDDED' ) ) {
    return;
}
 ...
 */
add_action( 'after_setup_theme', 'mncf_bootstrap', 9 );

// Checks if bootstrapping function already loaded
if ( !function_exists( 'mncf_bootstrap' ) ) {

    /**
     * Checks running mode (embedded or not) and resolves conflict
     * between embedded versions.
     * 
     * MNCF_RUNNING_EMBEDDED is defined if embedded code is already in use
    ...
    function mncf_bootstrap() {
        ...
        if ( defined( 'TYPES_LOAD_EMBEDDED' ) ) {
            ...
            if ( !defined( 'MNCF_VERSION' ) ) {
                 ...
                 */
            } else if ( defined( 'MNCF_VERSION' ) ) {
                // Check if plugin path
                $in_plugin = dirname( dirname( __FILE__ ) ) == MNCF_ABSPATH;
                /*
     ...
     * @param type $file
     */
    function mncf_load_instance( $file ) {
