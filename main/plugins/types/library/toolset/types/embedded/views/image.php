...
class Types_Image_View
{
    ...
    private static function __resizeImg( $img, $args = array() ) {
        if ( is_mn_error( $check = self::$__utils->checkEditRequirements( $img ) ) ) {
            return $check;
        }
        if ( is_mn_error( $imgData = self::$__utils->getImg( $img ) ) ) {
            return $imgData;
        }
        if ( is_mn_error( $path = self::$__utils->getWritablePath( $img ) ) ) {
            return $path;
        }
        $args = mn_parse_args( $args,
                array(
        ...
        if ( $args['width'] === null && $args['height'] === null ) {
            return new MN_Error( __CLASS__ . '::' . __METHOD__,
        ...
        $croppedImg = "{$path}{$basename}-mncf_{$suffix}.{$imgData->pathinfo['extension']}";
        if ( !$args['clear_cache'] ) {
            if ( !is_mn_error( $cropped = self::$__utils->getImg( $croppedImg ) ) ) {
                return $cropped;
            }
        }
        /*
        ...
        switch ( $args['resize'] ) {
            case 'stretch':
                $dst_w = $__dst_w;
                $dst_h = $__dst_h;
                $new_image = mn_imagecreatetruecolor( $__dst_w, $__dst_h );
                break;

            case 'crop':
            default:
                list($dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h) = $dims;
                $new_image = mn_imagecreatetruecolor( $dst_w, $dst_h );
                break;
        ...
        try {
            ...
            imagedestroy( $new_image );
            return new MN_Error( __CLASS__ . '::' . __METHOD__,
                    __( 'Resize path invalid', 'mncf' ), $croppedImg );
        }
...
class Types_Image_Utils
{
    ...
    public static function checkEditRequirements( $img ) {
        ...
        // Check writable path
        if ( is_mn_error( $path = self::getWritablePath( $img ) ) ) {
    ...
    public static function getWritablePath( $img = null ) {
        if ( !is_null( $img ) ) {
            ...
            if ( !is_writable( $dir ) || !is_dir( $dir ) ) {
                return self::$__cache->setCache( "writable_path_$img",
                                new MN_Error( __CLASS__ . '::' . __METHOD__,
        ...
        if ( !mn_mkdir_p( $dir ) || !is_writable( $dir ) || !is_dir( $dir ) ) {
            return self::$__cache->setCache( 'temp_writable_path',
                            new MN_Error( __CLASS__ . '::' . __METHOD__,
    ...
    public static function getImgObject( $img, $width, $height, $type, $mime ) {
        ...
        if ( self::inUploadAbsPath( $img ) ) {
            ...
            MNCF_Loader::loadClass( 'path' );
            $url = MNCF_Path::getFileUrl( $img, false ) . '/' . basename( $img );
        }
        $data = array(
            'width' => $width,
            'height' => $height,
    ...
    public static function loadImg( $img ) {
        ...
        @ini_set( 'memory_limit',
                        apply_filters( 'image_memory_limit', MN_MAX_MEMORY_LIMIT ) );
    ...
    public static function uploadInfo() {
        ...
        if ( $upload_info === null ) {
            ...
            if ( $parse_url ) {
                ...
                $baseurlpath = 'mn-content/uploads';
            }
    ...
    public static function getAbsPath( $imgUrl ) {
        ...
        $parsed_mn = parse_url( get_site_url() );
        if ( $upload_dir ) {
            ...
            if ( (!is_multisite() || is_main_site()) && strpos( $parsed['path'],
                ...
                if ( !empty( $parsed_mn['path'] ) ) {
                    $abspath = dirname( str_replace( $parsed_mn['path'] . '/',
                                    ABSPATH, $parsed['path'] ) );
                } else {
    ...
    public static function onDomain( $imgUrl )
    {
        ...
        if ( preg_match( '/(?P<domain>[a-z0-9][a-z0-9\-]{1,63}\.[a-z\.]{2,6})$/i', $parsed_mn['host'], $regs ) ) {
            $parsed_mn['domain'] = $regs['domain'];
        } else {
            $parsed_mn['domain'] = $parsed_mn['host'];
        } 
        return $parsed['domain'] == $parsed_mn['domain'];
    }
 ...
 * @since the year 2013
 */
class Types_Error extends MN_Error
{
    ...
    public function addError( $message, $data = array() ) {
        ...
        if ( $message instanceof MN_Error ) {
