 ...
 * @return type
 */
function mncf_fields_skype() {

    return array(
        'id' => 'mncf-skype',
        'title' => __( 'Skype', 'mncf' ),
        'description' => __( 'Skype', 'mncf' ),
        'validate' => array(
            'required' => array(
...
add_filter( 'mncf_pr_fields_type_skype_value_save',
        'mncf_pr_fields_type_skype_value_save_filter', 10, 3 );
add_filter( 'mncf_repetitive_field', 'mncf_field_skype_repetitive', 10, 4 );

// Add filter when using mnv_condition()
add_filter( 'mnv_condition', 'mncf_fields_skype_mnv_conditional_trigger' );
add_filter( 'mnv_condition_end', 'mncf_fields_skype_mnv_conditional_trigger_end' );
 ...
 * @param type $field
 */
function mncf_fields_skype_meta_box_form( $field ) {
    add_thickbox();
    if ( isset( $field['value'] ) ) {
        $field['value'] = maybe_unserialize( $field['value'] );
    }
    $form = array();
    add_filter( 'mncf_fields_shortcode_slug_' . $field['slug'], 'mncf_fields_skype_shortcode_filter', 10, 2 );
    $rand = mncf_unique_id( serialize( $field ) );
    $form['skypename'] = array(
        '#type' => 'textfield',
        '#value' => isset( $field['value']['skypename'] ) ? $field['value']['skypename'] : '',
        '#name' => 'mncf[' . $field['slug'] . '][skypename]',
        '#id' => 'mncf-fields-skype-' . $field['slug'] . '-' . $rand . '-skypename',
        '#inline' => true,
        '#suffix' => '&nbsp;' . __( 'Skype name', 'mncf' ),
        '#description' => '',
        '#prefix' => !empty( $field['description'] ) ? mncf_translate( 'field ' . $field['id'] . ' description',
                        $field['description'] )
                . '<br /><br />' : '',
        '#attributes' => array('style' => 'width:60%;'),
        '#_validate_this' => true,
        '#before' => '<div class="mncf-skype">',
    );

    $form['style'] = array(
        '#type' => 'hidden',
        '#value' => isset( $field['value']['style'] ) ? $field['value']['style'] : 'btn2',
        '#name' => 'mncf[' . $field['slug'] . '][style]',
        '#id' => 'mncf-fields-skype-' . $field['slug'] . '-' . $rand . '-style',
    );
    ...
    $preview = mncf_fields_skype_get_button_image( $preview_skypename,
            $preview_style );

    // Set button
    // TODO MNML move
    if ( isset( $field['disable'] ) || mncf_mnml_field_is_copied( $field ) ) {
        $edit_button = '';
    } else {
        $edit_button = ''
                . '<a href="'
                . admin_url( 'admin-ajax.php?action=mncf_ajax&amp;'
                        . 'mncf_action=insert_skype_button&amp;_mnnonce='
                        . mn_create_nonce( 'insert_skype_button' )
                        . '&amp;update=mncf-fields-skype-'
                        . $field['slug'] . '-' . $rand . '&amp;skypename=' . $preview_skypename
                        . '&amp;button_style=' . $preview_style
                        . '&amp;keepThis=true&amp;TB_iframe=true&amp;width=500&amp;height=500' )
                . '"'
                . ' class="thickbox mncf-fields-skype button-secondary"'
                . ' title="' . __( 'Edit Skype button', 'mncf' ) . '"'
                . '>'
                . __( 'Edit Skype button', 'mncf' ) . '</a>';
    }

    $form['markup'] = array(
        '#type' => 'markup',
        '#markup' => '<div class="mncf-form-item">'
        . '<div id="mncf-fields-skype-'
 ...
 */
function mncf_fields_skype_editor_callback( $field, $settings, $meta_type, $post ) {
    // Get saved button style if any
    if ( $meta_type == 'usermeta' ) {
        global $current_user;
        $_field = new MNCF_Usermeta_Field;
        $_field->set( $current_user->ID, $field );
    } else {
        $_field = new MNCF_Field;
    ...
    return array(
        ...
        'tabs' => array(
            'display' => array(
                ...
                'content' => MNCF_Loader::template( 'skype-select-button',
                        $settings ),
            ),
        ),
    );
 ...
 * Editor submit.
 */
function mncf_fields_skype_editor_submit( $data, $field, $context ) {
    $add = '';
 ...
 * @return type
 */
function mncf_fields_skype_shortcode_filter( $shortcode, $field ) {
    return $shortcode;
    $add = '';
 ...
 */
function mncf_fields_skype_meta_box_submit() {
    $update = esc_attr( $_GET['update'] );
    $preview = mncf_fields_skype_get_button_image( esc_attr( $_POST['skypename'] ),
 ...
 */
function mncf_fields_skype_meta_box_ajax() {
    if ( isset( $_POST['_mnnonce_mncf_form'] ) && mn_verify_nonce( $_POST['_mnnonce_mncf_form'],
                    'mncf-form' ) ) {
        add_action( 'admin_head_mncf_ajax', 'mncf_fields_skype_meta_box_submit' );
    }
    mn_enqueue_script( 'jquery' );
    mncf_admin_ajax_head( __( 'Insert skype button', 'mncf' ) );

    ?>
    <form method="post" action="">
        <h2><?php
    _e( 'Enter your Skype Name', 'mncf' );
    ...
    ?></h2>
        ...
        <?php
        echo MNCF_Loader::template( 'skype-select-button', $_GET );

        ?>
        <?php
        mn_nonce_field( 'mncf-form', '_mnnonce_mncf_form' );

        ?>
        <br /><br />
        <input type="submit" class="button-primary" value="<?php
    _e( 'Insert skype button', 'mncf' );
 ...
 * @return type
 */
function mncf_fields_skype_get_button( $skypename, $template = '',
        $class = false ) {
 ...
 * @return type
 */
function mncf_fields_skype_get_button_image( $skypename = '', $template = '' ) {
 ...
 *
 */
function mncf_fields_skype_view( $params )
{
    ...
    $content = mncf_fields_skype_get_button( $params['field_value']['skypename'], $button_style, $class );
    return $content;
}
 ...
 * @return type
 */
function mncf_pr_fields_type_skype_value_save_filter( $data, $meta_key = null,
        $post_id = null ) {
 ...
 * @return string
 */
function mncf_field_skype_repetitive( $element, $post, $field, $array_key ) {
    ...
    switch ( $array_key ) {
        case 'skypename':
            ...
            if ( mncf_mnml_field_is_copied( $field ) ) {
                $element['#after'] .= '<input type="hidden" name="mncf_repetitive_copy['
                        . $field['id'] . '][' . $mncf->repeater->index
                        . ']" value="1" />';
            }
            ...
            if ( defined( 'DOING_AJAX' ) ) {
                $field['value'] = '__mncf_repetitive_new_field';
                $element['#value'] = '';
            }
            break;
 ...
 * @return type
 */
function mncf_fields_skype_mnv_conditional_trigger( $post ) {
    add_filter( 'get_post_metadata',
            'mncf_fields_skype_conditional_filter_post_meta', 10, 4 );
}
 ...
 * @return type
 */
function mncf_fields_skype_conditional_filter_post_meta( $null, $object_id,
        $meta_key, $single ) {
 ...
 * @return type
 */
function mncf_fields_skype_mnv_conditional_trigger_end( $post ) {
