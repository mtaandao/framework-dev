 ...
 * @return type 
 */
function mncf_fields_date() {
    $settings = array(
        'id' => 'mncf-date',
        'title' => __( 'Date', 'mncf' ),
        'description' => __( 'Date', 'mncf' ),
        'validate' => array(
            'required' => array(
    ...
    if ( !defined( 'MNTOOLSET_FORMS_ABSPATH' ) ) {
        ...
        if ( in_array( $date_format, $localized_date_formats ) ) {
            ...
            $localized_js = array(
                'src' => file_exists( MNCF_EMBEDDED_RES_ABSPATH . '/js/i18n/jquery.ui.datepicker-'
                        . $locale . '.js' ) ? MNCF_EMBEDDED_RES_RELPATH . '/js/i18n/jquery.ui.datepicker-'
                        . $locale . '.js' : '',
                'deps' => array('jquery-ui-core'),
            );
        }
        $settings['meta_box_js'] = array(
            'mncf-jquery-fields-date' => array(
                'src' => MNCF_EMBEDDED_RES_RELPATH . '/js/jquery.ui.datepicker.min.js',
                'deps' => array('jquery-ui-core'),
            ),
            'mncf-jquery-fields-date-inline' => array(
                'inline' => 'mncf_fields_date_meta_box_js_inline',
            ),
            'mncf-jquery-fields-date-localization' => $localized_js,
        );
        $settings['meta_box_css'] = array(
            'mncf-jquery-ui' => array(
                'src' => MNCF_EMBEDDED_RES_RELPATH
...
// Set date formats
require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields/date/date-formats.php';

// Include helper functions
require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields/date/functions.php';

// Include calendar
require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields/date/calendar.php';

// Include JS
require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields/date/js.php';
 ...
 * 
 * Called from MNCF_Field:: _get_meta()
 * 
 * Returns array(
 *  'timestamp' => 14435346,
 ...
 *  'minute' => 9
 */
add_filter( 'mncf_fields_type_date_value_get',
        'mncf_fields_date_value_get_filter', 10, 4 );

/*
 * 
 * Used to convert submitted data (array) to timestamp before saving field.
 * Called from MNCF_Field::_filter_save_value()
 * 
 * Returns timestamp
 */
add_filter( 'mncf_fields_type_date_value_save',
        'mncf_fields_date_value_save_filter', 10, 3 );
 ...
 * Returns timestamp
 */
add_filter( 'mncf_conditional_display_compare_condition_value',
        'mncf_fields_date_conditional_condition_filter', 10, 5 );
/*
 * This only applied when Checking in AJAX call
 */
if ( defined( 'DOING_AJAX' ) ) {
    add_filter( 'mncf_conditional_display_compare_meta_value',
            'mncf_fields_date_conditional_value_filter', 10, 5 );
}
...
//        'mncf_fields_custom_conditional_statement_hook' );
/*
 * 1.3 Changed to use 'mnv_condition' hook.
 */
add_action( 'mnv_condition', 'mncf_fields_custom_conditional_statement_hook' );
 ...
 */
function mncf_fields_date_meta_box_form( $field, $field_object = null ) {
     ...
     */
    $value = $field['value'] = mncf_fields_date_value_get_filter( $field['value'],
            $field_object );

    // TODO MNML Set disable_in_form or similar to true, use hook for MNML
    if ( mncf_mnml_field_is_copied( $field ) ) {
        $attributes = array('style' => 'width:150px;');
    } else {
        $attributes = array('class' => 'mncf-datepicker', 'style' => 'width:150px;');
    }
     ...
     */
    $js_trigger = defined( 'DOING_AJAX' ) ? '<script type="text/javascript">mncfFieldsDateInit(\'\');</script>' : '';
     ...
     * Set Form
     */
    $unique_id = mncf_unique_id( serialize( $field ) );
    $form = array();
    $form[$unique_id . '-datepicker'] = array(
        '#type' => 'textfield',
        '#title' => '&nbsp;' . $field['name'],
        '#attributes' => $attributes,
        '#name' => 'mncf[' . $field['slug'] . '][datepicker]',
        '#id' => 'mncf-date-' . $field['slug'] . '-datepicker-' . $unique_id,
...
//        '#inline' => true,
    ...
    if ( !fields_date_timestamp_neg_supported() ) {
        ...
        $form[$unique_id . '-warning'] = array(
            '#type' => 'markup',
            '#markup' => '<div class="mncf-form-error"' . $_visible
            . '><p>' . __( 'Please enter a date after 1 January 1970', 'mncf' )
            . '</p></div>',
        );
    }
    ...
    if ( !empty( $field['data']['date_and_time'] )
        ...
        $form[$unique_id . 'time_hour'] = array(
            ...
            '#name' => 'mncf[' . $field['slug'] . '][hour]',
            '#id' => 'mncf-date-' . $field['slug'] . '-select-hour-'
            . $unique_id,
            '#inline' => true,
        );
        ...
        $form[$unique_id . 'time_minute'] = array(
            '#type' => 'select',
            '#title' => __( 'Minute', 'mncf' ),
            '#after' => '<br /><br />',
            '#inline' => true,
            '#options' => $options,
            '#default_value' => $value['minute'],
            '#name' => 'mncf[' . $field['slug'] . '][minute]',
            '#id' => 'mncf-date-' . $field['slug'] . '-minute-'
            . $unique_id,
        );
    }
 ...
 */
function mncf_fields_date_value_get_filter( $value, $field, $return = 'array',
    ...
    if ( $context != 'check_leftover' ) {
        $value = __mncf_fields_date_check_leftover( $value, $field );
    }
    ...
    if ( empty( $value ) ) {
        ...
        if (
            ...
            //https://mn-types.com/forums/topic/pre-1971-dates-throw-error/#post-288989
        ...
        $value = mncf_fields_date_value_check( $value );
    } else {
        $value = array(
            'timestamp' => $value,
 ...
 * @param type $params 
 */
function mncf_fields_date_view( $params ) {
    ...
    $params = mn_parse_args( $params, $defaults );
    $output = '';

    // Make sure value is right
    $__timestamp = mncf_fields_date_value_get_filter( $params['field_value'],
    ...
    switch ( $params['style'] ) {
        case 'calendar':
            $output .= mncf_fields_date_get_calendar( $params, true, false );
            break;
 ...
 * TinyMCE editor form.
 */
function mncf_fields_date_editor_callback( $field, $settings ) {
    ...
    return array(
        ...
        'tabs' => array(
            'display' => array(
                ...
                'content' => MNCF_Loader::template( 'editor-modal-date', $data ),
            ),
        )
    );
}
 ...
 * @return type 
 */
function mncf_fields_date_editor_submit( $data, $field, $context ) {
