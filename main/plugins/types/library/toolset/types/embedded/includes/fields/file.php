 ...
 * @return type
 */
function mncf_fields_file() {
    return array(
        'id' => 'mncf-file',
        'title' => __( 'File', 'mncf' ),
        'description' => __( 'File', 'mncf' ),
        'validate' => array(
            'required' => array(
 ...
 * @param type $field
 */
function mncf_fields_file_meta_box_form( $field ) {
    add_thickbox();
    $button_text = sprintf( __( 'Select %s', 'mncf' ), $field['type'] );
    // Set ID
    $element_id = 'mncf-fields-' . mncf_unique_id( serialize( func_get_args() ) );
    $attachment_id = false;
    ...
    if ( !mncf_mnml_field_is_copied( $field ) ) {
        if ( !empty( $attachment_id ) ) {
            $preview = mn_get_attachment_image( $attachment_id, 'thumbnail' );
        } else {
    ...
    $form = array(
        ...
        . ' class="mncf-fields-file-preview">' . $preview . '</div>',
        '#attributes' => array('class' => 'mncf-fields-file-textfield'),
    );
 ...
 * @return type
 */
function mncf_fields_file_attachment_fields_to_edit_filter( $form_fields, $post ) {
    // Reset form
    $form_fields = array();
    $type = (strpos( $post->post_mime_type, 'image/' ) !== false) ? 'image' : 'file';
    /*
     * Since Types 1.3.2 we use mn_get_attachment_url()
     * See
     * /mn-includes/post.php
     ...
     */
//    $url = $post->guid;
    $url = mn_get_attachment_url( $post->ID );
    $form_fields['mncf_fields_file'] = array(
        'label' => __( 'Types', 'mncf' ),
        'input' => 'html',
        'html' => '<a href="#" title="' . $url
        . '" class="mncf-fields-file-insert-button'
        . ' button-primary" onclick="mncfFieldsFileMediaTrigger(\''
        . $url . '\', \'' . $type . '\')">'
        . __( 'Use as field value', 'mncf' ) . '</a><br /><br />',
    );
    return $form_fields;
}
 ...
 * @param type $params
 */
function mncf_fields_file_view( $params ) {
    $output = '';
 ...
 *
 */
function mncf_fields_file_editor_callback( $field, $data, $meta_type, $post ) {
    ...
    return array(
        ...
        'tabs' => array(
            'display' => array(
                ...
                'content' => MNCF_Loader::template( 'editor-modal-file', $data ),
            )
        ),
        'settings' => $data,
    );
 ...
 */
function mncf_fields_file_editor_submit( $data, $field, $context ) {
    $add = '';
    $add .= ' link="true"';
    if ( !empty( $data['title'] ) ) {
 ...
 * @return type
 */
function mncf_fields_file_media_upload_tabs_filter( $tabs ) {
    unset( $tabs['type_url'] );
    return $tabs;
}
