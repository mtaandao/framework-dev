 ...
 * @return type
 */
function mncf_fields_checkbox()
{
    return array(
        'id' => 'mncf-checkbox',
        'title' => __( 'Checkbox', 'mncf' ),
        'description' => __( 'Checkbox', 'mncf' ),
        'validate' => array(
            'required' => array(
...
add_action( 'save_post', 'mncf_fields_checkbox_save_check', 15, 1 );
add_action( 'edit_attachment', 'mncf_fields_checkbox_save_check', 15, 1 );
 ...
 * @deprecated seems
 */
function mncf_fields_checkbox_meta_box_form($field, $field_object)
{
    global $mncf;
    $checked = false;
    ...
    return array(
        ...
        '#after' => '<input type="hidden" name="_mncf_check_checkbox['
 ...
 */
function mncf_fields_checkbox_editor_callback($field, $settings)
{
    $value_not_selected = '';
    $value_selected = '';
    ...
    $data = array_merge( array(
        'selected' => MNCF_Editor::sanitizeParams( $value_selected ),
        'not_selected' => MNCF_Editor::sanitizeParams( $value_not_selected ),
            ), $settings );
    ...
    return array(
        ...
        'tabs' => array(
            'display' => array(
                ...
                'content' => MNCF_Loader::template( 'editor-modal-checkbox',
                        $data ),
            )
        ),
    );
 ...
 */
function mncf_fields_checkbox_editor_submit($data, $field, $context)
{
    $add = '';
    if ( $context == 'usermeta' ) {
...
function mncf_fields_checkbox_view($params)
{
    ...
    if (
        ...
        $field = mncf_fields_get_field_by_slug( $params['field']['slug'], $option_name );

        if( is_array( $field ) && isset( $field['id'] ) ) {
            $output = mncf_translate( 'field ' . $field['id'] . ' checkbox value', $output );
        }
        ...
        if ( !empty( $params['field']['data']['display_value_selected'] ) ) {
            ...
            $output = mncf_translate( 'field ' . $params['field']['id'] . ' checkbox value selected', $output );
        }
    } elseif ( $params['field']['data']['display'] == 'value'
        ...
        $output = mncf_translate( 'field ' . $params['field']['id'] . ' checkbox value not selected', $output );
    } else {
        return '__mncf_skip_empty';
    }
 ...
 * @param type $post_id
 */
function mncf_fields_checkbox_save_check($post_id)
{
    $meta_to_unset = array();
    ...
    $mode = 'save_main';
    if ( defined( 'DOING_AJAX' ) && isset( $_GET['mncf_action']) ) {
        switch ( $_GET['mncf_action']) {
        case 'pr_save_all':
            $mode = 'save_all';
            break;
    ...
    switch( $mode ) {
    case 'save_main':
        if( isset($_POST['_mntoolset_checkbox']) ){
            foreach ( array_keys( $_POST['_mntoolset_checkbox'] ) as $slug ) {
                if ( array_key_exists( 'mncf', $_POST ) ) {
                    mncf_fields_checkbox_update_one( $post_id, $slug, $_POST['mncf'] );
                } else {
                    $slug_without_form = preg_replace( '/cred_form_\d+_\d+_/', '', $slug);
                    mncf_fields_checkbox_update_one( $post_id, $slug_without_form, $_POST );
                }
            }
        }
        return;
    case 'save_child':
    case 'save_all':
        if ( !array_key_exists('_mntoolset_checkbox', $_POST) ) {
            break;
        }
        foreach(array_keys($_POST['mncf_post_relationship']) as $post_id) {
            /**
             ...
             */
            if ( !array_key_exists($post_id, $_POST['mncf_post_relationship']) ) {
                continue;
            }
            /**
             * stop if array is empty
             */
            if (!count($_POST['mncf_post_relationship'][$post_id])) {
                continue;
            }
            /**
             * prepare children id
             */
            $children = array();
            foreach(array_keys($_POST['mncf_post_relationship'][$post_id]) as $child_id) {
                $children[] = $child_id;
            }
            $re = sprintf('/\-(%s)$/', implode('|', $children));
            $checkboxes = array();
            foreach(array_keys($_POST['_mntoolset_checkbox']) as $key) {
                $checkboxes[] = preg_replace($re, '', $key);
            }
            foreach( $children as $child_id ) {
                foreach( array_unique($checkboxes) as $slug ) {
                    mncf_fields_checkbox_update_one($child_id, $slug, $_POST['mncf_post_relationship'][$post_id][$child_id]);
                }
            }
        }
        break;
    ...
    if ( isset( $_POST['_mncf_check_checkbox'] ) ) {

        // Loop and search in $_POST
        foreach ( $_POST['_mncf_check_checkbox'] as $child_id => $slugs ) {
            foreach ( $slugs as $slug => $true ) {
                ...
                if ( $mode == 'save_main'
                        && intval( $child_id ) == mncf_get_post_id() ) {
                    if ( !isset( $_POST['mncf'][$cf->cf['slug']] ) ) {
                ...
                if ( $mode == 'save_main' ) {
                    ...
                    continue;
                } elseif ( !empty( $_POST['mncf_post_relationship'] ) ) {
                    foreach ( $_POST['mncf_post_relationship'] as $_parent => $_children ) {
...
function mncf_fields_checkbox_update_one($post_id, $slug, $array_to_check)
{
    $cf = new MNCF_Field();
