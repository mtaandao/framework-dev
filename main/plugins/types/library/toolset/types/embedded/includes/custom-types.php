 ...
 *
 */
add_action( 'mncf_type', 'mncf_filter_type', 10, 2 );
 ...
 * @return array
 */
function mncf_custom_types_default() {
    return array(
        'labels' => array(
            'name' => '',
            'singular_name' => '',
 ...
 * Inits custom types.
 */
function mncf_custom_types_init() {
    ...
    if ( !empty( $custom_types ) ) {
        foreach ( $custom_types as $post_type => $data ) {
            ...
            if (
                ...
                || mncf_is_builtin_post_types($post_type)
            ) {
                continue;
            }
            mncf_custom_types_register( $post_type, $data );
        }
    }
    ...
    /** This filter is documented in admin/admin/edit-form-advanced.php */
    add_filter('enter_title_here', 'mncf_filter_enter_title_here', 10, 2);
}
	...
	if ( !empty( $custom_types ) ) {
		foreach( $custom_types as $post_type => $data ) {
			if( empty( $data )
				...
				|| strpos( $data['menu_position'],'--mncf-add-menu-after--' ) === false )
				continue;

			// at this point we have not only an integer as menu position
			$target_url = explode( '--mncf-add-menu-after--', $data['menu_position'] );
...
function types_menu_order_item_sort( &$menu, $current_url, $target_url ) {
    ...
    if( $current_index && $add_menu_after_index )
        mncf_custom_types_menu_order_move( $menu, $current_index, $add_menu_after_index );return $menu;
}
 ...
 */
function mncf_custom_types_menu_order_move( &$menu, $item_move, $item_target ) {
 ...
 * @param type $data 
 */
function mncf_custom_types_register( $post_type, $data ) {
    ...
    $args = register_post_type( $post_type, apply_filters( 'mncf_type', $data, $post_type ) );

    do_action( 'mncf_type_registered', $args );
}

/**
 * Revised rewrite.
 * 
 * We force slugs now. Submitted and sanitized slug. Set slugs localized (MNML).
 * More solid way to force MN slugs.
 * 
 * @see https://icanlocalize.basecamphq.com/projects/7393061-mn-views/todo_items/153925180/comments
 * @since 1.1.3.2
 * @param type $data
 * @param type $post_type
 * @return boolean 
 */
function mncf_filter_type( $data, $post_type ) {
    if ( !empty( $data['rewrite']['enabled'] ) ) {
        $data['rewrite']['with_front'] = !empty( $data['rewrite']['with_front'] );
        $data['rewrite']['feeds'] = !empty( $data['rewrite']['feeds'] );
        $data['rewrite']['pages'] = !empty( $data['rewrite']['pages'] );
        ...
        // to return handling slugs back to MN.
        // 
        // We unset slug settings and leave MN to handle it himself.
        // Let MN decide what slugs should be!
//        if (!empty($data['rewrite']['custom']) && $data['rewrite']['custom'] != 'normal') {
//            unset($data['rewrite']['slug']);
//        }
        // Just discard non-MN property
        unset( $data['rewrite']['custom'] );
    } else {
        $data['rewrite'] = false;
    }
 ...
 * @return type 
 */
function mncf_get_active_custom_types() {
    $types = get_option(MNCF_OPTION_NAME_CUSTOM_TYPES, array());
...
/** This action is documented in admin/includes/table.php */
add_filter('dashboard_glance_items', 'mncf_dashboard_glance_items');

/**
 * Add CPT info to "At a Glance"
 *
 * Add to "At a Glance" Mtaandao admin dashboard widget information
 * about number of posts.
 *
 * @since 1.6.6
 *
 */
function mncf_dashboard_glance_items($elements)
{
    // remove when https://core.trac.mtaandao.org/ticket/27414 is fixed
    mn_register_style( 'mncf-fix-mtaandao-core', MNCF_EMBEDDED_RES_RELPATH . '/css/fix-mtaandao-core.css', array(), MNCF_VERSION );
    mn_enqueue_style( 'mncf-fix-mtaandao-core' );
 ...
 */
function mncf_filter_enter_title_here($enter_title_here, $post)
{
    ...
    if ( empty($enter_title_here) ) {
        $enter_title_here = __('Enter title here', 'mncf');
    }
    ...
    if ( !empty( $custom_types ) ) {
        foreach ( $custom_types as $post_type => $data ) {
            ...
            if (
                ...
                if( isset( $mn_post_types[$data['slug']] ) ) {
                    // refer $l to post labels
                    $l = &$mn_post_types[$data['slug']]->labels;
