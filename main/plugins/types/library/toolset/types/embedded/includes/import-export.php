 ...
 * @return array
 */
function mncf_admin_import_data($data = '', $redirect = true, $context = 'types', $args = array() )
{
    global $mndb;
    ...
    if ( !$data ) {
		$return[] = array(
			'type'		=> 'error',
			'content'	=> __( 'Error parsing XML: ', 'mncf' ) . $error->message
		);
        libxml_clear_errors();
		return $return;
    }
    ...
    if ( $overwrite_settings && isset( $data->settings ) ) {
        $mncf_settings = mncf_get_settings();
        foreach( mncf_admin_import_export_simplexml2array( $data->settings ) as $key => $value ) {
            $mncf_settings[$key] =  $value;
        }
        mncf_save_settings( $mncf_settings );
		$return[] = array(
			'type'		=> 'success',
			'content'	=> __( 'Settings are updated.', 'mncf' )
		);
    }
            ...
            if ( (isset( $group['add'] ) && $group['add'] ) ) {
                ...
                if ( !empty( $group['meta'] ) ) {
                    foreach ( $group['meta'] as $meta_key => $meta_value ) {
                        if ( '_mncf_conditional_display' == $meta_key ) {
                            if ( !empty($meta_value) ) {
                                $meta_value = mncf_admin_import_export_simplexml2array( $meta_value );
                                if ( !is_array($meta_value) ) {
                                    ...
                                    if ( $show_import_fail_version_message ) {
										...
										$return[] = array(
											'type'		=> 'error',
											'content'	=> __( 'The Types settings were not fully imported because it contained unsecured data. You should re-export your Types settings using the latest version of Types', 'mncf' )
										);
                                    }
                                }
                            }
                        }
                        update_post_meta( $group_mn_id, $meta_key, $meta_value);
                    }
                }
                $groups_check[] = $group_mn_id;
                if ( !empty( $post_to_update ) ) {
            ...
            if ( !empty( $groups_to_delete ) ) {
                foreach ( $groups_to_delete as $group_to_delete ) {
                    if ( !in_array( $group_to_delete->ID, $groups_check ) ) {
                        $deleted = mn_delete_post( $group_to_delete->ID, true );
                        if ( !$deleted ) {
							$return[] = array(
								'type'		=> 'error',
								'content'	=> sprintf( 
												__( 'Group "%s" delete failed', 'mncf' ),
            ...
            if ( !empty( $_POST['groups-to-be-deleted'] ) ) {
                foreach ( $_POST['groups-to-be-deleted'] as $group_to_delete ) {
                    ...
                    if ( !empty( $group_to_delete_post ) && $group_to_delete_post->post_type == TYPES_CUSTOM_FIELD_GROUP_CPT_NAME ) {
                        $deleted = mn_delete_post( $group_to_delete, true );
                        if ( !$deleted ) {
							$return[] = array(
								'type'		=> 'error',
								'content'	=> sprintf( 
    ...
    if ( !empty( $data->user_groups ) && isset( $data->user_groups->group) ) {
        ...
        foreach ( $groups as $group_id => $group ) {
            ...
            if ( (isset( $group['add'] ) && $group['add'] ) ) {
                ...
                if ( $group['update'] || ($overwrite_groups && !empty( $post_to_update )) ) {
                    if ( !empty( $post_to_update ) ) {
                        ...
                        if ( !$group_mn_id ) {
							$return[] = array(
								'type'		=> 'error',
								'content'	=> sprintf( 
												__( 'User group "%s" update failed', 'mncf' ),
												$group['post_title']
											)
							);
                        } else {
							/*
                            mncf_admin_message_store( sprintf( __( 'User group "%s" updated',
                                                    'mncf' ),
                                            $group['post_title'] ) );
							*/
							$return[] = array(
                    ...
                    if ( is_mn_error( $group_mn_id ) ) {
						$return[] = array(
							'type'		=> 'error',
							'content'	=> sprintf( 
											__( 'User group "%s" insert failed', 'mncf' ),
											$group['post_title']
										)
						);
                    } else {
                ...
                if ( !empty( $group['meta'] ) ) {
                    foreach ( $group['meta'] as $meta_key => $meta_value ) {
                        update_post_meta( $group_mn_id, $meta_key, mncf_admin_import_export_simplexml2array( $meta_value ) );
                    }
                }
                $groups_check[] = $group_mn_id;
                if ( !empty( $post_to_update ) ) {
                    ...
                    if ( !in_array( $group_to_delete->ID, $groups_check ) ) {
                        ...
                        if ( !$deleted ) {
							$return[] = array(
								'type'		=> 'error',
								'content'	=> sprintf( 
												__( 'User group "%s" delete failed', 'mncf' ),
												$group_to_delete->post_title
											)
							);
                        } else {
    ...
    if ( !empty( $data->user_fields ) ) {
        ...
        foreach ( $fields as $field_id => $field ) {
			...
			$return[] = array(
				'type'		=> 'success',
				'content'	=> sprintf( 
								__( 'User field "%s" added/updated', 'mncf' ),
								$field['name']
							)
			);
        }
        ...
        update_option( 'mncf-usermeta', $fields_existing );
	...
	$term_group_results = $ie_controller->process_field_group_import_per_domain(
		...
		$overwrite_groups,
		$delete_groups,
		mncf_ensarr( mncf_getpost( 'term_groups' ) )
	);
	...
	$term_field_results = $ie_controller->process_field_definition_import_per_domain(
		...
		'term_fields',
		$delete_fields,
		mncf_ensarr( mncf_getpost( 'term_fields' ) )
	);
    ...
    if ( !empty($data->types) && isset($data->types->type) ) {
        ...
        foreach ( $types as $type_id => $type ) {
			...
			$return[] = array(
				'type'		=> 'success',
				'content'	=> sprintf( 
								__( 'Post Type "%s" added/updated', 'mncf' ),
								$type_id
							)
			);
        }
    ...
    if ( !empty( $data->taxonomies ) && isset($data->taxonomies->taxonomy)) {
        ...
        foreach ( $taxonomies as $taxonomy_id => $taxonomy ) {
			...
			$return[] = array(
				'type'		=> 'success',
				'content'	=> sprintf( 
								__( 'Taxonomy "%s" added/updated', 'mncf' ),
								$taxonomy_id
							)
			);
        }
    ...
    if ( !empty( $data->post_relationships ) && !empty( $_POST['post_relationship'] ) ) {
        ...
        if ( is_array( $relationship ) ) {
			...
			$return[] = array(
				'type'		=> 'success',
				'content'	=> __( 'Post relationships created', 'mncf' )
			);
        } else {
			$return[] = array(
				'type'		=> 'error',
				'content'	=> __( 'Post relationships settings were not imported because it contained unsecured data. You should re-export your Types settings using the latest version of Types', 'mncf' )
			);
        }
    }
 ...
 * @return string
 */
function mncf_admin_import_export_simplexml2array($element)
{
    ...
    if ( !is_array( $element ) && strval( $element ) == '0' ) {
        ...
        foreach ( $element as $k => $v ) {
            ...
            $add = mncf_admin_import_export_simplexml2array( $v );
...
/**
* mncf_filter_import_custom_taxonomy_data
*
...
add_filter( 'mncf_filter_import_custom_taxonomy', 'mncf_filter_import_custom_taxonomy_data' );

function mncf_filter_import_custom_taxonomy_data( $data = array() ) {
	if ( 
		isset( $data['supports'] ) 
