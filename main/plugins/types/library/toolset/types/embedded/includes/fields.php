...
function mncf_admin_fields_get_active_fields_by_post_type($post_type)
{
    $custom_fields = array();
    $groups = mncf_admin_fields_get_groups(TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, true, true);
    if ( empty($groups) ) {
        return $custom_fields;
    }
    foreach ($groups as $group) {
        $types = mncf_admin_get_post_types_by_group($group['id']);
 ...
 */
function mncf_admin_fields_get_groups( $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, $only_active = false, $add_fields = false ) {
    ...
    if ( false === $cached_object ) {
        ...
        mn_cache_add( $cache_key, $groups, $cache_group );
    } else {
    ...
    if ( !empty( $groups ) ) {
        foreach ( $groups as $k => $group ) {
            $group = mncf_admin_fields_adjust_group( $group, $add_fields );
 ...
 * @return array
 */
function mncf_admin_fields_get_group( $group_id, $post_type = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME,
        $add_fields = false ) {
    ...
    return mncf_admin_fields_adjust_group( $group, $add_fields );
}
 ...
 * @return array
 */
function mncf_admin_fields_adjust_group( $post, $add_fields = false ) {
    if ( empty( $post ) ) {
        return false;
    }
    $group = array();
    ...
    $group['filters_association'] = get_post_meta( $post->ID, '_mn_types_group_filters_association', true );
    $group[MNCF_AUTHOR] = $post->post_author;

    /**
     * add _mn_types_group_showfor
     *
     * "all" - for all groups
     * array() - if any
     */
    if ( TYPES_USER_META_FIELD_GROUP_CPT_NAME == $post->post_type ) {
        $data = get_post_meta($post->ID, '_mn_types_group_showfor', true);
        if ( 'all' != $data ) {
        ...
        $group['_mn_types_group_showfor'] = $data;
    }

    // Attach fields if required (since 1.3)
    if ( $add_fields ) {
        $active = $add_fields == 'fields_active' ? true : false;
        $group['fields'] = mncf_admin_fields_get_fields_by_group( $post->ID, 'slug', $active );
    }
 ...
 * @return string
 */
function mncf_admin_get_groups_admin_styles_by_group( $group_id ) {
	if (
		defined( 'TYPES_USE_STYLING_EDITOR' )
		&& TYPES_USE_STYLING_EDITOR
	) {
		$admin_styles = get_post_meta( $group_id, '_mn_types_group_admin_styles', true );
 ...
 */
function mncf_admin_fields_save_group_admin_styles( $group_id, $admin_styles ) {
    update_post_meta( $group_id, '_mn_types_group_admin_styles', $admin_styles );
}

/**
 * Gets all fields.
 *
 * @todo Move to MNCF_Fields
 ...
 * added param $use_cache by Gen (used when adding new fields to group)
 */
function mncf_admin_fields_get_fields( $only_active = false,
        $disabled_by_type = false, $strictly_active = false,
        $option_name = 'mncf-fields', $use_cache = true, $clear_cache = false ) {
    ...
    switch( $option_name ) {
	    case 'mncf-usermeta':
	    case 'mncf-fields':
	    case MNCF_Field_Definition_Factory_Term::FIELD_DEFINITIONS_OPTION:
		    break;
	    default:
		    $option_name = 'mncf-fields';
		    break;
    }
    ...
    foreach ( $fields as $k => $v ) {
        ...
        $data = mncf_fields_type_action( $v['type'] );
        if ( empty( $data ) ) {
            unset( $fields[$k] );
            continue;
        }
        ...
        $v['id'] = $k;
        $v['meta_key'] = mncf_types_get_meta_prefix( $v ) . $k;

	    $option_name_to_meta_type = array(
		    'mncf-fields' => 'postmeta',
		    'mncf-usermeta' => 'usermeta',
		    MNCF_Field_Definition_Factory_Term::FIELD_DEFINITIONS_OPTION => 'termmeta'
	    );

        $v['meta_type'] = $option_name_to_meta_type[ $option_name ];
        $fields[$k] = mncf_sanitize_field( $v );
...
add_filter( 'toolset_filter_field_definition_array', 'mncf_update_mandatory_validation_rules', 10, 2 );
 ...
 * @since 2.2.4
 */
function mncf_update_mandatory_validation_rules( $field_definition, $ignored ) {
    ...
    $validation_rules = mncf_ensarr( mncf_getnest( $field_definition, array( 'data', 'validate' ) ) );
    ...
    if ( $is_file_field ) {
        ...
        if ( ! $has_url2_validation ) {
            ...
            $default_validation_error_message = __( 'Please enter a valid URL address pointing to the file.', 'mncf' );
            $validation_error_messages = array(
                'file' => $default_validation_error_message,
                'audio' => __( 'Please enter a valid URL address pointing to the audio file.', 'mncf' ),
                'image' => __( 'Please enter a valid URL address pointing to the image file.', 'mncf' ),
                'video' => __( 'Please enter a valid URL address pointing to the video file.', 'mncf' )
            );
    ...
    //
    // These field types come via MNToolset_Types::filterValidation().
...
function mncf_admin_fields_get_field_by_meta_key( $meta_key )
{
 ...
 * @return array
 */
function mncf_admin_fields_get_field( $field_id, $only_active = false,
        $disabled_by_type = false, $strictly_active = false,
        $option_name = 'mncf-fields' ) {
    $fields = mncf_admin_fields_get_fields( $only_active, $disabled_by_type,
            $strictly_active, $option_name );
    $field = !empty( $fields[$field_id] ) ? $fields[$field_id] : array();
    if ( !empty( $field ) ) {
        return apply_filters( 'mncf_field', $field );
    }
    return array();
}
 ...
 * @return type
 */
function mncf_fields_get_field_by_slug( $slug, $meta_name = 'mncf-fields' ) {
    return mncf_admin_fields_get_field( $slug, false, false, false, $meta_name );
}

add_filter('mncf_fields_by_group', 'mncf_admin_fields_get_fields_by_group', 10, 1);
 ...
 * @return type
 */
function mncf_admin_fields_get_groups_by_term( $term_id = false,
    ...
    foreach ( $groups as $k => $post ) {
        $temp = get_post_meta( $post->ID, '_mn_types_group_post_types', true );
        if ( $fetch_empty && $temp == 'all' ) {
            $groups[$k] = mncf_admin_fields_adjust_group( $post );
...
add_filter('mncf_get_groups_by_post_type', 'mncf_admin_get_groups_by_post_type', 10, 1);
 ...
 * @return type
 */
function mncf_admin_get_groups_by_post_type( $post_type, $fetch_empty = true, $terms = null, $only_active = true )
{
    $args = array();
    $args['post_type'] = TYPES_CUSTOM_FIELD_GROUP_CPT_NAME;
    $args['numberposts'] = -1;
    ...
    if ( $fetch_empty ) {
        $args['meta_query'] = array(
            'relation' => 'OR',
            array(
                'key' => '_mn_types_group_post_types',
                'value' => ',' . $post_type . ',',
                'compare' => 'LIKE',
            ),
            array(
    ...
    if ( !empty( $groups ) ) {
        foreach ( $groups as $key => $group ) {
            $group = mncf_admin_fields_adjust_group( $group );
    ...
    if ( !is_null( $terms ) ) {
        if ( !empty( $terms ) ) {
            ...
            global $mndb;
            $terms_sql = "SELECT * FROM $mndb->posts p
                JOIN $mndb->postmeta m
                WHERE p.post_type=".TYPES_CUSTOM_FIELD_GROUP_CPT_NAME." AND p.post_status='publish'
                AND p.ID = m.post_id AND m.meta_key='_mn_types_group_terms'
                $terms_sql";
            $groups = $mndb->get_results( $terms_sql );
            if ( !empty( $groups ) ) {
                foreach ( $groups as $key => $group ) {
                    $group = mncf_admin_fields_adjust_group( $group );
 ...
 * @return type
 */
function mncf_admin_get_groups_by_template( $templates = array('default'),
 ...
 *
 */
function mncf_get_fullpath_by_field_type($basename)
{
    return sprintf(
        '%s/fields/%s.php',
        dirname( __FILE__ ),
 ...
 * @param type $args
 */
function mncf_fields_type_action( $type, $func = '', $args = array() ) {
    ...
    if ( !isset( $actions[$type . '-' . $func_in . '-' . $md5_args] ) ) {
        ...
        } else if ( defined( 'MNCF_INC_ABSPATH' ) ) {
            $file = MNCF_INC_ABSPATH . '/fields/' . $type . '.php';
        } else {
            $file = '';
        }
        $file_embedded = mncf_get_fullpath_by_field_type($type);
        if ( file_exists( $file ) || file_exists( $file_embedded ) ) {
            ...
            if ( empty( $func ) ) {
                $func = 'mncf_fields_' . $type;
            } else {
                $func = 'mncf_fields_' . $type . '_' . $func;
            }
 ...
 */
function mncf_fields_get_shortcode( $field, $add = '', $content = '' )
{
    $shortcode = '[';
    $shortcode .= "types field='" . $field['slug'] . "'" . $add;
    $shortcode .= ']' . $content . '[/types]';
    $shortcode = apply_filters( 'mncf_fields_shortcode', $shortcode, $field );
    $shortcode = apply_filters( 'mncf_fields_shortcode_type_' . $field['type'], $shortcode, $field );
    $shortcode = apply_filters( 'mncf_fields_shortcode_slug_' . $field['slug'], $shortcode, $field );
    return $shortcode;
}

function mncf_termmeta_get_shortcode( $field, $add = '', $content = '' ) {
    $shortcode = '[';
...
function mncf_get_termmeta_form_addon_submit() {
	$add = '';
 ...
 * @param type $add Additional attributes
 */
function mncf_usermeta_get_shortcode( $field, $add = '', $content = '' ) {
...
function mncf_get_usermeta_form_addon( $settings = array() ) {
	global $mndb;
	$form = array();
	$users = $mndb->get_results(
	...
	$form[] = array(
        ...
        '#inline' => true,
		'#attributes' => array('onclick' => 'mncf_showmore(false)')
	);
	$form[] = array(
		'#type' => 'radio',
	...
	$form[] = array(
		...
		'#after' => '',
        '#inline' => true,
		'#attributes' => array('onclick' => 'mncf_showmore(true)')
	);
	...
	$form[] = array(
		...
		'#after' => '<input type="text" class="mncf-form-textfield form-textfield textfield" name="display_username_for_suser_id_value" value="' . $__userid . '"'.
	...
	$form[] = array(
		...
		'#after' => '<input type="text" class="input mncf-form-textfield form-textfield textfield"'. $__hidden .
...
function mncf_get_usermeta_form_addon_submit() {
    $add = '';
 ...
 */
function mncf_admin_fields_get_available_types() {
    return MNCF_Fields::getFieldsTypesData();
}
 ...
 * @param type $field
 */
function mncf_sanitize_field( $field ) {
    // Sanitize name
    if ( isset( $field['name'] ) ) {
 ...
 * @param type $field_id
 */
function mncf_admin_fields_get_groups_by_field( $field_id,
 ...
 * @param type $append
 */
function mncf_admin_fields_save_field_last_settings( $field_id, $settings,
        $append = false, $overwrite = false ) {
    $data = get_user_meta( get_current_user_id(), 'mncf-field-settings', true );
    ...
    update_user_meta( get_current_user_id(), 'mncf-field-settings', $data );
}
 ...
 * @param type $field_id
 */
function mncf_admin_fields_get_field_last_settings( $field_id ) {
    $data = get_user_meta( get_current_user_id(), 'mncf-field-settings', true );
 ...
 */
function mncf_admin_get_allowed_fields_by_post_type($post_type)
{
    $allowed_fields = $active_groups = array();
    $all_groups = mncf_admin_get_groups_by_post_type($post_type);
    foreach( $all_groups as $group_id => $group_data ) {
        $active_groups[] = $group_data['slug'];
    }
    $all_groups = mncf_admin_fields_get_groups(TYPES_CUSTOM_FIELD_GROUP_CPT_NAME, true, true);
 ...
 */
function mncf_get_all_field_slugs_except_current_group( $current_group = false ) {
    $all_slugs       = array();
    $all_post_fields = get_option( 'mncf-fields', array() );
    $all_user_fields = get_option( 'mncf-usermeta', array() );
    $all_term_fields = get_option( 'mncf-termmeta', array() );
    ...
    if( $current_group && !empty( $all_fields ) ) {
        ...
        $current_group_fields = get_post_meta( $current_group, '_mn_types_group_fields', true );
...
add_action('mn_ajax_mncf_get_all_field_slugs_except_current_group', 'mncf_get_all_field_slugs_except_current_group');
