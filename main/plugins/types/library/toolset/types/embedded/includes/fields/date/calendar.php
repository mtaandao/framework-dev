 ...
 * @return type
 */
function mncf_fields_date_get_calendar( $params, $initial = true, $echo = true ) {

    global $mndb, $m, $mn_locale, $posts;

    // mncf Set our own date
    $monthnum = adodb_date( 'n', $params['field_value'] );
    $year = adodb_date( 'Y', $params['field_value'] );
    $mncf_date = adodb_date( 'j', $params['field_value'] );

    $cache = array();
    $key = md5( $params['field']['slug'] . $mncf_date );
    if ( $cache = mn_cache_get( 'get_calendar', 'calendar' ) ) {
    ...
    $calendar_output = '<table id="mn-calendar-' . md5( serialize( func_get_args() ) )
            . '" summary="' . esc_attr__( 'Calendar' ) . '"' . $class . '>
	<caption>' . sprintf( $calendar_caption,
                    $mn_locale->get_month( $thismonth ), adodb_date( 'Y', $unixmonth ) ) . '</caption>
	<thead>
	<tr>';
    ...
    foreach ( $myweek as $wd ) {
        $day_name = (true == $initial) ? $mn_locale->get_weekday_initial( $wd ) : $mn_locale->get_weekday_abbrev( $wd );
        $wd = esc_attr( $wd );
