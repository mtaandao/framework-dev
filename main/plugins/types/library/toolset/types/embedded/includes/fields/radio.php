 ...
 * @return type 
 */
function mncf_fields_radio() {

    return array(
        'id' => 'mncf-radio',
        'title' => __( 'Radio', 'mncf' ),
        'description' => __( 'Radio', 'mncf' ),
        'validate' => array(
            'required' => array(
 ...
 * @deprecated seems
 */
function mncf_fields_radio_meta_box_form( $field ) {
    $options = array();
    $default_value = '';
 ...
 */
function mncf_fields_radio_editor_callback( $field, $data ) {
    ...
    return array(
        ...
        'tabs' => array(
            'display' => array(
                ...
                'content' => MNCF_Loader::template( 'editor-modal-radio', $data ),
            )
        ),
    );
}
 ...
 */
function mncf_fields_radio_editor_submit( $data, $field, $context ) {
    $add = '';
	switch ( $context ) {
		case 'usermeta':
			$types_attr = 'usermeta';
 ...
 * @param type $params 
 */
function mncf_fields_radio_view( $params ) {
        ...
        foreach ( $field['data']['options'] as $option_key => $option ) {
            if ( isset( $option['value'] )
                ...
                if ( isset( $params['field']['data']['display'] )
					...
					// We need to translate here because the stored value is on the original language
					// When updaing the value in the Field group, we might have problems
                    $field_value = mncf_translate( 'field ' . $params['field']['id'] . ' option ' . $option_key . ' display value', $option['display_value'] );
                }
            }
        }
        $output = $field_value;
