 ...
 */
function mncf_fields_date_calculate_time( $value ) {
 ...
 * @param $field
 * @param $field_object MNCF_Field|MNCF_Usermeta_Field|null Since 1.9, this can also be null (because of term fields).
 *
 ...
 */
function mncf_fields_date_value_save_filter( $value, $field, $field_object ) {
    ...
    if ( defined( 'MNTOOLSET_FORMS_VERSION' ) ) {
		...
		if ( is_numeric( $value['datepicker'] ) ) {
			...
			$timestamp = mntoolset_strtotime( $value['datepicker'] );
		}
    ...
    if ( ( null != $field_object ) && isset( $field_object->post->ID ) ) {
        delete_post_meta(
                ...
                '_mncf_' . $field_object->cf['id'] . '_hour_and_minute' );
    }
    ...
    $value['timestamp'] = mncf_fields_date_convert_datepicker_to_timestamp( $value['datepicker'] );

    if ( !mncf_fields_date_timestamp_is_valid( $value['timestamp'] ) ) {
        $mncf->debug->errors['date_save_failed'][] = array(
            'value' => $value,
            'field' => $field,
        );
        return false;
    ...
    if ( isset( $value['hour'] ) && isset( $value['minute'] ) ) {
        $value['timestamp'] = mncf_fields_date_calculate_time( $value );
    }
 ...
 * @return type 
 */
function mncf_fields_date_conditional_value_filter( $value, $field, $operation,
        $field_compared, $post ) {

    global $mncf;

    $field = mncf_admin_fields_get_field( $mncf->field->__get_slug_no_prefix( $field ) );
    if ( !empty( $field ) && isset( $field['type'] ) && $field['type'] == 'date' && isset($value['datepicker']) ) {
        $value['timestamp'] = mncf_fields_date_convert_datepicker_to_timestamp( $value['datepicker'] );
        $value = mncf_fields_date_calculate_time( $value );
    }
    return $value;
}
 ...
 * @param type $post
 */
function mncf_fields_date_conditional_condition_filter( $value, $field,
        $operation, $field_compared, $post ) {
    global $mncf;

    $field = mncf_admin_fields_get_field( $mncf->field->__get_slug_no_prefix( $field ) );
    if ( !empty( $field ) && isset( $field['type'] ) && $field['type'] == 'date' ) {
        $_value = mncf_fields_date_convert_datepicker_to_timestamp( $value );
        if ( $_value ) {
            $value = $_value;
        } else {
            $value = mncf_fields_date_value_get_filter( $value, $field,
                    'timestamp' );
        }
    }
    return $value;
 ...
 */
function mncf_fields_custom_conditional_statement_hook() {
    // Enqueue after first filters in evaluate.php
    add_filter( 'get_post_metadata',
            'mncf_fields_date_custom_conditional_statement_filter', 20, 4 );
}
 ...
 */
function mncf_fields_date_custom_conditional_statement_filter( $null,
    ...
    if ( !empty( $null ) && !empty( $field ) && isset( $field['type'] ) && $field['type'] == 'date' ) {
        ...
        $null = mncf_fields_date_value_get_filter( $null, $field, 'timestamp' );
        if ( !is_numeric( $null ) ) {
            $null = -1;
        }
        /**
...
function mncf_get_date_format_text() {
...
function _mncf_date_convert_mn_to_js( $date_format ) {
 ...
 *
 */
function mncf_date_to_strftime( $format ) {
 ...
 * @return null
 */
function mncf_fields_date_value_check( $a ) {
    ...
    foreach ( $required as $key ) {
        switch ( $key ) {
            case 'timestamp':
                ...
                if ( !isset( $a[$key] ) || !mncf_fields_date_timestamp_is_valid( $a[$key] ) ) {
                    $_t = mncf_fields_date_convert_datepicker_to_timestamp( $a['datepicker'] );
                    if ( !$_t ) {
                        // Failed converting
                        return $empty_date;
                    }
            ...
            case 'datepicker':
                ...
                if ( empty( $a[$key] ) ) {
                    $_d = mncf_fields_date_convert_timestamp_to_datepicker( $a['timestamp'] );
                    if ( !$_d ) {
                        // Failed converting
                        return $empty_date;
                    }
				...
				/*
                if ( !mncf_fields_date_datepicker_is_valid( $a['datepicker'] ) ) {
                    return $empty_date;
                }
				*/
    ...
    if ( adodb_date( mncf_get_date_format(), $a['timestamp'] ) != $a['datepicker'] ) {
        // In this case we'll give advantage to timestamp
        $a['datepicker'] = mncf_fields_date_convert_timestamp_to_datepicker( $a['timestamp'] );
        if ( !$a['timestamp'] ) {
            // Failed converting
            return $empty_date;
        }
 ...
 * @return boolean
 */
function mncf_fields_date_convert_timestamp_to_datepicker( $timestamp ) {
    // Check if timestamp valid
    if ( !mncf_fields_date_timestamp_is_valid( $timestamp ) ) {
        return false;
    }
    $_d = adodb_date( mncf_get_date_format(), $timestamp );
	/*
    if ( !mncf_fields_date_datepicker_is_valid( $_d ) ) {
        // Failed converting
        return false;
    }
	*/
 ...
 * @return boolean
 */
function mncf_fields_date_convert_datepicker_to_timestamp( $datepicker ) {
    ...
    if ( $_t == false || !mncf_fields_date_timestamp_is_valid( $_t ) ) {
        // Failed converting
        return false;
    }
    return $_t;
 ...
 * @return type
 */
function mncf_fields_date_timestamp_is_valid( $timestamp ) {
    /*
	...
	return MNToolset_Field_Date_Scripts::_isTimestampInRange($timestamp);
 ...
 * @return boolean
 */
function mncf_fields_date_datepicker_is_valid( $datepicker ) {
    return (bool) mncf_fields_date_convert_datepicker_to_timestamp( $datepicker );
}
 ...
 * @param type $field
 */
function __mncf_fields_date_check_leftover( $value, $field, $use_cache = true ) {
    ...
    $field_slug = mncf_types_get_meta_prefix() . $field_id;
    ...
    $_meta = mncf_get_post_meta( $post_id,
            '_mncf_' . $field_id . '_hour_and_minute', true );
    ...
    if ( !empty( $_meta ) && is_array( $_meta ) && isset( $_meta[$meta_id] ) ) {
        ...
        if ( !empty( $meta['timestamp'] ) || !empty( $meta['datepicker'] ) ) {
            ...
            $meta['timestamp'] = mncf_fields_date_value_get_filter( $meta,
            ...
            if ( (isset( $meta['hour'] )
                ...
                $value = mncf_fields_date_calculate_time( $meta );
            }
        }
    }
