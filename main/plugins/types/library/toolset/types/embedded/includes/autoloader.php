 ...
 *
 * See:
 * @link https://git.onthegosystems.com/toolset/layouts/wikis/layouts-theme-integration#mnddl_theme_integration_autoloader
 *
...
final class MNCF_Autoloader {
	...
	protected function __construct() {
		...
		if( file_exists( MNCF_ABSPATH . '/autoload_classmap.php' ) ) {
			$this->classmap = include( MNCF_ABSPATH . '/autoload_classmap.php' );
		}
	}
	 ...
	 *
	 * Currently it recongnizes MNToolset_* and MNToolset_Field_* classes if they're named in
	...
	private function try_load_toolset_forms_class( $full_class_name ) {
		...
		// Handle classes named MNToolset_Someting_Something*
		$explode_class = explode( '_' , $full_class_name );
		if( count( $explode_class ) >= 2 && 'MNToolset' == $explode_class[0] ) {
			...
			$candidate_path = MNTOOLSET_FORMS_ABSPATH . '/classes/' . $filename;
