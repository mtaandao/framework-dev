 ...
 * Use global $mncf_api to add new objects.
 * 
 * @since Types 1.2
 */
...
function types_get_field( $field, $meta_type = 'postmeta' ) {
	...
	switch ( $meta_type ) {
		case 'usermeta':
			$meta_type_key = 'mncf-usermeta';
			break;
		case 'termmeta':
			$meta_type_key = 'mncf-termmeta';
			break;
		default:
			$meta_type_key = 'mncf-fields';
			break;
	}
    $cache[$cache_key] = mncf_admin_fields_get_field( strval( $field ), false,
...
function types_get_fields( $args = array(), $toolset = 'types' ){
    ...
    $fields = MNCF_Fields::getFields( $args, $toolset );
...
function types_get_field_meta_value( $field, $post_id = null, $raw = false ) {
    return mncf_api_field_meta_value( $field, $post_id, $raw );
}
 ...
 * @return type 
 */
function mncf_api_field_meta_value( $field, $post_id = null, $raw = false ) {
    ...
    if ( mncf_admin_is_repetitive( $field ) ) {
        return mncf_api_field_meta_value_repetitive( $field, $post_id );
    }
    ...
    $value = $raw ? $mncf->field->__meta : $mncf->field->meta;
...
function types_field_get_meta_value_repetitive( $field, $post_id = null,
        $meta_id = null ) {
    return mncf_api_field_meta_value_repetitive( $field, $post_id, $meta_id );
}
...
function types_is_repetitive( $field ) {
    ...
    return mncf_admin_is_repetitive( $field );
}
...
function types_get_fields_by_group( $group, $only_active = 'only_active' ){
    ...
    $group = mncf_admin_fields_get_group( $group );
    if ( !empty( $group['id'] ) ) {
        if ( $only_active && $group['is_active'] ) {
            $fields = mncf_admin_fields_get_fields_by_group( $group['id'],
                    'slug' );
            foreach ( $fields as $field ) {
                $results[$field['id']] = mncf_api_field_meta_value( $field );
            }
        }
    }
...
function types_child_posts( $post_type, $args = array() ) {
    ...
    // MN allows querying inactive post types
    if ( !isset( $mn_post_types[$post_type] )
            || !$mn_post_types[$post_type]->publicly_queryable ) {
        return array();
    }
    ...
    $child_posts = MNCF_Relationship_Model::getChildrenByPostType( $post,
                    $post_type, array(), array(), $args );

    foreach ( $child_posts as $child_post_key => $child_post ) {
        $child_posts[$child_post_key]->fields = array();
        $groups = mncf_admin_post_get_post_groups_fields( $child_post );
            ...
            if ( !empty( $group['fields'] ) ) {
                ...
                foreach ( $group['fields'] as $k => $field ) {
                    ...
                    if ( types_is_repetitive( $field ) ) {
                        $data = mncf_get_post_meta( $child_post->ID,
                                mncf_types_get_meta_prefix( $field ) . $field['slug'],
 ...
 * 
 * Wrapper for mncf_cd_post_edit_field_filter()
 * core function.
 * 
 * @param type $element
 * @param type $field
...
function types_conditional_evaluate( $field, $post_id = null ) {
    return mncf_conditional_evaluate( $post_id, $field );
}
...
function types_create_relationship( $post_id, $posts = array(), $error = false ) {
    $updated = mncf_pr_admin_update_belongs( $post_id, $posts );
 ...
 * @return type 
 */
function mncf_api_field_meta_value_repetitive( $field, $post_id = null,
        $meta_id = null ) {
    ...
    } else if ( isset( $mncf->repeater->meta['custom_order'] ) ) {
        // Return ordered
        $values = $mncf->repeater->meta['custom_order'];
    } else if ( isset( $mncf->repeater->meta['by_meta_id'] ) ) {
        // Return by_meta_id
        $values = $mncf->repeater->meta['by_meta_id'];
    }
 ...
 * @return type 
 */
function mncf_conditional_evaluate( $post = null, $field ) {
...
function types_meta_prefix( $field = array() ) {
    return mncf_types_get_meta_prefix( $field );
}
 ...
 * $_POST['mncf'][$field_slug_full]
 * e.g. 'mncf-img'
 *  * If field slug do not exist in $_POST['mncf'] - won't be saved.
 * 
 * @global type $mncf_api
 * @param type $post
 * @param type $field
 * @return type 
 */
function mncf_api_repetitive_save( $post, $field ) {
    global $mncf;
    $mncf->repeater->set( $post, $field );
    return $mncf->repeater->save();
}
 ...
 * @return type 
 */
function mncf_api_repetitive_get_meta( $post, $field ) {
    global $mncf;
    $mncf->repeater->set( $post, $field );
    return $mncf->repeater->_get_meta();
}
