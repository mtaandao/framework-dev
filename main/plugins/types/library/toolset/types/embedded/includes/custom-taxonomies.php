 ...
 * @return array
 */
function mncf_custom_taxonomies_default() {
    return array(
        'slug' => '',
        'description' => '',
        'supports' => array(),
 ...
 */
function mncf_custom_taxonomies_init()
{
    ...
    if ( !empty( $custom_taxonomies ) ) {
        foreach ( $custom_taxonomies as $taxonomy => $data ) {
            if ( 
                ...
                mncf_custom_taxonomies_register( $taxonomy, $data );
            }
        }
    }
}
function mncf_builtin_taxonomies_init() {
	$custom_taxonomies = get_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array() );
    if ( !empty( $custom_taxonomies ) ) {
        foreach ( $custom_taxonomies as $taxonomy => $data ) {
            if ( isset($data['_builtin']) && $data['_builtin']) {
                mncf_taxonomies_register( $taxonomy, $data );
            }
        }
    }
}
 ...
 *
 */
function mncf_taxonomies_register($taxonomy, $data)
{
    ...
    // hide
        ...
        $mn_taxonomies[$data['slug']]->public = false;
        $mn_taxonomies[$data['slug']]->show_ui = false;
        $mn_taxonomies[$data['slug']]->show_in_menu = false;
    }
}
 ...
 * @since unknown
 */
function mncf_custom_taxonomies_register( $taxonomy, $data ) {
	...
	$object_types_filtered = apply_filters( 'mncf_taxonomy_objects', $object_types, $taxonomy );
	$taxonomy_args = apply_filters( 'mncf_taxonomy_data', $data, $taxonomy, $object_types );

	// Suddenly, Mtaandao 4.7 (alpha) requires the 'name' argument and uses it as the slug.
	...
	$is_success = ( $result instanceof MN_Error ? false : true );
 ...
 * @return array
 */
function mncf_get_active_custom_taxonomies() {
