 ...
 * Fields table.
 */
function mncf_module_inline_table_fields()
{
    // dont add module manager meta box on new post type form
    if ( !defined( 'MODMAN_PLUGIN_NAME' ) ) {
        _e('There is a problem with Module Manager', 'mncf');
        return;
    }
    ...
    $group = mncf_admin_fields_get_group( (int) $_GET['group_id'] );
    if ( empty($group) ) {
        _e('Wrong group id.', 'mncf');
        return;
    }
    do_action(
        'mnmodules_inline_element_gui',
        array(
 ...
 * Post Types table.
 */
add_filter('mncf_meta_box_order_defaults', 'mncf_module_post_add_meta_box', 10, 2);

function mncf_module_post_add_meta_box($meta_box_order_defaults, $type)
{
    if ( !defined( 'MODMAN_PLUGIN_NAME' )) {
    ...
    case 'post_type':
        if ( isset( $_GET['mncf-post-type'] ) ) {
            $meta_box_order_defaults['module_manager_post'] = array(
                'callback' => 'mncf_admin_metabox_module_manager_post',
                'title' => __('Module Manager', 'mncf'),
                'default' => 'side',
                'priority' => 'low',
            );
        }
    ...
    case 'taxonomy':
        if (  isset( $_GET['mncf-tax'] ) ) {
            $meta_box_order_defaults['module_manager_post'] = array(
                'callback' => 'mncf_admin_metabox_module_manager_taxonomy',
                'title' => __('Module Manager', 'mncf'),
                'default' => 'side',
                'priority' => 'low',
            );
    ...
    case 'mn-types-group':
        if (  isset( $_GET['group_id'] ) ) {
            $meta_box_order_defaults['module_manager_post'] = array(
                'callback' => 'mncf_module_inline_table_fields',
                'title' => __('Module Manager', 'mncf'),
                'default' => 'side',
                'priority' => 'low',
            );
...
function mncf_admin_metabox_module_manager_post()
{
    return mncf_admin_metabox_module_manager('post');
}

function mncf_admin_metabox_module_manager_taxonomy()
{
    return mncf_admin_metabox_module_manager('taxonomy');
}

function mncf_admin_metabox_module_manager($type)
{
    $form = array();
    /**
     * box content
    ...
    switch($type) {
    case 'post':
        mncf_module_inline_table_post_types();
        break;
    case 'taxonomy':
        mncf_module_inline_table_post_taxonomies();
        break;
    default:
        _e('Wrong type!', 'mncf');
        break;
...
function mncf_module_inline_table_post_types() {
    // dont add module manager meta box on new post type form
    if ( defined( 'MODMAN_PLUGIN_NAME' ) && isset( $_GET['mncf-post-type'] ) ) {
        $_custom_types = get_option( MNCF_OPTION_NAME_CUSTOM_TYPES, array() );
        if ( isset( $_custom_types[$_GET['mncf-post-type']] ) ) {
            $_post_type = $_custom_types[$_GET['mncf-post-type']];
            ...
            do_action( 'mnmodules_inline_element_gui', $element );
        }
    }
}
 ...
 * Taxonomies table.
 */
function mncf_module_inline_table_post_taxonomies() {
    // dont add module manager meta box on new post type form
    if ( defined( 'MODMAN_PLUGIN_NAME' ) && isset( $_GET['mncf-tax'] ) ) {
        $_custom_taxes = get_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array() );
        if ( isset( $_custom_taxes[$_GET['mncf-tax']] ) ) {
            $_tax = $_custom_taxes[$_GET['mncf-tax']];
            // add module manager meta box to post type form
...
if ( defined( 'MODMAN_PLUGIN_NAME' ) ) {
    add_filter( 'mnmodules_register_sections', 'mncf_register_modules_sections',
            10, 1 );

    // Post Types
    add_filter( 'mnmodules_register_items_' . _TYPES_MODULE_MANAGER_KEY_,
            'mncf_register_modules_items_types', 10, 1 );
    add_filter( 'mnmodules_export_items_' . _TYPES_MODULE_MANAGER_KEY_,
            'mncf_export_modules_items_types', 10, 2 );
    add_filter( 'mnmodules_import_items_' . _TYPES_MODULE_MANAGER_KEY_,
            'mncf_import_modules_items_types', 10, 2 );

    // Groups
    add_filter( 'mnmodules_register_items_' . _GROUPS_MODULE_MANAGER_KEY_,
            'mncf_register_modules_items_groups', 10, 1 );
    add_filter( 'mnmodules_export_items_' . _GROUPS_MODULE_MANAGER_KEY_,
            'mncf_export_modules_items_groups', 10, 3 );
    add_filter( 'mnmodules_import_items_' . _GROUPS_MODULE_MANAGER_KEY_,
            'mncf_import_modules_items_groups', 10, 2 );

    // Taxonomies
    add_filter( 'mnmodules_register_items_' . _TAX_MODULE_MANAGER_KEY_,
            'mncf_register_modules_items_taxonomies', 10, 1 );
    add_filter( 'mnmodules_export_items_' . _TAX_MODULE_MANAGER_KEY_,
            'mncf_export_modules_items_taxonomies', 10, 2 );
    add_filter( 'mnmodules_import_items_' . _TAX_MODULE_MANAGER_KEY_,
            'mncf_import_modules_items_taxonomies', 10, 2 );

    // Check items
    add_filter( 'mnmodules_items_check_' . _TYPES_MODULE_MANAGER_KEY_,
            'mncf_modman_items_check_custom_post_types', 10, 2 );
    add_filter( 'mnmodules_items_check_' . _GROUPS_MODULE_MANAGER_KEY_,
            'mncf_modman_items_check_groups', 10, 2 );
    add_filter( 'mnmodules_items_check_' . _TAX_MODULE_MANAGER_KEY_,
            'mncf_modman_items_check_taxonomies', 10, 2 );

	//Module manager: Hooks for adding plugin version

	/*Export*/
    add_filter('mnmodules_export_pluginversions_'._GROUPS_MODULE_MANAGER_KEY_,'mncf_modman_get_plugin_version');
    add_filter('mnmodules_export_pluginversions_'._TYPES_MODULE_MANAGER_KEY_,'mncf_modman_get_plugin_version');
    add_filter('mnmodules_export_pluginversions_'._TAX_MODULE_MANAGER_KEY_,'mncf_modman_get_plugin_version');

    /*Import*/
    add_filter('mnmodules_import_pluginversions_'._GROUPS_MODULE_MANAGER_KEY_,'mncf_modman_get_plugin_version');
    add_filter('mnmodules_import_pluginversions_'._TYPES_MODULE_MANAGER_KEY_,'mncf_modman_get_plugin_version');
    add_filter('mnmodules_import_pluginversions_'._TAX_MODULE_MANAGER_KEY_,'mncf_modman_get_plugin_version');

    /*
     * Module Manager Functions
     */

    function mncf_modman_get_plugin_version() {
    ...
    function mncf_register_modules_items_types( $items ) {
        ...
        foreach ( $custom_types as $type ) {
            ...
            $_details = sprintf( __( '%s post type: %s', 'mncf' ), ucfirst( $type['public'] ), $type['labels']['name'] );
    ...
    function mncf_export_modules_items_types( $res, $items ) {
        ...
        $xmlstring = mncf_admin_export_selected_data( $items, 'types',
                'module_manager' );
        return $xmlstring;
    }

    function mncf_import_modules_items_types( $result, $xmlstring ) {
        require_once MNCF_EMBEDDED_INC_ABSPATH . '/import-export.php';
        $result2 = mncf_admin_import_data_from_xmlstring( $xmlstring, 'types',
                'modman' );
        if ( false === $result2 || is_mn_error( $result2 ) )
            return (false === $result2) ? __( 'Error during Post Types import', 'mncf' ) : $result2->get_error_message( $result2->get_error_code() );

        return $result2;
    }

    function mncf_register_modules_items_groups( $items ) {
        $groups = mncf_admin_fields_get_groups();
        foreach ( $groups as $group ) {
            $_details = sprintf( __( 'Fields group: %s', 'mncf' ),
                    $group['name'] );
    ...
    function mncf_export_modules_items_groups( $res, $items, $use_cache = false ) {
        foreach ( $items as $ii => $item ) {
            $items[$ii] = intval( str_replace( '12' . _GROUPS_MODULE_MANAGER_KEY_ . '21',
                            '', $item['id'] ) );
        }
        $xmlstring = mncf_admin_export_selected_data( $items, 'groups',
                'module_manager', $use_cache );
        return $xmlstring;
    }

    function mncf_import_modules_items_groups( $result, $xmlstring ) {
        require_once MNCF_EMBEDDED_INC_ABSPATH . '/import-export.php';
        $result2 = mncf_admin_import_data_from_xmlstring( $xmlstring, 'groups',
                'modman' );
        if ( false === $result2 || is_mn_error( $result2 ) )
            return (false === $result2) ? __( 'Error during Field Groups import', 'mncf' ) : $result2->get_error_message( $result2->get_error_code() );

        return $result2;
    }

    function mncf_register_modules_items_taxonomies( $items ) {
    ...
    function mncf_export_modules_items_taxonomies( $res, $items ) {
        foreach ( $items as $ii => $item ) {
            if ( isset( $item['id'] ) ) {
        ...
        $xmlstring = mncf_admin_export_selected_data( $items, 'taxonomies',
                'module_manager' );
        return $xmlstring;
    }

    function mncf_import_modules_items_taxonomies( $result, $xmlstring ) {
        require_once MNCF_EMBEDDED_INC_ABSPATH . '/import-export.php';
        $result2 = mncf_admin_import_data_from_xmlstring( $xmlstring,
                'taxonomies', 'modman' );
        if ( false === $result2 || is_mn_error( $result2 ) )
            return (false === $result2) ? __( 'Error during Taxonomies import', 'mncf' ) : $result2->get_error_message( $result2->get_error_code() );
 ...
 * @return string
 */
function mncf_admin_export_selected_data ( array $items, $_type = 'all', $return = 'download', $use_cache = false)
{
    global $mncf;

    $xml = new ICL_Array2XML();
    $data = array();
    $data['settings'] = mncf_get_settings();
    ...
    if ( 'user_groups' == $_type || 'all' == $_type ) {
        ...
        if ( empty( $items ) ) {
            ...
            foreach ( $items as $k => $item ) {
                if ( isset( $item['id'] ) ) {
                    $items[$k] = intval( mncf_modman_get_submitted_id( 'groups',
                        $item['id'] ) );
                }
            }
            $args = array(
        ...
        if ( !empty( $groups ) ) {
            ...
            foreach ( $groups as $key => $post ) {
                ...
                if ( !empty( $meta ) ) {
                    ...
                    foreach ( $meta as $meta_key => $meta_value ) {
                        if ( in_array( $meta_key,
                            array(
                                '_mn_types_group_showfor',
                                '_mn_types_group_fields',
                                '_mn_types_group_admin_styles'
                            )
                        )
                        ) {
                ...
                $_data['checksum'] = $_data['hash'] = $mncf->export->generate_checksum( 'group',
                    $post['ID'] );
        ...
        if ( !empty( $items ) ) {
            ...
            foreach ( $groups as $key => $post ) {
                $fields = array_merge( $fields,
                    mncf_admin_fields_get_fields_by_group( $post->ID,
                    'slug', false, false, false,
                    TYPES_USER_META_FIELD_GROUP_CPT_NAME, 'mncf-usermeta',
                    $use_cache ) );
            }
        } else {
            // Get fields
            $fields = mncf_admin_fields_get_fields( false, false, false,
                'mncf-usermeta' );
        }
        if ( !empty( $fields ) ) {

            // Add checksums before MNML
            foreach ( $fields as $field_id => $field ) {
                // TODO MNML and others should use hook
                $fields[$field_id] = apply_filters( 'mncf_export_field',
                    $fields[$field_id] );
                $fields[$field_id]['__types_id'] = $field_id;
                $fields[$field_id]['__types_title'] = $field['name'];
                $fields[$field_id]['checksum'] = $fields[$field_id]['hash'] = $mncf->export->generate_checksum(
                    'field', $field_id
                );
            }

            // MNML
	        // todo remove MNML dependency, see https://onthegosystems.myjetbrains.com/youtrack/issue/types-749#comment=102-105900
            global $iclTranslationManagement;
            if ( !empty( $iclTranslationManagement ) ) {
                foreach ( $fields as $field_id => $field ) {
                    // TODO Check this for all fields
                    if ( isset( $iclTranslationManagement->settings['custom_fields_translation'][mncf_types_get_meta_prefix( $field ) . $field_id] ) ) {
                        $fields[$field_id]['mnml_action'] = $iclTranslationManagement->settings['custom_fields_translation'][mncf_types_get_meta_prefix( $field ) . $field_id];
                    }
                }
            }
    ...
    if ( 'groups' == $_type || 'all' == $_type ) {
        // Get groups
        if ( empty( $items ) ) {
            $groups = get_posts( 'post_type=mn-types-group&post_status=null&numberposts=-1' );
        } else {
            /*
             *
             * This fails
            ...
            foreach ( $groups as $key => $post ) {
                ...
                if ( !empty( $meta ) ) {
                    ...
                    foreach ( $meta as $meta_key => $meta_value ) {
                        if ( in_array( $meta_key,
                            array(
                                '_mn_types_group_terms',
                                '_mn_types_group_post_types',
                                '_mn_types_group_fields',
                                '_mn_types_group_templates',
                                '_mncf_conditional_display',
                                '_mn_types_group_filters_association',
                                '_mn_types_group_admin_styles'
                            )
                        )
                        ) {
    ...
    if ( 'types' == $_type || 'all' == $_type ) {
        ...
        if ( !empty( $custom_types ) ) {
            foreach ( $custom_types as $key => $type ) {
                ...
                $custom_types[$key] = apply_filters( 'mncf_export_custom_post_type',
                    $custom_types[$key] );

                $custom_types[$key]['__types_id'] = $key;
                $custom_types[$key]['__types_title'] = $type['labels']['name'];
                $custom_types[$key]['checksum'] = $custom_types[$key]['hash'] = $mncf->export->generate_checksum(
        ...
        if ( !empty( $items ) ) {
            ...
            $relationships_all = get_option( 'mncf_post_relationship', array() );
            $relationships = array();
    ...
    if ( 'taxonomies' == $_type || 'all' == $_type ) {
        if ( !empty( $items ) ) {
             ...
             * 0 => array('id' => 'pt', ...)
             */
            //            $custom_taxonomies = array_intersect_key( get_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES,
        ...
        if ( !empty( $custom_taxonomies ) ) {
            foreach ( $custom_taxonomies as $key => $tax ) {
                ...
                $custom_taxonomies[$key] = apply_filters( 'mncf_filter_export_custom_taxonomy', $custom_taxonomies[$key] );
                $custom_taxonomies[$key]['__types_id'] = $key;
                $custom_taxonomies[$key]['__types_title'] = $tax['labels']['name'];
                $custom_taxonomies[$key]['checksum'] = $mncf->export->generate_checksum(
    ...
    if ( class_exists( 'ZipArchive' ) ) {
        ...
        $temp_dir = mncf_get_temporary_directory();
        if ( empty( $temp_dir ) ) {
            die(__('There is a problem with temporary directory.', 'mncf'));
        }
         ...
         * try use mn_upload_dir instead. if this fail too, send pure
         * xml file to user
         */
        if ( empty( $zip->filename ) ) {
            $temp_dir = mn_upload_dir();
 ...
 */
function mncf_admin_import_data_from_xmlstring( $data = '', $_type = 'types',
        $context = 'types' ) {
     ...
     *
     * TODO Types 1.3
     * Merge with mncf_admin_import_data()
     */
    ...
    if ( !$data ) {
        echo '<div class="message error"><p>' . __( 'Error parsing XML', 'mncf' ) . '</p></div>';
        foreach ( libxml_get_errors() as $error ) {
            return new MN_Error( 'error_parsing_xml', __( 'Error parsing XML', 'mncf' ) . ' ' . $error->message );
        }
        libxml_clear_errors();
        return false;
    }
    ...
    if ( !empty( $data->groups ) && 'groups' == $_type ) {
        ...
        foreach ( $data->groups->group as $group ) {
            $group = (array) $group;
            // TODO 1.2.1 Remove
//            $_id = mncf_modman_set_submitted_id( _GROUPS_MODULE_MANAGER_KEY_,
//                    $group['ID'] );
            ...
            $group = mncf_admin_import_export_simplexml2array( $group );
            $group['add'] = true;
            $group['update'] = false;
        ...
        foreach ( $groups as $group ) {
            ...
            if ( (isset( $group['add'] ) && $group['add'] ) ) {
                $post_to_update = $mndb->get_var(
                    $mndb->prepare(
                        "SELECT ID FROM $mndb->posts WHERE post_title = %s AND post_type = %s",
                        $group['post_title'],
                ...
                if ( $group['update'] || (!empty( $post_to_update )) ) {
                    if ( !empty( $post_to_update ) ) {
                         ...
                         */
                        $_checksum = $mncf->import->checksum( 'group',
                                $post_to_update, $group['checksum'] );

                        $group_mn_id = mn_update_post( $post );
                        if ( !$group_mn_id ) {
                            $errors[] = new MN_Error( 'group_update_failed', sprintf( __( 'Group "%s" update failed', 'mncf' ),
                                                    $group['post_title'] ) );
                            $result['errors'][] = sprintf( __( 'Group %s update failed', 'mncf' ), $group['post_title'] );
                            $result['failed'] += 1;
                        } else {
                            if ( !$_checksum ) {
                                $result['updated'] += 1;
                    ...
                    $group_mn_id = mn_insert_post( $post, true );
                    if ( is_mn_error( $group_mn_id ) ) {
                        $errors[] = new MN_Error( 'group_insert_failed', sprintf( __( 'Group "%s" insert failed', 'mncf' ),
                                                $group['post_title'] ) );
                        $result['errors'][] = sprintf( __( 'Group %s insert failed', 'mncf' ), $group['post_title'] );
                        $result['failed'] += 1;
                    } else {
                        $result['new'] += 1;
                    }
                ...
                if ( !empty( $group['meta'] ) ) {
                    foreach ( $group['meta'] as $meta_key => $meta_value ) {
                        update_post_meta( $group_mn_id, $meta_key,
                                maybe_unserialize( $meta_value ) );
                    }
                }
                $group_check[] = $group_mn_id;
                if ( !empty( $post_to_update ) ) {
        ...
        // Process fields
        if ( !empty( $data->fields ) ) {
            $fields_existing = mncf_admin_fields_get_fields();
            $fields = array();
            $fields_check = array();
            // Set insert data from XML
            foreach ( $data->fields->field as $field ) {
                $field = mncf_admin_import_export_simplexml2array( $field );
            ...
            foreach ( $fields as $field_id => $field ) {
                ...
                if ( $_new_field ) {
                    ...
                    $_checksum = $mncf->import->checksum( 'field',
                ...
                // MNML
                global $iclTranslationManagement;
                if ( !empty( $iclTranslationManagement ) && isset( $field['mnml_action'] ) ) {
                    $iclTranslationManagement->settings['custom_fields_translation'][mncf_types_get_meta_prefix( $field ) . $field_id] = $field['mnml_action'];
                    $iclTranslationManagement->save_settings();
                }
            }
            update_option( 'mncf-fields', $fields_existing );
        }
    }
    ...
    if ( !empty( $data->types ) && 'types' == $_type ) {
        ...
        $types_existing = get_option( MNCF_OPTION_NAME_CUSTOM_TYPES, array() );
        $types = array();
        $types_check = array();
        // Set insert data from XML
        foreach ( $data->types->type as $type ) {
            $type = (array) $type;
            $type = mncf_admin_import_export_simplexml2array( $type );
        ...
        foreach ( $types as $type_id => $type ) {
            ...
            if ( isset( $types_existing[$type_id] ) ) {
                 ...
                 * Compare checksum to see if updated
                 */
                $_checksum = $mncf->import->checksum( 'custom_post_type',
        ...
        update_option( MNCF_OPTION_NAME_CUSTOM_TYPES, $types_existing );

        // Add relationships
        /** EMERSON: Restore Types relationships when importing modules */
        if ( !empty( $data->post_relationships )) {
        	$relationship_existing = get_option( 'mncf_post_relationship', array() );
        	/**
        	...
        	*/
        	...
        	if ( is_array( $relationship ) ) {
        		...
        		update_option( 'mncf_post_relationship', $relationship );
        	}
        }
    }
    ...
    if ( !empty( $data->taxonomies ) && 'taxonomies' == $_type ) {
        ...
        $taxonomies_existing = get_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, array() );
        $taxonomies = array();
        $taxonomies_check = array();
        // Set insert data from XML
        foreach ( $data->taxonomies->taxonomy as $taxonomy ) {
            // TODO 1.2.1 Remove
//            $_id = mncf_modman_get_submitted_id( _TAX_MODULE_MANAGER_KEY_,
//                    $taxonomy['__types_id'] );
            ...
            $taxonomy = mncf_admin_import_export_simplexml2array( $taxonomy );
			$taxonomy = apply_filters( 'mncf_filter_import_custom_taxonomy', $taxonomy );
        ...
        foreach ( $taxonomies as $taxonomy_id => $taxonomy ) {
            ...
            if ( isset( $taxonomies_existing[$taxonomy_id] ) ) {
                 ...
                 * Compare checksum to see if updated
                 */
                $_checksum = $mncf->import->checksum( 'custom_taxonomy',
        ...
        update_option( MNCF_OPTION_NAME_CUSTOM_TAXONOMIES, $taxonomies_existing );
    }

    if ( $imported ) {
        // MNML bulk registration
        // TODO MNML move
        if ( mncf_get_settings( 'register_translations_on_import' ) ) {
            mncf_admin_bulk_string_translation();
        }

        // Flush rewrite rules
        mncf_init_custom_types_taxonomies();
        flush_rewrite_rules();
    }
 ...
 * @param type $items
 */
function mncf_modman_items_check_custom_post_types( $items ) {

    global $mncf;

    foreach ( $items as $k => $item ) {
        $item['exists'] = $mncf->import->item_exists( 'custom_post_type',
                $item['id'] );
        if ( $item['exists'] && isset( $item['hash'] ) ) {
            $item['is_different'] = $mncf->import->checksum( 'custom_post_type',
 ...
 * @param type $items
 */
function mncf_modman_items_check_groups( $items ) {
    ...
    if ( !empty( $items['__fields'] ) ) {
        foreach ( $items['__fields'] as $k => $item ) {
            ...
            $_item['title'] = sprintf( __( 'Field: %s', 'mncf' ), $item['title'] );
            $_item['exists'] = $mncf->import->item_exists( 'field', $item['id'] );
            if ( $_item['exists'] && isset( $item['hash'] ) ) {
                $_item['is_different'] = $mncf->import->checksum( 'field',
                                $item['id'], $item['hash'] ) ? false : true;
            }
            $_fields[] = $_item;
        }
    ...
    foreach ( $items as $k => $item ) {
        ...
        $_item['exists'] = $mncf->import->item_exists( 'group', $item['id'] );
        if ( $_item['exists'] && isset( $item['hash'] ) ) {
            $_item['is_different'] = $mncf->import->checksum( 'group',
                            $item['id'], $item['hash'] ) ? false : true;
        }
        $_items[] = $_item;
    }
 ...
 * @param type $items
 */
function mncf_modman_items_check_taxonomies( $items ) {

    global $mncf;

    foreach ( $items as $k => $item ) {
        $item['exists'] = $mncf->import->item_exists( 'custom_taxonomy',
                $item['id'] );
        if ( $item['exists'] && isset( $item['hash'] ) ) {
            $item['is_different'] = $mncf->import->checksum( 'custom_taxonomy',
                            $item['id'], $item['hash'] ) ? false : true;
        }
        $items[$k] = $item;
    }
 ...
 * @return type
 */
function mncf_modman_get_submitted_id( $set, $item ) {
 ...
 * @return type
 */
function mncf_modman_set_submitted_id( $set, $id ) {
    return '12' . $set . '21' . $id;
}


add_filter( 'mncf_filter_export_custom_taxonomy', 'mncf_fix_exported_taxonomy_assignment_to_cpt' );
 ...
 * @since unknown
 */
function mncf_fix_exported_taxonomy_assignment_to_cpt( $taxonomy_data = array() ) {
	...
	$supported_post_types = mncf_ensarr( mncf_getarr( $taxonomy_data, 'supports' ) );
	...
	// in the samve way.
	$legacy_supported_post_type_array = mncf_ensarr( mncf_getarr( $taxonomy_data, 'object_type' ) );
