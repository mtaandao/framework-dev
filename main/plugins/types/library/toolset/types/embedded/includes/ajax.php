 ...
 * @todo auth
 */
function mncf_ajax_embedded() {


    if ( isset( $_REQUEST['_typesnonce'] ) ) {
        if ( !mn_verify_nonce( $_REQUEST['_typesnonce'], '_typesnonce' ) ) {
        ...
        if (
            ...
            || !mn_verify_nonce( $_REQUEST['_mnnonce'], $_REQUEST['mncf_action'] ) 
        ) {
    ...
    switch ( $_REQUEST['mncf_action'] ) {
        ...
        case 'insert_skype_button':
            ...
            require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields/skype.php';
            mncf_fields_skype_meta_box_ajax();
            break;
        ...
        case 'editor_callback':
            ...
            if ( !empty( $field ) ) {
                // Editor
                MNCF_Loader::loadClass( 'editor' );
                $editor = new MNCF_Editor();
        ...
        case 'pr_add_child_post':
            global $current_user;
            $output = '<tr>' . __( 'Passed wrong parameters', 'mncf' ) . '</tr>';
			$id = 0;
			...
			if ( ! $has_permissions ) {
				$output = '<tr><td>' . __( 'You do not have rights to create new items', 'mncf' ) . '</td></tr>';
			} else if ( 
                ...
                && isset( $_GET['post_type_parent'] ) )
            {
                ...
                if ( !empty( $parent_post->ID ) ) {
                     ...
                     * CHECKPOINT
                     */
                    $id = $mncf->relationship->add_new_child( $parent_post->ID, $post_type );
                    ...
                    if ( is_mn_error( $id ) ) {
                        ...
                        if ( !empty( $parent->ID ) && !empty( $child->ID ) ) {
                            ...
                            // Set relationship :)
                            $mncf->relationship->_set( $parent, $child, $data );

                            // Render new row
                            $output = $mncf->relationship->child_row( $parent_post->ID, $id, $data );
                        } else {
                            $output = '<tr>' . __( 'Error creating post relationship', 'mncf' ) . '</tr>';
                        }
                    }
                } else {
                    $output = '<tr>' . __( 'Error getting parent post', 'mncf' ) . '</tr>';
                }
            }
            if ( !defined( 'MNTOOLSET_FORMS_VERSION' ) ) {
                echo json_encode( array(
                    'output' => $output . mncf_form_render_js_validation( '#post', false ),
                    'child_id' => $id,
                ) );
            } else {
                echo json_encode( array(
                    'output' => $output,
                    'conditionals' => array('#post' => mntoolset_form_get_conditional_data( 'post' )),
                    'child_id' => $id,
                ) );
            }
            break;
        ...
        case 'pr_save_all':
            ...
            if ( current_user_can( 'edit_posts' ) && isset( $_POST['post_id'] ) ) {
                ...
                if ( isset( $_POST['mncf_post_relationship'][$parent_id] ) ) {

                    $children = mncf_sanitize_post_realtionship_input( (array) $_POST['mncf_post_relationship'][$parent_id] );

                    $mncf->relationship->save_children( $parent_id, $children );
                    $output = $mncf->relationship->child_meta_form(
            ...
            if ( !defined( 'MNTOOLSET_FORMS_VERSION' ) ) {
                ...
                $output .= '<script type="text/javascript">mncfConditionalInit();</script>';
            }
            mncf_show_admin_messages('echo');
            $errors = ob_get_clean();
            if ( !defined( 'MNTOOLSET_FORMS_VERSION' ) ) {
                echo json_encode( array(
                    'output' => $output,
        ...
        case 'pr_save_child_post':
            ...
            if ( current_user_can( 'edit_posts' ) && isset( $_GET['post_id'] )
                    ...
                    && isset( $_GET['post_type_parent'] )
                    && isset( $_GET['post_type_child'] )
                    && isset( $_POST['mncf_post_relationship'] ) ) {
                ...
                if ( isset( $_POST['mncf_post_relationship'][$parent_id][$child_id] ) ) {
                    $fields = mncf_sanitize_post_relationship_input_fields( (array) $_POST['mncf_post_relationship'][$parent_id][$child_id] );
                    $mncf->relationship->save_child( $parent_id, $child_id, $fields );

                    $output = $mncf->relationship->child_row(
                            $parent_id,
                            $child_id,
                            $mncf->relationship->settings( $parent_post_type, $child_post_type ) );

                    if ( !defined( 'MNTOOLSET_FORMS_VERSION' ) ) {
                        // TODO Move to conditional
                        $output .= '<script type="text/javascript">mncfConditionalInit(\'#types-child-row-' . $child_id . '\');</script>';
                    }
                }
            }
            mncf_show_admin_messages('echo');
        ...
        case 'pr_delete_child_post':
            ...
            if ( current_user_can( 'edit_posts' ) && isset( $_GET['post_id'] ) ) {
                $output = mncf_pr_admin_delete_child_item( intval( $_GET['post_id'] ) );
            }
            echo json_encode( array(
                'output' => $output,
            ) );
        ...
        case 'pr_pagination':
            ...
            if ( current_user_can( 'edit_posts' ) && isset( $_GET['post_id'] ) && isset( $_GET['post_type'] ) ) {
                ...
                if ( !empty( $parent->ID ) ) {
                    ...
                    // Save items_per_page
                    $mncf->relationship->save_items_per_page(
                            $parent->post_type, $child_post_type,
                            intval( $_GET[$mncf->relationship->items_per_page_option_name] )
                    );
        ...
        case 'um_repetitive_add':
            ...
            if ( isset( $_GET['field_id'] )
                ...
                $field = mncf_admin_fields_get_field( sanitize_text_field( $_GET['field_id'] ), false,
                 ...
                 * Force empty values!
                 */
                $mncf->usermeta_repeater->cf['value'] = null;
                $mncf->usermeta_repeater->meta = null;
                $form = $mncf->usermeta_repeater->get_field_form( null, true );

                echo json_encode( array(
                    'output' => mncf_form_simple( $form )
                    . mncf_form_render_js_validation( '#your-profile', false ),
                ) );
            } else {
                echo json_encode( array(
        ...
        case 'um_repetitive_delete':
                ...
                && current_user_can( 'edit_user', intval( $_POST['user_id'] ) ) )
            {
                ...
                $field = mncf_admin_fields_get_field( sanitize_text_field( $_POST['field_id'] ), false,
                        false, false, 'mncf-usermeta' );
                ...
                if ( !empty( $field ) && !empty( $user_id ) && !empty( $meta_id ) ) {
                    ...
                    global $mncf;
                    $mncf->usermeta_repeater->set( $user_id, $field );
                    $mncf->usermeta_repeater->delete( $meta_id );
        ...
        case 'repetitive_add':
            if ( current_user_can( 'edit_posts' ) && isset( $_GET['field_id'] ) ) {
                require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields.php';
                require_once MNCF_EMBEDDED_INC_ABSPATH . '/fields-post.php';
                $field = mncf_admin_fields_get_field( sanitize_text_field( $_GET['field_id'] ) );
                $parent_post_id = intval( $_GET['post_id'] );
                ...
                global $mncf;
                $mncf->repeater->set( $parent_post, $field );
                /*
                 * 
                 * Force empty values!
                 */
                $mncf->repeater->cf['value'] = null;
                $mncf->repeater->meta = null;
                $form = $mncf->repeater->get_field_form( null, true );

                echo json_encode( array(
                    'output' => mncf_form_simple( $form )
                    . mncf_form_render_js_validation( '#post', false ),
                ) );
            } else {
                echo json_encode( array(
                    'output' => 'params missing',
        ...
        case 'repetitive_delete':
            if ( current_user_can( 'edit_posts' ) && isset( $_POST['post_id'] ) && isset( $_POST['field_id'] ) ) {
                ...
                $field = mncf_admin_fields_get_field( sanitize_text_field( $_POST['field_id'] ) );
                $meta_id = intval( $_POST['meta_id'] );
 ...
 *
 * @param array $children_raw See MNCF_Relationship::save_children().
 * @return array Data with the same structure as input, but sanitized.
 *
 * @todo since
 * @todo move to better location if such exists
 */
function mncf_sanitize_post_realtionship_input( $children_raw ) {
    $children = array();
    foreach( $children_raw as $child_id_raw => $child_fields_raw ) {
        $child_id = intval( $child_id_raw );
        $children[ $child_id ] = mncf_sanitize_post_relationship_input_fields( $child_fields_raw );
    }
    return $children;
}
 ...
 *
 * @param array $fields_raw See MNCF_Relationship::save_child().
 * @return array Data with the same structure as input, but sanitized.
 *
 * @todo since
 * @todo move to better location if such exists
 */
function mncf_sanitize_post_relationship_input_fields( $fields_raw ) {
    $fields = array();
