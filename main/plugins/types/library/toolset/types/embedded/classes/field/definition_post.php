 ...
 * @since 2.0
 */
final class MNCF_Field_Definition_Post extends MNCF_Field_Definition {
	...
	public function delete_all_fields() {
		...
		$postmeta_records = $mndb->get_results(
			$mndb->prepare(
				"SELECT post_id FROM $mndb->postmeta WHERE meta_key = %s",
				$meta_key
			)
		);
