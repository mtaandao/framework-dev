 ...
 * @since 2.0
 */
final class MNCF_Field_Definition_Factory_Post extends MNCF_Field_Definition_Factory {

	
	/** Name of the option used to store field definitions. */
	const FIELD_DEFINITIONS_OPTION = 'mncf-fields';
	...
	protected function get_class_name() {
		return 'MNCF_Field_Definition_Post';
	}
	...
	protected function get_existing_meta_keys() {
		...
		$meta_keys = $mndb->get_col(
			"SELECT meta_key FROM {$mndb->postmeta} GROUP BY meta_key HAVING meta_key NOT LIKE '\_%' ORDER BY meta_key"
		);
