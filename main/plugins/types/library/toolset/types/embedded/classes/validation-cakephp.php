...
class Wpcf_Cake_Validation
{
    ...
    function comparison( $check1, $operator = null, $check2 = null ) {
        ...
        switch ( $operator ) {
            ...
            default:
                $_this = &Wpcf_Cake_Validation::getInstance();
                $_this->errors[] = __( 'You must define the $operator parameter for Wpcf_Cake_Validation::comparison()', 'mncf' );
                break;
        }
        return false;
    }
    ...
    function custom( $check, $regex = null ) {
        ...
        if ( $_this->regex === null ) {
            $_this->errors[] = __( 'You must define a regular expression for Wpcf_Cake_Validation::custom()', 'mncf' );
            return false;
        }
        return $_this->_check();
    }
    ...
    function _pass( $method, $check, $classPrefix ) {
        ...
        if ( !class_exists( $className ) ) {
            trigger_error( sprintf( __( 'Could not find %s class, unable to complete validation.', 'mncf' ), $className ), E_USER_WARNING );
            return false;
        }
        if ( !is_callable( array($className, $method) ) ) {
            trigger_error( sprintf( __( 'Method %s does not exist on %s unable to complete validation.', 'mncf' ), $method, $className ),
                    E_USER_WARNING );
            return false;
        }
        $check = (array) $check;
