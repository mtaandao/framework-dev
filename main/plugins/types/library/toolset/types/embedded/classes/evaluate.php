        ...
        if ( empty( $post->ID ) ) {
            ...
            if ( $o->context == 'relationship' ) {
                add_filter( 'get_post_metadata',
                        array('MNCF_Evaluate', 'relationship_custom_statement_meta_ajax_validation_filter'),
                        10, 4 );
            } else {
                add_filter( 'get_post_metadata',
                        array('MNCF_Evaluate', 'meta_ajax_validation_filter'),
                        10, 4 );
            }
            ...
            if ( empty( $matches ) ) {
                ...
                foreach ( $matches[1] as $field_name ) {
                     ...
                     */
                    $f = mncf_admin_fields_get_field( trim( strval( $field_name ) ) );
                    if ( empty( $f ) ) {
                        return false;
                    }
                ...
                $check = mnv_condition( $fields, $post );
            ...
            foreach ( $field['data']['conditional_display']['conditions'] as $condition ) {
                 ...
                 * This field value is checked
                 */
                $f = mncf_admin_fields_get_field( trim( strval( $condition['field'] ) ) );
                if ( empty( $f ) ) {
                    return false;
                }
                 ...
                 * 
                 * Since Types 1.2
                 * meta is property of MNCF_Field::$__meta
                 * 
                 * BREAKPOINT
...
//                $value = defined( 'DOING_AJAX' ) ? $c->_get_meta( 'POST' ) : $c->__meta;
                 ...
                 * Apply filters
                 */
                $value = apply_filters( 'mncf_conditional_display_compare_meta_value',
                        $value, $c->cf['id'], $condition['operation'], $c->slug,
                        $post );
                $condition['value'] = apply_filters( 'mncf_conditional_display_compare_condition_value',
                ...
                $passed = mncf_cd_admin_compare( $condition['operation'],
    ...
    public static function relationship_custom_statement_meta_ajax_validation_filter( $null,
        ...
        $value = $mncf->relationship->get_submitted_data(
                $mncf->relationship->parent->ID, $mncf->relationship->child->ID,
                $meta_key );
