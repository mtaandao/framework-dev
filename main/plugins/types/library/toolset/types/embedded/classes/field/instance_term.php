 ...
 * @since 1.9
 */
class MNCF_Field_Instance_Term extends MNCF_Field_Instance {
	...
	private function filter_single_value_before_saving( $original_value ) {
		...
		$value = apply_filters( 'mncf_fields_termmeta_value_save', $original_value, $field_definition_array['type'], $definition, $field_definition_array, null );
		$value = apply_filters( 'mncf_fields_value_save', $value, $field_definition_array['type'], $definition->get_slug(), null );
		$value = apply_filters( 'mncf_fields_slug_' . $definition->get_slug() . '_value_save', $value, $field_definition_array, null );
		$value = apply_filters( 'mncf_fields_type_' . $field_definition_array['type'] . '_value_save', $value, $field_definition_array, null );
	...
	private function actions_after_single_value_saved( $value, $value_unfiltered, $meta_id ) {
		 ...
		 * @param mixed $ignored Sometimes it may be MNCF_Field and sometimes null. Don't rely in this at all.
		 ...
		 *     mncf_fields_value_save & co.
		 * @since unknown
		 */
		do_action( 'mncf_fields_save', $value, $field_definition_array, null, $meta_id, $value_unfiltered );
		do_action( 'mncf_fields_slug_' . $definition->get_slug() . '_save', $value, $field_definition_array, null, $meta_id, $value_unfiltered );
		do_action( 'mncf_fields_type_' . $field_definition_array['type'] . '_save', $value, $field_definition_array, null, $meta_id, $value_unfiltered );
	...
	public function update_all_values( $values ) {
		...
		if( ! $this->get_definition()->get_is_repetitive() ) {
			...
			// for MNML compatibility).
			do_action(
				'mncf_termmeta_before_adding_repetitive_field_values',
				$this->get_object_id(),
			...
			foreach( $values as $value ) {
				...
				if ( $total_value_count == $current_value_number ) {
					...
					do_action(
						'mncf_termmeta_before_adding_last_repetitive_field_value',
						$this->get_object_id(),
						$this->get_definition()->get_definition_array(),
						$this
					);
			...
			do_action(
				'mncf_termmeta_after_adding_repetitive_field_values',
				$this->get_object_id(),
				$this->get_definition()->get_definition_array(),
				$this
			);
	...
	/**
	 * @return MNCF_Field_Accessor_Abstract An accessor to get the sort order for repetitive fields.
	 */
	protected function get_order_accessor() {
		return new MNCF_Field_Accessor_Termmeta( $this->get_object_id(), $this->get_order_meta_name(), false );
	}
