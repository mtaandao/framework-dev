 ...
 * @since 1.9.1
 */
class MNCF_Field_Option_Checkboxes {
	...
	/**
	 * MNCF_Field_Value_Option constructor.
	 *
	...
	public function is_option_checked( $field_value ) {
		...
		$option_value = mncf_getnest( $field_value, array( $this->option_id, 0 ), null );
	...
	public function get_label() {
		$value = mncf_getarr( $this->config, 'title' );
		if( !is_string( $value ) ) {
			$value = '';
		}
	...
	public function get_display_value( $is_checked = true ) {
		$display_mode = mncf_getarr( $this->config, 'display', 'db', array( 'value', 'db' ) );
		if( 'db' == $display_mode ) {
			return ( $is_checked ? $this->get_value_to_store() : '' );
		} else {
			if ( $is_checked ) {
				return mncf_getarr( $this->config, 'display_value_selected' );
			} else {
				return mncf_getarr( $this->config, 'display_value_not_selected' );
			}
		}
	}
	...
	public function get_value_to_store() {
		$value = mncf_getarr( $this->config, 'set_value' );
		if( !is_string( $value ) ) {
			return '';
		}
