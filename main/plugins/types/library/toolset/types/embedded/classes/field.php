 ...
 * // Setup field
 * global $mncf;
 * $my_field = new MNCF_Field();
 * $my_field->set($mncf->post, mncf_admin_fields_get_field('image'));
 *
 * // Use it
 * $my_field->save();
 *
 * // Generic instance can be found in global $mncf.
 * global $mncf;
 * $mncf->field->set(...);
 *
...
class MNCF_Field
{
    ...
    function __construct( $config = array() ) {
        // Parse args
        extract( mn_parse_args( (array) $config, $this->config ) );
    }
    ...
    function set( $post, $cf ) {
        ...
        if ( is_string( $cf ) ) {
            ...
            $_cf = mncf_admin_fields_get_field( $this->__get_slug_no_prefix( $cf ) );
            // Check if found without prefix
            if ( empty( $_cf ) ) {
                $_cf = mncf_admin_fields_get_field( $cf );
            }
            if ( empty( $_cf ) ) {
                /*
    ...
    function _get_meta() {
        ...
        if ( $this->use_cache ) {
			if ( false != $cached_object && is_array( $cached_object ) && isset( $cached_object[0] ) ) {// Mtaandao cache
				...
				$all_postmeta = $mndb->get_results( $mndb->prepare( "SELECT * FROM {$mndb->postmeta} WHERE post_id=%d", $this->post->ID), OBJECT );
    ...
    function get_submitted_data() {
        $posted = isset( $_POST['mncf'][$this->cf['slug']] ) ? $_POST['mncf'][$this->cf['slug']] : null;
    ...
    function save( $value = null )
    {
         ...
         *
         */
        do_action('mncf_postmeta_before_delete', $this->post, $this->cf);
        delete_post_meta( $this->post->ID, $this->slug );
        do_action('mncf_postmeta_after_delete', $this->post, $this->cf);
        // Trim
        if ( is_string( $value ) ) {
            $value = trim( $value );
        }
    ...
    {
        // Apply filters
        $value = apply_filters( 'mncf_fields_value_save', $value, $this->cf['type'], $this->cf['slug'], $this->cf, $this );
        $value = apply_filters( 'mncf_fields_slug_' . $this->cf['slug'] . '_value_save', $value, $this->cf, $this );
        $value = apply_filters( 'mncf_fields_type_' . $this->cf['type'] . '_value_save', $value, $this->cf, $this );
    ...
    {
        $value = apply_filters( 'mncf_fields_postmeta_value_save', $value, $this->cf['type'], $this->cf['slug'], $this->cf, $this );
        return $value;
    }

    function _filter_save_usermeta_value( $value )
    {
        $value = apply_filters( 'mncf_fields_usermeta_value_save', $value, $this->cf['type'], $this->cf['slug'], $this->cf, $this );
        return $value;
    }
	
	function _filter_save_termmeta_value( $value )
    {
        $value = apply_filters( 'mncf_fields_termmeta_value_save', $value, $this->cf['type'], $this->cf['slug'], $this->cf, $this );
        return $value;
    }
    ...
    {
        do_action( 'mncf_fields_save', $value, $field, $this, $meta_id, $meta_value_original );
        do_action( 'mncf_fields_slug_' . $field['slug'] . '_save', $value, $field, $this, $meta_id, $meta_value_original );
        do_action( 'mncf_fields_type_' . $field['type'] . '_save', $value, $field, $this, $meta_id, $meta_value_original );
    }
    ...
    {
        $file = mncf_get_fullpath_by_field_type($basename);
        if ( file_exists( $file ) ) {
            include_once $file;
        }
        return $file;
    ...
    function _get_config() {
        $this->_include_file_by_field_type($this->cf['type']);
        $func = 'mncf_fields_' . $this->cf['type'];
        if ( is_callable( $func ) ) {
    ...
    function _get_meta_form( $meta_value = null, $meta_id = null, $wrap = true ) {
        ...
        if ( $wrap ) {
            $form['__meta_form_OPEN'] = array(
                '#type' => 'markup',
                '#markup' => ''
                . '<div id="mncf_wrapper_' . $this->unique_id
                . '" class="mncf-wrap mncf-meta-form">',
            );
        }
...
//                        'Since Types 1.2 we encourage developers to completely define fields' );
            ...
            if ( file_exists( $file ) ) {
                ...
                if ( function_exists( 'mncf_fields_'
                                . $this->config->inherited_field_type
                                . '_meta_box_form' ) ) {
                    $inherited = call_user_func_array( 'mncf_fields_'
                            . $this->config->inherited_field_type
                            . '_meta_box_form', array($this->cf, $this) );
                    // If single form - convert to array of elements
                    if ( isset( $inherited['#type'] ) ) {
                        $inherited = array('mncf_field_' . $this->unique_id => $inherited);
                    }
        ...
        $func = 'mncf_fields_' . $this->cf['type'] . '_meta_box_form';
        if ( is_callable( $func ) ) {
            /*
             *
             * From Types 1.2 use complete form setup
             */
            $form_meta_box = call_user_func_array( 'mncf_fields_'
                    . $this->cf['type'] . '_meta_box_form',
                    array($this->cf, $this) );

            // If single form - convert to array of elements
            if ( isset( $form_meta_box['#type'] ) ) {
                $form_meta_box = array('mncf_field_' . $this->unique_id => $form_meta_box);
            }
        ...
        if ( !empty( $form ) ) {
            ...
            foreach ( $form as $element_key => $element ) {
                ...
                if ( empty( $started ) ) {
                    ...
                    $element['#title'] = mncf_translate( 'field '
                    ...
                    $element['#description'] = mnautop( mncf_translate( 'field '
                ...
                // Process field
                $temp_processed = mncf_admin_post_process_field( $this );
                if ( !empty( $temp_processed['element'] ) ) {
                    $element = $temp_processed['element'];
                }

                // Set form element
                $form[$element_key] = apply_filters( 'mncf_post_edit_field', $element, $this->cf, $this->post, $this->context );
            }

            // Add to editor
            if ( $this->add_to_editor ) {
                mncf_admin_post_add_to_editor( $this->cf );
            }
        ...
        return apply_filters( 'mncf_meta_form', $_form );
    }
    ...
    function enqueue_script( $config = null ) {
        ...
        if ( !empty( $config->meta_box_js ) ) {
            foreach ( $config->meta_box_js as $handle => $data_script ) {
                ...
                mn_enqueue_script( $handle, $data_script['src'], $deps, $ver, $in_footer);
            }
        }
    }
    ...
    function is_under_control( $field_key ) {
        ...
        return MNCF_Fields::isUnderControl( $key );
    }
    ...
    function __get_slug_no_prefix( $meta_key ) {
        return strpos( $meta_key, MNCF_META_PREFIX ) === 0 ? substr( $meta_key,
                        strlen( MNCF_META_PREFIX ) ) : $meta_key;
    }
     ...
     * mncf_post_relationship[214][mncf-my-checkbox]
     *
     * Or if multi array
     * mncf_post_relationship[214][mncf-my-date][datepicker]
     * mncf_post_relationship[214][mncf-my-date][hour]
     *
     * @param type $prefix
     * @param type $name
     * @return type
...
class MNCF_Termmeta_Field extends MNCF_Field
{
    ...
    function _get_meta() {
        ...
        if ( $this->use_cache ) {
			if ( false != $cached_object && is_array( $cached_object ) && isset( $cached_object[0] ) ) {// Mtaandao cache
				...
				$all_termmeta = $mndb->get_results( $mndb->prepare( "SELECT * FROM {$mndb->termmeta} WHERE term_id=%d", $this->term_id), OBJECT );
