...
class MNCF_Import_Export
{
     ...
     * @var type 
     */
    var $_remove_data_keys = array('id', 'ID', 'menu_icon', 'mnml_action',
        'mncf-post-type', 'mncf-tax', 'hash', 'checksum');
    ...
    function get_group_checksum_data( $group_id ) {
        ...
        $checksum = array();
        $group = mncf_admin_fields_get_group( $group_id );
    ...
    function generate_checksum( $type, $item_id = null ) {
        switch ( $type ) {
            ...
            case 'field':
                $checksum = mncf_admin_fields_get_field( $item_id );
                ksort( $checksum, SORT_STRING );
                break;

            case 'custom_post_type':
                $checksum = mncf_get_custom_post_type_settings( $item_id );

                break;

            case 'custom_taxonomy':
                $checksum = mncf_get_custom_taxonomy_settings( $item_id );
                break;
    ...
    function item_exists( $type, $item_id ) {
        switch ( $type ) {
            case 'group':
                $check = mncf_admin_fields_get_group( $item_id );
                break;

            case 'field':
                $check = mncf_admin_fields_get_field( $item_id );
                break;

            case 'custom_post_type':
                $check = mncf_get_custom_post_type_settings( $item_id );
                break;

            case 'custom_taxonomy':
                $check = mncf_get_custom_taxonomy_settings( $item_id );
                break;
