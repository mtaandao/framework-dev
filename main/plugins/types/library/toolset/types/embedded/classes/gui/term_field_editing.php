 ...
 * @since 1.9
 */
final class MNCF_GUI_Term_Field_Editing {
	 ...
	 *
	 * @return MNCF_GUI_Term_Field_Editing
	 * @since 2.1
	 */
	...
	private function add_hooks() {
		...
		$is_term_listing_page = ( $main_controller->is_admin() && 'edit' != mncf_getget( 'action' ) );
	 ...
	 *
	 * @link https://developer.mtaandao.org/reference/hooks/taxonomy_edit_form_fields/
	 *
	 * @param MN_Term $term Term that is being edited.
	...
	public function on_term_update( $term_id, $tt_id ) {
		...
		if( !empty( $update_errors ) ) {
			foreach( $update_errors as $update_error ) {
				mncf_admin_message_store( $update_error->get_error_message(), 'error' );
			}
			mncf_admin_message_store(
				sprintf(
					'<strong>%s</strong>',
					__( 'There has been a problem while saving custom fields. Please fix it and try again.', 'mncf' )
				),
				'error'
			);
		}
	 ...
	 *
	 * @param int $term_id
	 * @param MNCF_Field_Definition[] $field_definitions
	 * @return MN_Error[]
	 */
	private function update_term_fields( $term_id, $field_definitions ) {
		...
		return $this->filter_mn_errors_flat( $update_results );
	}


	/**
	 * From an array that can contain booleans, MN_Error and arrays of MN_Error, create an array containing all
	 * MN_Error instances only.
	 *
	 * @param array $update_results
	 * @return MN_Error[]
	 */
	private function filter_mn_errors_flat( $update_results ) {
		$errors = array();
		foreach( $update_results as $update_result ) {
			if( $update_result instanceof MN_Error ) {
				$errors[] = $update_result;
			} else if( is_array( $update_result ) ) {
				foreach( $update_result as $error ) {
					if( $error instanceof MN_Error ) {
						$errors[] = $error;
					}
				}
			}
	 ...
	 * @param int $term_id
	 *
	 * @return MN_Error|MN_Error[]|true
	 */
	private function update_single_field( $field_definition, $term_id ) {
		$field = new MNCF_Field_Instance_Term( $field_definition, $term_id );
		$saver = new MNCF_Field_Data_Saver( $field, self::EDIT_FORM_ID );
		...
		foreach( $validation_results as $index => $validation_result ) {
			...
			if( $validation_result instanceof MN_Error ) {
				$error_message = sprintf( '%s %s',
					sprintf( __( 'Field "%s" not updated:', 'mncf' ), $field_definition->get_name() ),
					implode( ', ', $validation_result->get_error_data() )
				);
				$errors[] = new MN_Error( 'mncf_field_not_updated', $error_message );
			}
		}
	...
	private function add_toolset_forms_support() {
		...
		// Extra enqueuing of media assets needed since MNToolset_Field_File doesn't know about termmeta.
		mn_enqueue_media();
		...
		// This will load some assets needed for image field preview (specifically the 'mntoolset-forms-admin' style).
		// Hacky, but better than re-registering the toolset-forms stylesheet elsewhere.
		$faux_form_bootstrap = new MNToolset_Forms_Bootstrap();
	...
	public function render_js_validation_data() {
		mncf_form_render_js_validation( '.validate' );
	}
	...
	private function render_field_group_edit_page( $field_group, $term_id ) {
		...
		/** @var MNCF_Field_Definition_Term $field_definition */
		foreach( $field_definitions as $field_definition ) {
			printf(
	...
	private function render_field_group_add_page( $field_group, $term_id ) {
		...
		foreach( $field_definitions as $field_definition ) {
			printf(
				'<div class="form-field mncf-add-term-form-field">%s</div>',
	 ...
	 *
	 * @param MNCF_Field_Definition_Term $field_definition
	...
	private function get_toolset_forms_field( $field_definition, $form_id, $term_id, $hide_field_title ) {
		...
		if( null == $term_id ) {
			$field = new MNCF_Field_Instance_Unsaved( $field_definition );
		} else {
			$field = new MNCF_Field_Instance_Term( $field_definition, $term_id );
		}

		$tf_renderer = new MNCF_Field_Renderer_Toolset_Forms( $field, $form_id );
	...
	const LISTING_COLUMN_PREFIX = 'mncf_field_';
	 ...
	 * @link https://make.mtaandao.org/docs/plugin-developer-handbook/10-plugin-components/custom-list-table-columns/
	 * @since 1.9.1
	 */
	public function manage_term_listing_columns( $columns ) {

		$factory = Types_Field_Group_Term_Factory::get_instance();
		$taxonomy_slug = sanitize_text_field( mncf_getget( 'taxonomy' ) );
		$groups = $factory->get_groups_by_taxonomy( $taxonomy_slug );
	...
	public function manage_term_listing_cell( $value, $column_name, $term_id ) {
		...
		if( $this->is_term_field_column( $column_name ) ) {
			...
			try {
				...
				$field_definition = MNCF_Field_Definition_Factory_Term::get_instance()->load_field_definition( $field_slug );
				$field = new MNCF_Field_Instance_Term( $field_definition, $term_id );
				...
				$renderer = MNCF_Field_Renderer_Factory::get_instance()->create_preview_renderer( $field, $renderer_args );
	...
	public function filter_hidden_columns( $columns, $screen, $use_defaults ) {
		 ...
		 * @param MN_Screen $screen Current screen object.
	...
	private function annotate_last_displayed_column( $column_name ) {
		...
		mn_localize_script(
			...
			array(
				...
				'annotation' => sprintf(
					' <i class="js-mncf-tooltip mncf-tooltip dashicons dashicons-editor-help" data-tooltip="%s"></i>',
					__( 'Some columns with term fields have been automatically hidden. You can control their appearance through the Screen options (on top right of the page).', 'mncf' )
				)
			)
		);
