 ...
 *
 * Tries to be clever about MN attachments and use thumbnails instead of full size images whenever possible.
 *
 * For displaying the full image on clicking on the preview, you need to enqueue the 'mncf-js' script and
 * the 'mntoolset-forms-admin' style
 *
 * @since 1.9.1
 */
final class MNCF_Field_Renderer_Preview_Image extends MNCF_Field_Renderer_Preview_Base {
	...
	protected function render_single( $value ) {
		...
		$output = sprintf(
			'<div class="js-mnt-file-preview mnt-file-preview">
	...
	protected function try_finding_attachment( $url ) {
		...
		$attachment_src = mn_get_attachment_image_src( $attachment_id, 'thumbnail' );
	...
	protected function get_maximum_image_size() {
		return mncf_getarr( $this->args, 'maximum_image_size', self::DEFAULT_MAX_IMAGE_SIZE );
	}
