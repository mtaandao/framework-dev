...
class MNCF_Relationship
{
    ...
    var $items_per_page_option_name = '_mncf_relationship_items_per_page';
    var $child_form = null;
    ...
    function __construct()
    {
        ...
        $this->settings = get_option( 'mncf_post_relationship', array() );
        add_action( 'mn_ajax_add-types_reltax_add',
    ...
    function child_meta_form($parent, $post_type)
    {
        ...
        $this->child_form = new MNCF_Relationship_Child_Form(
                        $parent,
                        $post_type,
    ...
    function child_row($parent_id, $child_id)
    {
        ...
        if ( empty( $parent ) || empty( $child ) ) {
            return new MN_Error( 'mncf-relationship-save-child', 'no parent/child post' );
        }
        $output = '';
     ...
     * @param type $parent
     * @param type $child
     * @return \MNCF_Relationship_Child_Form
     */
    function _get_child_form($parent, $child)
    {
        require_once dirname( __FILE__ ) . '/relationship/form-child.php';
        return new MNCF_Relationship_Child_Form(
                        $parent,
                        $child->post_type,
    ...
    function save_child( $parent_id, $child_id, $save_fields = array() )
    {
        ...
        update_post_meta( $child->ID,
                '_mncf_belongs_' . $parent->post_type . '_id', $parent->ID );

        // Check if added via AJAX
        $check = get_post_meta( $child->ID, '_mncf_relationship_new', true );
        $new = !empty( $check );
        delete_post_meta( $child->ID, '_mncf_relationship_new' );

        // Set post data
        $post_data['ID'] = $child->ID;

        // Title needs to be checked if submitted at all
        if ( !isset( $save_fields['_mn_title'] ) ) {
            ...
            $post_title = $save_fields['_mn_title'];
        }


        $post_data['post_title'] = $post_title;
        $post_data['post_content'] = isset( $save_fields['_mn_body'] ) ? $save_fields['_mn_body'] : $child->post_content;
        $post_data['post_excerpt'] = isset( $save_fields['_mn_excerpt'] ) ? $save_fields['_mn_excerpt'] : $child->post_excerpt;
        ...
        $cf = new MNCF_Field;
        if (
            isset( $_POST['mncf_post_relationship'][$parent_id])
            && isset( $_POST['mncf_post_relationship'][$parent_id][$child_id] )
        ) {
            $_POST['mncf'] = array();
            foreach( $_POST['mncf_post_relationship'][$parent_id][$child_id] as $slug => $value ) {
                $_POST['mncf'][$cf->__get_slug_no_prefix( $slug )] = $value;
                $_POST['mncf'][$slug] = $value;
            }
        }
        unset($cf);
        ...
        if ( isset( $_POST['post_ID'] ) ) {
            ...
            $_POST = array();
            foreach( array('mncf_post_relationship', 'post_ID', '_mntoolset_checkbox', 'mncf', '_mnnonce') as $key ) {
        ...
        // Workaround for types-876, see mncf_admin_post_save_post_hook().
	    add_filter( 'types_updating_child_post', '__return_true' );

        $updated_id = mn_update_post( $post_data );
        ...
        if ( empty( $updated_id ) ) {
            return new MN_Error( 'relationship-update-post-failed', 'Updating post failed' );
        }
        ...
        if ( !empty( $save_fields['parents'] ) ) {
            foreach ( $save_fields['parents'] as $parent_post_type => $parent_post_id ) {
                update_post_meta( $child->ID,
                        '_mncf_belongs_' . $parent_post_type . '_id',
                        $parent_post_id );
            }
        }
        ...
        if ( !empty( $save_fields['taxonomies'] ) && is_array( $save_fields['taxonomies'] ) ) {
            ...
            mn_delete_object_term_relationships( $child->ID,
                    array_keys( $save_fields['taxonomies'] ) );
            foreach ( $_save_data as $_taxonomy => $_terms ) {
                mn_set_post_terms( $child->ID, $_terms, $_taxonomy,
                        $append = false );
            }
        }

        // Unset non-types
        unset( $save_fields['_mn_title'], $save_fields['_mn_body'],
        ...
        foreach ( $save_fields as $slug => $value ) {
            if ( defined( 'MNTOOLSET_FORMS_VERSION' ) ) {
                // Get field by slug
                $field = mncf_fields_get_field_by_slug( str_replace( MNCF_META_PREFIX, '', $slug ) );
                if ( empty( $field ) ) {
                    continue;
                }
                // Set config
                $config = mntoolset_form_filter_types_field( $field, $child->ID );
                // Check if valid
                $valid = mntoolset_form_validate_field( 'post', $config, $value );
                if ( is_mn_error( $valid ) ) {
                    $errors = $valid->get_error_data();
                    $msg = sprintf(
                        __( 'Child post "%s" field "%s" not updated:', 'mncf' ),
                        $child->post_title,
                        $field['name']
                    );
                    mncf_admin_message_store( $msg . ' ' . implode( ', ', $errors ), 'error' );
                    continue;
                }
            }
         ...
         * save feature image
         */
        if ( isset( $save_fields['_mn_featured_image']) ) {
            if ( $save_fields['_mn_featured_image'] ) {
                set_post_thumbnail( $updated_id, $save_fields['_mn_featured_image']);
            } else {
        ...
        do_action( 'mncf_relationship_save_child', $child, $parent );
    ...
    function add_new_child($parent_id, $post_type)
    {
        ...
        if ( empty( $parent ) ) {
            return new MN_Error( 'mncf-relationship-no-parent', 'No parent' );
        }
        $new_post = array(
            'post_title' => __('New Child', 'mncf'). ': '.$post_type,
            'post_type' => $post_type,
            'post_status' => 'draft',
        );
        $id = mn_insert_post( $new_post, true );
        /**
         * return mn_error
         */
        if ( is_mn_error( $id ) ) {
         ...
         */
        update_post_meta( $id, '_mncf_relationship_new', 1 );
        /**
         * Save relationship
         */
        update_post_meta( $id, '_mncf_belongs_' . $parent->post_type . '_id', $parent->ID );
        /**
         * Fix title
         */
        $mndb->update(
        ...
        do_action( 'mncf_relationship_add_child', get_post( $id ), $parent );
        mn_cache_flush();
        return $id;
    }
    ...
    function get_submitted_data($parent_id, $child_id, $field)
    {
        ...
        return isset( $_POST['mncf_post_relationship'][$parent_id][$child_id][$_field_slug] ) ? $_POST['mncf_post_relationship'][$parent_id][$child_id][$_field_slug] : null;
    }
    ...
    {
        $parents = array();
        $item_parents = mncf_pr_admin_get_belongs( $child->post_type );
        if ( $item_parents ) {
            foreach ( $item_parents as $post_type => $data ) {

                // Get parent ID
                $meta = mncf_get_post_meta( $child->ID,
                        '_mncf_belongs_' . $post_type . '_id', true );
    ...
    {
        return mncf_get_post_meta( $post_id,
                        '_mncf_belongs_' . $parent_post_type . '_id', true );
    }
    ...
    public function ajaxAddTax()
    {
        if ( isset( $_POST['types_reltax'] ) ) {
            ...
            _mn_ajax_add_hierarchical_term();
        }
        die();
    }
