 ...
 */
class MNCF_Repeater extends MNCF_Field
{
    ...
    function __construct() {
        parent::__construct();
        if ( is_admin() ) {
            mncf_admin_add_js_settings( 'mncf_repetitive_last_warning',
                    __( 'Sorry, can not delete all fields.', 'mncf' ) );
        }
    }
    ...
    function save( $data = null ) {
        ...
        // Delete all fields
        do_action('mncf_postmeta_before_delete_repetitive', $this->post, $this->cf);
        delete_post_meta( $this->post->ID, $this->slug );
        do_action('mncf_postmeta_after_delete_repetitive', $this->post, $this->cf);

        // Allow $data to replace $_POST
        if ( is_null( $data ) && isset( $_POST['mncf'][$this->cf['slug']] ) ) {
            $data = $_POST['mncf'][$this->cf['slug']];
        }

        // Set data
        if ( !empty( $data ) ) {

            do_action('mncf_postmeta_before_add_repetitive', $this->post, $this->cf);
            ...
            foreach ( $data as $meta_value ) {
                ...
                if ( is_array( $meta_value ) && isset( $meta_value['new_value'] ) ) {
                    ...
                    $mncf->debug->deprecated['repetitive_new_value_used'] = 'repetitive_new_value_used';
                }
                ...
                if ( count($data) == $i++ ) {
                    do_action('mncf_postmeta_before_add_last_repetitive', $this->post, $this->cf);
                }
            ...
            do_action('mncf_postmeta_after_add_repetitive', $this->post, $this->cf);
    ...
    function _get_meta() {
        ...
        $cached_object_field = mn_cache_get( $cache_key_field, $cache_group );

        if ( $this->use_cache ) {
			if ( false != $cached_object_field && is_array( $cached_object_field ) ) {// Mtaandao cache
        ...
        // Cache it
        mn_cache_add( $cache_key, $_meta, $cache_group );// Mtaandao cache
    ...
    function get_fields_form() {
        $form = array();
        $form_id = $this->cf['id'];
        $unique_id = mncf_unique_id( serialize( $this->cf ) );
         ...
         * TODO Move this to MNCF_Conditional
         */
        $show = true;
        if ( $mncf->conditional->is_conditional( $this->cf ) ) {
            $mncf->conditional->set( $this->post, $this->cf );
            $show = $mncf->conditional->evaluate();
        }
        ...
        $form[$unique_id . '_repetitive_wrapper_open'] = array(
            ...
            . '" class="mncf-wrap mncf-repetitive-wrapper" style="' . $css_cd . '">',
        );
         ...
         *
         * @see mncf_admin_post_save_post_hook()
         */
        $form[$form_id . '_hidden_mark'] = array(
            '#type' => 'hidden',
            '#name' => '__mncf_repetitive[' . $this->slug . ']',
            '#value' => 1,
        ...
        $form[$form_id . '_repetitive_sortable_open'] = array(
            ...
            '#markup' => '<div id="mncf_'
            . $form_id
            . '_repetitive_sortable_' . mncf_unique_id( serialize( $this->cf ) )
            . '" class="mncf-repetitive-sortable-wrapper">',
        ...
        if ( !defined( 'DOING_AJAX' ) ) {
            $form[$form_id . '_repetitive_ajax_response'] = array(
                '#type' => 'markup',
                '#markup' => '<div class="mncf-repetitive-response"></div>',
            ...
            $form[$unique_id . '_main_description'] = array(
                '#type' => 'markup',
                '#markup' => '<div class="mncf-repetitive-description">'
                . mnautop( stripslashes( strip_tags( $this->description ) ) )
                . '</div>',
            );
            ...
            $form[$form_id . '_repetitive_form'] = array(
                '#type' => 'markup',
                '#markup' => mncf_repetitive_form( $this->cf, $this->post ),
    ...
    function get_field_form( $meta_value = null, $meta_id = null ) {
        ...
        if ( is_null( $meta_value ) ) {
            $key = 'mncf_field_' . mncf_unique_id( md5( $this->index ) . $meta_id );
        } else {
            $key = 'mncf_field_' . md5( maybe_serialize( $meta_value ) . $meta_id );
        }
        /*
         *
         *
...
//        if ( is_null( $meta_value ) || is_array( $meta_value ) ) {
        ...
        $form[$key . '_drag_open'] = array(
            '#type' => 'markup',
            '#markup' => '<div class="mncf-repetitive-drag-and-drop">'
        );

        // Use MNCF_Field::get_field_form()
        ...
        foreach ( $field_form as $k => $field ) {
            ...
            // Apply filters
            $field_form[$k] = apply_filters( 'mncf_repetitive_field', $field,
                    $this->post, $this->cf, $k );
...
//            $_loop = true;
        ...
        $form[$key . '_control_open'] = array(
            '#type' => 'markup',
            '#markup' => '<div class="mncf-repetitive-control">'
        );

        // Drag button
        $form[$key . '_drag_button'] = array(
            '#type' => 'markup',
            '#markup' => mncf_repetitive_drag_button( $this->cf, $this->post ),
        );

        // 'Delete' button
        $form[$key . '_delete_button'] = array(
            '#type' => 'markup',
            '#markup' => mncf_repetitive_delete_button( $this->cf, $this->post,
                    $meta_id ),
        );
        ...
        mncf_admin_add_js_settings( 'mncf_repetitive_count_' . md5( $this->cf['id'] ),
                $this->index );
    ...
    function delete( $meta_id ) {
        ...
        if ( $r === false ) {
            return MN_Error( 'mncf_repeater_delete_field',
...
class MNCF_Termmeta_Repeater extends MNCF_Termmeta_Field
{
    ...
    function _get_meta() {
        ...
        $cached_object_termfield = mn_cache_get( $cache_key_termfield, $cache_group );

        if ( $this->use_cache ) {
			if ( false != $cached_object_termfield && is_array( $cached_object_termfield ) ) {// Mtaandao cache
    ...
    function get_fields_form( $is_profile = '' ) {
        ...
        if ( $is_profile == 1 ) {
            $addTagHeaderStart = '<div class="mncf-repeater-profile-line-left">';
            $addTagHeaderEnd = '</div><div class="mncf-repeater-profile-line-right">';
        }
        // Set title
        $form[$unique_id . '_main_title'] = array(
            '#type' => 'markup',
        ...
        if ( !defined( 'DOING_AJAX' ) ) {
            ...
            $form[$form_id . '_repetitive_form'] = array(
                '#type' => 'markup',
                '#markup' => mncf_repetitive_umform( $this->cf,
                        $this->currentUID ),
                '#id' => $form_id . '_repetitive_form',
            );
        }
