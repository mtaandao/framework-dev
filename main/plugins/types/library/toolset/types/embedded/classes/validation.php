 ...
 */
class MNCF_Validation
{

    function __construct() {
        add_filter( 'mncf_field', array($this, 'filter_field') );
    }

    /**
     * Filters each field.
     * 
     * Uses 'mncf_field' hook. Filters field settings.
     * 
     * @param type $field
     * @return int
     */
        ...
        if ( $field['type'] == 'date' ) {
            if ( !fields_date_timestamp_neg_supported() ) {
                ...
                $field['data']['validate']['negativeTimestamp'] = array(
                    'active' => 1,
                    'value' => 'true',
                    'message' => mncf_admin_validation_messages( 'negativeTimestamp' ),
                );
            }
        }
        return $field;
     ...
     * @return string
     */
    static function renderJsonData( $selector = '.mncf-form-validate' ) {

        // Get collected validation rules
        $elements = mncf_form_add_js_validation( 'get' );
        ...
        foreach ( $elements as $id => $element ) {
            ...
            foreach ( $element['#validate'] as $method => $args ) {
                ...
                if ( empty( $args['message'] ) ) {
                    $args['message'] = mncf_admin_validation_messages( $method,
                            $args['value'] );
                }
                //Check if mtaandao date format is d/m/Y and use ITA validation
                if ( $method == 'date' && isset( $element['mncf-type'] ) && $element['mncf-type'] == 'date' && get_option( 'date_format' ) == 'd/m/Y' ) {
                    $method = 'dateITA:true';
                }
                // Set JSON data
        ...
        mn_enqueue_script( 'types-validation' );
