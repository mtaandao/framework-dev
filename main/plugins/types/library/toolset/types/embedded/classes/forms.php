...
class Enlimbo_Forms_Wpcf
{
    ...
    {
        // Auto-add mnnonce field
        $form['_mnnonce'] = array(
            '#type' => 'markup',
            '#markup' => mn_nonce_field( $id, '_mnnonce_mncf', true, false )
        );

        $this->_id = $id;
        $this->_elements = $form;

        do_action( 'mncf_form_autohandle', $id, $form, $this );
        do_action( 'mncf_form_autohandle_' . $id, $form, $this );

        // get submitted data
        if ( $this->isSubmitted() ) {

            do_action( 'mncf_form_autohandle_submit', $id, $form, $this );
            do_action( 'mncf_form_autohandle_submit_' . $id, $form, $this );

            // check if errors (validation)
            $this->validate( $this->_elements );

            do_action( 'mncf_form_autohandle_validate', $id, $form, $this );
            do_action( 'mncf_form_autohandle_validate_' . $id, $form, $this );
            ...
            if ( empty( $this->_errors ) ) {
                ...
                if ( empty( $this->_errors ) ) {
                    // redirect
                    do_action( 'mncf_form_autohandle_redirection', $id, $form,
                            $this );
                    do_action( 'mncf_form_autohandle_redirection_' . $id, $form,
                            $this );
                    if( ! headers_sent() ) {
    ...
    public function isSubmitted( $id = '' )
    {
        ...
        return (
            isset( $_REQUEST['_mnnonce_mncf'] )
            && mn_verify_nonce( $_REQUEST['_mnnonce_mncf'], $id )
        );
    }
    ...
    public function validateElement( &$element )
    {
        ...
        if ( isset( $check['error'] ) ) {
            ...
            if ( isset( $element['mncf-id'] ) ) {
                $this->_elements_not_valid[$element['mncf-id']] = $element;
            }
        }
    }
        ...
        if ( is_callable( array($this, $method) ) ) {
            if ( !isset( $element['#id'] ) ) {
                if ( isset( $element['#attributes']['id'] ) ) {
                    ...
                    $element['#id'] = sprintf('mncf-%s-%s', $element['#type'], md5($element['#name']));
                } else {
            ...
            if ( !empty( $element['#validate'] ) ) {
                mncf_form_add_js_validation( $element );
            }
...
//        }
    ...
    public function markup( $element )
    {
        if ( isset( $element['#pattern'] ) ) {
            $element = $this->_setRender( $element );
            $element['_render']['label'] = isset($element['#title'])? $element['#title']:__('[no title]', 'mncf');
    ...
    public function thumbnail( $element )
    {
        ...
        $image = $element['#value']? mn_get_attachment_image($element['#value']):'';
        $element['#attributes'] = array(
            'class' => 'feature-image-id',
        );
        $element['#after'] = sprintf(
            '<div class="mnt-file-preview">%s</div><a href="#" id="%s_media" class="feature-image" data-set="%s" data-remove="%s" data-value="%d" data-mnt-type="image">%s</a>',
            $image,
            $element['#id'],
            __('Set featured image', 'mncf'),
            __('Remove featured image', 'mncf'),
            $element['#value']? $element['#value']:0,
            $image? __('Remove featured image', 'mncf'):__('Set featured image', 'mncf')
        );
