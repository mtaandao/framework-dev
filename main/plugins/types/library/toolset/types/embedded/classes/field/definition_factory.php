 ...
 * @since 1.9
 */
abstract class MNCF_Field_Definition_Factory {

	/**
	 * Singleton parent.
	 *
	 * @link http://stackoverflow.com/questions/3126130/extending-singletons-in-php
	 * @return MNCF_Field_Definition_Factory Instance of calling class.
	 */
	 ...
	 * @return MNCF_Field_Definition_Factory Factory instance for given domain.
	...
	public static function get_factory_by_domain( $domain ) {
		switch( $domain ) {
			case Types_Field_Utils::DOMAIN_POSTS:
				return MNCF_Field_Definition_Factory_Post::get_instance();
			...
			case Types_Field_Utils::DOMAIN_TERMS:
				return MNCF_Field_Definition_Factory_Term::get_instance();
			default:
	 ...
	 * equal to the key).
	 * @return null|MNCF_Field_Definition Field definition or null if it can't be loaded.
	 */
		...
		if( !array_key_exists( $field_key, $this->field_definitions ) ) {
			...
			if( in_array( $field_key, array_keys( $fields_from_options ) ) ) {
				...
				foreach( $fields_from_options as $field_from_options ) {
					if( mncf_getarr( $field_from_options, 'slug' ) == $field_key ) {
			...
			$field_type_slug = mncf_getarr( $field_configuration, 'type', null );
			...
			try {
				...
				/** @var MNCF_Field_Definition $field_definition */
	 ...
	 * different for meta_keys with the mncf- prefix from the ones without it. More details in the code.
	 *
	...
	final public function create_field_definition_for_existing_fields( $meta_key ) {
		...
		// If the meta_key has our mncf- prefix, we will not use it in the slug.
		$field_slug = preg_replace( '/^mncf\-/', '', $meta_key );
		...
		// prefix ('mncf-'), we assume that this was most probably a Types field whose definition got lost. If not,
		...
		//
		// Name of the legacy function is: mncf_types_cf_under_control().
		//
		// I assume this setting is somehow related to toolset-forms.
		$adding_field_with_prefix = (
			substr( $meta_key, 0, strlen( MNCF_Field_Definition::FIELD_META_KEY_PREFIX ) ) == MNCF_Field_Definition::FIELD_META_KEY_PREFIX
		);
	...
	private function get_fields_from_options() {
		return mncf_admin_fields_get_fields( false, false, false, $this->get_option_name() );
	}
	 ...
	 *
	 * It also completely clears the cache of the (legacy) mncf_admin_fields_get_fields.
	...
	public function clear_definition_storage( $field_slug = null ) {
		...
		mncf_admin_fields_get_fields( false, false, false, $this->get_option_name(), false, true );
	}
	...
	private function get_types_field_slugs() {
		...
		foreach ( $fields_from_options as $field_configuration ) {
			$slug = mncf_getarr( $field_configuration, 'slug' );
			if( !empty( $slug ) ) {
				$field_slugs[] = $slug;
			}
		}
	 ...
	 * @return bool|MNCF_Field_Definition|null
	 * @since 1.9
	 */
	...
	/**
	 * @return MNCF_Field_Definition[] All existing Types field definitions.
	 */
	...
	/**
	 * @var null|MNCF_Field_Definition_Generic[] Cache.
	 */
	private $generic_field_definitions = null;


	/**
	 * @return MNCF_Field_Definition_Generic[] Definitions of all generic fields that exist in the database within
	 *     current domain.
	 */
	public function load_generic_field_definitions() {
		...
		if( null == $this->generic_field_definitions ) {
			...
			foreach( $existing_meta_keys as $meta_key ) {
				...
				$results[] = new MNCF_Field_Definition_Generic( $meta_key );
			}
	...
	/**
	 * @return MNCF_Field_Definition_Abstract[] All field definitions (generic and Types-controlled).
	 */
	 ...
	 *
	 * @param MNCF_Field_Definition_Abstract[] $definitions
	 * @param string $orderby 'name'|'slug'|'is_under_types_control'|'field_type'
	 * @param string $order 'asc'|'desc'
	 *
	 * @return MNCF_Field_Definition_Abstract[] Reordered array.
	 */
	public function order_definitions( $definitions, $orderby = 'name', $order = 'asc' ) {
		...
		usort( $definitions, array( $this, mncf_getarr( $orderby_to_comparison_function, $orderby, 'compare_definitions_by_name' ) ) );
	 ...
	 *
	 * @param $first MNCF_Field_Definition_Abstract
	 * @param $second MNCF_Field_Definition_Abstract
	 *
	 * @return int
	 */
	...
	public function compare_definitions_by_field_type( $first, $second ) {
		if( $first->is_under_types_control() == $second->is_under_types_control() ) {
			if( $first->is_under_types_control() ) {
				...
				/** @var MNCF_Field_Definition $first_t */
				$first_t = $first;
				/** @var MNCF_Field_Definition $second_t */
				$second_t = $second;
	 ...
	 *
	 * @return MNCF_Field_Definition_Abstract[] Field definitions that match query arguments.
	 *
	 * @since 1.9
	 */
	public function query_definitions( $args ) {

		$args = mn_parse_args( $args,  array( 'filter' => 'all' ) );

		// Get only certain type of field definitions (generic, Types or both)
		$filter = mncf_getarr( $args, 'filter' );
		if( 'types' == $filter ) {
		...
		$search_string = mncf_getarr( $args, 'search', '' );
		...
		$group_source = mncf_getarr( $args, 'group_slug', (int) mncf_getarr( $args, 'group_id' ) );
		if( !empty( $group_source ) ) {
			...
			if( null != $group ) {
				foreach( $results as $field_definition ) {
					if( $field_definition instanceof MNCF_Field_Definition
		...
		// Sort results
		$orderby = mncf_getarr( $args, 'orderby', 'name' );
		$order = mncf_getarr( $args, 'order', 'asc', array( 'asc', 'desc' ) );
	 ...
	 *
	 * @param MNCF_Field_Definition $field_definition
