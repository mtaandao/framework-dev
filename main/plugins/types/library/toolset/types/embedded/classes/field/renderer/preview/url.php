...
class MNCF_Field_Renderer_Preview_URL extends MNCF_Field_Renderer_Preview_Base {
	...
	protected function get_link_label( $url_components ) {
		...
		$url_query = mncf_getarr( $url_components, 'query' );
		$url_query = ( empty( $url_query ) ? '' : '?' . $url_query );

		$url_path = mncf_getarr( $url_components, 'path' );
		...
		$label = sprintf(
			'%s%s%s',
			mncf_getarr( $url_components, 'host' ),
			$url_path,
			$url_query
		);
