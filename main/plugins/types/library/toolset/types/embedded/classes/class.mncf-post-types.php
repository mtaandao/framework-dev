...
class MNCF_Post_Types
{
    ...
    function __construct()
    {
        ...
        global $mn_version;
		if ( version_compare( $mn_version, '4.4' ) < 0 ) {
			add_action('admin_head-nav-menus.php', array($this, 'add_filters'));
			add_filter('mn_setup_nav_menu_item',  array( $this, 'setup_archive_item'));
			add_filter('mn_nav_menu_objects', array( $this, 'maybe_make_current'));
		}
    }
    /**
    ...
    public function pre_get_posts($query)
    {
        ...
        if (
            ...
            $custom_post_types = mncf_get_active_custom_types();
            /**
    ...
    public function manage_custom_columns($column, $post_id)
    {
        ...
        $field = mncf_admin_fields_get_field_by_meta_key($column);
        if ( isset( $field['type'] ) ) {
            switch( $field['type'] ) {
            case 'image':
                $default_width = '100px';
                 ...
                 */
                $width = apply_filters('mncf_field_image_max_width', $default_width);
                if (empty($width)) {
                    $width = $default_width;
                }
                $value = sprintf(
            ...
            case 'date':
                ...
                $value = MNToolset_Field_Date::timetodate($value);
                break;
            }
        }
        if ( is_string($value ) ) {
    ...
    public function setup_archive_item( $menu_item ) {
        ...
        if (post_type_exists( $post_type )) {
            ...
            $menu_item->type_label = sprintf( __( 'Archive for %s', 'mncf' ), $data->labels->name);
    ...
    public function add_archive_checkbox( $posts, $args, $post_type )
    {
        ...
        global $_nav_menu_placeholder, $mn_rewrite;
    ...
    {
        return mncf_get_custom_post_type_settings( $post_type );
    }
