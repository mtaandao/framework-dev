 ...
 * @since 1.9.1
 */
class MNCF_Field_Option_Radio extends MNCF_Field_Option_Checkboxes {
	...
	public function get_value_to_store() {
		$value = mncf_getarr( $this->config, 'value' );
		if( !is_string( $value ) ) {
			return '';
		}
	...
	public function get_display_value( $is_checked = true ) {
		...
		$display_mode = mncf_getnest( $field_definition_array, array( 'data', 'display' ), 'db' );
		...
		if( 'db' == $display_mode ) {
			...
			if ( $is_checked ) {
				return mncf_getarr( $this->config, 'display_value' );
			} else {
				return '';
			}
		}
