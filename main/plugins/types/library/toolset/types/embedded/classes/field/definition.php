 ...
 *
 * Children of this class must be instantiated exclusively through a factory class inherited from MNCF_Field_Definition_Factory.
 *
 ...
 * @since 1.9
 */
abstract class MNCF_Field_Definition extends MNCF_Field_Definition_Abstract {
	 ...
	 */
	const FIELD_META_KEY_PREFIX = 'mncf-';
	...
	/** @var MNCF_Field_Definition_Factory The factory object that manages this field definition */
	private $factory;


	/**
	 * MNCF_Field_Definition constructor.
	 *
	 ...
	 * @param MNCF_Field_Definition_Factory $factory
	 *
	 * @since 1.9
	 */
	public function __construct( $type, $definition_array, $factory ) {
		...
		if( ! $factory instanceof MNCF_Field_Definition_Factory ) {
			throw new InvalidArgumentException( 'Invalid field definition factory object.' );
		}
		
		$this->factory = $factory;

		$this->definition_array = $this->get_type()->sanitize_field_definition_array( mncf_ensarr( $definition_array ) );

		$this->slug = mncf_getarr( $this->definition_array, 'slug' );

		if( sanitize_title( $this->slug ) != $this->slug ) {
			throw new InvalidArgumentException( 'Invalid slug.' );
		}

		$this->name = sanitize_text_field( mncf_getarr( $this->definition_array, 'name', $this->get_slug() ) );
	...
	public function get_display_name() {
		...
		$display_name = MNToolset_Types::translate( $string_name, $this->get_name() );
	 ...
	 */
	public function get_description() { return sanitize_text_field( mncf_getarr( $this->definition_array, 'description' ) ); }
	...
	public function is_under_types_control() {
		$is_disabled = (bool) mncf_getnest( $this->definition_array, array( 'data', 'disabled' ), false );
		return !$is_disabled;
	}
	...
	/**
	 * @return string Meta_key use to store this field's values. Defaults to mncf-$slug.
	 */
	public function get_meta_key() {
		if( null == $this->meta_key ) {
			$this->meta_key = sanitize_title(
				mncf_getarr( $this->definition_array, 'meta_key', self::FIELD_META_KEY_PREFIX . $this->get_slug() )
			);
		}
	...
	public function get_is_repetitive() {
		if( null === $this->is_repetitive ) {
			$this->is_repetitive = ( mncf_getnest( $this->definition_array, array( 'data', 'repetitive' ), 0 ) != 0 );
		}
	...
	public function get_forced_value() {
		return mncf_getnest( $this->definition_array, array( 'data', 'set_value' ), null );
	}
	...
	public function get_should_save_empty_value() {
		return ( 'yes' == mncf_getnest( $this->definition_array, array( 'data', 'save_empty' ), 'no' ) );
	}


	public function get_is_required() {
		return ( 1 == mncf_getnest( $this->definition_array, array( 'data', 'validate', 'required', 'active' ), 0 ) );
	}
	 ...
	 * @return MNCF_Field_Option_Checkboxes[] An option_id => option_data array.
	 * @since 1.9
	 */
	public function get_field_options() {
		...
		$options_definition = mncf_ensarr( mncf_getnest( $this->definition_array, array( 'data', 'options' ) ) );
		$results = array();
		...
		$default = mncf_getarr( $options_definition, 'default', 'no-default' );
		if( $has_default ) {
			...
			try {
				switch( $this->get_type()->get_slug() ) {
					case Types_Field_Type_Definition_Factory::RADIO:
						$option = new MNCF_Field_Option_Radio( $option_id, $option_config, $default, $this );
						break;
					case Types_Field_Type_Definition_Factory::SELECT:
						$option = new MNCF_Field_Option_Select( $option_id, $option_config, $default, $this );
						break;
					case Types_Field_Type_Definition_Factory::CHECKBOXES:
						$option = new MNCF_Field_Option_Checkboxes( $option_id, $option_config, $default );
						break;
					default:
						throw new InvalidArgumentException( 'Invalid field type' );
				}
	...
	public function get_datetime_option() {
		...
		$value = mncf_getnest( $this->definition_array, array( 'data', 'date_and_time' ) );
	 ...
	 *
	 * @return MNCF_Field_DataMapper_Abstract
	 */
	public function get_data_mapper() {
		switch( $this->get_type()->get_slug() ) {
			case Types_Field_Type_Definition_Factory::CHECKBOXES:
				return new MNCF_Field_DataMapper_Checkboxes( $this );
			case Types_Field_Type_Definition_Factory::CHECKBOX:
				return new MNCF_Field_DataMapper_Checkbox( $this );
			default:
				return new MNCF_Field_DataMapper_Identity( $this );
		}
	}
	...
	protected function check_allowed_types( $allowed_field_types ) {
		...
		$allowed_field_types = mncf_wraparr( $allowed_field_types );
	...
	const XML_KEY_MNML_ACTION = 'mnml_action';
	...
	private function add_checksum_to_export_object( $data ) {
		...
		$conditional_display = mncf_getnest( $checksum_source, array( self::XML_KEY_DATA, self::XML_KEY_DATA_CONDITIONAL_DISPLAY ), null );
		...
		$checksum_source[ self::XML_KEY_DATA ][ self::XML_KEY_DATA_IS_REPEATING ] =
			(int) mncf_getnest( $checksum_source, array( self::XML_KEY_DATA, self::XML_KEY_DATA_IS_REPEATING ), 0 );

		$checksum_source = $ie_controller->add_checksum_to_object(
			$checksum_source,
			null,
			array(
				self::XML_KEY_ID, self::XML_KEY_MENU_ICON, self::XML_KEY_MNML_ACTION,
	...
	public function get_export_object() {
		...
		// legacy filter
		$data = apply_filters( 'mncf_export_field', $data );
		...
		// Export MNML TM setting for this field's translation, if available.
		$mnml_tm_settings = apply_filters( 'mnml_setting', null, 'translation-management' );
		$custom_field_translation_setting = mncf_getnest( $mnml_tm_settings, array( 'custom_fields_translation', $this->get_meta_key() ), null );
		if( null !== $custom_field_translation_setting ) {
			$data[ self::XML_KEY_MNML_ACTION ] = $custom_field_translation_setting;
		}
	...
	/**
	 * @return MNCF_Field_Definition_Factory Factory object that is responsible for creating this object.
	 * @since 2.0
	 */
	...
	private function set_data_key_safely( $key, $value ) {
		if( !is_array( mncf_getarr( $this->definition_array, 'data' ) ) ) {
