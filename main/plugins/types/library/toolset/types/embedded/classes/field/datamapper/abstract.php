 ...
 *
 * The correct data mapper for a field is chosen in MNCF_Field_Definition::get_data_mapper().
 *
 * Note that data mappers do not deal with displaying field values (except in form inputs). To properly display a field,
 * use one of existing renderers (MNCF_Field_Renderer_Factory) or create new one.
 *
 * @since 1.9
 */
abstract class MNCF_Field_DataMapper_Abstract {

	protected $field_definition;

	public function __construct( $field_definition ) {
		if( ! $field_definition instanceof MNCF_Field_Definition_Abstract ) {
