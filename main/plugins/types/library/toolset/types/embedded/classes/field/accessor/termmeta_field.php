 ...
 * @since 1.9
 */
final class MNCF_Field_Accessor_Termmeta_Field extends MNCF_Field_Accessor_Termmeta {

	/** @var MNCF_Field_Instance_Abstract */
	private $field;


	/**
	 * MNCF_Field_Accessor_Termmeta_Field constructor.
	 *
	 * @param int $object_id
	 * @param string $meta_key
	 * @param bool $is_repetitive
	 * @param MNCF_Field_Instance_Abstract $field_instance
	...
	public function __construct( $object_id, $meta_key, $is_repetitive, $field_instance ) {
		...
		if( ! $field_instance instanceof MNCF_Field_Instance_Abstract ) {
	...
	public function delete_raw_value( $value = '' ) {
		...
		do_action( "mncf_termeta_before_{$action}", $this->field );

		$result = delete_term_meta( $this->object_id, $this->meta_key, $value );

		do_action( "mncf_termmeta_after_{$action}", $this->field );
