 ...
 * 
 * @see MNCF_Validation::renderJsonData( $selector ) how rules are added
 * Use mn_enqueue_script( 'types-validation' ) to enqueue this script.
 * Use mncf_form_render_js_validation( $selector ) to render validation data used here.
 * 
...
var typesValidation = (function($) {
    ...
    function _initValidation(selector) {
        $(selector).validate({
            ...
            ignore: 'input[type="hidden"], .mncf-form-groups-support-post-type, .mncf-form-groups-support-tax, .mncf-form-groups-support-templates, :not(.js-types-validate)',
    ...
    function conditionalIsHidden(object) {
        ...
        if (object.parents('.mncf-conditional').length > 0
                && object.parents('.inside').is(':hidden')) {
            if (object.parents('.mncf-conditional').css('display') == 'none') {
                return true;
            }
            return false;
        } else {
            return object.parents('.mncf-conditional').length > 0 && object.is(':hidden');
        }
    }
