/*
 * Example of ted created by MNCF_Editor::renderTedSettings()
 */
/*var ted = {
    fieldID: null,
    fieldType: null,
...
var tedFrame = (function(window, $){
    ...
    function init()
    {
        ...
        modal.find('.media-modal-close, .media-button-cancel').click(function(){
            if (ted.callback == 'views_wizard') {
                window.parent.typesMNViews.wizardCancel();
                return false;
            }
    ...
    {
        if (ted.callback == 'views_wizard') {
            window.parent.typesMNViews.wizardSendShortcode(shortcode);
            return true;
        }
		if (ted.callback == 'admin_bar') {
            window.parent.typesMNViews.adminBarCreateShortcode(shortcode);
            return true;
        }
        // Check if there is custom handler
        if (window.parent.mncfFieldsEditorCallback_redirect) {
            eval(window.parent.mncfFieldsEditorCallback_redirect['function'] + '(\''+esc_shortcode+'\', window.parent.mncfFieldsEditorCallback_redirect[\'params\'])');
            // Reset redirect
            window.parent.mncfFieldsEditorCallback_redirect = null;
        } else {
            // Use default handler
...
jQuery(document).ready(function($){
    ...
    /* Generic function to display native MN Tooltip */
...
/* Generic function to display native MN Tooltip END */
});
...
jQuery(function(){
    ...
    parent.jQuery.colorbox.resize({
        innerHeight: jQuery('#mncf-ajax').height()
    });
});
