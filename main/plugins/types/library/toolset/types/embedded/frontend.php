...
if ( version_compare( $mn_version, '3.3', '<' ) ) {
    // add a the_content filter to allow types shortcodes to be closed.
    // This is a bit of a HACK for version 3.2.1 and less

    add_filter( 'the_content', 'mncf_fix_closed_types_shortcodes', 9, 1 );
    add_filter( 'the_content', 'mncf_fix_closed_types_shortcodes_after', 11, 1 );

    function mncf_fix_closed_types_shortcodes( $content ) {
        $content = str_replace( '][/types', ']###TYPES###[/types', $content );
        return $content;
    }

    function mncf_fix_closed_types_shortcodes_after( $content ) {
...
add_shortcode( 'types', 'mncf_shortcode' );

/**
 * Shortcode processing.
 *
 * Called by MN when rendering post on frontend.
 ...
 * @return string
 */
function mncf_shortcode( $atts, $content = null, $code = '' ) {

    global $mncf;

    // Switch the post if there is an attribute of 'id' in the shortcode.
    $post_id_atts = new MNV_mncf_switch_post_from_attr_id( $atts );

    if ( !is_array( $atts ) ) {
        $mncf->errors['shortcode_malformed'][] = func_get_args();
        return '';
    }
...
function types_render_field( $field_id = null, $params = array(), $content = null, $code = '', $allow_parent = true ) {
	...
	if ( isset( $params['post_id'] ) ) {
		...
		if ( is_numeric( $params['post_id'] ) ) {
			...
			$post_id = intval( MNCF_Relationship::get_parent( $post_id, trim( $params['post_id'], '$' ) ) );
		}
	}
	...
	if ( empty( $field ) ) {
		...
		global $mnlogger;
		$mnlogger->log( 'types_render_field call for missing field \'' . $field_id . '\'', MNLOG_DEBUG );
	...
	if ( types_is_repetitive( $field ) ) {
		...
		$meta_key = mncf_types_get_meta_prefix( $field ) . $field['slug'];
		$params['field_value'] = mncf_get_post_meta( $post_id, $meta_key, true );

		if ( $params['field_value'] == '' && $field['type'] != 'checkbox' ) {
			return '';
		}

		$html = types_render_field_single( $field, $params, $content, $code, $mncf->field->meta_object->meta_id );
	}

	return $mncf->field->html( $html, $params );
}

function types_render_termmeta( $field_id, $params, $content = null, $code = '' ) {

    global $mncf, $mndb, $MN_Views;
    // HTML var holds actual output
    $html = '';

    //Get User id from views loop
    if ( 
		isset( $MN_Views->taxonomy_data['term']->term_id ) 
		&& ! empty( $MN_Views->taxonomy_data['term']->term_id ) 
	) {
        $params['term_id'] = $MN_Views->taxonomy_data['term']->term_id;
    }
    ...
    if ( empty( $field ) ) {
        ...
        global $mnlogger;
        $mnlogger->log( 'types_render_field call for missing field \''
                . $field_id . '\'', MNLOG_DEBUG );
    ...
    if ( mncf_admin_is_repetitive( $field ) ) {
        ...
        $mncf->termmeta_repeater->set( $term_id, $field );
        $_meta = $mncf->termmeta_repeater->_get_meta();
        $meta = '';
        ...
        $params['field_value'] = get_term_meta( $term_id, mncf_types_get_meta_prefix( $field ) . $field['slug'], true );
        if ( $params['field_value'] == '' && $field['type'] != 'checkbox' ) {
            return '';
        }
    ...
    // API filter
    $mncf->termmeta_field->set( $term_id, $field );
    return $mncf->termmeta_field->html( $html, $params );
}
...
function types_render_usermeta( $field_id, $params, $content = null, $code = '' ) {
    ...
    global $mncf, $post, $mndb, $MN_Views;
    ...
    // Get User id from views loop
    if ( 
		isset( $MN_Views->users_data['term']->ID ) 
		&& ! empty( $MN_Views->users_data['term']->ID ) 
	) {
        $params['user_id'] = $MN_Views->users_data['term']->ID;
    }
    ...
    if ( isset( $params['user_id'] ) ) {
        ...
        $user_id = $mndb->get_var(
            $mndb->prepare(
                "SELECT * FROM " . $mndb->users . " WHERE user_login = %s",
                $params['user_name']
            )
        );
    ...
    if ( types_is_repetitive( $field ) ) {
        ...
        $params['field_value'] = get_user_meta( $user_id, mncf_types_get_meta_prefix( $field ) . $field['slug'], true );
        if ( $params['field_value'] == '' && $field['type'] != 'checkbox' ) {
            return '';
        }
        $html = types_render_field_single( $field, $params, $content, $code );
    ...
    return $mncf->usermeta_field->html( $html, $params );
}
...
function types_render_field_single( $field, $params, $content = null, $code = '', $meta_id = null )
{
    ...
    $params['field_value'] = apply_filters( 'mncf_fields_value_display', $params['field_value'], $params, $post->ID, $field['id'], $meta_id );

    $params['field_value'] = apply_filters( 'mncf_fields_slug_' . $field['slug'] . '_value_display', $params['field_value'], $params, $post->ID, $field['id'], $meta_id );

    $params['field_value'] = apply_filters( 'mncf_fields_type_' . $field['type'] . '_value_display', $params['field_value'], $params, $post->ID, $field['id'], $meta_id );
    // To make sure
    if ( is_string( $params['field_value'] ) ) {
	...
	// When a variable string or label output needs translating we do it on 'mncf_fields_type_' . $field['type'] . '_value_display' on a field type basis

    $field['name'] = mncf_translate( 'field ' . $field['id'] . ' name', $field['name'] );
    $params['field'] = $field;
    ...
    if ( (isset( $params['raw'] ) && $params['raw'] == 'true')
        || (isset( $params['output'] ) && $params['output'] == 'raw') ) {
             ...
             */
            $output = '';
            $_view_func = 'mncf_fields_' . strtolower( $field['type'] ) . '_view';
	    ...
	    if ( Toolset_Utils::is_field_value_truly_empty( $output ) && isset( $params['field_value'] )
                && $params['field_value'] !== "" ) {
                    $output = $params['field_value'];
                } else if ( $output == '__mncf_skip_empty' ) {
                    $output = '';
                }

            if (isset($params['output']) && $params['output'] == 'html') {
                $output = mncf_frontend_compat_html_output( $output, $field, $content, $params );
            } else {
...
function mncf_frontend_compat_html_output( $output, $field, $content, $params ) {
    ...
    if ( empty( $output ) && !empty( $params['field_value'] ) ) {
        $output = mncf_frontend_wrap_field_value( $field,
                $params['field_value'], $params );
        $output = mncf_frontend_wrap_field( $field, $output, $params );
    } else if ( $output != '__mncf_skip_empty' ) {
        $output = mncf_frontend_wrap_field_value( $field, $output, $params );
        $output = mncf_frontend_wrap_field( $field, $output, $params );
    } else {
        $output = '';
    }
    ...
    if ( isset( $count[$field['slug']] ) && intval( $count[$field['slug']] ) > 1 ) {
        ...
        $output = str_replace( 'id="mncf-field-' . $field['slug'] . '"',
                'id="mncf-field-' . $field['slug'] . $add . '"', $output );
    }
    return $output;
}
 ...
 * @return type
 */
function mncf_frontend_wrap_field( $field, $content, $params = array() ) {
    if ( isset( $params['output'] ) && $params['output'] == 'html' ) {
        $class = array();
        ...
        $class[] = 'mncf-field-' . $field['type'] . ' mncf-field-'
                . $field['slug'];
        // Add name if needed
        if ( isset( $params['show_name'] ) && $params['show_name'] == 'true'
                && strpos( $content,
                        'class="mncf-field-' . $field['type']
                        . '-name ' ) === false ) {
            $content = mncf_frontend_wrap_field_name( $field, $field['name'],
                            $params ) . $content;
        }
        $output = '<div id="mncf-field-' . $field['slug'] . '"'
                . ' class="' . implode( ' ', $class ) . '"';
        ...
        if ( isset( $params['show_name'] ) && $params['show_name'] == 'true'
            ...
            $content = mncf_frontend_wrap_field_name( $field,
 ...
 * @return type
 */
function mncf_frontend_wrap_field_name( $field, $content, $params = array() ) {
    if ( isset( $params['output'] ) && $params['output'] == 'html' ) {
        $class = array();
        ...
        $class[] = 'mncf-field-name mncf-field-' . $field['type'] . ' mncf-field-'
                . $field['slug'] . '-name';
 ...
 * @return type
 */
function mncf_frontend_wrap_field_value( $field, $content, $params = array() ) {
    if ( isset( $params['output'] ) && $params['output'] == 'html' ) {
        $class = array();
        if ( $field['type'] == 'checkboxes' && isset( $params['option'] ) ) {
        ...
        $class[] = 'mncf-field-value mncf-field-' . $field['type']
                . '-value mncf-field-' . $field['slug'] . '-value';
...
add_filter( 'mnv_filter_query', 'mncf_views_post_query', 12, 2 ); // after custom fields.
add_filter( 'mnv_filter_taxonomy_query', 'mncf_views_term_query', 42, 2 ); // after termmeta fields.
add_filter( 'mnv_filter_user_query', 'mncf_views_user_query', 72, 2 ); // after usermeta fields.

function mncf_views_post_query( $query, $view_settings ) {
	$query = mncf_views_query( $query, $view_settings, 'mncf-fields' );
	return $query;
}

function mncf_views_term_query( $query, $view_settings ) {
	$query = mncf_views_query( $query, $view_settings, 'mncf-termmeta' );
	return $query;
}

function mncf_views_user_query( $query, $view_settings ) {
	$query = mncf_views_query( $query, $view_settings, 'mncf-usermeta' );
	return $query;
}
 ...
 * @return string
 */
function mncf_views_query( $query, $view_settings, $meta_key = 'mncf-fields' ) {
	
	if ( ! in_array( $meta_key, array( 'mncf-fields', 'mncf-usermeta', 'mncf-termmeta' ) ) ) {
		return $query;
	}
    ...
    if ( isset( $query['meta_query'] ) ) {
        foreach ( $query['meta_query'] as $index => $meta ) {
            if ( is_array( $meta ) && isset( $meta['key'] ) ) {
                $field_name = $meta['key'];
                if ( _mncf_is_checkboxes_field( $field_name, $meta_key ) ) {
                    ...
                    /* According to http://codex.mtaandao.org/Class_Reference/MN_Meta_Query#Accepted_Arguments,
					...
					if ( is_array( $meta['value'] ) ) {
						...
						$values = _mncf_views_query_recursive_add_comma_meta_values( $values );
					...
					if ( version_compare( $mn_version, '4.1', '<' ) ) {
    ...
    if ( $meta_filter_required ) {
        add_filter( 'get_meta_sql', 'mncf_views_get_meta_sql', 10, 6 );
    }
    return $query;
}

function _mncf_is_checkboxes_field( $field_name, $key = 'mncf-fields' ) {
	if ( ! in_array( $key, array( 'mncf-fields', 'mncf-usermeta', 'mncf-termmeta' ) ) ) {
		return false;
	}
    $opt = get_option( $key );
    if ( $opt && strpos( $field_name, 'mncf-' ) === 0 ) {
        $field_name = substr( $field_name, 5 );
...
function _mncf_views_query_recursive_add_comma_meta_values( $values ) {
 ...
 * @return type
 */
function mncf_views_get_meta_sql( $clause, $queries, $type, $primary_table,
    ...
    remove_filter( 'get_meta_sql', 'mncf_views_get_meta_sql', 10, 6 );

    return $clause;
}


/** Fix shortcode rendering for MN 4.2.3 security fixes.
 ...
 *  adding filter with priority 5 before do_shortcode and other MN standard filters
 *
 *  Heavily inspired in do_shortcodes_in_html_tags
 */

add_filter( 'the_content', 'mncf_preprocess_shortcodes_in_html_elements', 5 );

function mncf_preprocess_shortcodes_in_html_elements( $content ) {
	...
	$textarr = mncf_html_split( $content );
	...
	foreach ( $textarr as &$element ) {
		...
		if ( $counts > 0 ) {
			foreach ( $matches[0] as $index => &$match ) {
				...
				if ( $inner_content ) {
					$new_inner_content = mncf_preprocess_shortcodes_in_html_elements( $inner_content );
...
/**
 * Separate HTML elements and comments from the text. Needed for mncf_preprocess_shortcodes_in_html_elements.
 *
 * Heavily inspired in mn_html_split
 *
 * @param string $input The text which has to be formatted.
 * @return array The formatted text.
 */
function mncf_html_split( $input ) {
	static $regex;
