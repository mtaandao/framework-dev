jQuery( document ).ready( function( $ ) {    	
	...
	if (!($.isEmptyObject(js_array))) {
		...
		for(var key in js_array) {
			...
			if ((!(js_array[key]['is_commercial_plugin_tab'])) && (!(installer_theme_install_localize.no_associated_themes))) {
			    $('div.mn-filter ul.filter-links').append('<li><a data-sort="'+key+'" href="#">'+ js_array[key]['the_hyperlink_text'] +'</a></li>');
			}
		}
	}
	...
	if (loaded_browsing_tab.length > 0) {
		...
		//Mtaandao defaults to 'Featured' when theme install is loaded without the browse parameter		
