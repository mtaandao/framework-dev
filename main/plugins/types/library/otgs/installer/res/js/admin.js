    var otgs_mn_installer = {
    
    init: function(){
        
        jQuery('.otgs_mn_installer_table').on('click', '.enter_site_key_js', otgs_mn_installer.show_site_key_form);
        jQuery('.otgs_mn_installer_table').on('click', '.cancel_site_key_js', otgs_mn_installer.hide_site_key_form);
        
        jQuery('.otgs_mn_installer_table').on('click', '.remove_site_key_js', otgs_mn_installer.remove_site_key);
        jQuery('.otgs_mn_installer_table').on('click', '.update_site_key_js', otgs_mn_installer.update_site_key);
            
        jQuery('.otgs_mn_installer_table').on('submit', '.otgsi_site_key_form', otgs_mn_installer.save_site_key);
        jQuery('.otgs_mn_installer_table').on('submit', '.otgsi_downloads_form', otgs_mn_installer.download_downloads);
        jQuery('.otgs_mn_installer_table').on('change', '.otgsi_downloads_form :checkbox[name="downloads[]"]', otgs_mn_installer.update_downloads_form);
        
        jQuery('.installer-dismiss-nag').click(otgs_mn_installer.dismiss_nag);
        
        jQuery('.otgs_mn_installer_table').on('click', '.installer_expand_button', otgs_mn_installer.toggle_subpackages);

        otgs_mn_installer.scroll_to_repository();

        if( typeof pagenow != 'undefined' && pagenow == 'plugins'){

            jQuery(document).ajaxSuccess(function(event, xhr, settings) {
                var data = otgs_mn_installer.getQueryParameters(settings.data);
    ...
    show_site_key_form: function(){
        ...
        otgs_mn_installer.reset_errors();
    ...
    save_site_key: function(){
        ...
        jQuery.ajax({url: ajaxurl, type: 'POST', dataType:'json', data: data, success: 
            function(ret){
                if(!ret.error){                    
                    otgs_mn_installer.saved_site_key();
                }else{
                    otgs_mn_installer.show_error(thisf.find('[name=repository_id]').val(), ret.error);
    ...
    remove_site_key: function(){
        ...
        if( jQuery(this).attr('disabled') == 'disabled' ){
            ...
            if(confirm(jQuery(this).data('confirmation'))){
                ...
                jQuery.ajax({url: ajaxurl, type: 'POST', data: data, success: otgs_mn_installer.removed_site_key});
            }
            ...
            complete: function( event, xhr, settings ){
                ...
                if(xhr == 'success') {
                    ...
                    if(ret.error){
                        ...
                        otgs_mn_installer.updated_site_key(ret);
                    }
                }else{
    ...
    download_downloads: function(){
        ...
        function download_and_activate( elem ){
            ...
            if(is_update){
                otgs_mn_installer.show_download_progress_status(downloads_form, installer_strings.updating.replace('%s', plugin_name));
            }else{
                otgs_mn_installer.show_download_progress_status(downloads_form, installer_strings.installing.replace('%s', plugin_name));
            }
            ...
            jQuery.ajax({
                ...
                success: function(ret){
                    ...
                    if(!ret.success){
                        ...
                        if(ret.message){
                            installed.closest('.otgs_mn_installer_table').find('.installer-error-box').html('<p>' + ret.message + '</p>').show();
                        }else{
                            installed.closest('.otgs_mn_installer_table').find('.installer-error-box').html('<p>' + downloads_form.find('.installer-revalidate-message').html() + '</p>').show();
                        }
                    ...
                    if(ret.success && activate){
                        ...
                        otgs_mn_installer.show_download_progress_status(downloads_form, installer_strings.activating.replace('%s', plugin_name));
                        activating.show();
                        spinner.show();
                        ...
                        jQuery.ajax({
                            ...
                            success: function(ret){
                                ...
                                if( typeof checkboxes[idx] != 'undefined' ){
                                    ...
                                    otgs_mn_installer.hide_download_progress_status(downloads_form);
    ...
    toggle_subpackages: function(){
        var list = jQuery(this).closest('td').find('.otgs_mn_installer_subtable');
            ...
            if(typeof split[1] != 'undefined'){
                ...
                if(repo_element.length){
                    ...
                    if(package_element.length && !package_element.is(':visible')){
                        package_element.parents('.otgs_mn_installer_subtable').slideDown();
...
jQuery(document).ready(otgs_mn_installer.init);