...
class Installer_Dependencies{
    ...
    function __construct(){
        ...
        add_action('mn_ajax_update-plugin', array($this, 'prevent_plugins_update_on_updates_screen'), 0); // high priority, before MN
    ...
    public function is_win_paths_exception($repository_id){
        ...
        if(!isset($this->is_win_paths_exception[$repository_id])) {
            ...
            if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
                ...
                $longest_path['mnml']       = 109;
                $longest_path['toolset']    = 99;

                $margin                     = 15;

                $upgrade_path_length = strlen( MN_CONTENT_DIR . '/upgrade' );

                $installer_settings = MN_Installer()->settings;
                ...
                if ( is_array( $installer_settings['repositories'][$repository_id]['data']['downloads']['plugins'] ) ) {
                    ...
                    $url = MN_Installer()->append_site_key_to_download_url( $a_plugin['url'], 'xxxxxx', $repository_id );
                    $tmpfname = mn_tempnam( $url );
    ...
    public function is_uploading_allowed(){
        ...
        if(!isset($this->uploading_allowed)){
            ...
            require_once MN_Installer()->plugin_path() . '/includes/installer-upgrader-skins.php';

            $upgrader_skins = new Installer_Upgrader_Skins(); //use our custom (mute) Skin
            $upgrader = new Plugin_Upgrader($upgrader_skins);

            ob_start();
            $res = $upgrader->fs_connect( array(MN_CONTENT_DIR, MN_PLUGIN_DIR) );
            ob_end_clean();
    ...
    public function win_paths_exception_message(){
        return __('Downloading is not possible. Mtaandao cannot create required folders because of the
                    ...
                    foreach ($plugins as $plugin_id => $plugin) {
                        ...
                        if( in_array( $plugin_id, $plugins_with_updates ) ) {
                            ...
                            foreach ($repositories_plugins as $slug => $name) {
                                if ($mn_plugin_slug == $slug || $name == $plugin['Name'] || $name == $plugin['Title']) { //match order: slug, name, title

                                    remove_action("after_plugin_row_$plugin_id", 'mn_plugin_update_row', 10, 2);
                                    add_action("after_plugin_row_$plugin_id", array($this, 'mn_plugin_update_row_win_exception'), 10, 2);
    ...
    public function mn_plugin_update_row_win_exception(){
        $mn_list_table = _get_list_table('MN_Plugins_List_Table');
        echo '<tr class="plugin-update-tr">';
        echo '<td  class="plugin-update colspanchange" colspan="' . esc_attr( $mn_list_table->get_column_count() ) .
                                ...
                                foreach ($repository['data']['packages'] as $package) {
                                    ...
                                    foreach ($package['products'] as $product) {
                                        ...
                                        foreach ($product['plugins'] as $plugin_slug) {
                                            ...
                                            if ($download['slug'] == $mn_plugin_slug && empty($download['free-on-mnorg']) ) {
