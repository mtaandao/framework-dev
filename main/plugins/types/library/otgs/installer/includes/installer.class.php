...
final class MN_Installer{
    ...
    public function __construct(){
        ...
        //add_filter('mn_installer_buy_url', array($this, 'append_parameters_to_buy_url'));
    ...
    public function init(){
        ...
        $this->_using_icl     = function_exists('mnml_site_uses_icl') && mnml_site_uses_icl();
        $this->_mnml_version  = defined('ICL_SITEPRESS_VERSION') ? ICL_SITEPRESS_VERSION : '';

        mn_enqueue_script('installer-admin', $this->res_url() . '/res/js/admin.js', array('jquery'), $this->version());
        mn_enqueue_style('installer-admin', $this->res_url() . '/res/css/admin.css', array(), $this->version());
        ...
        mn_localize_script( 'installer-admin', 'installer_strings', $translation_array );
        ...
        if(defined('DOING_AJAX')){
            add_action('mn_ajax_save_site_key', array($this, 'save_site_key'));
            add_action('mn_ajax_remove_site_key', array($this, 'remove_site_key_ajax'));
            add_action('mn_ajax_update_site_key', array($this, 'update_site_key'));

            add_action('mn_ajax_installer_download_plugin', array($this, 'download_plugin_ajax_handler'));
            add_action('mn_ajax_installer_activate_plugin', array($this, 'activate_plugin'));

            add_action('mn_ajax_installer_dismiss_nag', array($this, 'dismiss_nag'));
        }
        ...
        // MN 4.2
        if(defined('DOING_AJAX')){
            add_action('mn_ajax_update-plugin', array($this, 'plugin_upgrade_custom_errors'), 0); // high priority, before MN
        }
    ...
    protected function log($message){
        if( defined('MNML_INSTALLER_LOGGING') && MNML_INSTALLER_LOGGING ){
    ...
    private function _pre_1_0_clean_up(){
        ...
        if(!defined('MNRC_VERSION')){
            $old_tables = array(
                $mndb->prefix . 'mnrc_cached_requests',
                $mndb->prefix . 'mnrc_extension_types',
                $mndb->prefix . 'mnrc_extensions',
                $mndb->prefix . 'mnrc_repositories',
                $mndb->prefix . 'mnrc_repositories_relationships',
            );

            foreach($old_tables as $table){
                $mndb->query(sprintf("DROP TABLE IF EXISTS %s", $table));
            }
        ...
        if( !empty($this->settings['repositories']) ) {
            ...
            foreach ( $this->settings['repositories'] as $repository_id => $repository ) {
                ...
                foreach ( $plugins as $plugin_id => $plugin ) {
                    ...
                    $mn_plugin_slug = dirname( $plugin_id );
                    if ( empty($mn_plugin_slug) ) {
                        $mn_plugin_slug = basename( $plugin_id, '.php' );
                    }
                    ...
                    foreach ( $repositories_plugins as $repository_id => $r_plugins ) {
                        ...
                        foreach ( $r_plugins as $slug => $r_plugin ) {
                            ...
                            if ( $mn_plugin_slug == $slug || $r_plugin['name'] == $plugin['Name'] || $r_plugin['name'] == $plugin['Title'] ) { //match order: slug, name, title
    ...
    public function save_settings(){
        ...
        update_option( 'mn_installer_settings', $_settings );
        ...
        if( is_multisite() && is_main_site() && isset($this->settings['repositories']) ){
            ...
            update_site_option( 'mn_installer_network', $network_settings );
    ...
    public function get_settings($refresh = false){
        ...
        if($refresh || empty($this->settings)){
            ...
            $_settings = get_option('mn_installer_settings');
            ...
            if (is_multisite() && isset($this->settings['repositories'])) {
                $network_settings = maybe_unserialize(get_site_option('mn_installer_network'));
                if ($network_settings) {
                    foreach ($this->settings['repositories'] as $rep_id => $repository) {
                    ...
                    if ( $site_key_missing || $site_key_changed ) {
                        ...
                        $response = $this->save_site_key(
                            array(
                                ...
                                'site_key' => $site_key,
                                'return' => true,
                                'nonce' => mn_create_nonce( 'save_site_key_' . $repository_id )
                            )
                        );
                        ...
                        if ( !empty($response['error']) ) {
                            ...
                            $this->admin_messages[] = array(
                                'type' => 'error',
                                'text' => sprintf( __( 'You are using an invalid site key defined as the constant %s (most likely in mn-config.php). 
    ...
    public function load_repositories_list(){
        global $mn_installer_instances;

        foreach ($mn_installer_instances as $instance) {
    ...
    public function refresh_repositories_data(){
        ...
        if( defined('OTGS_DISABLE_AUTO_UPDATES') && OTGS_DISABLE_AUTO_UPDATES && empty($_GET['force-check']) || $checked ){
            ...
            if(empty($this->settings['repositories']) && $this->is_repositories_page()){
                ...
                $error = sprintf(__("Installer cannot display the products information because the automatic updating for %s was explicitly disabled with the configuration below (usually in mn-config.php):", 'installer'), strtoupper( join(', ', $repository_names) ));
        ...
        foreach($this->repositories as $id => $data){
            ...
            $response = mn_remote_get($data['products']);
            ...
            if(is_mn_error($response)){
                ...
                $error = sprintf(__("Installer cannot contact our updates server to get information about the available products and check for new versions. If you are seeing this message for the first time, you can ignore it, as it may be a temporary communication problem. If the problem persists and your Mtaandao admin is slowing down, you can disable automated version checks. Add the following line to your mn-config.php file:", 'installer'), strtoupper($id));
    ...
    public function get_extra_url_parameters(){
        ...
        $parameters['installer_version']  = MN_INSTALLER_VERSION;
        $parameters['theme']              = mn_get_theme()->get( 'Name' );
    ...
    public function append_parameters_to_buy_url($url, $repository_id, $args = array()){
        ...
        if($repository_id == 'mnml'){
            $url = add_query_arg(array('using_icl' => $this->_using_icl, 'mnml_version' => $this->_mnml_version), $url);
        }

        $url = apply_filters('mn_installer_buy_url', $url);
    ...
    public function save_site_key($args = array()){
        ...
        if($repository_id && $nonce && mn_create_nonce('save_site_key_' . $repository_id) == $nonce){
    ...
    /**
     * Alias for MN_Installer::get_repository_site_key
     * @see MN_Installer::get_repository_site_key()
     *
     * @param string $repository_id
     * @return string (site key) or bool
     */
    public function get_site_key($repository_id){
        return MN_Installer::get_repository_site_key( $repository_id );
    }
    ...
    public function remove_site_key_ajax(){
        if($_POST['nonce'] == mn_create_nonce('remove_site_key_' . $_POST['repository_id'])){
    ...
    public function validate_repository_subscription(){
        ...
        if($repository_id){
            ...
            mn_redirect($this->menu_url() . '#repository-' . $repository_id);
            exit;
    ...
    public function update_site_key(){
        ...
        $repository_id = sanitize_text_field ( $_POST['repository_id'] );
        if($_POST['nonce'] == mn_create_nonce('update_site_key_' . $repository_id )){
    ...
    public function api_debug_log($text){
        ...
        if(defined('MNML_DEBUG_INSTALLER') && MNML_DEBUG_INSTALLER){
    ...
    public function fetch_subscription_data( $repository_id, $site_key, $source = self::SITE_KEY_VALIDATION_SOURCE_OTHER ){
        ...
        if($repository_id == 'mnml'){
            ...
            $args['body']['mnml_version']   = $this->_mnml_version;
        }

        $args['body']['installer_version']  = MN_INSTALLER_VERSION;
        $args['body']['theme']              = mn_get_theme()->get( 'Name' );
        ...
        $response = mn_remote_post($this->repositories[$repository_id]['api-url'], $args);
        ...
        if( !is_mn_error($response) ){
            $datas = mn_remote_retrieve_body($response);
    ...
    public function plugins_renew_warning($plugin_file, $plugin_data, $status){
        ...
        <tr class="plugin-update-tr"><td colspan="<?php echo $mn_list_table->get_column_count(); ?>" class="plugin-update colspanchange">
    ...
    public function plugin_is_installed($name, $slug, $version = null){
        ...
        foreach($plugins as $plugin_id => $plugin){
            ...
            if( $mn_plugin_slug == $slug || $plugin['Name'] == $name  || $plugin['Title'] == $name || ( $mn_plugin_slug == $slug . '-embedded' || $plugin['Name'] == $name . ' Embedded' ) ){
                if($version){
    ...
    public function plugin_is_embedded_version($name, $slug){
        ...
        foreach($plugins as $plugin_id => $plugin){
            ...
            // TBD
            $mn_plugin_slug = dirname($plugin_id);
            if( $mn_plugin_slug == $slug . '-embedded' &&  $plugin['Name'] == $name . ' Embedded'){
                $is = true;
                break;
            }
    ...
    public function download_plugin_ajax_handler(){
        ...
        if($subscription_data && !is_mn_error($subscription_data) && $this->repository_has_valid_subscription($data['repository_id'])){

            if($data['nonce'] == mn_create_nonce('install_plugin_' . $data['url'])){
                ...
                foreach($plugins as $id => $plugin){
                    $mn_plugin_slug = dirname($id);
                    $is_embedded = $this->plugin_is_embedded_version(preg_replace('/ Embedded$/', '', $plugin['Name']), preg_replace('/-embedded$/', '', $mn_plugin_slug));

                    if($mn_plugin_slug == $data['slug'] || $is_embedded && preg_replace('/-embedded$/', '', $mn_plugin_slug) == $data['slug']){
                        $plugin_id = $id;
                        break;
                    }
                }
                ...
                if($plugin_id && empty($is_embedded)){ //upgrade
                    ...
                    if(!$ret && !empty($upgrader->skin->installer_error)){
                        if(is_mn_error($upgrader->skin->installer_error)){
                    ...
                    if($plugin_is_active){
                        //prevent redirects
                        add_filter('mn_redirect', '__return_false');
        ...
        $response['nonce']       = mn_create_nonce('activate_' . $plugin_id);
    ...
    public function activate_plugin(){
        ...
        if(isset($_POST['nonce']) &&  $plugin_id && $_POST['nonce'] == mn_create_nonce('activate_' . $plugin_id )){

            // Deactivate any embedded version
            $plugin_slug = dirname($plugin_id);
            $active_plugins = get_option('active_plugins');
            foreach($active_plugins as $plugin){
                $mn_plugin_slug = dirname($plugin);
                if($mn_plugin_slug == $plugin_slug . '-embedded'){
            ...
            //prevent redirects
            add_filter('mn_redirect', '__return_false', 10000);
    ...
    public function custom_plugins_api_call($false, $action, $args){
        ...
        if($action == 'plugin_information'){
            ...
            $plugin_names = array();
            foreach( $plugins as $plugin_id => $plugin ) {
                // plugins by MN slug which (plugin folder) which can be different
                    ...
                    foreach($package['products'] as $product){
                        ...
                        foreach($product['plugins'] as $plugin_slug){
                            ...
                            if( $download['slug'] == $slug ||
                                ...
                                if( !empty( $download['free-on-mnorg'] ) ){
                                    return false; // use data from mtaandao.org
                                }
    ...
    public function show_purchase_notice_under_plugin($plugin_file, $plugin_data, $status){
        ...
        $mn_list_table = _get_list_table('MN_Plugins_List_Table');
        $mn_version = preg_replace( '/-(.+)$/', '', $GLOBALS['mn_version'] );

        if( version_compare( $mn_version, '4.6', '>=' ) ){
        ...
        if ( isset($_REQUEST['action']) ) {
                ...
                if(isset($plugins) && is_array($plugins)) {
                    ...
                    foreach ($plugins as $k => $plugin) {
                        ...
                        if ($plugin_repository) {
                            ...
                            if(!$site_key && !empty($free_on_mnorg)){ // allow the download from mn.org
                                continue;
                            }
            ...
            if( 'upgrade-plugin' == $action || 'update-plugin' == $action ) {
                ...
                if($plugin_repository) {
                    ...
                    if ( (empty($site_key) || empty($subscription_data)) && empty($free_on_mnorg)) {

                        $error_message = sprintf(__("%s cannot update because your site's registration is not valid. Please %sregister %s%s again for this site first.", 'installer'),
                            '<strong>'.$plugin_name . '</strong>', '<a href="' . $this->menu_url() . '&validate_repository=' . $plugin_repository .
                            '#repository-' . $plugin_repository . '">', $product_name, '</a>');

                        if(defined('DOING_AJAX')){ //MN 4.2
                            ...
                            $status['errorCode'] = 'mn_installer_invalid_subscription';
