 ...
 *
 * Supports automatic updates and installation of Toolset/MNML Themes
 *
...
class Installer_Theme_Class {
    ...
    public function __construct() {
        ...
        $installer_repositories = MN_Installer()->get_repositories();
        ...
        if ( is_array( $repos_with_themes ) ) {
            ...
            foreach ( $repos_with_themes as $k => $repo ) {
                ...
                if ( (isset($installer_repositories[$repo]['api-url'])) && (isset($installer_repositories[$repo]['products'])) ) {
                    ...
                    $this->installer_site_url[$repo] = MN_Installer()->get_installer_site_url( $repo );
                    $this->installer_site_key[$repo] = MN_Installer()->get_site_key( $repo );
                    $this->theme_user_registration[$repo] = false;

                    if ( MN_Installer()->repository_has_valid_subscription( $repo ) ) {

                        $this->installer_theme_subscription_type = MN_Installer()->get_subscription_type_for_repository( $repo );
                        $this->installer_themes_option[$repo] = 'mn_installer_' . $repo . '_themes';
                        $this->installer_themes_available_updates[$repo] = 'mn_installer_' . $repo . '_updated_themes';
    ...
    public function init() {
        ...
        add_action( 'http_api_debug', array($this, 'installer_theme_sync_native_mn_api'), 10, 5 );
        add_filter( 'installer_theme_hook_response_theme', array($this, 'installer_theme_add_num_ratings'), 10, 1 );
        add_filter( 'themes_update_check_locales', array($this, 'installer_theme_sync_call_mn_theme_api'), 10, 1 );
        ...
        add_action( 'mn_ajax_installer_theme_frontend_selected_tab', array($this, 'installer_theme_frontend_selected_tab'), 0 );
        add_action( 'mn_loaded', array($this, 'installer_themes_support_set_up_func') );
    }
    ...
    public function installer_theme_enqueue_scripts() {
        ...
        if ( ('theme-install' == $current_screen) || ($commercial_plugin_screen) || ('theme-install-network' == $current_screen) ) {
            ...
            if ( !(empty($js_array)) ) {
                mn_enqueue_script( 'installer-theme-install', MN_Installer()->res_url() . '/res/js/installer_theme_install.js', array('jquery', 'installer-admin'), MN_Installer()->version() );
                ...
                mn_localize_script( 'installer-theme-install', 'installer_theme_install_localize',
                    array(
                        'js_array_installer' => $js_array,
                        'ajaxurl' => $installer_ajax_url,
                        'no_associated_themes' => $subscription_js_check,
                        'installer_theme_frontend_selected_tab_nonce' => mn_create_nonce( 'installer_theme_frontend_selected_tab' )
                    )
                );
            }
        }
    ...
    /** Override Mtaandao Themes API response with our own themes API*/
    public function installer_theme_api_override_response( $res, $action, $args ) {
        ...
        if ( true === $res ) {
            ...
            //Default MN Themes here
            $client_side_active_tab = get_option( 'mn_installer_clientside_active_tab' );
    ...
    private function installer_theme_get_themes( $product_url = '', $repo_source = '' ) {
        ...
        $current_installer_settings = MN_Installer()->get_settings();
        ...
        if ( (is_array( $current_installer_settings )) && (!(empty($current_installer_settings))) ) {
            ...
            //Call API
            $response = mn_remote_get( $query_remote_url );
    ...
    /** Format response in compatibility with Mtaandao Theme API response */
        ...
        if ( ('theme_information' == $action) && (!(empty($slug))) ) {
            ...
            foreach ( $themes as $k => $theme ) {
                if ( $slug == $theme['basename'] ) {
                    $theme['download_link'] = MN_Installer()->append_site_key_to_download_url( $theme['url'], $this->installer_site_key[$this->installer_theme_active_tab], $this->installer_theme_active_tab );
            ...
            //Let's add themes to the overriden Mtaandao API Theme response
            /** Installer 1.7.6: Update to compatible data format response from MN Theme API */
    ...
    /** When Mtaandao queries its own Themes API, we sync with our own */
    public function installer_theme_sync_native_mn_api( $response, $responsetext, $class, $args, $url ) {

        $api_native_string = 'api.mtaandao.org/themes/';
        if ( (strpos( $url, $api_native_string ) !== false) ) {
            //Mtaandao is querying its own themes API
            $installer_repositories = MN_Installer()->get_repositories();
    ...
    private function installer_theme_get_repo_product_name( $theme_repo ) {
        ...
        if ( isset(MN_Installer()->settings['repositories'][$theme_repo]['data']['product-name']) ) {
            //Set
            $prod_name = MN_Installer()->settings['repositories'][$theme_repo]['data']['product-name'];
            if ( !(empty($prod_name)) ) {
    ...
    private function installer_theme_general_api_query() {
        $products = false;
        $response = mn_remote_get( $this->repository_theme_products );
        if ( !(is_mn_error( $response )) ) {
            //Not MN error
            //Evaluate response
            if ( $response && isset($response['response']['code']) && $response['response']['code'] == 200 ) {
    ...
    /** MN Theme API compatibility- added num ratings */
    ...
    /** When Mtaandao.org makes a call to its repository, let's run our own upgrade checks too */
    public function installer_theme_sync_call_mn_theme_api( $locales ) {
                    ...
                    foreach ( $installed_themes as $theme_slug => $theme_object ) {
                        if ( array_key_exists( $theme_slug, $simplified_available_themes ) ) {
                            ...
                            if ( version_compare( $repository_version, $local_version, '>' ) ) {
                                ...
                                $package_url = MN_Installer()->append_site_key_to_download_url( $theme_download_url, $this->installer_site_key [$repo_slug], $repo_slug );
    ...
    public function installer_theme_add_query_arg_tab( $url, $path, $blog_id = null ) {
        ...
        $mn_install_string = 'update.php?action=install-theme';
        if ( $path == $mn_install_string ) {
    ...
    public function installer_theme_frontend_selected_tab() {
        if ( isset($_POST["frontend_tab_selected"]) ) {
            ...
            if ( !(empty($frontend_tab_selected)) ) {
                ...
                update_option( 'mn_installer_clientside_active_tab', $frontend_tab_selected, false );
    ...
    protected function installer_theme_get_themes_by_subscription( $subscription_type, $repo ) {
        ...
        if ( isset(MN_Installer()->settings['repositories'][$repo]['data']['packages']) ) {
            //Set
            $packages = MN_Installer()->settings['repositories'][$repo]['data']['packages'];
    ...
    /** Hook to mn_loaded, fires when all Installer theme class is ready */
