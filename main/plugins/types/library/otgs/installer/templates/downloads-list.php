                                ...
                                <td>
                                    ...
                                    <?php 
                                        ...
                                        $download_data = array(
                                            'url'           => $url, 
                                            'slug'          => $download['slug'],
                                            'nonce'         => mn_create_nonce('install_plugin_' . $url),
                                            'repository_id' => $repository_id
                                        );
                                    ?>
                                    <input type="checkbox" name="downloads[]" value="<?php echo base64_encode(json_encode($download_data)); ?>" <?php 
                                        if($this->plugin_is_installed($download['name'], $download['slug'], $download['version']) && !$this->plugin_is_embedded_version($download['name'], $download['slug']) || MN_Installer()->dependencies->cant_download($repository_id) ): ?>disabled="disabled"<?php endif; ?> />&nbsp;
                    ...
                    <?php if( !MN_Installer()->dependencies->is_uploading_allowed() ): ?>
                        <p><?php printf(__('Downloading is not possible because Mtaandao cannot write into the plugins folder. %sHow to fix%s.', 'installer'),
                                '<a href="http://codex.mtaandao.org/Changing_File_Permissions">', '</a>') ?></p>
                    <?php elseif( MN_Installer()->dependencies->is_win_paths_exception($repository_id) ): ?>
                        <p><?php echo MN_Installer()->dependencies->win_paths_exception_message() ?></p>
                    <?php endif; ?>
                    </div>
