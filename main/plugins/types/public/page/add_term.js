 ...
 *
 * See admin/edit-tags.php and admin/js/tags.php for details.
 *
 * @since 2.1
 */
Types.page.addTerm = (function($) {
     ...
     *
     * This is used to outrun the binding on the original submit button from that's defined in admin/js/tags.js
    ...
    self.createFauxSubmitButton = function() {
        ...
        var handleSubmitAction = function(e) {
            ...
            if(form.valid()) {
                MNV_Toolset.Utils.Spinner.show(MNV_Toolset.Utils.Spinner.find(form));
        ...
        submitSection.append(customSubmitButton).append(MNV_Toolset.Utils.Spinner.create());
    ...
    self.catchAfterTermCreatedEvent = function(event, xhr, settings) {
        ...
        var action = MNV_Toolset.Utils.getParameterByName('action', data);
        ...
        if('add-tag' == action && self.hasTypesFields()) {
            ...
            if(hasAjaxError) {
                MNV_Toolset.Utils.Spinner.hide(MNV_Toolset.Utils.Spinner.find($(self.formSelector)));
                $('html, body').animate({ scrollTop: 0 }, "slow");
            } else {
                var newUrl = MNV_Toolset.Utils.updateUrlQuery('message', '1');
