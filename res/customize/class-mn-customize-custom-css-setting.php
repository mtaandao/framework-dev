<?php
/**
 * Customize API: MN_Customize_Custom_CSS_Setting class
 *
...
/**
 * Custom Setting to handle MN Custom CSS.
 *
 * @since 4.7.0
 *
 * @see MN_Customize_Setting
 */
final class MN_Customize_Custom_CSS_Setting extends MN_Customize_Setting {
	...
	/**
	 * MN_Customize_Custom_CSS_Setting constructor.
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 ...
	 *
	 * @param MN_Customize_Manager $manager The Customize Manager class.
	...
	public function preview() {
		...
		add_filter( 'mn_get_custom_css', array( $this, 'filter_previewed_mn_get_custom_css' ), 9, 2 );
		return true;
	}

	/**
	 * Filter `mn_get_custom_css` for applying the customized value.
	 *
	 * This is used in the preview when `mn_get_custom_css()` is called for rendering the styles.
	 *
	 * @since 4.7.0
	 * @access private
	 * @see mn_get_custom_css()
	 *
	 * @param string $css        Original CSS.
	 * @param string $stylesheet Current stylesheet.
	 * @return string CSS.
	 */
	public function filter_previewed_mn_get_custom_css( $css, $stylesheet ) {
	 ...
	 * @since 4.7.0
	 * @access public
	 * @see MN_Customize_Setting::value()
	 *
	 * @return string
	 */
	public function value() {
		...
		$value = '';
		$post = mn_get_custom_css_post( $this->stylesheet );
		if ( $post ) {
	...
	public function update( $css ) {
		...
		$r = mn_update_custom_css_post( $css, array(
			'stylesheet' => $this->stylesheet,
		) );

		if ( $r instanceof MN_Error ) {
			return false;
		}
		$post_id = $r->ID;
