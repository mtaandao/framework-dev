<?php
/**
 * HTTP API: MN_Http_Streams class
 *
 * @package Mtaandao
 * @subpackage HTTP
 * @since 4.4.0
 ...
 */
class MN_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see MN_Http::request For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	...
	public function request($url, $args = array()) {
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str )
				return new MN_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );

			return new MN_Error('http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $r['stream'] ) {
			if ( ! MN_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new MN_Error( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
			...
			while ( ! feof($handle) && $keep_reading ) {
				...
				if ( ! $bodyStarted ) {
					...
					if ( strpos( $strResponse, "\r\n\r\n" ) ) {
						$process = MN_Http::processResponse( $strResponse );
						$bodyStarted = true;
						$block = $process['body'];
						unset( $strResponse );
						$process['body'] = '';
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new MN_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$arrHeaders = MN_Http::processHeaders( $process['headers'], $url );
		...
		// Handle redirects.
		if ( false !== ( $redirect_response = MN_Http::handle_redirects( $url, $r, $response ) ) )
			return $redirect_response;

		// If the body was chunk encoded, then decode it.
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
			$process['body'] = MN_Http::chunkTransferDecode($process['body']);

		if ( true === $r['decompress'] && true === MN_Http_Encoding::should_decode($arrHeaders['headers']) )
			$process['body'] = MN_Http_Encoding::decompress( $process['body'] );
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( MN_Http::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of MN_Http directly through its API.
 *
 * @see MN_HTTP::request
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use MN_HTTP::request() directly
 */
class MN_HTTP_Fsockopen extends MN_HTTP_Streams {
