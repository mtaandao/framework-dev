<?php
/**
 * Meta API: MN_Metadata_Lazyloader class
 *
 * @package Mtaandao
 * @subpackage Meta
 * @since 4.5.0
 ...
 *
 * When loading many objects of a given type, such as posts in a MN_Query loop, it often makes
 ...
 *
 * MN uses the MN_Metadata_Lazyloader class to queue objects for metadata cache priming. The class
 * then detects the relevant get_*_meta() function call, and queries the metadata of all queued objects.
 *
 * Do not access this class directly. Use the mn_metadata_lazyloader() function.
 *
 * @since 4.5.0
 */
class MN_Metadata_Lazyloader {
	/**
	 ...
	 * @return bool|MN_Error True on success, MN_Error on failure.
	 */
	public function queue_objects( $object_type, $object_ids ) {
		if ( ! isset( $this->settings[ $object_type ] ) ) {
			return new MN_Error( 'invalid_object_type', __( 'Invalid object type' ) );
		}
		 ...
		 * @param MN_Metadata_Lazyloader $lazyloader  The lazy-loader object.
		 */
	 ...
	 * directly, from either inside or outside the `MN_Query` object.
	 *
	 * @since 4.5.0
	 *
