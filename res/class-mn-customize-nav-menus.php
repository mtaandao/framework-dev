...
final class MN_Customize_Nav_Menus {
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xmn/mn-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L469-L499
		...
		add_action( 'mn_ajax_customize-nav-menus-insert-auto-draft', array( $this, 'ajax_insert_auto_draft_post' ) );
	...
	public function ajax_load_available_items() {
		...
		if ( isset( $_POST['item_types'] ) && is_array( $_POST['item_types'] ) ) {
			$item_types = mn_unslash( $_POST['item_types'] );
		} elseif ( isset( $_POST['type'] ) && isset( $_POST['object'] ) ) { // Back compat.
			$item_types[] = array(
				'type' => mn_unslash( $_POST['type'] ),
				'object' => mn_unslash( $_POST['object'] ),
		...
		mn_send_json_success( array( 'items' => $all_items ) );
	}
	...
	public function search_available_items_query( $args = array() ) {
		...
		if ( 1 === $args['pagenum'] && $nav_menus_created_posts_setting && count( $nav_menus_created_posts_setting ) > 0 ) {
			$stub_post_query = new MN_Query( array_merge(
				$query,
				array(
	...
	public function customize_register() {
		 ...
		 * See https://github.com/xmn/mn-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L506-L543
		 */
		...
		$this->manager->add_setting( new MN_Customize_Filter_Setting( $this->manager, 'nav_menus_created_posts', array(
			'transport' => 'postMessage',
	 ...
	 * }
	 * @return MN_Post|MN_Error Inserted auto-draft post object or error.
	 */
	public function insert_auto_draft_post( $postarr ) {
		if ( ! isset( $postarr['post_type'] ) || ! post_type_exists( $postarr['post_type'] )  ) {
			return new MN_Error( 'unknown_post_type', __( 'Unknown post type' ) );
		}
		if ( empty( $postarr['post_title'] ) ) {
			return new MN_Error( 'empty_title', __( 'Empty title' ) );
		}
		if ( ! empty( $postarr['post_status'] ) ) {
			return new MN_Error( 'status_forbidden', __( 'Status is forbidden' ) );
		}
		...
		add_filter( 'mn_insert_post_empty_content', '__return_false', 1000 );
		$r = mn_insert_post( mn_slash( $postarr ), true );
		remove_filter( 'mn_insert_post_empty_content', '__return_false', 1000 );
	...
	public function ajax_insert_auto_draft_post() {
		if ( ! check_ajax_referer( 'customize-menus', 'customize-menus-nonce', false ) ) {
			mn_send_json_error( 'bad_nonce', 400 );
		}

		if ( ! current_user_can( 'customize' ) ) {
			mn_send_json_error( 'customize_not_allowed', 403 );
		}

		if ( empty( $_POST['params'] ) || ! is_array( $_POST['params'] ) ) {
			mn_send_json_error( 'missing_params', 400 );
		}

		$params = mn_unslash( $_POST['params'] );
		$illegal_params = array_diff( array_keys( $params ), array( 'post_type', 'post_title' ) );
		if ( ! empty( $illegal_params ) ) {
			mn_send_json_error( 'illegal_params', 400 );
		}
	...
	public function make_auto_draft_status_previewable() {
		...
		$mn_post_statuses['auto-draft']->protected = true;
	}
	...
	public function sanitize_nav_menus_created_posts( $value ) {
		$post_ids = array();
		foreach ( mn_parse_id_list( $value ) as $post_id ) {
			if ( empty( $post_id ) ) {
				continue;
			}
	 ...
	 * `MN_Customize_Nav_Menu_Items::sanitize_nav_menus_created_posts()` to
	 ...
	 * @access public
	 *
	 * @param MN_Customize_Setting $setting Customizer setting object.
	 */
	public function save_nav_menus_created_posts( $setting ) {
		...
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				...
				// Note that mn_publish_post() cannot be used because unique slugs need to be assigned.
				mn_update_post( mn_slash( $args ) );
