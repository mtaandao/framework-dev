...
// Users
add_filter( 'mnmu_validate_user_signup', 'signup_nonce_check' );
add_action( 'init', 'maybe_add_existing_user_to_blog' );
add_action( 'mnmu_new_user', 'newuser_notify_siteadmin' );
add_action( 'mnmu_activate_user', 'add_new_user_to_blog', 10, 3 );
add_action( 'mnmu_activate_user', 'mnmu_welcome_user_notification', 10, 3 );
add_action( 'after_signup_user', 'mnmu_signup_user_notification', 10, 4 );
add_action( 'network_site_new_created_user',   'mn_send_new_user_notifications' );
add_action( 'network_site_users_created_user', 'mn_send_new_user_notifications' );
add_action( 'network_user_new_created_user',   'mn_send_new_user_notifications' );
add_filter( 'sanitize_user', 'strtolower' );

// Blogs
add_filter( 'mnmu_validate_blog_signup', 'signup_nonce_check' );
add_action( 'mnmu_new_blog', 'mnmu_log_new_registrations', 10, 2 );
add_action( 'mnmu_new_blog', 'newblog_notify_siteadmin', 10, 2 );
add_action( 'mnmu_activate_blog', 'mnmu_welcome_notification', 10, 5 );
add_action( 'after_signup_site', 'mnmu_signup_blog_notification', 10, 7 );
...
// Counts
add_action( 'admin_init', 'mn_schedule_update_network_counts');
add_action( 'update_network_counts', 'mn_update_network_counts');
foreach ( array( 'user_register', 'deleted_user', 'mnmu_new_user', 'make_spam_user', 'make_ham_user' ) as $action )
	add_action( $action, 'mn_maybe_update_network_user_counts' );
foreach ( array( 'make_spam_blog', 'make_ham_blog', 'archive_blog', 'unarchive_blog', 'make_delete_blog', 'make_undelete_blog' ) as $action )
	add_action( $action, 'mn_maybe_update_network_site_counts' );
unset( $action );

// Files
add_filter( 'mn_upload_bits', 'upload_is_file_too_big' );
...
// MN_HOME and MN_SITEURL should not have any effect in MS
remove_filter( 'option_siteurl', '_config_mn_siteurl' );
remove_filter( 'option_home',    '_config_mn_home'    );
