...
class MN_Term_Query {
	...
	public function get_terms() {
		...
		if ( ! empty( $this->query_vars['object_ids'] ) ) {
			$join .= " INNER JOIN {$mndb->term_relationships} AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id";
		}
		...
		$last_changed = mn_cache_get_last_changed( 'terms' );
		...
		if ( 'count' == $_fields ) {
			$count = $mndb->get_var( $this->request );
			mn_cache_set( $cache_key, $count, 'terms' );
			return $count;
		}
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', mn_parse_id_list( $this->query_vars['include'] ) );
