...
mnList = {
	 ...
	 * 4. `_mnnonce` value in element's href attribute.
	 * 5. `_mnnonce` input field that is a descendant of element.
	...
	nonce: function( element, settings ) {
		var url      = mnAjax.unserialize( element.attr( 'href' ) ),
			$element = $( '#' + settings.element );

		return settings.nonce || url._ajax_nonce || $element.find( 'input[name="_ajax_nonce"]' ).val() || url._mnnonce || $element.find( 'input[name="_mnnonce"]' ).val() || 0;
	},

	/**
	 * Extract list item data from a DOM element.
	 *
	 * Example 1: data-mn-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-mn-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		var data = [], mnListsData;

		try {
			mnListsData = $( element ).data( 'mn-lists' ) || '';
			mnListsData = mnListsData.match( new RegExp( type + ':[\\S]+' ) );

			if ( mnListsData ) {
				data = mnListsData[0].split( ':' );
			}
		} catch ( error ) {}
	...
	pre: function( list, settings, action ) {
		...
		settings = $.extend( {}, this.mnList.settings, {
			element: null,
			nonce:   0,
			target:  list.get( 0 )
		}, settings || {} );
	...
	ajaxAdd: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mnList.parseData( $element, 'add' ),
			formValues, formData, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = mnList.pre.call( list, $element, settings, 'add' );
		...
		if ( ! $element.is( '[id="' + settings.element + '-submit"]' ) ) {
			return ! mnList.add.call( list, $element, settings );
		}
		...
		settings.nonce  = mnList.nonce( $element, settings );

		if ( ! mnAjax.validateForm( '#' + settings.element ) ) {
			return false;
		}
		...
		formValues = $( '#' + settings.element + ' :input' ).not( '[name="_ajax_nonce"], [name="_mnnonce"], [name="action"]' );
		...
		settings.success = function( response ) {
			parsedResponse   = mnAjax.parseAjaxResponse( response, settings.response, settings.element );
			...
			$.each( parsedResponse.responses, function() {
				mnList.add.call( list, this.data, $.extend( {}, settings, { // this.firstChild.nodevalue
			...
			list.mnList.recolor();
			$( list ).trigger( 'mnListAddEnd', [ settings, list.mnList ] );
			mnList.clear.call( list, '#' + settings.element );
		};
	...
	ajaxDel: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mnList.parseData( $element, 'delete' ),
			$eventTarget, parsedResponse, returnedResponse;

		settings = settings || {};
		settings = mnList.pre.call( list, $element, settings, 'delete' );
		...
		if ( 'none' !== settings.delColor ) {
			$eventTarget.css( 'backgroundColor', settings.delColor ).fadeOut( 350, function() {
				list.mnList.recolor();
				$( list ).trigger( 'mnListDelEnd', [ settings, list.mnList ] );
			} );
		} else {
			list.mnList.recolor();
			$( list ).trigger( 'mnListDelEnd', [ settings, list.mnList ] );
	...
	ajaxDim: function( element, settings ) {
		var list     = this,
			$element = $( element ),
			data     = mnList.parseData( $element, 'dim' ),
		...
		settings = settings || {};
		settings = mnList.pre.call( list, $element, settings, 'dim' );
		...
		color        = mnList.getColor( $eventTarget );
			...
			$eventTarget
				...
				.animate( { backgroundColor: color }, {
					complete: function() {
						...
						$( list ).trigger( 'mnListDimEnd', [ settings, list.mnList ] );
					}
				} );
		} else {
			$( list ).trigger( 'mnListDimEnd', [ settings, list.mnList ] );
	...
	add: function( element, settings ) {
		...
		settings = $.extend( { position: 0, id: 0, oldId: null }, this.mnList.settings, settings );
		...
		if ( 'none' !== settings.addColor ) {
			$element.css( 'backgroundColor', settings.addColor ).animate( { backgroundColor: mnList.getColor( $element ) }, {
				complete: function() {
					$( this ).css( 'backgroundColor', '' );
				}
			} );
	...
	clear: function( elementId ) {
		...
		if ( list.mnList && $element.parents( '#' + list.id ).length ) {
			return;
		}
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-mn-lists^="add:' + list.id + ':"]', function() {
			return list.mnList.add( this );
		} );

		$element.on( 'click', 'a[data-mn-lists^="add:' + list.id + ':"], input[data-mn-lists^="add:' + list.id + ':"]', function() {
			return list.mnList.add( this );
		} );

		$element.on( 'click', '[data-mn-lists^="delete:' + list.id + ':"]', function() {
			return list.mnList.del( this );
		} );

		$element.on( 'click', '[data-mn-lists^="dim:' + list.id + ':"]', function() {
	...
	recolor: function() {
		...
		items.filter( evenOdd[0] ).addClass( list.mnList.settings.alt ).end();
		items.filter( evenOdd[1] ).removeClass( list.mnList.settings.alt );
	},
	...
	init: function() {
		...
		$list.mnList.process = function( element ) {
			$list.each( function() {
				this.mnList.process( element );
			} );
		};

		$list.mnList.recolor = function() {
			$list.each( function() {
				this.mnList.recolor();
			} );
		};
 ...
 *
 * @param {mnList~confirm}   settings.confirm     Callback that's run before a request is made. Default: null.
 * @param {mnList~addBefore} settings.addBefore   Callback that's run before an item gets added to the list.
 *                                                Default: null.
 * @param {mnList~addAfter}  settings.addAfter    Callback that's run after an item got added to the list.
 *                                                Default: null.
 * @param {mnList~delBefore} settings.delBefore   Callback that's run before an item gets deleted from the list.
 *                                                Default: null.
 * @param {mnList~delAfter}  settings.delAfter    Callback that's run after an item got deleted from the list.
 *                                                Default: null.
 * @param {mnList~dimBefore} settings.dimBefore   Callback that's run before an item gets dim'd. Default: null.
 * @param {mnList~dimAfter}  settings.dimAfter    Callback that's run after an item got dim'd. Default: null.
 * @returns {$.fn} mnList API function.
 */
$.fn.mnList = function( settings ) {
	this.each( function( index, list ) {
		list.mnList = {
			settings: $.extend( {}, mnList.settings, { what: mnList.parseData( list, 'list' )[1] || '' }, settings )
		};

		$.each( functions, function( func, callback ) {
			list.mnList[func] = function( element, setting ) {
				return mnList[callback].call( list, element, setting );
			};
		} );
	} );
