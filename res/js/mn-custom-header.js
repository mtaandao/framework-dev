...
(function( window, settings ) {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'mn-custom-header-video-loaded' );
						break;
					}
				}
			}
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'mn-custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'mn-custom-header-video-button' );
			button.setAttribute( 'class', 'mn-custom-header-video-button mn-custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'mn-custom-header-video-button mn-custom-header-video-play';
				button.innerHTML = settings.l10n.pause;
				if ( 'a11y' in window.mn ) {
					window.mn.a11y.speak( settings.l10n.playSpeak);
				}
			});

			this.container.addEventListener( 'pause', function() {
				button.className = 'mn-custom-header-video-button mn-custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window.mn ) {
					window.mn.a11y.speak( settings.l10n.pauseSpeak);
				}
			});
	...
	NativeHandler = BaseHandler.extend({
		...
		ready: function() {
			...
			video.id = 'mn-custom-header-video';
	...
	window.mn.customHeader = new CustomHeader();
	document.addEventListener( 'DOMContentLoaded', window.mn.customHeader.initialize.bind( window.mn.customHeader ), false );

	// Selective refresh support in the Customizer.
	if ( 'customize' in window.mn ) {
		window.mn.customize.selectiveRefresh.bind( 'render-partials-response', function( response ) {
			if ( 'custom_header_settings' in response ) {
				settings = response.custom_header_settings;
			}
		});

		window.mn.customize.selectiveRefresh.bind( 'partial-content-rendered', function( placement ) {
			if ( 'custom_header' === placement.partial.id ) {
				window.mn.customHeader.initialize();
			}
		});
	}

})( window, window._mnCustomHeaderSettings || {} );
