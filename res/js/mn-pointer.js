...
(function($){
	...
	$.widget('mn.pointer', {
		options: {
			pointerClass: 'mn-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var close  = ( mnPointerL10n ) ? mnPointerL10n.dismiss : 'Dismiss',
		...
		_create: function() {
			...
			this.content = $('<div class="mn-pointer-content"></div>');
			this.arrow   = $('<div class="mn-pointer-arrow"><div class="mn-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="mn-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /mn-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'mn-pointer-' + edge );
		},
