...
(function( exports, $ ){
	...
	api.Preview = api.Messenger.extend({
		...
		handleLinkClick: function( event ) {
			...
			if ( ! api.isLinkPreviewable( link[0] ) ) {
				mn.a11y.speak( api.settings.l10n.linkUnpreviewable );
		...
		handleFormSubmit: function( event ) {
			...
			if ( 'GET' !== form.prop( 'method' ).toUpperCase() || ! api.isLinkPreviewable( urlParser ) ) {
				mn.a11y.speak( api.settings.l10n.formUnpreviewable );
				event.preventDefault();
				return;
			}
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		// Skip mn login and signup pages.
		if ( /\/mn-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}
		...
		if ( /\/mn-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		if ( $( element ).closest( '#mnadminbar' ).length ) {
			return;
		}
