(function( window, undefined ) {
	...
	window.mn            = window.mn || {};
	mn.api               = mn.api || new MN_API();
	mn.api.versionString = mn.api.versionString || 'mn/v2/';
...
(function( window, undefined ) {
	 ...
	 * @param {Date} date.
	 */
	mn.api.utils.parseISO8601 = function( date ) {
	 ...
	 */
	mn.api.utils.getRootUrl = function() {
	 ...
	 */
	mn.api.utils.capitalize = function( str ) {
	 ...
	 */
	mn.api.utils.extractRoutePart = function( route, part ) {
		var routeParts;

		part  = part || 1;

		// Remove versions string from route to avoid returning it.
		route = route.replace( mn.api.versionString, '' );
	 ...
	 */
	mn.api.utils.extractParentName = function( route ) {
		var name,
	 ...
	 *                                to add the args to.
	 */
	mn.api.utils.decorateFromRoute = function( routeEndpoints, modelInstance ) {
	 ...
	 */
	mn.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. Mtaandao expects dates in UTC.
				 ...
				 *
				 * Mtaandao returns 'date' and 'date_modified' in the timezone of the server as well as
				...
				getDate: function( field ) {
					...
					return new Date( mn.api.utils.parseISO8601( theISODate ) );
				}
			},
			...
			buildModelGetter = function( parentModel, modelId, modelName, embedSourcePoint, embedCheckField ) {
				...
				getModel = new mn.api.models[ modelName ]( attributes );
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a mn.api.collections[ collectionName ]
				 * collection.
				 */
				...
				getObjects = new mn.api.collections[ collectionName ]( properties, classProperties );
			...
			TagsMixin = {
				...
				getTags: function() {
					var tagIds = this.get( 'tags' ),
						tags  = new mn.api.collections.Tags();
				...
				setTags: function( tags ) {
					...
					if ( _.isArray( tags ) ) {
						...
						// Get all the tags.
						allTags = new mn.api.collections.Tags();
						allTags.fetch( {
							data:    { per_page: 100 },
							success: function( alltags ) {
								...
								_.each( tags, function( tag ) {
									newTag = new mn.api.models.Tag( alltags.findWhere( { slug: tag } ) );
								...
								tags = new mn.api.collections.Tags( newTags );
			...
			CategoriesMixin = {
				...
				getCategories: function() {
					var categoryIds = this.get( 'categories' ),
						categories  = new mn.api.collections.Categories();
				...
				setCategories: function( categories ) {
					...
					if ( _.isArray( categories ) ) {
						...
						allCategories = new mn.api.collections.Categories();
						allCategories.fetch( {
							data:    { per_page: 100 },
							success: function( allcats ) {
								...
								_.each( categories, function( category ) {
									newCategory = new mn.api.models.Category( allcats.findWhere( { slug: category } ) );
								...
								categories = new mn.api.collections.Categories( newCategories );
			...
			FeaturedMediaMixin = {
				getFeaturedMedia: function() {
					return buildModelGetter( this, this.get( 'featured_media' ), 'Media', 'mn:featuredmedia', 'source_url' );
				}
			};
...
(function() {
	...
	var mnApiSettings = window.mnApiSettings || {};

	/**
	 * Backbone base model for all models.
	 */
	mn.api.MNApiBaseModel = Backbone.Model.extend(
		/** @lends MNApiBaseModel.prototype  */
		{
			/**
			...
			sync: function( method, model, options ) {
				...
				if ( ! _.isUndefined( mnApiSettings.nonce ) && ! _.isNull( mnApiSettings.nonce ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-MN-Nonce', mnApiSettings.nonce );
	 ...
	 */
	mn.api.models.Schema = mn.api.MNApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				mn.api.MNApiBaseModel.prototype.initialize.call( model, attributes, options );

				model.apiRoot = options.apiRoot || mnApiSettings.root;
				model.versionString = options.versionString || mnApiSettings.versionString;
			},
...
( function() {
	 ...
	 */
	mn.api.MNApiBaseCollection = Backbone.Collection.extend(
		/** @lends BaseCollection.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( 'undefined' !== typeof mnApiSettings.nonce ) {
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-MN-Nonce', mnApiSettings.nonce );
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-mn-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-mn-total' ), 10 );
						}
...
( function() {
	...
	var Endpoint, initializedDeferreds = {},
		mnApiSettings = window.mnApiSettings || {};
	window.mn = window.mn || {};
	mn.api    = mn.api || {};

	// If mnApiSettings is unavailable, try the default.
	if ( _.isEmpty( mnApiSettings ) ) {
		mnApiSettings.root = window.location.origin + '/mn-json/';
	}

	Endpoint = Backbone.Model.extend( {
		defaults: {
			apiRoot: mnApiSettings.root,
			versionString: mn.api.versionString,
			schema: null,
			models: {},
			collections: {}
		},
		...
		initialize: function() {
			...
			model.schemaModel = new mn.api.models.Schema( null, {
			...
			if ( model.get( 'schema' ) ) {
				...
				( _.isUndefined( mnApiSettings.cacheSchema ) || mnApiSettings.cacheSchema ) &&
				sessionStorage.getItem( 'mn-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'mn-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( mnApiSettings.cacheSchema ) || mnApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'mn-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
		...
		constructFromSchema: function() {
			 ...
			 *
			 * Localizing a variable mnApiSettings.mapping will over-ride the default mapping options.
			 *
			 */
			mapping = mnApiSettings.mapping || {
				models: {
					'Categories':      'Category',
					'Comments':        'Comment',
					'Pages':           'Page',
			...
			modelRoutes      = [];
			collectionRoutes = [];
			schemaRoot       = routeModel.get( 'apiRoot' ).replace( mn.api.utils.getRootUrl(), '' );
			loadingObjects   = {};
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				var modelClassName,
						routeName  = mn.api.utils.extractRoutePart( modelRoute.index, 2 ),
						parentName = mn.api.utils.extractRoutePart( modelRoute.index, 4 ),
						routeEnd   = mn.api.utils.extractRoutePart( modelRoute.index, 1 );
				...
				if ( '' !== parentName && parentName !== routeName ) {
					modelClassName = mn.api.utils.capitalize( parentName ) + mn.api.utils.capitalize( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = mn.api.MNApiBaseModel.extend( {
					...
					modelClassName = mn.api.utils.capitalize( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = mn.api.MNApiBaseModel.extend( {
				...
				mn.api.utils.decorateFromRoute( modelRoute.route.endpoints, loadingObjects.models[ modelClassName ] );
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						...
						parentName = mn.api.utils.extractRoutePart( collectionRoute.index, 3 );

				// If the collection has a parent in its route, add that to its class name.
				if ( '' !== parentName && parentName !== routeName ) {

					collectionClassName = mn.api.utils.capitalize( parentName ) + mn.api.utils.capitalize( routeName );
					...
					loadingObjects.collections[ collectionClassName ] = mn.api.MNApiBaseCollection.extend( {
					...
					collectionClassName = mn.api.utils.capitalize( routeName );
					modelClassName      = mapping.models[ collectionClassName ] || collectionClassName;
					collectionClassName = mapping.collections[ collectionClassName ] || collectionClassName;
					loadingObjects.collections[ collectionClassName ] = mn.api.MNApiBaseCollection.extend( {
				...
				// Add defaults to the new model, pulled form the endpoint.
				mn.api.utils.decorateFromRoute( collectionRoute.route.endpoints, loadingObjects.collections[ collectionClassName ] );
			} );

			// Add mixins and helpers for each of the models.
			_.each( loadingObjects.models, function( model, index ) {
				loadingObjects.models[ index ] = mn.api.utils.addMixinsAndHelpers( model, index, loadingObjects );
			} );
	...
	mn.api.endpoints = new Backbone.Collection( {
		model: Endpoint
	} );

	/**
	 * Initialize the mn-api, optionally passing the API root.
	 *
	 * @param {object} [args]
	 * @param {string} [args.apiRoot] The api root. Optional, defaults to mnApiSettings.root.
	 * @param {string} [args.versionString] The version string. Optional, defaults to mnApiSettings.root.
	 * @param {object} [args.schema] The schema. Optional, will be fetched from API if not provided.
	 */
	mn.api.init = function( args ) {
		var endpoint, attributes = {}, deferred, promise;

		args                     = args || {};
		attributes.apiRoot       = args.apiRoot || mnApiSettings.root;
		attributes.versionString = args.versionString || mnApiSettings.versionString;
		attributes.schema        = args.schema || null;
		if ( ! attributes.schema && attributes.apiRoot === mnApiSettings.root && attributes.versionString === mnApiSettings.versionString ) {
			attributes.schema = mnApiSettings.schema;
		}

		if ( ! initializedDeferreds[ attributes.apiRoot + attributes.versionString ] ) {
			endpoint = mn.api.endpoints.findWhere( { apiRoot: attributes.apiRoot, versionString: attributes.versionString } );
			if ( ! endpoint ) {
				endpoint = new Endpoint( attributes );
				mn.api.endpoints.add( endpoint );
			}
			deferred = jQuery.Deferred();
			...
			endpoint.schemaConstructed.done( function( endpoint ) {
				...
				mn.api.models      = _.extend( endpoint.get( 'models' ), mn.api.models );
				mn.api.collections = _.extend( endpoint.get( 'collections' ), mn.api.collections );
				deferred.resolveWith( mn.api, [ endpoint ] );
			} );
	...
	// The mn.api.init function returns a promise that will resolve with the endpoint once it is ready.
	mn.api.loadPromise = mn.api.init();
