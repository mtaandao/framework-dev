( function( tinymce, mn, settings ) {
	tinymce.PluginManager.add( 'mnemoji', function( editor ) {
		var typing,
			env = tinymce.Env,
		...
		if ( ! mn || ! mn.emoji || settings.supports.everything ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-mn-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			mn.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after mn.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-mn-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-mn-emoji' ) ) {
				event.preventDefault();
			}
		} );
	} );
} )( window.tinymce, window.mn, window._mnemojiSettings );
