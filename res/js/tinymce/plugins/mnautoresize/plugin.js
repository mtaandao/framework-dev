...
// Forked for Mtaandao so it can be turned on/off after loading.
 ...
 * it's initialized.
 */
tinymce.PluginManager.add( 'mnautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'mn-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'mn-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'mn-autoresize' );
	...
	function off() {
		...
		if ( ! settings.mn_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'mn-autoresize' );
	...
	editor.addCommand( 'mnAutoResize', resize );

	// On/off
	editor.addCommand( 'mnAutoResizeOn', on );
	editor.addCommand( 'mnAutoResizeOff', off );
});
