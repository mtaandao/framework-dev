 ...
 * @since 4.7.0
 *
 * @param int|MN_User $user_id User's ID or a MN_User object. Defaults to current user.
 * @return string The locale of the user.
 */
function get_user_locale( $user_id = 0 ) {
	$user = false;
	if ( 0 === $user_id && function_exists( 'mn_get_current_user' ) ) {
		$user = mn_get_current_user();
	} elseif ( $user_id instanceof MN_User ) {
		$user = $user_id;
 ...
 * @param string $plugin_rel_path Optional. Relative path to PLUGIN_DIR where the .mo file resides.
 *                                Default false.
...
function _get_path_to_translation_from_lang_dir( $domain ) {
	...
	$path = MN_LANG_DIR . '/plugins/' . $mofile;
	if ( in_array( $path, $cached_mofiles ) ) {
		return $path;
	}

	$path = MN_LANG_DIR . '/themes/' . $mofile;
	if ( in_array( $path, $cached_mofiles ) ) {
		return $path;
	}
...
function is_rtl() {
	global $mn_locale;
	if ( ! ( $mn_locale instanceof MN_Locale ) ) {
		return false;
	}
 ...
 * @since 4.7.0
 *
 * @global MN_Locale_Switcher $mn_locale_switcher
 *
 * @param string $locale The locale.
 * @return bool True on success, false on failure.
 */
function switch_to_locale( $locale ) {
	/* @var MN_Locale_Switcher $mn_locale_switcher */
	global $mn_locale_switcher;

	return $mn_locale_switcher->switch_to_locale( $locale );
}
...
function restore_previous_locale() {
	...
	return $mn_locale_switcher->restore_previous_locale();
}
...
function restore_current_locale() {
	...
	return $mn_locale_switcher->restore_current_locale();
}
...
function is_locale_switched() {
	...
	return $mn_locale_switcher->is_switched();
}
