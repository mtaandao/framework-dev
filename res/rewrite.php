...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$mn_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$mn_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $mn_rewrite;
	$mn_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$mn_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see MN_Rewrite::remove_permastruct()
 * @global MN_Rewrite $mn_rewrite Mtaandao rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$mn_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $function ) {
	...
	if ( ! in_array( $feedname, $mn_rewrite->feeds ) ) {
		$mn_rewrite->feeds[] = $feedname;
	}
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $mn_rewrite;
	$mn_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _mn_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to MN_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, mn_unique_post_slug() prevents the creation of post slugs
 ...
 *                          MN::parse_request(). Default empty array.
 * @return array Returns the original array of query vars, with date/post conflicts resolved.
 */
function mn_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' == get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof MN_Post ) {
	...
	if ( !$mn_rewrite->using_index_permalinks() )
		$url = str_replace( $mn_rewrite->index . '/', '', $url );
	...
	foreach ( (array)$rewrite as $match => $query) {
		...
		if ( preg_match("#^$match#", $request_match, $matches) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( $key, $mn->public_query_vars ) ){
					$query[$key] = $value;
			...
			$query = mn_resolve_numeric_slug_conflicts( $query );
