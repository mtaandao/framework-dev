...
function _mn_scripts_maybe_doing_it_wrong( $function ) {
	if ( did_action( 'init' ) || did_action( 'admin_enqueue_scripts' ) || did_action( 'mn_enqueue_scripts' ) || did_action( 'login_enqueue_scripts' ) ) {
		return;
	}

	_doing_it_wrong( $function, sprintf(
		/* translators: 1: mn_enqueue_scripts, 2: admin_enqueue_scripts, 3: login_enqueue_scripts */
...
function mn_deregister_script( $handle ) {
	...
	if ( ( is_admin() && 'admin_enqueue_scripts' !== $current_filter ) ||
		...
		if ( in_array( $handle, $no ) ) {
			$message = sprintf(
				/* translators: 1: script name, 2: mn_enqueue_scripts */
 ...
 *                                    Default 'false'.
 */
function mn_enqueue_script( $handle, $src = '', $deps = array(), $ver = false, $in_footer = false ) {
	$mn_scripts = mn_scripts();
