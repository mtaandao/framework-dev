<?php
/**
 * REST API: MN_REST_Meta_Fields class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 */
abstract class MN_REST_Meta_Fields {
	 ...
	 * @return MN_Error|object Object containing the meta values by name, otherwise MN_Error object.
	 */
	 ...
	 * @param MN_REST_Request $request Current request object.
	 ...
	 * @param int             $object_id Object ID to fetch meta for.
	 * @return MN_Error|null MN_Error if one occurs, null on success.
	 */
	 ...
	 * @return bool|MN_Error True if meta field is deleted, MN_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( ! delete_metadata( $meta_type, $object_id, mn_slash( $meta_key ) ) ) {
			return new MN_Error(
				'rest_meta_database_error',
				__( 'Could not delete meta value from database.' ),
				array( 'key' => $name, 'status' => MN_Http::INTERNAL_SERVER_ERROR )
			);
		}
	 ...
	 * @return bool|MN_Error True if meta fields are updated, MN_Error otherwise.
	 */
	protected function update_multi_meta_value( $object_id, $meta_key, $name, $values ) {
		...
		foreach ( $to_remove as $value ) {
			if ( ! delete_metadata( $meta_type, $object_id, mn_slash( $meta_key ), mn_slash( $value ) ) ) {
				return new MN_Error(
					'rest_meta_database_error',
		...
		foreach ( $to_add as $value ) {
			if ( ! add_metadata( $meta_type, $object_id, mn_slash( $meta_key ), mn_slash( $value ) ) ) {
				return new MN_Error(
					'rest_meta_database_error',
					__( 'Could not update meta value in database.' ),
					array( 'key' => $name, 'status' => MN_Http::INTERNAL_SERVER_ERROR )
	 ...
	 * @return bool|MN_Error True if the meta field was updated, MN_Error otherwise.
	 */
	protected function update_meta_value( $object_id, $meta_key, $name, $value ) {
		...
		$meta_key   = mn_slash( $meta_key );
		$meta_value = mn_slash( $value );
	 ...
	 * @return MN_Error|string The meta array, if valid, otherwise an error.
	 */
