<?php
/**
 * REST API: MN_REST_Taxonomies_Controller class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MN_REST_Taxonomies_Controller extends MN_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		if ( 'edit' === $request['context'] ) {
			...
			return new MN_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to manage terms in this taxonomy.' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return MN_REST_Response Response object on success, or MN_Error object on failure.
	 */
	 ...
	 *
	 * @param  MN_REST_Request $request Full details about the request.
	 * @return true|MN_Error True if the request has read access for the item, otherwise false or MN_Error object.
	 */
	public function get_item_permissions_check( $request ) {
		...
		if ( $tax_obj ) {
			...
			if ( 'edit' === $request['context'] && ! current_user_can( $tax_obj->cap->manage_terms ) ) {
				return new MN_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to manage terms in this taxonomy.' ), array( 'status' => rest_authorization_required_code() ) );
			}
		}
	...
	public function get_item( $request ) {
		$tax_obj = get_taxonomy( $request['taxonomy'] );
		if ( empty( $tax_obj ) ) {
			return new MN_Error( 'rest_taxonomy_invalid', __( 'Invalid taxonomy.' ), array( 'status' => 404 ) );
		}
