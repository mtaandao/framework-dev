<?php
/**
 * REST API: MN_REST_Post_Statuses_Controller class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MN_REST_Post_Statuses_Controller extends MN_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( empty( $status ) ) {
			return new MN_Error( 'rest_status_invalid', __( 'Invalid status.' ), array( 'status' => 404 ) );
		}

		$check = $this->check_read_permission( $status );

		if ( ! $check ) {
			return new MN_Error( 'rest_cannot_read_status', __( 'Cannot view status.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	 ...
	 * @param MN_REST_Request $request Full details about the request.
	 * @return MN_REST_Response Post status data.
	 */
	public function prepare_item_for_response( $status, $request ) {
		...
		if ( 'publish' === $status->name ) {
			$response->add_link( 'archives', rest_url( 'mn/v2/posts' ) );
		} else {
			$response->add_link( 'archives', add_query_arg( 'status', $status->name, rest_url( 'mn/v2/posts' ) ) );
		}
