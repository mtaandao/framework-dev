<?php
/**
 * REST API: MN_REST_Post_Types_Controller class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MN_REST_Post_Types_Controller extends MN_REST_Controller {
	 ...
	 * @return MN_Error|true True if the request has read access, MN_Error object otherwise.
	 */
	public function get_items_permissions_check( $request ) {
		if ( 'edit' === $request['context'] ) {
			...
			return new MN_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to edit posts in this post type.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item( $request ) {
		...
		if ( empty( $obj ) ) {
			return new MN_Error( 'rest_type_invalid', __( 'Invalid post type.' ), array( 'status' => 404 ) );
		}

		if ( empty( $obj->show_in_rest ) ) {
			return new MN_Error( 'rest_cannot_read_type', __( 'Cannot view post type.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function prepare_item_for_response( $post_type, $request ) {
		$taxonomies = mn_list_filter( get_object_taxonomies( $post_type->name, 'objects' ), array( 'show_in_rest' => true ) );
		$taxonomies = mn_list_pluck( $taxonomies, 'name' );
		...
		$response->add_links( array(
			...
			'https://api.w.org/items' => array(
				'href' => rest_url( sprintf( 'mn/v2/%s', $base ) ),
			),
		) );
