<?php
/**
 * REST API: MN_REST_Terms_Controller class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MN_REST_Terms_Controller extends MN_REST_Controller {
	...
	public function __construct( $taxonomy ) {
		...
		$this->meta = new MN_REST_Term_Meta_Fields( $taxonomy );
	}
	 ...
	 * @return bool|MN_Error True if the request has read access, otherwise false or MN_Error object.
	 */
	public function get_items_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && ! current_user_can( $tax_obj->cap->edit_terms ) ) {
			return new MN_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function get_items( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @link https://developer.mtaandao.co.ke/reference/functions/get_terms/
		 *
		...
		if ( ! empty( $prepared_args['post'] )  ) {
			$query_result = mn_get_object_terms( $prepared_args['post'], $this->taxonomy, $prepared_args );

			// Used when calling mn_count_terms() below.
		...
		$total_terms = mn_count_terms( $this->taxonomy, $count_args );
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return bool|MN_Error True if the request has read access for the item, otherwise false or MN_Error object.
	 */
	public function get_item_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && ! current_user_can( 'edit_term', (int) $request['id'] ) ) {
			return new MN_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit this term.' ), array( 'status' => rest_authorization_required_code() ) );
		}
		return true;
	}
	...
	public function get_item( $request ) {
		...
		if ( ! $term || $term->taxonomy !== $this->taxonomy ) {
			return new MN_Error( 'rest_term_invalid', __( "Term doesn't exist." ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return bool|MN_Error True if the request has access to create items, false or MN_Error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( $taxonomy_obj->cap->edit_terms ) ) {
			return new MN_Error( 'rest_cannot_create', __( 'Sorry, you are not allowed to create new terms.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new MN_Error( 'rest_taxonomy_not_hierarchical', __( 'Can not set parent term, taxonomy is not hierarchical.' ), array( 'status' => 400 ) );
			}

			$parent = get_term( (int) $request['parent'], $this->taxonomy );

			if ( ! $parent ) {
				return new MN_Error( 'rest_term_invalid', __( "Parent term doesn't exist." ), array( 'status' => 400 ) );
			}
		}

		$prepared_term = $this->prepare_item_for_database( $request );

		$term = mn_insert_term( mn_slash( $prepared_term->name ), $this->taxonomy, mn_slash( (array) $prepared_term ) );
		if ( is_mn_error( $term ) ) {
			/*
		 ...
		 * @since 4.7.0
		 *
		 * @param MN_Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return bool|MN_Error True if the request has access to update the item, false or MN_Error object otherwise.
	 */
	public function update_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'edit_term', $term->term_id ) ) {
			return new MN_Error( 'rest_cannot_update', __( 'Sorry, you are not allowed to edit this term.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			$update = mn_update_term( $term->term_id, $term->taxonomy, mn_slash( (array) $prepared_term ) );
		...
		/* This action is documented in lib/endpoints/class-mn-rest-terms-controller.php */
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return bool|MN_Error True if the request has access to delete the item, otherwise false or MN_Error object.
	 */
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'delete_term', $term->term_id ) ) {
			return new MN_Error( 'rest_cannot_delete', __( 'Sorry, you are not allowed to delete this term.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new MN_Error( 'rest_trash_not_supported', __( 'Terms do not support trashing. Set force=true to delete.' ), array( 'status' => 501 ) );
		}
		...
		if ( ! $retval ) {
			return new MN_Error( 'rest_cannot_delete', __( 'The term cannot be deleted.' ), array( 'status' => 500 ) );
		}
	 ...
	 * @param MN_REST_Request $request Request object.
	 * @return MN_REST_Response $response Response object.
	 */
	...
	protected function prepare_links( $term ) {
		...
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( sprintf( 'mn/v2/taxonomies/%s', $this->taxonomy ) ),
			),
		);
		...
		foreach ( $taxonomy_obj->object_type as $type ) {
			...
			$post_type_links[] = array(
				'href' => add_query_arg( $this->rest_base, $term->term_id, rest_url( sprintf( 'mn/v2/%s', $rest_base ) ) ),
			);
		}
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal MN_Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set MN_Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
		 * @param $params JSON Schema-formatted collection parameters.
		 * @param MN_Taxonomy $taxonomy_obj Taxonomy object.
		 */
