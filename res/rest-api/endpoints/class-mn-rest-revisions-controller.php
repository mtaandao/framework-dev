<?php
/**
 * REST API: MN_REST_Revisions_Controller class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class MN_REST_Revisions_Controller extends MN_REST_Controller {
	...
	public function __construct( $parent_post_type ) {
		...
		$this->parent_controller = new MN_REST_Posts_Controller( $parent_post_type );
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! current_user_can( $parent_post_type_obj->cap->edit_post, $parent->ID ) ) {
			return new MN_Error( 'rest_cannot_read', __( 'Sorry, you are not allowed to view revisions of this post.' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_items( $request ) {
		...
		if ( ! $request['parent'] || ! $parent || $this->parent_post_type !== $parent->post_type ) {
			return new MN_Error( 'rest_post_invalid_parent', __( 'Invalid post parent ID.' ), array( 'status' => 404 ) );
		}

		$revisions = mn_get_post_revisions( $request['parent'] );
	...
	public function get_item( $request ) {
		...
		if ( ! $revision || 'revision' !== $revision->post_type ) {
			return new MN_Error( 'rest_post_invalid_id', __( 'Invalid revision ID.' ), array( 'status' => 404 ) );
		}
	 ...
	 * @return bool|MN_Error True if the request has access to delete the item, MN_Error object otherwise.
	 */
	public function delete_item_permissions_check( $request ) {

		$response = $this->get_items_permissions_check( $request );
		if ( ! $response || is_mn_error( $response ) ) {
			return $response;
		}
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return true|MN_Error True on success, or MN_Error object on failure.
	 */
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new MN_Error( 'rest_trash_not_supported', __( 'Revisions do not support trashing. Set force=true to delete.' ), array( 'status' => 501 ) );
		}

		$revision = get_post( $request['id'] );
		$previous = $this->prepare_item_for_response( $revision, $request );

		$result = mn_delete_post( $request['id'], true );
	 ...
	 * @access public
	 *
	 * @param MN_Post         $post    Post revision object.
	...
	public function prepare_item_for_response( $post, $request ) {
		 ...
		 * @param MN_Post          $post     The original revision object.
