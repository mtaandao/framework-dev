...
final class MN_Customize_Manager {
	...
	public function __construct( $args = array() ) {
		...
		if ( ! isset( $args['changeset_uuid'] ) ) {
			$args['changeset_uuid'] = mn_generate_uuid4();
		}
		...
		if ( ! isset( $args['theme'] ) ) {
			if ( isset( $_REQUEST['customize_theme'] ) ) {
				$args['theme'] = mn_unslash( $_REQUEST['customize_theme'] );
			} elseif ( isset( $_REQUEST['theme'] ) ) { // Deprecated.
				$args['theme'] = mn_unslash( $_REQUEST['theme'] );
			}
		}
		if ( ! isset( $args['messenger_channel'] ) && isset( $_REQUEST['customize_messenger_channel'] ) ) {
			$args['messenger_channel'] = sanitize_key( mn_unslash( $_REQUEST['customize_messenger_channel'] ) );
		}

		$this->original_stylesheet = get_stylesheet();
		$this->theme = mn_get_theme( $args['theme'] );
		...
		require_once( ABSPATH . RES . '/customize/class-mn-customize-background-position-control.php' );
		...
		require_once( ABSPATH . RES . '/customize/class-mn-customize-custom-css-setting.php' );
	...
	protected function mn_die( $ajax_message, $message = null ) {
		...
		if ( $this->messenger_channel ) {
			ob_start();
			mn_enqueue_scripts();
			mn_print_scripts( array( 'customize-base' ) );

			$settings = array(
				'messengerArgs' => array(
					'channel' => $this->messenger_channel,
					'url' => mn_customize_url(),
				),
				'error' => $ajax_message,
			);
			?>
			...
			} )( mn.customize, <?php echo mn_json_encode( $settings ) ?> );
			</script>
			<?php
	...
	public function setup_theme() {
		...
		if ( ! preg_match( '/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/', $this->_changeset_uuid ) ) {
			$this->mn_die( -1, __( 'Invalid changeset UUID' ) );
		}
		...
		if ( ! current_user_can( 'customize' ) && ! $this->changeset_post_id() ) {
			$this->mn_die( $this->messenger_channel ? 0 : -1, __( 'Non-existent changeset UUID.' ) );
		}
	...
	public function find_changeset_post_id( $uuid ) {
		...
		if ( ! empty( $changeset_post_query->posts ) ) {
			...
			mn_cache_set( $this->_changeset_uuid, $changeset_post_id, $cache_group );
			return $changeset_post_id;
		}
	...
	function import_theme_starter_content( $starter_content = array() ) {
		...
		$all_post_slugs = array_merge(
			mn_list_pluck( $attachments, 'post_name' ),
			mn_list_pluck( $posts, 'post_name' )
		);

		// Re-use auto-draft starter content posts referenced in the current customized state.
		$existing_starter_content_posts = array();
		if ( ! empty( $starter_content_auto_draft_post_ids ) ) {
			$existing_posts_query = new MN_Query( array(
			...
			foreach ( $attachments as $symbol => $attachment ) {
				...
				if ( isset( $existing_starter_content_posts[ 'attachment:' . $attachment['post_name'] ] ) ) {
					...
					if ( empty( $attached_file ) || ! file_exists( $attached_file ) ) {
						...
						$metadata = mn_generate_attachment_metadata( $attachment_post->ID, $attached_file );
				...
				if ( ! $attachment_id ) {
					...
					$temp_file_name = mn_tempnam( basename( $file_path ) );
					...
					$attachment_post_data = array_merge(
						mn_array_slice_assoc( $attachment, array( 'post_title', 'post_content', 'post_excerpt' ) ),
						array(
		...
		if ( ! empty( $posts ) ) {
			foreach ( array_keys( $posts ) as $post_symbol ) {
				...
				if ( ! empty( $posts[ $post_symbol ]['template'] ) ) {
					$posts[ $post_symbol ]['meta_input']['_mn_page_template'] = $posts[ $post_symbol ]['template'];
				}

				$r = $this->nav_menus->insert_auto_draft_post( $posts[ $post_symbol ] );
				if ( $r instanceof MN_Post ) {
					$posts[ $post_symbol ]['ID'] = $r->ID;
				}
			}

			$starter_content_auto_draft_post_ids = array_merge( $starter_content_auto_draft_post_ids, mn_list_pluck( $posts, 'ID' ) );
		}
		...
		foreach ( $theme_mods as $name => $value ) {
			...
			if ( 'header_image' === $name ) {
				...
				$metadata = mn_get_attachment_metadata( $value );
				if ( empty( $metadata ) ) {
					continue;
				}
				$value = array(
					'attachment_id' => $value,
					'url' => mn_get_attachment_url( $value ),
					'height' => $metadata['height'],
					'width' => $metadata['width'],
				);
			} elseif ( 'background_image' === $name ) {
				$value = mn_get_attachment_url( $value );
			}
	...
	protected function prepare_starter_content_attachments( $attachments ) {
		...
		foreach ( $attachments as $symbol => $attachment ) {
			...
			$checked_filetype = mn_check_filetype( $file_name );
	 ...
	 * 4. Any programmatically-set post values via `MN_Customize_Manager::set_post_value()`.
	 *
	...
	public function unsanitized_post_values( $args = array() ) {
		...
		if ( ! $this->is_theme_active() ) {
			...
			if ( isset( $stashed_theme_mods[ $stylesheet ] ) ) {
				$values = array_merge( $values, mn_list_pluck( $stashed_theme_mods[ $stylesheet ], 'value' ) );
			}
		}
		...
		if ( ! $args['exclude_post_data'] ) {
			if ( ! isset( $this->_post_values ) ) {
				if ( isset( $_POST['customized'] ) ) {
					$post_values = json_decode( mn_unslash( $_POST['customized'] ), true );
				} else {
					$post_values = array();
				}
	...
	public function customize_preview_init() {
		 ...
		 * allowed then the auth cookies would not be sent and Mtaandao would
		...
		add_filter( 'mn_headers', array( $this, 'filter_iframe_security_headers' ) );
		...
		if ( $this->messenger_channel && ! current_user_can( 'customize' ) ) {
			$this->mn_die( -1, __( 'Unauthorized. You may remove the customize_messenger_channel param to preview as frontend.' ) );
			return;
		}

		$this->prepare_controls();

		add_filter( 'mn_redirect', array( $this, 'add_state_query_params' ) );

		mn_enqueue_script( 'customize-preview' );
		add_action( 'mn_head', array( $this, 'customize_preview_loading_style' ) );
		add_action( 'mn_head', array( $this, 'remove_frameless_preview_messenger_channel' ) );
	 ...
	 * @access public
	 * @see mn_redirect()
	 * @see MN_Customize_Manager::get_allowed_url()
	 *
	 * @param string $url URL.
	 * @return string URL.
	 */
	public function add_state_query_params( $url ) {
		$parsed_original_url = mn_parse_url( $url );
		$is_allowed = false;
		foreach ( $this->get_allowed_urls() as $allowed_url ) {
			$parsed_allowed_url = mn_parse_url( $allowed_url );
			$is_allowed = (
	...
	public function customize_preview_settings() {
		...
		$self_url = empty( $_SERVER['REQUEST_URI'] ) ? home_url( '/' ) : esc_url_raw( mn_unslash( $_SERVER['REQUEST_URI'] ) );
		...
		foreach ( $allowed_urls as $allowed_url ) {
			$parsed = mn_parse_url( $allowed_url );
	 ...
	 * to values that pass validation, and values that become null or `MN_Error`
	...
	public function validate_setting_values( $setting_values, $options = array() ) {
		$options = mn_parse_args( $options, array(
		...
		foreach ( $setting_values as $setting_id => $unsanitized_value ) {
			...
			if ( ! $setting ) {
				if ( $options['validate_existence'] ) {
					$validities[ $setting_id ] = new MN_Error( 'unrecognized', __( 'Setting does not exist or is unrecognized.' ) );
				}
				continue;
			}
			if ( $options['validate_capability'] && ! current_user_can( $setting->capability ) ) {
				$validity = new MN_Error( 'unauthorized', __( 'Unauthorized to modify setting due to capability.' ) );
			} else {
			...
			if ( ! is_mn_error( $validity ) ) {
				...
				$late_validity = apply_filters( "customize_validate_{$setting->id}", new MN_Error(), $unsanitized_value, $setting );
	...
	/**
	 * Handle customize_save MN Ajax request to save/update a changeset.
	 *
	 * @since 3.4.0
	...
	public function save() {
		if ( ! is_user_logged_in() ) {
			mn_send_json_error( 'unauthenticated' );
		}
		...
		if ( empty( $changeset_post_id ) ) {
			if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->create_posts ) ) {
				mn_send_json_error( 'cannot_create_changeset_post' );
			}
		} else {
			if ( ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->edit_post, $changeset_post_id ) ) {
				mn_send_json_error( 'cannot_edit_changeset_post' );
			}
		}

		if ( ! empty( $_POST['customize_changeset_data'] ) ) {
			$input_changeset_data = json_decode( mn_unslash( $_POST['customize_changeset_data'] ), true );
			if ( ! is_array( $input_changeset_data ) ) {
				mn_send_json_error( 'invalid_customize_changeset_data' );
			}
		} else {
		...
		if ( isset( $_POST['customize_changeset_title'] ) ) {
			$changeset_title = sanitize_text_field( mn_unslash( $_POST['customize_changeset_title'] ) );
		}
		...
		if ( isset( $_POST['customize_changeset_status'] ) ) {
			$changeset_status = mn_unslash( $_POST['customize_changeset_status'] );
			if ( ! get_post_status_object( $changeset_status ) || ! in_array( $changeset_status, array( 'draft', 'pending', 'publish', 'future' ), true ) ) {
				mn_send_json_error( 'bad_customize_changeset_status', 400 );
			}
			...
			if ( $is_publish && ! current_user_can( get_post_type_object( 'customize_changeset' )->cap->publish_posts ) ) {
				mn_send_json_error( 'changeset_publish_unauthorized', 403 );
			}
		}

		/*
		 * Validate changeset date param. Date is assumed to be in local time for
		 * the MN if in MySQL format (YYYY-MM-DD HH:MM:SS). Otherwise, the date
		...
		if ( isset( $_POST['customize_changeset_date'] ) ) {
			$changeset_date = mn_unslash( $_POST['customize_changeset_date'] );
			if ( preg_match( '/^\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d$/', $changeset_date ) ) {
				...
				$valid_date = mn_checkdate( $mm, $jj, $aa, $changeset_date );
				if ( ! $valid_date ) {
					mn_send_json_error( 'bad_customize_changeset_date', 400 );
				}
		...
		if ( is_mn_error( $r ) ) {
			...
			if ( 'publish' === $response['changeset_status'] ) {
				$response['next_changeset_uuid'] = mn_generate_uuid4();
			}
		}
	 ...
	 * }
	 *
	 * @return array|MN_Error Returns array on success and MN_Error with array data on error.
	 */
	function save_changeset_post( $args = array() ) {
		...
		if ( $changeset_post_id ) {
			...
			if ( 'publish' === $existing_status || 'trash' === $existing_status ) {
				return new MN_Error( 'changeset_already_published' );
			}
		...
		if ( 'publish' === $args['status'] && false === has_action( 'transition_post_status', '_mn_customize_publish_changeset' ) ) {
			return new MN_Error( 'missing_publish_callback' );
		}
		...
		if ( $args['date_gmt'] ) {
			...
			if ( ! $is_future_dated ) {
				return new MN_Error( 'not_future_date' ); // Only future dates are allowed.
			}

			if ( ! $this->is_theme_active() && ( 'future' === $args['status'] || $is_future_dated ) ) {
				return new MN_Error( 'cannot_schedule_theme_switches' ); // This should be allowed in the future, when theme is a regular setting.
			}
			...
			if ( $will_remain_auto_draft ) {
				return new MN_Error( 'cannot_supply_date_for_auto_draft_changeset' );
			}
			...
			if ( mysql2date( 'U', $changeset_post->post_date_gmt, false ) <= mysql2date( 'U', $now, false ) ) {
				return new MN_Error( 'not_future_date' );
			}
		}

		// The request was made via mn.customize.previewer.save().
		...
		if ( $update_transactionally && $invalid_setting_count > 0 ) {
			...
			return new MN_Error( 'transaction_fail', '', $response );
		}
		...
		foreach ( $args['data'] as $setting_id => $setting_params ) {
			...
			if ( isset( $setting_validities[ $setting_id ] ) && is_mn_error( $setting_validities[ $setting_id ] ) ) {
				continue;
			}
		...
		$filter_context = array(
			...
			'previous_data' => is_mn_error( $original_changeset_data ) ? array() : $original_changeset_data,
			'manager' => $this,
		);
		 ...
		 *     @type MN_Customize_Manager $manager       Manager instance.
		 * }
		 */
		...
		// Gather the data for mn_insert_post()/mn_update_post().
		$json_options = 0;
		...
		$json_options |= JSON_PRETTY_PRINT; // Also introduced in PHP 5.4, but MN defines constant for back compat. See MN Trac #30139.
		$post_array = array(
			'post_content' => mn_json_encode( $data, $json_options ),
		);
		if ( $args['title'] ) {
		...
		add_filter( 'mn_save_post_revision_post_has_changed', array( $this, '_filter_revision_post_has_changed' ), 5, 3 );

		// Update the changeset post. The publish_customize_changeset action will cause the settings in the changeset to be saved via MN_Customize_Setting::save().
		$has_kses = ( false !== has_filter( 'content_save_pre', 'mn_filter_post_kses' ) );
		if ( $has_kses ) {
			kses_remove_filters(); // Prevent KSES from corrupting JSON in post_content.
		}

		// Note that updating a post with publish status will trigger MN_Customize_Manager::publish_changeset_values().
		if ( $changeset_post_id ) {
			$post_array['edit_date'] = true; // Prevent date clearing.
			$r = mn_update_post( mn_slash( $post_array ), true );
		} else {
			$r = mn_insert_post( mn_slash( $post_array ), true );
			if ( ! is_mn_error( $r ) ) {
		...
		$this->_changeset_data = null; // Reset so MN_Customize_Manager::changeset_data() will re-populate with updated contents.

		remove_filter( 'mn_save_post_revision_post_has_changed', array( $this, '_filter_revision_post_has_changed' ) );
		...
		if ( is_mn_error( $r ) ) {
			...
			return new MN_Error( 'changeset_post_save_failure', '', $response );
		}
	 ...
	 * `_mn_customize_publish_changeset()` when a customize_changeset post is
	 * transitioned to the `publish` status. As such, this method should not be
	 * called directly and instead `mn_publish_post()` should be used.
	 *
	 ...
	 * @since 4.7.0
	 * @access private
	 * @see _mn_customize_publish_changeset()
	 *
	 * @param int $changeset_post_id ID for customize_changeset post. Defaults to the changeset for the current manager instance.
	 * @return true|MN_Error True or error info.
	 */
	public function _publish_changeset_values( $changeset_post_id ) {
		...
		if ( is_mn_error( $publishing_changeset_data ) ) {
		 ...
		 * on the $mn_customize object passed to hooks during the save logic.
		 */
		 ...
		 * @since 3.4.0
		 *
		 * @param MN_Customize_Manager $manager MN_Customize_Manager instance.
		 */
		...
		foreach ( $changeset_setting_ids as $setting_id ) {
			...
			if ( $setting ) {
				 ...
				 * when a scheduled changeset publishes via MN Cron.
				 */
				if ( isset( $setting_user_ids[ $setting_id ] ) ) {
					mn_set_current_user( $setting_user_ids[ $setting_id ] );
				} else {
					mn_set_current_user( $original_user_id );
				}
	 ...
	 *
	 * @since 3.4.0
	 * @deprecated 4.7.0 Use mn_list_sort()
	 *
	...
	protected function _cmp_priority( $a, $b ) {
		_deprecated_function( __METHOD__, '4.7.0', 'mn_list_sort' );
	...
	public function prepare_controls() {
		...
		$controls = array();
		$this->controls = mn_list_sort( $this->controls, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
		...
		// Prepare sections.
		$this->sections = mn_list_sort( $this->sections, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
		$sections = array();
		...
		foreach ( $this->sections as $section ) {
			...
			$section->controls = mn_list_sort( $section->controls, array(
				'priority'        => 'ASC',
				'instance_number' => 'ASC',
			) );
		...
		// Prepare panels.
		$this->panels = mn_list_sort( $this->panels, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
		$panels = array();
		...
		foreach ( $this->panels as $panel ) {
			...
			$panel->sections = mn_list_sort( $panel->sections, array(
				'priority'        => 'ASC',
				'instance_number' => 'ASC',
			), 'ASC', true );
		...
		$this->containers = mn_list_sort( $this->containers, array(
			'priority'        => 'ASC',
			'instance_number' => 'ASC',
		), 'ASC', true );
	}
	...
	public function is_cross_domain() {
		$admin_origin = mn_parse_url( admin_url() );
		$home_origin = mn_parse_url( home_url() );
	...
	public function register_controls() {
		...
		$this->register_control_type( 'MN_Customize_Background_Position_Control' );
		...
		if ( current_theme_supports( 'custom-header', 'video' ) ) {
			...
			// @todo Customizer sections should support having notifications just like controls do. See <https://core.trac.mtaandao.co.ke/ticket/38794>.
		...
		$this->add_control( new MN_Customize_Media_Control( $this, 'header_video', array(
			'theme_supports' => array( 'custom-header', 'video' ),
			'label'          => __( 'Header Video' ),
			'description'    => $control_description,
			'section'        => 'header_image',
			'mime_type'      => 'video',
			// @todo These button_labels can be removed once MN_Customize_Media_Control provides mime_type-specific labels automatically. See <https://core.trac.mtaandao.co.ke/ticket/38796>.
			'button_labels'  => array(
		...
		$this->add_control( new MN_Customize_Background_Position_Control( $this, 'background_position', array(
		...
		/*
		 * Static Front Page
		 * See also https://core.trac.mtaandao.co.ke/ticket/19627 which introduces the the static-front-page theme_support.
		...
		$this->add_section( 'custom_css', array(
			...
			'description'        => sprintf( '%s<br /><a href="%s" class="external-link" target="_blank">%s<span class="screen-reader-text">%s</span></a>',
				...
				esc_url( __( 'https://mtaandao.github.io/CSS' ) ),
				__( 'Learn more about CSS' ),
				__( '(link opens in a new window)' )
			),
		) );

		$custom_css_setting = new MN_Customize_Custom_CSS_Setting( $this, sprintf( 'custom_css[%s]', get_stylesheet() ), array(
			'capability' => 'edit_css',
	 ...
	 * @param MN_Customize_Setting $setting Setting.
	 * @return string|MN_Error Background value or validation error.
	 */
	public function _sanitize_background_setting( $value, $setting ) {
		if ( 'background_repeat' === $setting->id ) {
			if ( ! in_array( $value, array( 'repeat-x', 'repeat-y', 'repeat', 'no-repeat' ) ) ) {
				return new MN_Error( 'invalid_value', __( 'Invalid value for background repeat.' ) );
			}
		} elseif ( 'background_attachment' === $setting->id ) {
			if ( ! in_array( $value, array( 'fixed', 'scroll' ) ) ) {
				return new MN_Error( 'invalid_value', __( 'Invalid value for background attachment.' ) );
			}
		} elseif ( 'background_position_x' === $setting->id ) {
			if ( ! in_array( $value, array( 'left', 'center', 'right' ), true ) ) {
				return new MN_Error( 'invalid_value', __( 'Invalid value for background position X.' ) );
			}
		} elseif ( 'background_position_y' === $setting->id ) {
			if ( ! in_array( $value, array( 'top', 'center', 'bottom' ), true ) ) {
				return new MN_Error( 'invalid_value', __( 'Invalid value for background position Y.' ) );
			}
		} elseif ( 'background_size' === $setting->id ) {
			if ( ! in_array( $value, array( 'auto', 'contain', 'cover' ), true ) ) {
				return new MN_Error( 'invalid_value', __( 'Invalid value for background size.' ) );
			}
			...
			return new MN_Error( 'unrecognized_setting', __( 'Unrecognized background setting.' ) );
		}
		return $value;
	}
	 ...
	 * @param MN_Customize_Selective_Refresh $selective_refresh Selective refresh component.
